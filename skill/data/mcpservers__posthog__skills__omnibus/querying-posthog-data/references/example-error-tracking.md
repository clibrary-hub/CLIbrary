...
SELECT
    ...
    sumForEach(arrayMap(bin -> if(and(greater(timestamp, bin), lessOrEquals(dateDiff('seconds', bin, timestamp), divide(dateDiff('seconds', toDateTime(toDateTime('2026-05-29 16:41:36.851758')), toDateTime(toDateTime('2026-05-30 16:41:36.852270'))), 20))), 1, 0), arrayMap(i -> dateAdd(toDateTime(toDateTime('2026-05-29 16:41:36.851758')), toIntervalSecond(multiply(i, divide(dateDiff('seconds', toDateTime(toDateTime('2026-05-29 16:41:36.851758')), toDateTime(toDateTime('2026-05-30 16:41:36.852270'))), 20)))), range(0, 20)))) AS volumeRange,
    argMin(tuple(uuid, distinct_id, timestamp, properties), timestamp) AS first_event,
    argMax(properties.$lib, timestamp) AS library
FROM
    events AS e
WHERE
    and(equals(event, '$exception'), isNotNull(e.issue_id), equals(properties.tag, 'max_ai'), greaterOrEquals(timestamp, toDateTime(toDateTime('2026-05-29 16:41:36.851758'))), lessOrEquals(timestamp, toDateTime(toDateTime('2026-05-30 16:41:36.852270'))), or(greater(position(lower(properties.$exception_types), lower('constant')), 0), greater(position(lower(properties.$exception_values), lower('constant')), 0), greater(position(lower(properties.$exception_sources), lower('constant')), 0), greater(position(lower(properties.$exception_functions), lower('constant')), 0), greater(position(lower(properties.email), lower('constant')), 0), greater(position(lower(person.properties.email), lower('constant')), 0)))
GROUP BY
    id
ORDER BY
    last_seen DESC

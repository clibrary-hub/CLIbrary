...
SELECT
    ...
    date_diff('DAY', toDateTime('2026-05-30 16:41:41.117475'), expiry_time) AS recording_ttl,
    greaterOrEquals(max(s._timestamp), toDateTime('2026-05-30 16:36:41.116545')) AS ongoing,
...
WHERE
    and(greaterOrEquals(s.min_first_timestamp, toDateTime('2026-05-27 00:00:00.000000')), lessOrEquals(s.min_first_timestamp, toDateTime('2026-05-30 16:41:41.116746')))
GROUP BY
    session_id
HAVING
    and(greaterOrEquals(expiry_time, toDateTime('2026-05-30 16:41:41.117353')), equals(max(s.is_deleted), 0), greater(active_seconds, 5.0))
ORDER BY
    start_time DESC,
    session_id DESC
LIMIT 50000

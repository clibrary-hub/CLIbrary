...
| 介面 | CLI（`click` 框架） |
| 設定檔 | `~/.config/mail-draft/config.json` |
...
[project.scripts]
mail-draft = "mail_draft.cli:main"
```
...
```bash
mail-draft [OPTIONS] COMMAND [ARGS]...
```
...
# 指定輸出格式
mail-draft run --output json

# 詳細模式
mail-draft run --verbose

# 靜音（適合 pipe 到其他工具）
mail-draft run --quiet | jq .

# 查看設定
mail-draft config show

# 匯出 JSON
mail-draft export --output json > result.json
```
...
from .core import MailDraftCore
...
def config():
    """顯示或修改設定"""
    import json, pathlib
    cfg_path = pathlib.Path.home() / ".config" / "mail-draft" / "config.json"
    if cfg_path.exists():
...
DB_PATH = pathlib.Path.home() / ".config" / "mail-draft" / "data.db"
...
{
  "name": "mail-draft",
  "description": "AI 草稿產生器。解決問題：信件草稿要手寫",
  "input_schema": {
    "type": "object",
    "properties": {
      "action": {
...
```bash
# Agent 呼叫範例
result=$(mail-draft run --quiet --output json)
echo $result | jq .data
```
...
def call_mail_draft(action="run") -> dict:
    proc = subprocess.run(
        ["mail-draft", action, "--output", "json", "--quiet"],
...
```bash
# 管線串接範例
mail-draft run --quiet --output json \
  | jq -r '.data' \
  | other-tool process
```
...
# 執行測試
pytest tests/ -v --cov=mail_draft --cov-report=term-missing
...
```python
import pytest
from mail_draft.core import MailDraftCore
...
*工具編號 #164 ／ 類別 I — 郵件與通訊 ／ 計畫版本 v1.0 ／ 2026-04-27*

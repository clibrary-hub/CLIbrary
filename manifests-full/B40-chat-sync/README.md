...
| 介面 | CLI（`click` 框架） |
| 設定檔 | `~/.config/chat-sync/config.json` |
...
dependencies = ["rsync", "click", "rich"]

[project.scripts]
chat-sync = "chat_sync.cli:main"
```
...
```bash
chat-sync [OPTIONS] COMMAND [ARGS]...
```
...
# 指定輸出格式
chat-sync run --output json

# 詳細模式
chat-sync run --verbose

# 靜音（適合 pipe 到其他工具）
chat-sync run --quiet | jq .

# 查看設定
chat-sync config show

# 匯出 JSON
chat-sync export --output json > result.json
```
...
def config():
    """顯示或修改設定"""
    import json, pathlib
    cfg_path = pathlib.Path.home() / ".config" / "chat-sync" / "config.json"
    if cfg_path.exists():
...
DB_PATH = pathlib.Path.home() / ".config" / "chat-sync" / "data.db"
...
{
  "name": "chat-sync",
  "description": "對話跨裝置同步。解決問題：對話備份難跨裝置",
  "input_schema": {
    "type": "object",
    "properties": {
      "action": {
...
```bash
# Agent 呼叫範例
result=$(chat-sync run --quiet --output json)
echo $result | jq .data
```
...
def call_chat_sync(action="run") -> dict:
    proc = subprocess.run(
        ["chat-sync", action, "--output", "json", "--quiet"],
...
```bash
# 管線串接範例
chat-sync run --quiet --output json \
  | jq -r '.data' \
  | other-tool process
```
...
# 執行測試
pytest tests/ -v --cov=chat_sync --cov-report=term-missing
...
```python
import pytest
from chat_sync.core import ChatSyncCore
...
*工具編號 #040 ／ 類別 B — 對話與記憶管理 ／ 計畫版本 v1.0 ／ 2026-04-27*

...
| 介面 | CLI（`click` 框架） |
| 設定檔 | `~/.config/order-hub/config.json` |
...
```bash
order-hub [OPTIONS] COMMAND [ARGS]...
```
...
# 指定輸出格式
order-hub run --output json

# 詳細模式
order-hub run --verbose

# 靜音（適合 pipe 到其他工具）
order-hub run --quiet | jq .

# 查看設定
order-hub config show

# 匯出 JSON
order-hub export --output json > result.json
```
...
def config():
    """顯示或修改設定"""
    import json, pathlib
    cfg_path = pathlib.Path.home() / ".config" / "order-hub" / "config.json"
    if cfg_path.exists():
...
DB_PATH = pathlib.Path.home() / ".config" / "order-hub" / "data.db"
...
```bash
# Agent 呼叫範例
result=$(order-hub run --quiet --output json)
echo $result | jq .data
```
...
def call_order_hub(action="run") -> dict:
    proc = subprocess.run(
        ["order-hub", action, "--output", "json", "--quiet"],
...
```bash
# 管線串接範例
order-hub run --quiet --output json \
  | jq -r '.data' \
  | other-tool process
```
...
# 執行測試
pytest tests/ -v --cov=order_hub --cov-report=term-missing
...
```python
import pytest
from order_hub.core import OrderHubCore
...
*工具編號 #404 ／ 類別 U — 購物與訂閱管理 ／ 計畫版本 v1.0 ／ 2026-04-27*

...
| 介面 | CLI（`click` 框架） |
| 設定檔 | `~/.config/agent-acl/config.json` |
...
```bash
agent-acl [OPTIONS] COMMAND [ARGS]...
```
...
# 指定輸出格式
agent-acl run --output json

# 詳細模式
agent-acl run --verbose

# 靜音（適合 pipe 到其他工具）
agent-acl run --quiet | jq .

# 查看設定
agent-acl config show

# 匯出 JSON
agent-acl export --output json > result.json
```
...
def config():
    """顯示或修改設定"""
    import json, pathlib
    cfg_path = pathlib.Path.home() / ".config" / "agent-acl" / "config.json"
    if cfg_path.exists():
...
DB_PATH = pathlib.Path.home() / ".config" / "agent-acl" / "data.db"
...
{
  "name": "agent-acl",
  "description": "Agent 權限控管。解決問題：Agent 信任邊界不清",
  "input_schema": {
    "type": "object",
    "properties": {
      "action": {
...
```bash
# Agent 呼叫範例
result=$(agent-acl run --quiet --output json)
echo $result | jq .data
```
...
def call_agent_acl(action="run") -> dict:
    proc = subprocess.run(
        ["agent-acl", action, "--output", "json", "--quiet"],
...
```bash
# 管線串接範例
agent-acl run --quiet --output json \
  | jq -r '.data' \
  | other-tool process
```
...
# 執行測試
pytest tests/ -v --cov=agent_acl --cov-report=term-missing
...
```python
import pytest
from agent_acl.core import AgentAclCore
...
```json
{
  "name": "agent-acl",
  "version": "0.1.0",
  "description": "Agent 權限控管",
  "category": "F",
  "category_name": "Agent間通訊與協作",
  "number": 105,
  "pain_point": "Agent 信任邊界不清",
  "commercial": "✅",
  "language": "Python",
  "entry_point": "agent-acl",
  "dependencies": ["click", "rich", "pydantic"],
...
*工具編號 #105 ／ 類別 F — Agent間通訊與協作 ／ 計畫版本 v1.0 ／ 2026-04-27*

...
| 介面 | CLI（`click` 框架） |
| 設定檔 | `~/.config/night-wind/config.json` |
...
[project.scripts]
night-wind = "night_wind.cli:main"
```
...
```bash
night-wind [OPTIONS] COMMAND [ARGS]...
```
...
# 指定輸出格式
night-wind run --output json

# 詳細模式
night-wind run --verbose

# 靜音（適合 pipe 到其他工具）
night-wind run --quiet | jq .

# 查看設定
night-wind config show

# 匯出 JSON
night-wind export --output json > result.json
```
...
from .core import NightWindCore
...
def config():
    """顯示或修改設定"""
    import json, pathlib
    cfg_path = pathlib.Path.home() / ".config" / "night-wind" / "config.json"
    if cfg_path.exists():
...
DB_PATH = pathlib.Path.home() / ".config" / "night-wind" / "data.db"
...
{
  "name": "night-wind",
  "description": "睡前清場器。解決問題：想睡前自動關閉所有應用",
  "input_schema": {
    "type": "object",
    "properties": {
      "action": {
...
```bash
# Agent 呼叫範例
result=$(night-wind run --quiet --output json)
echo $result | jq .data
```
...
def call_night_wind(action="run") -> dict:
    proc = subprocess.run(
        ["night-wind", action, "--output", "json", "--quiet"],
...
```bash
# 管線串接範例
night-wind run --quiet --output json \
  | jq -r '.data' \
  | other-tool process
```
...
# 執行測試
pytest tests/ -v --cov=night_wind --cov-report=term-missing
...
```python
import pytest
from night_wind.core import NightWindCore
...
```json
{
  ...
  "category_name": "個人生產力",
  "number": 126,
  "pain_point": "想睡前自動關閉所有應用",
  "commercial": "✅",
  "language": "Python",
  "entry_point": "night-wind",
  "dependencies": ["psutil", "click", "rich"],
...
*工具編號 #126 ／ 類別 G — 個人生產力 ／ 計畫版本 v1.0 ／ 2026-04-27*

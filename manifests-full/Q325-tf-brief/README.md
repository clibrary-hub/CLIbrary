...
> **Terraform 摘要器**
> 分類：Q — DevOps與基礎設施 ｜ 編號：#325 ｜ 商用：✅

## 問題背景

**用戶痛點**：Terraform plan 摘要
...
| 介面 | CLI（`click` 框架） |
| 設定檔 | `~/.config/tf-brief/config.json` |
...
# 指定輸出格式
tf-brief run --output json

# 詳細模式
tf-brief run --verbose

# 靜音（適合 pipe 到其他工具）
tf-brief run --quiet | jq .

# 查看設定
tf-brief config show

# 匯出 JSON
tf-brief export --output json > result.json
```
...
def config():
    """顯示或修改設定"""
    import json, pathlib
    cfg_path = pathlib.Path.home() / ".config" / "tf-brief" / "config.json"
    if cfg_path.exists():
...
DB_PATH = pathlib.Path.home() / ".config" / "tf-brief" / "data.db"
...
{
  "name": "tf-brief",
  "description": "Terraform 摘要器。解決問題：Terraform plan 摘要",
  "input_schema": {
    "type": "object",
    "properties": {
      "action": {
...
```bash
# Agent 呼叫範例
result=$(tf-brief run --quiet --output json)
echo $result | jq .data
```
...
def call_tf_brief(action="run") -> dict:
    proc = subprocess.run(
        ["tf-brief", action, "--output", "json", "--quiet"],
...
```bash
# 管線串接範例
tf-brief run --quiet --output json \
  | jq -r '.data' \
  | other-tool process
```
...
# 執行測試
pytest tests/ -v --cov=tf_brief --cov-report=term-missing
...
```python
import pytest
from tf_brief.core import TfBriefCore
...
```json
{
  "name": "tf-brief",
  "version": "0.1.0",
  "description": "Terraform 摘要器",
  "category": "Q",
  "category_name": "DevOps與基礎設施",
  "number": 325,
  "pain_point": "Terraform plan 摘要",
  "commercial": "✅",
  "language": "Python",
  "entry_point": "tf-brief",
  "dependencies": ["anthropic", "click", "rich"],
...
*工具編號 #325 ／ 類別 Q — DevOps與基礎設施 ／ 計畫版本 v1.0 ／ 2026-04-27*

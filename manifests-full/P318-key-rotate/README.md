...
| 介面 | CLI（`click` 框架） |
| 設定檔 | `~/.config/key-rotate/config.json` |
...
[project.scripts]
key-rotate = "key_rotate.cli:main"
```
...
```bash
key-rotate [OPTIONS] COMMAND [ARGS]...
```
...
# 指定輸出格式
key-rotate run --output json

# 詳細模式
key-rotate run --verbose

# 靜音（適合 pipe 到其他工具）
key-rotate run --quiet | jq .

# 查看設定
key-rotate config show

# 匯出 JSON
key-rotate export --output json > result.json
```
...
from .core import KeyRotateCore
...
def config():
    """顯示或修改設定"""
    import json, pathlib
    cfg_path = pathlib.Path.home() / ".config" / "key-rotate" / "config.json"
    if cfg_path.exists():
...
DB_PATH = pathlib.Path.home() / ".config" / "key-rotate" / "data.db"
...
{
  "name": "key-rotate",
  "description": "金鑰輪轉器。解決問題：KMS 金鑰輪轉",
  "input_schema": {
    "type": "object",
    "properties": {
      "action": {
...
```bash
# Agent 呼叫範例
result=$(key-rotate run --quiet --output json)
echo $result | jq .data
```
...
def call_key_rotate(action="run") -> dict:
    proc = subprocess.run(
        ["key-rotate", action, "--output", "json", "--quiet"],
...
```bash
# 管線串接範例
key-rotate run --quiet --output json \
  | jq -r '.data' \
  | other-tool process
```
...
# 執行測試
pytest tests/ -v --cov=key_rotate --cov-report=term-missing
...
```python
import pytest
from key_rotate.core import KeyRotateCore
...
*工具編號 #318 ／ 類別 P — 安全與隱私 ／ 計畫版本 v1.0 ／ 2026-04-27*

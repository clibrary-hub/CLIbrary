...
> **結構化資料提取**
> 分類：M — 網路爬蟲與監控 ｜ 編號：#254 ｜ 商用：⚠️

## 問題背景

**用戶痛點**：抓 JSON-LD 結構化資料
...
| 介面 | CLI（`click` 框架） |
| 設定檔 | `~/.config/ld-extract/config.json` |
...
[project.scripts]
ld-extract = "ld_extract.cli:main"
```
...
```bash
ld-extract [OPTIONS] COMMAND [ARGS]...
```
...
# 指定輸出格式
ld-extract run --output json

# 詳細模式
ld-extract run --verbose

# 靜音（適合 pipe 到其他工具）
ld-extract run --quiet | jq .

# 查看設定
ld-extract config show

# 匯出 JSON
ld-extract export --output json > result.json
```
...
from .core import LdExtractCore
...
def config():
    """顯示或修改設定"""
    import json, pathlib
    cfg_path = pathlib.Path.home() / ".config" / "ld-extract" / "config.json"
    if cfg_path.exists():
...
DB_PATH = pathlib.Path.home() / ".config" / "ld-extract" / "data.db"
...
{
  "name": "ld-extract",
  "description": "結構化資料提取。解決問題：抓 JSON-LD 結構化資料",
  "input_schema": {
    "type": "object",
    "properties": {
      "action": {
...
```bash
# Agent 呼叫範例
result=$(ld-extract run --quiet --output json)
echo $result | jq .data
```
...
def call_ld_extract(action="run") -> dict:
    proc = subprocess.run(
        ["ld-extract", action, "--output", "json", "--quiet"],
...
```bash
# 管線串接範例
ld-extract run --quiet --output json \
  | jq -r '.data' \
  | other-tool process
```
...
# 執行測試
pytest tests/ -v --cov=ld_extract --cov-report=term-missing
...
```python
import pytest
from ld_extract.core import LdExtractCore
...
```json
{
  ...
  "number": 254,
  "pain_point": "抓 JSON-LD 結構化資料",
  "commercial": "⚠️",
  "language": "Python",
  "entry_point": "ld-extract",
  "dependencies": ["requests", "beautifulsoup4", "click"],
...
*工具編號 #254 ／ 類別 M — 網路爬蟲與監控 ／ 計畫版本 v1.0 ／ 2026-04-27*

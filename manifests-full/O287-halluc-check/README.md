...
| 介面 | CLI（`click` 框架） |
| 設定檔 | `~/.config/halluc-check/config.json` |
...
[project.scripts]
halluc-check = "halluc_check.cli:main"
```
...
```bash
halluc-check [OPTIONS] COMMAND [ARGS]...
```
...
# 指定輸出格式
halluc-check run --output json

# 詳細模式
halluc-check run --verbose

# 靜音（適合 pipe 到其他工具）
halluc-check run --quiet | jq .

# 查看設定
halluc-check config show

# 匯出 JSON
halluc-check export --output json > result.json
```
...
from .core import HallucCheckCore
...
def config():
    """顯示或修改設定"""
    import json, pathlib
    cfg_path = pathlib.Path.home() / ".config" / "halluc-check" / "config.json"
    if cfg_path.exists():
...
DB_PATH = pathlib.Path.home() / ".config" / "halluc-check" / "data.db"
...
{
  "name": "halluc-check",
  "description": "幻覺檢查器。解決問題：LLM 幻覺偵測",
  "input_schema": {
    "type": "object",
    "properties": {
      "action": {
...
```bash
# Agent 呼叫範例
result=$(halluc-check run --quiet --output json)
echo $result | jq .data
```
...
def call_halluc_check(action="run") -> dict:
    proc = subprocess.run(
        ["halluc-check", action, "--output", "json", "--quiet"],
...
```bash
# 管線串接範例
halluc-check run --quiet --output json \
  | jq -r '.data' \
  | other-tool process
```
...
# 執行測試
pytest tests/ -v --cov=halluc_check --cov-report=term-missing

# 快速冒煙測試
halluc-check run --help
halluc-check config show
```
...
```python
import pytest
from halluc_check.core import HallucCheckCore
...
*工具編號 #287 ／ 類別 O — AI/ML輔助 ／ 計畫版本 v1.0 ／ 2026-04-27*

...
| 介面 | CLI（`click` 框架） |
| 設定檔 | `~/.config/wiki-pub/config.json` |
...
# 指定輸出格式
wiki-pub run --output json

# 詳細模式
wiki-pub run --verbose

# 靜音（適合 pipe 到其他工具）
wiki-pub run --quiet | jq .

# 查看設定
wiki-pub config show

# 匯出 JSON
wiki-pub export --output json > result.json
```
...
def config():
    """顯示或修改設定"""
    import json, pathlib
    cfg_path = pathlib.Path.home() / ".config" / "wiki-pub" / "config.json"
    if cfg_path.exists():
...
DB_PATH = pathlib.Path.home() / ".config" / "wiki-pub" / "data.db"
...
{
  "name": "wiki-pub",
  "description": "知識庫發佈器。解決問題：知識庫匯出靜態網站",
  "input_schema": {
    "type": "object",
    "properties": {
      "action": {
...
```bash
# Agent 呼叫範例
result=$(wiki-pub run --quiet --output json)
echo $result | jq .data
```
...
def call_wiki_pub(action="run") -> dict:
    proc = subprocess.run(
        ["wiki-pub", action, "--output", "json", "--quiet"],
...
```bash
# 管線串接範例
wiki-pub run --quiet --output json \
  | jq -r '.data' \
  | other-tool process
```
...
# 執行測試
pytest tests/ -v --cov=wiki_pub --cov-report=term-missing
...
```python
import pytest
from wiki_pub.core import WikiPubCore
...
```json
{
  ...
  "number": 356,
  "pain_point": "知識庫匯出靜態網站",
  "commercial": "✅",
  "language": "Python",
  "entry_point": "wiki-pub",
  "dependencies": ["mkdocs", "click", "rich"],
...
*工具編號 #356 ／ 類別 R — 知識管理與筆記 ／ 計畫版本 v1.0 ／ 2026-04-27*

...
| 介面 | CLI（`click` 框架） |
| 設定檔 | `~/.config/trip-conflict/config.json` |
...
[project.scripts]
trip-conflict = "trip_conflict.cli:main"
```
...
```bash
trip-conflict [OPTIONS] COMMAND [ARGS]...
```
...
# 指定輸出格式
trip-conflict run --output json

# 詳細模式
trip-conflict run --verbose

# 靜音（適合 pipe 到其他工具）
trip-conflict run --quiet | jq .

# 查看設定
trip-conflict config show

# 匯出 JSON
trip-conflict export --output json > result.json
```
...
### `trip_conflict/cli.py`

```python
import click
from rich.console import Console
from .core import TripConflictCore
...
def config():
    """顯示或修改設定"""
    import json, pathlib
    cfg_path = pathlib.Path.home() / ".config" / "trip-conflict" / "config.json"
    if cfg_path.exists():
...
DB_PATH = pathlib.Path.home() / ".config" / "trip-conflict" / "data.db"
...
```bash
# Agent 呼叫範例
result=$(trip-conflict run --quiet --output json)
echo $result | jq .data
```
...
def call_trip_conflict(action="run") -> dict:
    proc = subprocess.run(
        ["trip-conflict", action, "--output", "json", "--quiet"],
...
```bash
# 管線串接範例
trip-conflict run --quiet --output json \
  | jq -r '.data' \
  | other-tool process
```
...
# 執行測試
pytest tests/ -v --cov=trip_conflict --cov-report=term-missing

# 快速冒煙測試
trip-conflict run --help
trip-conflict config show
```
...
```python
import pytest
from trip_conflict.core import TripConflictCore
...
*工具編號 #400 ／ 類別 T — 旅遊與生活 ／ 計畫版本 v1.0 ／ 2026-04-27*

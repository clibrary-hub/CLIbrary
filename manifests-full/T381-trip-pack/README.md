...
| 介面 | CLI（`click` 框架） |
| 設定檔 | `~/.config/trip-pack/config.json` |
...
```bash
trip-pack [OPTIONS] COMMAND [ARGS]...
```
...
# 指定輸出格式
trip-pack run --output json

# 詳細模式
trip-pack run --verbose

# 靜音（適合 pipe 到其他工具）
trip-pack run --quiet | jq .

# 查看設定
trip-pack config show

# 匯出 JSON
trip-pack export --output json > result.json
```
...
def config():
    """顯示或修改設定"""
    import json, pathlib
    cfg_path = pathlib.Path.home() / ".config" / "trip-pack" / "config.json"
    if cfg_path.exists():
...
DB_PATH = pathlib.Path.home() / ".config" / "trip-pack" / "data.db"
...
```bash
# Agent 呼叫範例
result=$(trip-pack run --quiet --output json)
echo $result | jq .data
```
...
def call_trip_pack(action="run") -> dict:
    proc = subprocess.run(
        ["trip-pack", action, "--output", "json", "--quiet"],
...
```bash
# 管線串接範例
trip-pack run --quiet --output json \
  | jq -r '.data' \
  | other-tool process
```
...
# 執行測試
pytest tests/ -v --cov=trip_pack --cov-report=term-missing
...
```python
import pytest
from trip_pack.core import TripPackCore
...
*工具編號 #381 ／ 類別 T — 旅遊與生活 ／ 計畫版本 v1.0 ／ 2026-04-27*

...
| 介面 | CLI（`click` 框架） |
| 設定檔 | `~/.config/pack-list/config.json` |
...
```bash
pack-list [OPTIONS] COMMAND [ARGS]...
```
...
# 指定輸出格式
pack-list run --output json

# 詳細模式
pack-list run --verbose

# 靜音（適合 pipe 到其他工具）
pack-list run --quiet | jq .

# 查看設定
pack-list config show

# 匯出 JSON
pack-list export --output json > result.json
```
...
def config():
    """顯示或修改設定"""
    import json, pathlib
    cfg_path = pathlib.Path.home() / ".config" / "pack-list" / "config.json"
    if cfg_path.exists():
...
DB_PATH = pathlib.Path.home() / ".config" / "pack-list" / "data.db"
...
```bash
# Agent 呼叫範例
result=$(pack-list run --quiet --output json)
echo $result | jq .data
```
...
def call_pack_list(action="run") -> dict:
    proc = subprocess.run(
        ["pack-list", action, "--output", "json", "--quiet"],
...
```bash
# 管線串接範例
pack-list run --quiet --output json \
  | jq -r '.data' \
  | other-tool process
```
...
# 執行測試
pytest tests/ -v --cov=pack_list --cov-report=term-missing
...
```python
import pytest
from pack_list.core import PackListCore
...
*工具編號 #387 ／ 類別 T — 旅遊與生活 ／ 計畫版本 v1.0 ／ 2026-04-27*

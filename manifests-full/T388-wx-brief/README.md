...
| 介面 | CLI（`click` 框架） |
| 設定檔 | `~/.config/wx-brief/config.json` |
...
# 指定輸出格式
wx-brief run --output json

# 詳細模式
wx-brief run --verbose

# 靜音（適合 pipe 到其他工具）
wx-brief run --quiet | jq .

# 查看設定
wx-brief config show

# 匯出 JSON
wx-brief export --output json > result.json
```
...
def config():
    """顯示或修改設定"""
    import json, pathlib
    cfg_path = pathlib.Path.home() / ".config" / "wx-brief" / "config.json"
    if cfg_path.exists():
...
DB_PATH = pathlib.Path.home() / ".config" / "wx-brief" / "data.db"
...
```bash
# Agent 呼叫範例
result=$(wx-brief run --quiet --output json)
echo $result | jq .data
```
...
def call_wx_brief(action="run") -> dict:
    proc = subprocess.run(
        ["wx-brief", action, "--output", "json", "--quiet"],
...
```bash
# 管線串接範例
wx-brief run --quiet --output json \
  | jq -r '.data' \
  | other-tool process
```
...
# 執行測試
pytest tests/ -v --cov=wx_brief --cov-report=term-missing
...
```python
import pytest
from wx_brief.core import WxBriefCore
...
*工具編號 #388 ／ 類別 T — 旅遊與生活 ／ 計畫版本 v1.0 ／ 2026-04-27*

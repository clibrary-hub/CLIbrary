...
| 介面 | CLI（`click` 框架） |
| 設定檔 | `~/.config/pivot-quick/config.json` |
...
[project.scripts]
pivot-quick = "pivot_quick.cli:main"
```
...
```bash
pivot-quick [OPTIONS] COMMAND [ARGS]...
```
...
# 指定輸出格式
pivot-quick run --output json

# 詳細模式
pivot-quick run --verbose

# 靜音（適合 pipe 到其他工具）
pivot-quick run --quiet | jq .

# 查看設定
pivot-quick config show

# 匯出 JSON
pivot-quick export --output json > result.json
```
...
from .core import PivotQuickCore
...
def config():
    """顯示或修改設定"""
    import json, pathlib
    cfg_path = pathlib.Path.home() / ".config" / "pivot-quick" / "config.json"
    if cfg_path.exists():
...
DB_PATH = pathlib.Path.home() / ".config" / "pivot-quick" / "data.db"
...
```bash
# Agent 呼叫範例
result=$(pivot-quick run --quiet --output json)
echo $result | jq .data
```
...
def call_pivot_quick(action="run") -> dict:
    proc = subprocess.run(
        ["pivot-quick", action, "--output", "json", "--quiet"],
...
```bash
# 管線串接範例
pivot-quick run --quiet --output json \
  | jq -r '.data' \
  | other-tool process
```
...
# 執行測試
pytest tests/ -v --cov=pivot_quick --cov-report=term-missing

# 快速冒煙測試
pivot-quick run --help
pivot-quick config show
```
...
```python
import pytest
from pivot_quick.core import PivotQuickCore
...
*工具編號 #264 ／ 類別 N — 資料分析與視覺化 ／ 計畫版本 v1.0 ／ 2026-04-27*

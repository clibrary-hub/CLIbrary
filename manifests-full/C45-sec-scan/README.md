...
| 介面 | CLI（`click` 框架） |
| 設定檔 | `~/.config/sec-scan/config.json` |
...
dependencies = ["bandit", "click", "rich"]

[project.scripts]
sec-scan = "sec_scan.cli:main"
```
...
# 指定輸出格式
sec-scan run --output json

# 詳細模式
sec-scan run --verbose

# 靜音（適合 pipe 到其他工具）
sec-scan run --quiet | jq .

# 查看設定
sec-scan config show

# 匯出 JSON
sec-scan export --output json > result.json
```
...
def config():
    """顯示或修改設定"""
    import json, pathlib
    cfg_path = pathlib.Path.home() / ".config" / "sec-scan" / "config.json"
    if cfg_path.exists():
...
DB_PATH = pathlib.Path.home() / ".config" / "sec-scan" / "data.db"
...
{
  "name": "sec-scan",
  "description": "安全掃描器。解決問題：產出有安全漏洞",
  "input_schema": {
    "type": "object",
    "properties": {
      "action": {
...
```bash
# Agent 呼叫範例
result=$(sec-scan run --quiet --output json)
echo $result | jq .data
```
...
def call_sec_scan(action="run") -> dict:
    proc = subprocess.run(
        ["sec-scan", action, "--output", "json", "--quiet"],
...
```bash
# 管線串接範例
sec-scan run --quiet --output json \
  | jq -r '.data' \
  | other-tool process
```
...
# 執行測試
pytest tests/ -v --cov=sec_scan --cov-report=term-missing
...
```python
import pytest
from sec_scan.core import SecScanCore
...
*工具編號 #045 ／ 類別 C — 程式碼品質與開發輔助 ／ 計畫版本 v1.0 ／ 2026-04-27*

...
| 介面 | CLI（`click` 框架） |
| 設定檔 | `~/.config/rtl-check/config.json` |
...
dependencies = ["click", "rich", "re"]

[project.scripts]
rtl-check = "rtl_check.cli:main"
```
...
```bash
rtl-check [OPTIONS] COMMAND [ARGS]...
```
...
# 指定輸出格式
rtl-check run --output json

# 詳細模式
rtl-check run --verbose

# 靜音（適合 pipe 到其他工具）
rtl-check run --quiet | jq .

# 查看設定
rtl-check config show

# 匯出 JSON
rtl-check export --output json > result.json
```
...
def config():
    """顯示或修改設定"""
    import json, pathlib
    cfg_path = pathlib.Path.home() / ".config" / "rtl-check" / "config.json"
    if cfg_path.exists():
...
DB_PATH = pathlib.Path.home() / ".config" / "rtl-check" / "data.db"
...
{
  "name": "rtl-check",
  "description": "RTL 檢查器。解決問題：RTL 語系排版檢查",
  "input_schema": {
    "type": "object",
    "properties": {
      "action": {
...
```bash
# Agent 呼叫範例
result=$(rtl-check run --quiet --output json)
echo $result | jq .data
```
...
def call_rtl_check(action="run") -> dict:
    proc = subprocess.run(
        ["rtl-check", action, "--output", "json", "--quiet"],
...
```bash
# 管線串接範例
rtl-check run --quiet --output json \
  | jq -r '.data' \
  | other-tool process
```
...
# 執行測試
pytest tests/ -v --cov=rtl_check --cov-report=term-missing
...
```python
import pytest
from rtl_check.core import RtlCheckCore
...
```json
{
  ...
  "number": 453,
  "pain_point": "RTL 語系排版檢查",
  "commercial": "✅",
  "language": "Python",
  "entry_point": "rtl-check",
  "dependencies": ["click", "rich", "re"],
...
*工具編號 #453 ／ 類別 W — 翻譯與在地化 ／ 計畫版本 v1.0 ／ 2026-04-27*

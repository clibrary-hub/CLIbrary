...
> **Feature Flag CLI**
> 分類：Q — DevOps與基礎設施 ｜ 編號：#329 ｜ 商用：⚠️

## 問題背景

**用戶痛點**：feature flag 管理
...
| 介面 | CLI（`click` 框架） |
| 設定檔 | `~/.config/flag-cli/config.json` |
...
# 指定輸出格式
flag-cli run --output json

# 詳細模式
flag-cli run --verbose

# 靜音（適合 pipe 到其他工具）
flag-cli run --quiet | jq .

# 查看設定
flag-cli config show

# 匯出 JSON
flag-cli export --output json > result.json
```
...
def config():
    """顯示或修改設定"""
    import json, pathlib
    cfg_path = pathlib.Path.home() / ".config" / "flag-cli" / "config.json"
    if cfg_path.exists():
...
DB_PATH = pathlib.Path.home() / ".config" / "flag-cli" / "data.db"
...
{
  "name": "flag-cli",
  "description": "Feature Flag CLI。解決問題：feature flag 管理",
  "input_schema": {
    "type": "object",
    "properties": {
      "action": {
...
```bash
# Agent 呼叫範例
result=$(flag-cli run --quiet --output json)
echo $result | jq .data
```
...
def call_flag_cli(action="run") -> dict:
    proc = subprocess.run(
        ["flag-cli", action, "--output", "json", "--quiet"],
...
```bash
# 管線串接範例
flag-cli run --quiet --output json \
  | jq -r '.data' \
  | other-tool process
```
...
# 執行測試
pytest tests/ -v --cov=flag_cli --cov-report=term-missing
...
```python
import pytest
from flag_cli.core import FlagCliCore
...
```json
{
  "name": "flag-cli",
  "version": "0.1.0",
  "description": "Feature Flag CLI",
  "category": "Q",
  "category_name": "DevOps與基礎設施",
  "number": 329,
  "pain_point": "feature flag 管理",
  "commercial": "⚠️",
  "language": "Python",
  "entry_point": "flag-cli",
  "dependencies": ["click", "rich", "yaml"],
...
*工具編號 #329 ／ 類別 Q — DevOps與基礎設施 ／ 計畫版本 v1.0 ／ 2026-04-27*

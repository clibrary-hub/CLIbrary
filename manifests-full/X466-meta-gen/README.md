...
| 介面 | CLI（`click` 框架） |
| 設定檔 | `~/.config/meta-gen/config.json` |
...
# 指定輸出格式
meta-gen run --output json

# 詳細模式
meta-gen run --verbose

# 靜音（適合 pipe 到其他工具）
meta-gen run --quiet | jq .

# 查看設定
meta-gen config show

# 匯出 JSON
meta-gen export --output json > result.json
```
...
def config():
    """顯示或修改設定"""
    import json, pathlib
    cfg_path = pathlib.Path.home() / ".config" / "meta-gen" / "config.json"
    if cfg_path.exists():
...
DB_PATH = pathlib.Path.home() / ".config" / "meta-gen" / "data.db"
...
{
  "name": "meta-gen",
  "description": "Meta 標籤產生器。解決問題：SEO meta 自動產",
  "input_schema": {
    "type": "object",
    "properties": {
      "action": {
...
```bash
# Agent 呼叫範例
result=$(meta-gen run --quiet --output json)
echo $result | jq .data
```
...
def call_meta_gen(action="run") -> dict:
    proc = subprocess.run(
        ["meta-gen", action, "--output", "json", "--quiet"],
...
```bash
# 管線串接範例
meta-gen run --quiet --output json \
  | jq -r '.data' \
  | other-tool process
```
...
# 執行測試
pytest tests/ -v --cov=meta_gen --cov-report=term-missing
...
```python
import pytest
from meta_gen.core import MetaGenCore
...
```json
{
  "name": "meta-gen",
  "version": "0.1.0",
  "description": "Meta 標籤產生器",
  "category": "X",
  "category_name": "寫作與內容創作",
  "number": 466,
  "pain_point": "SEO meta 自動產",
  "commercial": "✅",
  "language": "Python",
  "entry_point": "meta-gen",
  "dependencies": ["anthropic", "click", "rich"],
...
*工具編號 #466 ／ 類別 X — 寫作與內容創作 ／ 計畫版本 v1.0 ／ 2026-04-27*

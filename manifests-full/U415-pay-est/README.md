...
| 介面 | CLI（`click` 框架） |
| 設定檔 | `~/.config/pay-est/config.json` |
...
# 指定輸出格式
pay-est run --output json

# 詳細模式
pay-est run --verbose

# 靜音（適合 pipe 到其他工具）
pay-est run --quiet | jq .

# 查看設定
pay-est config show

# 匯出 JSON
pay-est export --output json > result.json
```
...
def config():
    """顯示或修改設定"""
    import json, pathlib
    cfg_path = pathlib.Path.home() / ".config" / "pay-est" / "config.json"
    if cfg_path.exists():
...
DB_PATH = pathlib.Path.home() / ".config" / "pay-est" / "data.db"
...
{
  "name": "pay-est",
  "description": "跨幣付款試算。解決問題：跨幣別實付試算",
  "input_schema": {
    "type": "object",
    "properties": {
      "action": {
...
```bash
# Agent 呼叫範例
result=$(pay-est run --quiet --output json)
echo $result | jq .data
```
...
def call_pay_est(action="run") -> dict:
    proc = subprocess.run(
        ["pay-est", action, "--output", "json", "--quiet"],
...
# 執行測試
pytest tests/ -v --cov=pay_est --cov-report=term-missing
...
```python
import pytest
from pay_est.core import PayEstCore
...
*工具編號 #415 ／ 類別 U — 購物與訂閱管理 ／ 計畫版本 v1.0 ／ 2026-04-27*

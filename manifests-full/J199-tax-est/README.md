...
| 介面 | CLI（`click` 框架） |
| 設定檔 | `~/.config/tax-est/config.json` |
...
# 指定輸出格式
tax-est run --output json

# 詳細模式
tax-est run --verbose

# 靜音（適合 pipe 到其他工具）
tax-est run --quiet | jq .

# 查看設定
tax-est config show

# 匯出 JSON
tax-est export --output json > result.json
```
...
def config():
    """顯示或修改設定"""
    import json, pathlib
    cfg_path = pathlib.Path.home() / ".config" / "tax-est" / "config.json"
    if cfg_path.exists():
...
DB_PATH = pathlib.Path.home() / ".config" / "tax-est" / "data.db"
...
```bash
# Agent 呼叫範例
result=$(tax-est run --quiet --output json)
echo $result | jq .data
```
...
def call_tax_est(action="run") -> dict:
    proc = subprocess.run(
        ["tax-est", action, "--output", "json", "--quiet"],
...
# 執行測試
pytest tests/ -v --cov=tax_est --cov-report=term-missing
...
```python
import pytest
from tax_est.core import TaxEstCore
...
*工具編號 #199 ／ 類別 J — 財務與帳務 ／ 計畫版本 v1.0 ／ 2026-04-27*

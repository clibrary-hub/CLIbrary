...
| 介面 | CLI（`click` 框架） |
| 設定檔 | `~/.config/syntax-rep/config.json` |
...
[project.scripts]
syntax-rep = "syntax_rep.cli:main"
```
...
```bash
syntax-rep [OPTIONS] COMMAND [ARGS]...
```
...
# 指定輸出格式
syntax-rep run --output json

# 詳細模式
syntax-rep run --verbose

# 靜音（適合 pipe 到其他工具）
syntax-rep run --quiet | jq .

# 查看設定
syntax-rep config show

# 匯出 JSON
syntax-rep export --output json > result.json
```
...
from .core import SyntaxRepCore
...
def config():
    """顯示或修改設定"""
    import json, pathlib
    cfg_path = pathlib.Path.home() / ".config" / "syntax-rep" / "config.json"
    if cfg_path.exists():
...
DB_PATH = pathlib.Path.home() / ".config" / "syntax-rep" / "data.db"
...
```bash
# Agent 呼叫範例
result=$(syntax-rep run --quiet --output json)
echo $result | jq .data
```
...
def call_syntax_rep(action="run") -> dict:
    proc = subprocess.run(
        ["syntax-rep", action, "--output", "json", "--quiet"],
...
```bash
# 管線串接範例
syntax-rep run --quiet --output json \
  | jq -r '.data' \
  | other-tool process
```
...
# 執行測試
pytest tests/ -v --cov=syntax_rep --cov-report=term-missing
...
```python
import pytest
from syntax_rep.core import SyntaxRepCore
...
*工具編號 #477 ／ 類別 X — 寫作與內容創作 ／ 計畫版本 v1.0 ／ 2026-04-27*

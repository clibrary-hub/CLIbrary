...
| 介面 | CLI（`click` 框架） |
| 設定檔 | `~/.config/trip-split/config.json` |
...
[project.scripts]
trip-split = "trip_split.cli:main"
```
...
```bash
trip-split [OPTIONS] COMMAND [ARGS]...
```
...
# 指定輸出格式
trip-split run --output json

# 詳細模式
trip-split run --verbose

# 靜音（適合 pipe 到其他工具）
trip-split run --quiet | jq .

# 查看設定
trip-split config show

# 匯出 JSON
trip-split export --output json > result.json
```
...
from .core import TripSplitCore
...
def config():
    """顯示或修改設定"""
    import json, pathlib
    cfg_path = pathlib.Path.home() / ".config" / "trip-split" / "config.json"
    if cfg_path.exists():
...
DB_PATH = pathlib.Path.home() / ".config" / "trip-split" / "data.db"
...
{
  "name": "trip-split",
  "description": "行程分帳器。解決問題：多人合分行程帳",
  "input_schema": {
    "type": "object",
    "properties": {
      "action": {
...
```bash
# Agent 呼叫範例
result=$(trip-split run --quiet --output json)
echo $result | jq .data
```
...
def call_trip_split(action="run") -> dict:
    proc = subprocess.run(
        ["trip-split", action, "--output", "json", "--quiet"],
...
```bash
# 管線串接範例
trip-split run --quiet --output json \
  | jq -r '.data' \
  | other-tool process
```
...
# 執行測試
pytest tests/ -v --cov=trip_split --cov-report=term-missing
...
```python
import pytest
from trip_split.core import TripSplitCore
...
*工具編號 #397 ／ 類別 T — 旅遊與生活 ／ 計畫版本 v1.0 ／ 2026-04-27*

...
| 介面 | CLI（`click` 框架） |
| 設定檔 | `~/.config/corr-matrix/config.json` |
...
[project.scripts]
corr-matrix = "corr_matrix.cli:main"
```
...
```bash
corr-matrix [OPTIONS] COMMAND [ARGS]...
```
...
# 指定輸出格式
corr-matrix run --output json

# 詳細模式
corr-matrix run --verbose

# 靜音（適合 pipe 到其他工具）
corr-matrix run --quiet | jq .

# 查看設定
corr-matrix config show

# 匯出 JSON
corr-matrix export --output json > result.json
```
...
from .core import CorrMatrixCore
...
def config():
    """顯示或修改設定"""
    import json, pathlib
    cfg_path = pathlib.Path.home() / ".config" / "corr-matrix" / "config.json"
    if cfg_path.exists():
...
DB_PATH = pathlib.Path.home() / ".config" / "corr-matrix" / "data.db"
...
{
  "name": "corr-matrix",
  "description": "相關矩陣器。解決問題：散點/相關矩陣",
  "input_schema": {
    "type": "object",
    "properties": {
      "action": {
...
```bash
# Agent 呼叫範例
result=$(corr-matrix run --quiet --output json)
echo $result | jq .data
```
...
def call_corr_matrix(action="run") -> dict:
    proc = subprocess.run(
        ["corr-matrix", action, "--output", "json", "--quiet"],
...
```bash
# 管線串接範例
corr-matrix run --quiet --output json \
  | jq -r '.data' \
  | other-tool process
```
...
# 執行測試
pytest tests/ -v --cov=corr_matrix --cov-report=term-missing

# 快速冒煙測試
corr-matrix run --help
corr-matrix config show
```
...
```python
import pytest
from corr_matrix.core import CorrMatrixCore
...
*工具編號 #271 ／ 類別 N — 資料分析與視覺化 ／ 計畫版本 v1.0 ／ 2026-04-27*

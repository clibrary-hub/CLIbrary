...
| 介面 | CLI（`click` 框架） |
| 設定檔 | `~/.config/page-watch/config.json` |
...
[project.scripts]
page-watch = "page_watch.cli:main"
```
...
```bash
page-watch [OPTIONS] COMMAND [ARGS]...
```
...
# 指定輸出格式
page-watch run --output json

# 詳細模式
page-watch run --verbose

# 靜音（適合 pipe 到其他工具）
page-watch run --quiet | jq .

# 查看設定
page-watch config show

# 匯出 JSON
page-watch export --output json > result.json
```
...
from .core import PageWatchCore
...
def config():
    """顯示或修改設定"""
    import json, pathlib
    cfg_path = pathlib.Path.home() / ".config" / "page-watch" / "config.json"
    if cfg_path.exists():
...
DB_PATH = pathlib.Path.home() / ".config" / "page-watch" / "data.db"
...
```bash
# Agent 呼叫範例
result=$(page-watch run --quiet --output json)
echo $result | jq .data
```
...
def call_page_watch(action="run") -> dict:
    proc = subprocess.run(
        ["page-watch", action, "--output", "json", "--quiet"],
...
```bash
# 管線串接範例
page-watch run --quiet --output json \
  | jq -r '.data' \
  | other-tool process
```
...
# 執行測試
pytest tests/ -v --cov=page_watch --cov-report=term-missing
...
```python
import pytest
from page_watch.core import PageWatchCore
...
*工具編號 #243 ／ 類別 M — 網路爬蟲與監控 ／ 計畫版本 v1.0 ／ 2026-04-27*

...
| 介面 | CLI（`click` 框架） |
| 設定檔 | `~/.config/caption-gen/config.json` |
...
[project.scripts]
caption-gen = "caption_gen.cli:main"
```
...
```bash
caption-gen [OPTIONS] COMMAND [ARGS]...
```
...
# 指定輸出格式
caption-gen run --output json

# 詳細模式
caption-gen run --verbose

# 靜音（適合 pipe 到其他工具）
caption-gen run --quiet | jq .

# 查看設定
caption-gen config show

# 匯出 JSON
caption-gen export --output json > result.json
```
...
from .core import CaptionGenCore
...
def config():
    """顯示或修改設定"""
    import json, pathlib
    cfg_path = pathlib.Path.home() / ".config" / "caption-gen" / "config.json"
    if cfg_path.exists():
...
DB_PATH = pathlib.Path.home() / ".config" / "caption-gen" / "data.db"
...
{
  "name": "caption-gen",
  "description": "圖說產生器。解決問題：圖表自動產 caption",
  "input_schema": {
    "type": "object",
    "properties": {
      "action": {
...
```bash
# Agent 呼叫範例
result=$(caption-gen run --quiet --output json)
echo $result | jq .data
```
...
def call_caption_gen(action="run") -> dict:
    proc = subprocess.run(
        ["caption-gen", action, "--output", "json", "--quiet"],
...
```bash
# 管線串接範例
caption-gen run --quiet --output json \
  | jq -r '.data' \
  | other-tool process
```
...
# 執行測試
pytest tests/ -v --cov=caption_gen --cov-report=term-missing

# 快速冒煙測試
caption-gen run --help
caption-gen config show
```
...
```python
import pytest
from caption_gen.core import CaptionGenCore
...
```json
{
  ...
  "category_name": "學習與研究",
  "number": 379,
  "pain_point": "圖表自動產 caption",
  "commercial": "✅",
  "language": "Python",
  "entry_point": "caption-gen",
  "dependencies": ["anthropic", "click", "rich"],
...
*工具編號 #379 ／ 類別 S — 學習與研究 ／ 計畫版本 v1.0 ／ 2026-04-27*

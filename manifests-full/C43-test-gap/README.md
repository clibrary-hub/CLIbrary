...
| 介面 | CLI（`click` 框架） |
| 設定檔 | `~/.config/test-gap/config.json` |
...
pip install coverage click rich
...
dependencies = ["coverage", "click", "rich"]

[project.scripts]
test-gap = "test_gap.cli:main"
```
...
# 指定輸出格式
test-gap run --output json

# 詳細模式
test-gap run --verbose

# 靜音（適合 pipe 到其他工具）
test-gap run --quiet | jq .

# 查看設定
test-gap config show

# 匯出 JSON
test-gap export --output json > result.json
```
...
def config():
    """顯示或修改設定"""
    import json, pathlib
    cfg_path = pathlib.Path.home() / ".config" / "test-gap" / "config.json"
    if cfg_path.exists():
...
DB_PATH = pathlib.Path.home() / ".config" / "test-gap" / "data.db"
...
{
  "name": "test-gap",
  "description": "覆蓋率缺口分析器。解決問題：測試覆蓋率不足",
  "input_schema": {
    "type": "object",
    "properties": {
      "action": {
...
```bash
# Agent 呼叫範例
result=$(test-gap run --quiet --output json)
echo $result | jq .data
```
...
def call_test_gap(action="run") -> dict:
    proc = subprocess.run(
        ["test-gap", action, "--output", "json", "--quiet"],
...
```bash
# 管線串接範例
test-gap run --quiet --output json \
  | jq -r '.data' \
  | other-tool process
```
...
# 執行測試
pytest tests/ -v --cov=test_gap --cov-report=term-missing
...
```python
import pytest
from test_gap.core import TestGapCore
...
*工具編號 #043 ／ 類別 C — 程式碼品質與開發輔助 ／ 計畫版本 v1.0 ／ 2026-04-27*

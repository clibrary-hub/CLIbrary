...
| 介面 | CLI（`click` 框架） |
| 設定檔 | `~/.config/gift-list/config.json` |
...
```bash
gift-list [OPTIONS] COMMAND [ARGS]...
```
...
# 指定輸出格式
gift-list run --output json

# 詳細模式
gift-list run --verbose

# 靜音（適合 pipe 到其他工具）
gift-list run --quiet | jq .

# 查看設定
gift-list config show

# 匯出 JSON
gift-list export --output json > result.json
```
...
def config():
    """顯示或修改設定"""
    import json, pathlib
    cfg_path = pathlib.Path.home() / ".config" / "gift-list" / "config.json"
    if cfg_path.exists():
...
DB_PATH = pathlib.Path.home() / ".config" / "gift-list" / "data.db"
...
```bash
# Agent 呼叫範例
result=$(gift-list run --quiet --output json)
echo $result | jq .data
```
...
def call_gift_list(action="run") -> dict:
    proc = subprocess.run(
        ["gift-list", action, "--output", "json", "--quiet"],
...
```bash
# 管線串接範例
gift-list run --quiet --output json \
  | jq -r '.data' \
  | other-tool process
```
...
# 執行測試
pytest tests/ -v --cov=gift_list --cov-report=term-missing
...
```python
import pytest
from gift_list.core import GiftListCore
...
*工具編號 #410 ／ 類別 U — 購物與訂閱管理 ／ 計畫版本 v1.0 ／ 2026-04-27*

...
| 介面 | CLI（`click` 框架） |
| 設定檔 | `~/.config/invoice-gen/config.json` |
...
[project.scripts]
invoice-gen = "invoice_gen.cli:main"
```
...
```bash
invoice-gen [OPTIONS] COMMAND [ARGS]...
```
...
# 指定輸出格式
invoice-gen run --output json

# 詳細模式
invoice-gen run --verbose

# 靜音（適合 pipe 到其他工具）
invoice-gen run --quiet | jq .

# 查看設定
invoice-gen config show

# 匯出 JSON
invoice-gen export --output json > result.json
```
...
from .core import InvoiceGenCore
...
def config():
    """顯示或修改設定"""
    import json, pathlib
    cfg_path = pathlib.Path.home() / ".config" / "invoice-gen" / "config.json"
    if cfg_path.exists():
...
DB_PATH = pathlib.Path.home() / ".config" / "invoice-gen" / "data.db"
...
```bash
# Agent 呼叫範例
result=$(invoice-gen run --quiet --output json)
echo $result | jq .data
```
...
def call_invoice_gen(action="run") -> dict:
    proc = subprocess.run(
        ["invoice-gen", action, "--output", "json", "--quiet"],
...
```bash
# 管線串接範例
invoice-gen run --quiet --output json \
  | jq -r '.data' \
  | other-tool process
```
...
# 執行測試
pytest tests/ -v --cov=invoice_gen --cov-report=term-missing

# 快速冒煙測試
invoice-gen run --help
invoice-gen config show
```
...
```python
import pytest
from invoice_gen.core import InvoiceGenCore
...
*工具編號 #192 ／ 類別 J — 財務與帳務 ／ 計畫版本 v1.0 ／ 2026-04-27*

...
| 介面 | CLI（`click` 框架） |
| 設定檔 | `~/.config/pii-mask/config.json` |
...
# 指定輸出格式
pii-mask run --output json

# 詳細模式
pii-mask run --verbose

# 靜音（適合 pipe 到其他工具）
pii-mask run --quiet | jq .

# 查看設定
pii-mask config show

# 匯出 JSON
pii-mask export --output json > result.json
```
...
def config():
    """顯示或修改設定"""
    import json, pathlib
    cfg_path = pathlib.Path.home() / ".config" / "pii-mask" / "config.json"
    if cfg_path.exists():
...
DB_PATH = pathlib.Path.home() / ".config" / "pii-mask" / "data.db"
...
{
  "name": "pii-mask",
  "description": "PII 遮罩器。解決問題：隱私資料遮罩",
  "input_schema": {
    "type": "object",
    "properties": {
      "action": {
...
```bash
# Agent 呼叫範例
result=$(pii-mask run --quiet --output json)
echo $result | jq .data
```
...
def call_pii_mask(action="run") -> dict:
    proc = subprocess.run(
        ["pii-mask", action, "--output", "json", "--quiet"],
...
```bash
# 管線串接範例
pii-mask run --quiet --output json \
  | jq -r '.data' \
  | other-tool process
```
...
# 執行測試
pytest tests/ -v --cov=pii_mask --cov-report=term-missing
...
```python
import pytest
from pii_mask.core import PiiMaskCore
...
*工具編號 #306 ／ 類別 P — 安全與隱私 ／ 計畫版本 v1.0 ／ 2026-04-27*

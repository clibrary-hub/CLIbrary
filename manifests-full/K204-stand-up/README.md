...
| 介面 | CLI（`click` 框架） |
| 設定檔 | `~/.config/stand-up/config.json` |
...
# 指定輸出格式
stand-up run --output json

# 詳細模式
stand-up run --verbose

# 靜音（適合 pipe 到其他工具）
stand-up run --quiet | jq .

# 查看設定
stand-up config show

# 匯出 JSON
stand-up export --output json > result.json
```
...
def config():
    """顯示或修改設定"""
    import json, pathlib
    cfg_path = pathlib.Path.home() / ".config" / "stand-up" / "config.json"
    if cfg_path.exists():
...
DB_PATH = pathlib.Path.home() / ".config" / "stand-up" / "data.db"
...
{
  "name": "stand-up",
  "description": "起身提醒器。解決問題：久坐缺乏起身提醒",
  "input_schema": {
    "type": "object",
    "properties": {
      "action": {
...
```bash
# Agent 呼叫範例
result=$(stand-up run --quiet --output json)
echo $result | jq .data
```
...
def call_stand_up(action="run") -> dict:
    proc = subprocess.run(
        ["stand-up", action, "--output", "json", "--quiet"],
...
```bash
# 管線串接範例
stand-up run --quiet --output json \
  | jq -r '.data' \
  | other-tool process
```
...
# 執行測試
pytest tests/ -v --cov=stand_up --cov-report=term-missing
...
```python
import pytest
from stand_up.core import StandUpCore
...
*工具編號 #204 ／ 類別 K — 健康與健身 ／ 計畫版本 v1.0 ／ 2026-04-27*

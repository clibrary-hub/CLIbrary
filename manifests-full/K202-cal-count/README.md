...
| 介面 | CLI（`click` 框架） |
| 設定檔 | `~/.config/cal-count/config.json` |
...
```bash
cal-count [OPTIONS] COMMAND [ARGS]...
```
...
# 指定輸出格式
cal-count run --output json

# 詳細模式
cal-count run --verbose

# 靜音（適合 pipe 到其他工具）
cal-count run --quiet | jq .

# 查看設定
cal-count config show

# 匯出 JSON
cal-count export --output json > result.json
```
...
def config():
    """顯示或修改設定"""
    import json, pathlib
    cfg_path = pathlib.Path.home() / ".config" / "cal-count" / "config.json"
    if cfg_path.exists():
...
DB_PATH = pathlib.Path.home() / ".config" / "cal-count" / "data.db"
...
{
  "name": "cal-count",
  "description": "熱量計算器。解決問題：飲食卡路里難算",
  "input_schema": {
    "type": "object",
    "properties": {
      "action": {
...
```bash
# Agent 呼叫範例
result=$(cal-count run --quiet --output json)
echo $result | jq .data
```
...
def call_cal_count(action="run") -> dict:
    proc = subprocess.run(
        ["cal-count", action, "--output", "json", "--quiet"],
...
```bash
# 管線串接範例
cal-count run --quiet --output json \
  | jq -r '.data' \
  | other-tool process
```
...
# 執行測試
pytest tests/ -v --cov=cal_count --cov-report=term-missing
...
```python
import pytest
from cal_count.core import CalCountCore
...
*工具編號 #202 ／ 類別 K — 健康與健身 ／ 計畫版本 v1.0 ／ 2026-04-27*

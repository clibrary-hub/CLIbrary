...
| 介面 | CLI（`click` 框架） |
| 設定檔 | `~/.config/run-analyze/config.json` |
...
[project.scripts]
run-analyze = "run_analyze.cli:main"
```
...
```bash
run-analyze [OPTIONS] COMMAND [ARGS]...
```
...
# 指定輸出格式
run-analyze run --output json

# 詳細模式
run-analyze run --verbose

# 靜音（適合 pipe 到其他工具）
run-analyze run --quiet | jq .

# 查看設定
run-analyze config show

# 匯出 JSON
run-analyze export --output json > result.json
```
...
from .core import RunAnalyzeCore
...
def config():
    """顯示或修改設定"""
    import json, pathlib
    cfg_path = pathlib.Path.home() / ".config" / "run-analyze" / "config.json"
    if cfg_path.exists():
...
DB_PATH = pathlib.Path.home() / ".config" / "run-analyze" / "data.db"
...
{
  "name": "run-analyze",
  "description": "跑步分析器。解決問題：配速心率分析麻煩",
  "input_schema": {
    "type": "object",
    "properties": {
      "action": {
...
```bash
# Agent 呼叫範例
result=$(run-analyze run --quiet --output json)
echo $result | jq .data
```
...
def call_run_analyze(action="run") -> dict:
    proc = subprocess.run(
        ["run-analyze", action, "--output", "json", "--quiet"],
...
```bash
# 管線串接範例
run-analyze run --quiet --output json \
  | jq -r '.data' \
  | other-tool process
```
...
# 執行測試
pytest tests/ -v --cov=run_analyze --cov-report=term-missing

# 快速冒煙測試
run-analyze run --help
run-analyze config show
```
...
```python
import pytest
from run_analyze.core import RunAnalyzeCore
...
*工具編號 #208 ／ 類別 K — 健康與健身 ／ 計畫版本 v1.0 ／ 2026-04-27*

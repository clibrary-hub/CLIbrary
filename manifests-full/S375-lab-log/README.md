...
| 介面 | CLI（`click` 框架） |
| 設定檔 | `~/.config/lab-log/config.json` |
...
# 指定輸出格式
lab-log run --output json

# 詳細模式
lab-log run --verbose

# 靜音（適合 pipe 到其他工具）
lab-log run --quiet | jq .

# 查看設定
lab-log config show

# 匯出 JSON
lab-log export --output json > result.json
```
...
def config():
    """顯示或修改設定"""
    import json, pathlib
    cfg_path = pathlib.Path.home() / ".config" / "lab-log" / "config.json"
    if cfg_path.exists():
...
DB_PATH = pathlib.Path.home() / ".config" / "lab-log" / "data.db"
...
```bash
# Agent 呼叫範例
result=$(lab-log run --quiet --output json)
echo $result | jq .data
```
...
def call_lab_log(action="run") -> dict:
    proc = subprocess.run(
        ["lab-log", action, "--output", "json", "--quiet"],
...
# 執行測試
pytest tests/ -v --cov=lab_log --cov-report=term-missing
...
```python
import pytest
from lab_log.core import LabLogCore
...
*工具編號 #375 ／ 類別 S — 學習與研究 ／ 計畫版本 v1.0 ／ 2026-04-27*

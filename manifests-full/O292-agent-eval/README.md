...
> **Agent 評測器**
> 分類：O — AI/ML輔助 ｜ 編號：#292 ｜ 商用：✅

## 問題背景

**用戶痛點**：多 Agent benchmark
...
| 介面 | CLI（`click` 框架） |
| 設定檔 | `~/.config/agent-eval/config.json` |
...
# 指定輸出格式
agent-eval run --output json

# 詳細模式
agent-eval run --verbose

# 靜音（適合 pipe 到其他工具）
agent-eval run --quiet | jq .

# 查看設定
agent-eval config show

# 匯出 JSON
agent-eval export --output json > result.json
```
...
from .core import AgentEvalCore
...
def config():
    """顯示或修改設定"""
    import json, pathlib
    cfg_path = pathlib.Path.home() / ".config" / "agent-eval" / "config.json"
    if cfg_path.exists():
...
DB_PATH = pathlib.Path.home() / ".config" / "agent-eval" / "data.db"
...
{
  "name": "agent-eval",
  "description": "Agent 評測器。解決問題：多 Agent benchmark",
  "input_schema": {
    "type": "object",
    "properties": {
      "action": {
...
```bash
# Agent 呼叫範例
result=$(agent-eval run --quiet --output json)
echo $result | jq .data
```
...
def call_agent_eval(action="run") -> dict:
    proc = subprocess.run(
        ["agent-eval", action, "--output", "json", "--quiet"],
...
```bash
# 管線串接範例
agent-eval run --quiet --output json \
  | jq -r '.data' \
  | other-tool process
```
...
# 執行測試
pytest tests/ -v --cov=agent_eval --cov-report=term-missing
...
```python
import pytest
from agent_eval.core import AgentEvalCore
...
```json
{
  ...
  "number": 292,
  "pain_point": "多 Agent benchmark",
  "commercial": "✅",
  "language": "Python",
  "entry_point": "agent-eval",
  "dependencies": ["anthropic", "click", "rich"],
...
*工具編號 #292 ／ 類別 O — AI/ML輔助 ／ 計畫版本 v1.0 ／ 2026-04-27*

...
| 介面 | CLI（`click` 框架） |
| 設定檔 | `~/.config/csv-sql/config.json` |
...
dependencies = ["duckdb", "click", "rich"]

[project.scripts]
csv-sql = "csv_sql.cli:main"
```
...
# 指定輸出格式
csv-sql run --output json

# 詳細模式
csv-sql run --verbose

# 靜音（適合 pipe 到其他工具）
csv-sql run --quiet | jq .

# 查看設定
csv-sql config show

# 匯出 JSON
csv-sql export --output json > result.json
```
...
def config():
    """顯示或修改設定"""
    import json, pathlib
    cfg_path = pathlib.Path.home() / ".config" / "csv-sql" / "config.json"
    if cfg_path.exists():
...
DB_PATH = pathlib.Path.home() / ".config" / "csv-sql" / "data.db"
...
{
  "name": "csv-sql",
  "description": "CSV SQL 引擎。解決問題：想跑 SQL on CSV",
  "input_schema": {
    "type": "object",
    "properties": {
      "action": {
...
```bash
# Agent 呼叫範例
result=$(csv-sql run --quiet --output json)
echo $result | jq .data
```
...
def call_csv_sql(action="run") -> dict:
    proc = subprocess.run(
        ["csv-sql", action, "--output", "json", "--quiet"],
...
# 執行測試
pytest tests/ -v --cov=csv_sql --cov-report=term-missing
...
```python
import pytest
from csv_sql.core import CsvSqlCore
...
```json
{
  "name": "csv-sql",
  "version": "0.1.0",
  "description": "CSV SQL 引擎",
  "category": "N",
  "category_name": "資料分析與視覺化",
  "number": 263,
  "pain_point": "想跑 SQL on CSV",
  "commercial": "✅",
  "language": "Python",
  "entry_point": "csv-sql",
  "dependencies": ["duckdb", "click", "rich"],
...
*工具編號 #263 ／ 類別 N — 資料分析與視覺化 ／ 計畫版本 v1.0 ／ 2026-04-27*

...
| 介面 | CLI（`click` 框架） |
| 設定檔 | `~/.config/db-snap/config.json` |
...
# 指定輸出格式
db-snap run --output json

# 詳細模式
db-snap run --verbose

# 靜音（適合 pipe 到其他工具）
db-snap run --quiet | jq .

# 查看設定
db-snap config show

# 匯出 JSON
db-snap export --output json > result.json
```
...
def config():
    """顯示或修改設定"""
    import json, pathlib
    cfg_path = pathlib.Path.home() / ".config" / "db-snap" / "config.json"
    if cfg_path.exists():
...
DB_PATH = pathlib.Path.home() / ".config" / "db-snap" / "data.db"
...
{
  "name": "db-snap",
  "description": "DB 備份器。解決問題：DB 備份還原",
  "input_schema": {
    "type": "object",
    "properties": {
      "action": {
...
```bash
# Agent 呼叫範例
result=$(db-snap run --quiet --output json)
echo $result | jq .data
```
...
def call_db_snap(action="run") -> dict:
    proc = subprocess.run(
        ["db-snap", action, "--output", "json", "--quiet"],
...
# 執行測試
pytest tests/ -v --cov=db_snap --cov-report=term-missing
...
```python
import pytest
from db_snap.core import DbSnapCore
...
*工具編號 #335 ／ 類別 Q — DevOps與基礎設施 ／ 計畫版本 v1.0 ／ 2026-04-27*

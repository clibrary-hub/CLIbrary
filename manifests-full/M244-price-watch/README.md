...
| 介面 | CLI（`click` 框架） |
| 設定檔 | `~/.config/price-watch/config.json` |
...
[project.scripts]
price-watch = "price_watch.cli:main"
```
...
```bash
price-watch [OPTIONS] COMMAND [ARGS]...
```
...
# 指定輸出格式
price-watch run --output json

# 詳細模式
price-watch run --verbose

# 靜音（適合 pipe 到其他工具）
price-watch run --quiet | jq .

# 查看設定
price-watch config show

# 匯出 JSON
price-watch export --output json > result.json
```
...
from .core import PriceWatchCore
...
def config():
    """顯示或修改設定"""
    import json, pathlib
    cfg_path = pathlib.Path.home() / ".config" / "price-watch" / "config.json"
    if cfg_path.exists():
...
DB_PATH = pathlib.Path.home() / ".config" / "price-watch" / "data.db"
...
```bash
# Agent 呼叫範例
result=$(price-watch run --quiet --output json)
echo $result | jq .data
```
...
def call_price_watch(action="run") -> dict:
    proc = subprocess.run(
        ["price-watch", action, "--output", "json", "--quiet"],
...
```bash
# 管線串接範例
price-watch run --quiet --output json \
  | jq -r '.data' \
  | other-tool process
```
...
# 執行測試
pytest tests/ -v --cov=price_watch --cov-report=term-missing

# 快速冒煙測試
price-watch run --help
price-watch config show
```
...
```python
import pytest
from price_watch.core import PriceWatchCore
...
*工具編號 #244 ／ 類別 M — 網路爬蟲與監控 ／ 計畫版本 v1.0 ／ 2026-04-27*

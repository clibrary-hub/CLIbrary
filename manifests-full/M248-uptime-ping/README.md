...
| 介面 | CLI（`click` 框架） |
| 設定檔 | `~/.config/uptime-ping/config.json` |
...
[project.scripts]
uptime-ping = "uptime_ping.cli:main"
```
...
```bash
uptime-ping [OPTIONS] COMMAND [ARGS]...
```
...
# 指定輸出格式
uptime-ping run --output json

# 詳細模式
uptime-ping run --verbose

# 靜音（適合 pipe 到其他工具）
uptime-ping run --quiet | jq .

# 查看設定
uptime-ping config show

# 匯出 JSON
uptime-ping export --output json > result.json
```
...
from .core import UptimePingCore
...
def config():
    """顯示或修改設定"""
    import json, pathlib
    cfg_path = pathlib.Path.home() / ".config" / "uptime-ping" / "config.json"
    if cfg_path.exists():
...
DB_PATH = pathlib.Path.home() / ".config" / "uptime-ping" / "data.db"
...
{
  "name": "uptime-ping",
  "description": "可達性測試器。解決問題：網站可達性測試",
  "input_schema": {
    "type": "object",
    "properties": {
      "action": {
...
```bash
# Agent 呼叫範例
result=$(uptime-ping run --quiet --output json)
echo $result | jq .data
```
...
def call_uptime_ping(action="run") -> dict:
    proc = subprocess.run(
        ["uptime-ping", action, "--output", "json", "--quiet"],
...
```bash
# 管線串接範例
uptime-ping run --quiet --output json \
  | jq -r '.data' \
  | other-tool process
```
...
# 執行測試
pytest tests/ -v --cov=uptime_ping --cov-report=term-missing

# 快速冒煙測試
uptime-ping run --help
uptime-ping config show
```
...
```python
import pytest
from uptime_ping.core import UptimePingCore
...
*工具編號 #248 ／ 類別 M — 網路爬蟲與監控 ／ 計畫版本 v1.0 ／ 2026-04-27*

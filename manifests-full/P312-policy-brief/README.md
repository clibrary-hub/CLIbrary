...
| 介面 | CLI（`click` 框架） |
| 設定檔 | `~/.config/policy-brief/config.json` |
...
[project.scripts]
policy-brief = "policy_brief.cli:main"
```
...
```bash
policy-brief [OPTIONS] COMMAND [ARGS]...
```
...
# 指定輸出格式
policy-brief run --output json

# 詳細模式
policy-brief run --verbose

# 靜音（適合 pipe 到其他工具）
policy-brief run --quiet | jq .

# 查看設定
policy-brief config show

# 匯出 JSON
policy-brief export --output json > result.json
```
...
from .core import PolicyBriefCore
...
def config():
    """顯示或修改設定"""
    import json, pathlib
    cfg_path = pathlib.Path.home() / ".config" / "policy-brief" / "config.json"
    if cfg_path.exists():
...
DB_PATH = pathlib.Path.home() / ".config" / "policy-brief" / "data.db"
...
{
  "name": "policy-brief",
  "description": "隱私政策摘要。解決問題：隱私政策 NL 摘要",
  "input_schema": {
    "type": "object",
    "properties": {
      "action": {
...
```bash
# Agent 呼叫範例
result=$(policy-brief run --quiet --output json)
echo $result | jq .data
```
...
def call_policy_brief(action="run") -> dict:
    proc = subprocess.run(
        ["policy-brief", action, "--output", "json", "--quiet"],
...
```bash
# 管線串接範例
policy-brief run --quiet --output json \
  | jq -r '.data' \
  | other-tool process
```
...
# 執行測試
pytest tests/ -v --cov=policy_brief --cov-report=term-missing

# 快速冒煙測試
policy-brief run --help
policy-brief config show
```
...
```python
import pytest
from policy_brief.core import PolicyBriefCore
...
```json
{
  ...
  "category_name": "安全與隱私",
  "number": 312,
  "pain_point": "隱私政策 NL 摘要",
  "commercial": "✅",
  "language": "Python",
  "entry_point": "policy-brief",
  "dependencies": ["anthropic", "click", "rich"],
...
*工具編號 #312 ／ 類別 P — 安全與隱私 ／ 計畫版本 v1.0 ／ 2026-04-27*

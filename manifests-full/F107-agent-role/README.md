...
| 介面 | CLI（`click` 框架） |
| 設定檔 | `~/.config/agent-role/config.json` |
...
[project.scripts]
agent-role = "agent_role.cli:main"
```
...
```bash
agent-role [OPTIONS] COMMAND [ARGS]...
```
...
# 指定輸出格式
agent-role run --output json

# 詳細模式
agent-role run --verbose

# 靜音（適合 pipe 到其他工具）
agent-role run --quiet | jq .

# 查看設定
agent-role config show

# 匯出 JSON
agent-role export --output json > result.json
```
...
from .core import AgentRoleCore
...
def config():
    """顯示或修改設定"""
    import json, pathlib
    cfg_path = pathlib.Path.home() / ".config" / "agent-role" / "config.json"
    if cfg_path.exists():
...
DB_PATH = pathlib.Path.home() / ".config" / "agent-role" / "data.db"
...
{
  "name": "agent-role",
  "description": "角色定義器。解決問題：Agent 角色無形式化",
  "input_schema": {
    "type": "object",
    "properties": {
      "action": {
...
```bash
# Agent 呼叫範例
result=$(agent-role run --quiet --output json)
echo $result | jq .data
```
...
def call_agent_role(action="run") -> dict:
    proc = subprocess.run(
        ["agent-role", action, "--output", "json", "--quiet"],
...
```bash
# 管線串接範例
agent-role run --quiet --output json \
  | jq -r '.data' \
  | other-tool process
```
...
# 執行測試
pytest tests/ -v --cov=agent_role --cov-report=term-missing
...
```python
import pytest
from agent_role.core import AgentRoleCore
...
```json
{
  ...
  "number": 107,
  "pain_point": "Agent 角色無形式化",
  "commercial": "✅",
  "language": "Python",
  "entry_point": "agent-role",
  "dependencies": ["click", "rich", "pydantic"],
...
*工具編號 #107 ／ 類別 F — Agent間通訊與協作 ／ 計畫版本 v1.0 ／ 2026-04-27*

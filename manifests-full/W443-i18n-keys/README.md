...
| 介面 | CLI（`click` 框架） |
| 設定檔 | `~/.config/i18n-keys/config.json` |
...
```bash
i18n-keys [OPTIONS] COMMAND [ARGS]...
```
...
# 指定輸出格式
i18n-keys run --output json

# 詳細模式
i18n-keys run --verbose

# 靜音（適合 pipe 到其他工具）
i18n-keys run --quiet | jq .

# 查看設定
i18n-keys config show

# 匯出 JSON
i18n-keys export --output json > result.json
```
...
def config():
    """顯示或修改設定"""
    import json, pathlib
    cfg_path = pathlib.Path.home() / ".config" / "i18n-keys" / "config.json"
    if cfg_path.exists():
...
DB_PATH = pathlib.Path.home() / ".config" / "i18n-keys" / "data.db"
...
{
  "name": "i18n-keys",
  "description": "字串鍵管理器。解決問題：App 字串 i18n 管理",
  "input_schema": {
    "type": "object",
    "properties": {
      "action": {
...
```bash
# Agent 呼叫範例
result=$(i18n-keys run --quiet --output json)
echo $result | jq .data
```
...
def call_i18n_keys(action="run") -> dict:
    proc = subprocess.run(
        ["i18n-keys", action, "--output", "json", "--quiet"],
...
```bash
# 管線串接範例
i18n-keys run --quiet --output json \
  | jq -r '.data' \
  | other-tool process
```
...
# 執行測試
pytest tests/ -v --cov=i18n_keys --cov-report=term-missing
...
```python
import pytest
from i18n_keys.core import I18NKeysCore
...
```json
{
  ...
  "number": 443,
  "pain_point": "App 字串 i18n 管理",
  "commercial": "✅",
  "language": "Python",
  "entry_point": "i18n-keys",
  "dependencies": ["click", "rich", "pyyaml"],
...
*工具編號 #443 ／ 類別 W — 翻譯與在地化 ／ 計畫版本 v1.0 ／ 2026-04-27*

...
| 介面 | CLI（`click` 框架） |
| 設定檔 | `~/.config/tr-cost/config.json` |
...
# 指定輸出格式
tr-cost run --output json

# 詳細模式
tr-cost run --verbose

# 靜音（適合 pipe 到其他工具）
tr-cost run --quiet | jq .

# 查看設定
tr-cost config show

# 匯出 JSON
tr-cost export --output json > result.json
```
...
def config():
    """顯示或修改設定"""
    import json, pathlib
    cfg_path = pathlib.Path.home() / ".config" / "tr-cost" / "config.json"
    if cfg_path.exists():
...
DB_PATH = pathlib.Path.home() / ".config" / "tr-cost" / "data.db"
...
```bash
# Agent 呼叫範例
result=$(tr-cost run --quiet --output json)
echo $result | jq .data
```
...
def call_tr_cost(action="run") -> dict:
    proc = subprocess.run(
        ["tr-cost", action, "--output", "json", "--quiet"],
...
# 執行測試
pytest tests/ -v --cov=tr_cost --cov-report=term-missing
...
```python
import pytest
from tr_cost.core import TrCostCore
...
*工具編號 #456 ／ 類別 W — 翻譯與在地化 ／ 計畫版本 v1.0 ／ 2026-04-27*

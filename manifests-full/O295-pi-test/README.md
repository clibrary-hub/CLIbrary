...
| 介面 | CLI（`click` 框架） |
| 設定檔 | `~/.config/pi-test/config.json` |
...
# 指定輸出格式
pi-test run --output json

# 詳細模式
pi-test run --verbose

# 靜音（適合 pipe 到其他工具）
pi-test run --quiet | jq .

# 查看設定
pi-test config show

# 匯出 JSON
pi-test export --output json > result.json
```
...
def config():
    """顯示或修改設定"""
    import json, pathlib
    cfg_path = pathlib.Path.home() / ".config" / "pi-test" / "config.json"
    if cfg_path.exists():
...
DB_PATH = pathlib.Path.home() / ".config" / "pi-test" / "data.db"
...
{
  "name": "pi-test",
  "description": "提示注入測試器。解決問題：提示注入測試",
  "input_schema": {
    "type": "object",
    "properties": {
      "action": {
...
```bash
# Agent 呼叫範例
result=$(pi-test run --quiet --output json)
echo $result | jq .data
```
...
def call_pi_test(action="run") -> dict:
    proc = subprocess.run(
        ["pi-test", action, "--output", "json", "--quiet"],
...
# 執行測試
pytest tests/ -v --cov=pi_test --cov-report=term-missing
...
```python
import pytest
from pi_test.core import PiTestCore
...
*工具編號 #295 ／ 類別 O — AI/ML輔助 ／ 計畫版本 v1.0 ／ 2026-04-27*

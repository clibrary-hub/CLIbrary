...
| 介面 | CLI（`click` 框架） |
| 設定檔 | `~/.config/otp-collect/config.json` |
...
[project.scripts]
otp-collect = "otp_collect.cli:main"
```
...
```bash
otp-collect [OPTIONS] COMMAND [ARGS]...
```
...
# 指定輸出格式
otp-collect run --output json

# 詳細模式
otp-collect run --verbose

# 靜音（適合 pipe 到其他工具）
otp-collect run --quiet | jq .

# 查看設定
otp-collect config show

# 匯出 JSON
otp-collect export --output json > result.json
```
...
from .core import OtpCollectCore
...
def config():
    """顯示或修改設定"""
    import json, pathlib
    cfg_path = pathlib.Path.home() / ".config" / "otp-collect" / "config.json"
    if cfg_path.exists():
...
DB_PATH = pathlib.Path.home() / ".config" / "otp-collect" / "data.db"
...
{
  "name": "otp-collect",
  "description": "OTP 集中器。解決問題：SMS 驗證碼分散",
  "input_schema": {
    "type": "object",
    "properties": {
      "action": {
...
```bash
# Agent 呼叫範例
result=$(otp-collect run --quiet --output json)
echo $result | jq .data
```
...
def call_otp_collect(action="run") -> dict:
    proc = subprocess.run(
        ["otp-collect", action, "--output", "json", "--quiet"],
...
```bash
# 管線串接範例
otp-collect run --quiet --output json \
  | jq -r '.data' \
  | other-tool process
```
...
# 執行測試
pytest tests/ -v --cov=otp_collect --cov-report=term-missing

# 快速冒煙測試
otp-collect run --help
otp-collect config show
```
...
```python
import pytest
from otp_collect.core import OtpCollectCore
...
```json
{
  ...
  "category_name": "郵件與通訊",
  "number": 174,
  "pain_point": "SMS 驗證碼分散",
  "commercial": "⚠️",
  "language": "Python",
  "entry_point": "otp-collect",
  "dependencies": ["click", "rich", "sqlite3"],
...
*工具編號 #174 ／ 類別 I — 郵件與通訊 ／ 計畫版本 v1.0 ／ 2026-04-27*

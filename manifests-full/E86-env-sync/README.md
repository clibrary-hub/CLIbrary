...
| 介面 | CLI（`click` 框架） |
| 設定檔 | `~/.config/env-sync/config.json` |
...
dependencies = ["python-dotenv", "click", "rich"]

[project.scripts]
env-sync = "env_sync.cli:main"
```
...
# 指定輸出格式
env-sync run --output json

# 詳細模式
env-sync run --verbose

# 靜音（適合 pipe 到其他工具）
env-sync run --quiet | jq .

# 查看設定
env-sync config show

# 匯出 JSON
env-sync export --output json > result.json
```
...
def config():
    """顯示或修改設定"""
    import json, pathlib
    cfg_path = pathlib.Path.home() / ".config" / "env-sync" / "config.json"
    if cfg_path.exists():
...
DB_PATH = pathlib.Path.home() / ".config" / "env-sync" / "data.db"
...
class EnvSyncCore:
    ...
    核心邏輯類別。
    問題：環境變數管理混亂
    解法：透過 python-dotenv, click 實現自動化處理。
    """
...
{
  "name": "env-sync",
  "description": "環境變數同步器。解決問題：環境變數管理混亂",
  "input_schema": {
    "type": "object",
    "properties": {
      "action": {
...
```bash
# Agent 呼叫範例
result=$(env-sync run --quiet --output json)
echo $result | jq .data
```
...
def call_env_sync(action="run") -> dict:
    proc = subprocess.run(
        ["env-sync", action, "--output", "json", "--quiet"],
...
```bash
# 管線串接範例
env-sync run --quiet --output json \
  | jq -r '.data' \
  | other-tool process
```
...
# 執行測試
pytest tests/ -v --cov=env_sync --cov-report=term-missing
...
```python
import pytest
from env_sync.core import EnvSyncCore
...
*工具編號 #086 ／ 類別 E — 資料處理與整合 ／ 計畫版本 v1.0 ／ 2026-04-27*

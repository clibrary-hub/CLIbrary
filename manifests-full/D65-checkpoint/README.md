...
| 介面 | CLI（`click` 框架） |
| 設定檔 | `~/.config/checkpoint/config.json` |
...
```bash
checkpoint [OPTIONS] COMMAND [ARGS]...
```
...
# 指定輸出格式
checkpoint run --output json

# 詳細模式
checkpoint run --verbose

# 靜音（適合 pipe 到其他工具）
checkpoint run --quiet | jq .

# 查看設定
checkpoint config show

# 匯出 JSON
checkpoint export --output json > result.json
```
...
from .core import CheckpointCore
...
def config():
    """顯示或修改設定"""
    import json, pathlib
    cfg_path = pathlib.Path.home() / ".config" / "checkpoint" / "config.json"
    if cfg_path.exists():
...
DB_PATH = pathlib.Path.home() / ".config" / "checkpoint" / "data.db"
...
{
  "name": "checkpoint",
  "description": "斷點恢復器。解決問題：連續作業沒有斷點恢復",
  "input_schema": {
    "type": "object",
    "properties": {
      "action": {
...
```bash
# Agent 呼叫範例
result=$(checkpoint run --quiet --output json)
echo $result | jq .data
```
...
def call_checkpoint(action="run") -> dict:
    proc = subprocess.run(
        ["checkpoint", action, "--output", "json", "--quiet"],
...
```bash
# 管線串接範例
checkpoint run --quiet --output json \
  | jq -r '.data' \
  | other-tool process
```
...
# 執行測試
pytest tests/ -v --cov=checkpoint --cov-report=term-missing
...
```python
import pytest
from checkpoint.core import CheckpointCore
...
*工具編號 #065 ／ 類別 D — 自動化與工作流程 ／ 計畫版本 v1.0 ／ 2026-04-27*

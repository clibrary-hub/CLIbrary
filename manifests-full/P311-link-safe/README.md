...
> **連結安全檢查**
> 分類：P — 安全與隱私 ｜ 編號：#311 ｜ 商用：⚠️

## 問題背景

**用戶痛點**：防 Phishing 連結檢查
...
| 介面 | CLI（`click` 框架） |
| 設定檔 | `~/.config/link-safe/config.json` |
...
```bash
link-safe [OPTIONS] COMMAND [ARGS]...
```
...
# 指定輸出格式
link-safe run --output json

# 詳細模式
link-safe run --verbose

# 靜音（適合 pipe 到其他工具）
link-safe run --quiet | jq .

# 查看設定
link-safe config show

# 匯出 JSON
link-safe export --output json > result.json
```
...
def config():
    """顯示或修改設定"""
    import json, pathlib
    cfg_path = pathlib.Path.home() / ".config" / "link-safe" / "config.json"
    if cfg_path.exists():
...
DB_PATH = pathlib.Path.home() / ".config" / "link-safe" / "data.db"
...
{
  "name": "link-safe",
  "description": "連結安全檢查。解決問題：防 Phishing 連結檢查",
  "input_schema": {
    "type": "object",
    "properties": {
      "action": {
...
```bash
# Agent 呼叫範例
result=$(link-safe run --quiet --output json)
echo $result | jq .data
```
...
def call_link_safe(action="run") -> dict:
    proc = subprocess.run(
        ["link-safe", action, "--output", "json", "--quiet"],
...
```bash
# 管線串接範例
link-safe run --quiet --output json \
  | jq -r '.data' \
  | other-tool process
```
...
# 執行測試
pytest tests/ -v --cov=link_safe --cov-report=term-missing
...
```python
import pytest
from link_safe.core import LinkSafeCore
...
```json
{
  ...
  "category_name": "安全與隱私",
  "number": 311,
  "pain_point": "防 Phishing 連結檢查",
  "commercial": "⚠️",
  "language": "Python",
  "entry_point": "link-safe",
  "dependencies": ["requests", "click", "rich"],
...
*工具編號 #311 ／ 類別 P — 安全與隱私 ／ 計畫版本 v1.0 ／ 2026-04-27*

...
| 介面 | CLI（`click` 框架） |
| 設定檔 | `~/.config/pwn-check/config.json` |
...
```bash
pwn-check [OPTIONS] COMMAND [ARGS]...
```
...
# 指定輸出格式
pwn-check run --output json

# 詳細模式
pwn-check run --verbose

# 靜音（適合 pipe 到其他工具）
pwn-check run --quiet | jq .

# 查看設定
pwn-check config show

# 匯出 JSON
pwn-check export --output json > result.json
```
...
def config():
    """顯示或修改設定"""
    import json, pathlib
    cfg_path = pathlib.Path.home() / ".config" / "pwn-check" / "config.json"
    if cfg_path.exists():
...
DB_PATH = pathlib.Path.home() / ".config" / "pwn-check" / "data.db"
...
{
  "name": "pwn-check",
  "description": "帳密洩漏查詢。解決問題：密碼洩漏檢查",
  "input_schema": {
    "type": "object",
    "properties": {
      "action": {
...
```bash
# Agent 呼叫範例
result=$(pwn-check run --quiet --output json)
echo $result | jq .data
```
...
def call_pwn_check(action="run") -> dict:
    proc = subprocess.run(
        ["pwn-check", action, "--output", "json", "--quiet"],
...
```bash
# 管線串接範例
pwn-check run --quiet --output json \
  | jq -r '.data' \
  | other-tool process
```
...
# 執行測試
pytest tests/ -v --cov=pwn_check --cov-report=term-missing
...
```python
import pytest
from pwn_check.core import PwnCheckCore
...
*工具編號 #301 ／ 類別 P — 安全與隱私 ／ 計畫版本 v1.0 ／ 2026-04-27*

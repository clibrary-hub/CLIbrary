...
| 介面 | CLI（`click` 框架） |
| 設定檔 | `~/.config/fx-rate/config.json` |
...
# 指定輸出格式
fx-rate run --output json

# 詳細模式
fx-rate run --verbose

# 靜音（適合 pipe 到其他工具）
fx-rate run --quiet | jq .

# 查看設定
fx-rate config show

# 匯出 JSON
fx-rate export --output json > result.json
```
...
def config():
    """顯示或修改設定"""
    import json, pathlib
    cfg_path = pathlib.Path.home() / ".config" / "fx-rate" / "config.json"
    if cfg_path.exists():
...
DB_PATH = pathlib.Path.home() / ".config" / "fx-rate" / "data.db"
...
```bash
# Agent 呼叫範例
result=$(fx-rate run --quiet --output json)
echo $result | jq .data
```
...
def call_fx_rate(action="run") -> dict:
    proc = subprocess.run(
        ["fx-rate", action, "--output", "json", "--quiet"],
...
# 執行測試
pytest tests/ -v --cov=fx_rate --cov-report=term-missing
...
```python
import pytest
from fx_rate.core import FxRateCore
...
*工具編號 #185 ／ 類別 J — 財務與帳務 ／ 計畫版本 v1.0 ／ 2026-04-27*

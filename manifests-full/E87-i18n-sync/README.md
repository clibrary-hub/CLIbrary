...
| 介面 | CLI（`click` 框架） |
| 設定檔 | `~/.config/i18n-sync/config.json` |
...
```bash
i18n-sync [OPTIONS] COMMAND [ARGS]...
```
...
# 指定輸出格式
i18n-sync run --output json

# 詳細模式
i18n-sync run --verbose

# 靜音（適合 pipe 到其他工具）
i18n-sync run --quiet | jq .

# 查看設定
i18n-sync config show

# 匯出 JSON
i18n-sync export --output json > result.json
```
...
def config():
    """顯示或修改設定"""
    import json, pathlib
    cfg_path = pathlib.Path.home() / ".config" / "i18n-sync" / "config.json"
    if cfg_path.exists():
...
DB_PATH = pathlib.Path.home() / ".config" / "i18n-sync" / "data.db"
...
{
  "name": "i18n-sync",
  "description": "翻譯同步器。解決問題：多語翻譯檔難維護",
  "input_schema": {
    "type": "object",
    "properties": {
      "action": {
...
```bash
# Agent 呼叫範例
result=$(i18n-sync run --quiet --output json)
echo $result | jq .data
```
...
def call_i18n_sync(action="run") -> dict:
    proc = subprocess.run(
        ["i18n-sync", action, "--output", "json", "--quiet"],
...
```bash
# 管線串接範例
i18n-sync run --quiet --output json \
  | jq -r '.data' \
  | other-tool process
```
...
# 執行測試
pytest tests/ -v --cov=i18n_sync --cov-report=term-missing
...
```python
import pytest
from i18n_sync.core import I18NSyncCore
...
*工具編號 #087 ／ 類別 E — 資料處理與整合 ／ 計畫版本 v1.0 ／ 2026-04-27*

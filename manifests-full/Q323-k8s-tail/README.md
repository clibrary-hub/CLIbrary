...
| 介面 | CLI（`click` 框架） |
| 設定檔 | `~/.config/k8s-tail/config.json` |
...
pip install kubernetes click rich
...
dependencies = ["kubernetes", "click", "rich"]

[project.scripts]
k8s-tail = "k8s_tail.cli:main"
```
...
# 指定輸出格式
k8s-tail run --output json

# 詳細模式
k8s-tail run --verbose

# 靜音（適合 pipe 到其他工具）
k8s-tail run --quiet | jq .

# 查看設定
k8s-tail config show

# 匯出 JSON
k8s-tail export --output json > result.json
```
...
def config():
    """顯示或修改設定"""
    import json, pathlib
    cfg_path = pathlib.Path.home() / ".config" / "k8s-tail" / "config.json"
    if cfg_path.exists():
...
DB_PATH = pathlib.Path.home() / ".config" / "k8s-tail" / "data.db"
...
class K8STailCore:
    ...
    核心邏輯類別。
    問題：k8s log tail
    解法：透過 kubernetes, click 實現自動化處理。
    """
...
{
  "name": "k8s-tail",
  "description": "k8s 多 Pod 日誌。解決問題：k8s log tail",
  "input_schema": {
    "type": "object",
    "properties": {
      "action": {
...
```bash
# Agent 呼叫範例
result=$(k8s-tail run --quiet --output json)
echo $result | jq .data
```
...
def call_k8s_tail(action="run") -> dict:
    proc = subprocess.run(
        ["k8s-tail", action, "--output", "json", "--quiet"],
...
```bash
# 管線串接範例
k8s-tail run --quiet --output json \
  | jq -r '.data' \
  | other-tool process
```
...
# 執行測試
pytest tests/ -v --cov=k8s_tail --cov-report=term-missing
...
```python
import pytest
from k8s_tail.core import K8STailCore
...
```json
{
  "name": "k8s-tail",
  "version": "0.1.0",
  "description": "k8s 多 Pod 日誌",
  "category": "Q",
  "category_name": "DevOps與基礎設施",
  "number": 323,
  "pain_point": "k8s log tail",
  "commercial": "✅",
  "language": "Python",
  "entry_point": "k8s-tail",
  "dependencies": ["kubernetes", "click", "rich"],
...
*工具編號 #323 ／ 類別 Q — DevOps與基礎設施 ／ 計畫版本 v1.0 ／ 2026-04-27*

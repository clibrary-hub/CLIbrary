...
| 介面 | CLI（`click` 框架） |
| 設定檔 | `~/.config/sub-share/config.json` |
...
```bash
sub-share [OPTIONS] COMMAND [ARGS]...
```
...
# 指定輸出格式
sub-share run --output json

# 詳細模式
sub-share run --verbose

# 靜音（適合 pipe 到其他工具）
sub-share run --quiet | jq .

# 查看設定
sub-share config show

# 匯出 JSON
sub-share export --output json > result.json
```
...
def config():
    """顯示或修改設定"""
    import json, pathlib
    cfg_path = pathlib.Path.home() / ".config" / "sub-share" / "config.json"
    if cfg_path.exists():
...
DB_PATH = pathlib.Path.home() / ".config" / "sub-share" / "data.db"
...
```bash
# Agent 呼叫範例
result=$(sub-share run --quiet --output json)
echo $result | jq .data
```
...
def call_sub_share(action="run") -> dict:
    proc = subprocess.run(
        ["sub-share", action, "--output", "json", "--quiet"],
...
```bash
# 管線串接範例
sub-share run --quiet --output json \
  | jq -r '.data' \
  | other-tool process
```
...
# 執行測試
pytest tests/ -v --cov=sub_share --cov-report=term-missing
...
```python
import pytest
from sub_share.core import SubShareCore
...
*工具編號 #420 ／ 類別 U — 購物與訂閱管理 ／ 計畫版本 v1.0 ／ 2026-04-27*

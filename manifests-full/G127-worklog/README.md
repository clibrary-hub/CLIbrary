...
| 介面 | CLI（`click` 框架） |
| 設定檔 | `~/.config/worklog/config.json` |
...
pip install sqlite3 gitpython rich
...
dependencies = ["sqlite3", "gitpython", "rich"]
...
# 指定輸出格式
worklog run --output json

# 詳細模式
worklog run --verbose

# 靜音（適合 pipe 到其他工具）
worklog run --quiet | jq .

# 查看設定
worklog config show

# 匯出 JSON
worklog export --output json > result.json
```
...
def config():
    """顯示或修改設定"""
    import json, pathlib
    cfg_path = pathlib.Path.home() / ".config" / "worklog" / "config.json"
    if cfg_path.exists():
...
DB_PATH = pathlib.Path.home() / ".config" / "worklog" / "data.db"
...
class WorklogCore:
    ...
    核心邏輯類別。
    問題：找不到上週做了什麼
    解法：透過 sqlite3, gitpython 實現自動化處理。
    """
...
{
  "name": "worklog",
  "description": "自動工時日誌。解決問題：找不到上週做了什麼",
  "input_schema": {
    "type": "object",
    "properties": {
      "action": {
...
```bash
# Agent 呼叫範例
result=$(worklog run --quiet --output json)
echo $result | jq .data
```
...
def call_worklog(action="run") -> dict:
    proc = subprocess.run(
        ["worklog", action, "--output", "json", "--quiet"],
...
```python
import pytest
from worklog.core import WorklogCore
...
*工具編號 #127 ／ 類別 G — 個人生產力 ／ 計畫版本 v1.0 ／ 2026-04-27*

...
| 介面 | CLI（`click` 框架） |
| 設定檔 | `~/.config/pre-check/config.json` |
...
```bash
pre-check [OPTIONS] COMMAND [ARGS]...
```
...
# 指定輸出格式
pre-check run --output json

# 詳細模式
pre-check run --verbose

# 靜音（適合 pipe 到其他工具）
pre-check run --quiet | jq .

# 查看設定
pre-check config show

# 匯出 JSON
pre-check export --output json > result.json
```
...
def config():
    """顯示或修改設定"""
    import json, pathlib
    cfg_path = pathlib.Path.home() / ".config" / "pre-check" / "config.json"
    if cfg_path.exists():
...
DB_PATH = pathlib.Path.home() / ".config" / "pre-check" / "data.db"
...
{
  "name": "pre-check",
  "description": "環境前置檢查器。解決問題：前置條件檢查繁瑣",
  "input_schema": {
    "type": "object",
    "properties": {
      "action": {
...
```bash
# Agent 呼叫範例
result=$(pre-check run --quiet --output json)
echo $result | jq .data
```
...
def call_pre_check(action="run") -> dict:
    proc = subprocess.run(
        ["pre-check", action, "--output", "json", "--quiet"],
...
```bash
# 管線串接範例
pre-check run --quiet --output json \
  | jq -r '.data' \
  | other-tool process
```
...
# 執行測試
pytest tests/ -v --cov=pre_check --cov-report=term-missing
...
```python
import pytest
from pre_check.core import PreCheckCore
...
*工具編號 #064 ／ 類別 D — 自動化與工作流程 ／ 計畫版本 v1.0 ／ 2026-04-27*

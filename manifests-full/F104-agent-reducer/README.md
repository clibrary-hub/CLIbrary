...
| 介面 | CLI（`click` 框架） |
| 設定檔 | `~/.config/agent-reducer/config.json` |
...
[project.scripts]
agent-reducer = "agent_reducer.cli:main"
```
...
```bash
agent-reducer [OPTIONS] COMMAND [ARGS]...
```
...
# 指定輸出格式
agent-reducer run --output json

# 詳細模式
agent-reducer run --verbose

# 靜音（適合 pipe 到其他工具）
agent-reducer run --quiet | jq .

# 查看設定
agent-reducer config show

# 匯出 JSON
agent-reducer export --output json > result.json
```
...
### `agent_reducer/cli.py`

```python
import click
from rich.console import Console
from .core import AgentReducerCore
...
def config():
    """顯示或修改設定"""
    import json, pathlib
    cfg_path = pathlib.Path.home() / ".config" / "agent-reducer" / "config.json"
    if cfg_path.exists():
...
DB_PATH = pathlib.Path.home() / ".config" / "agent-reducer" / "data.db"
...
{
  "name": "agent-reducer",
  "description": "結果聚合器。解決問題：Agent 結果難以聚合",
  "input_schema": {
    "type": "object",
    "properties": {
      "action": {
...
```bash
# Agent 呼叫範例
result=$(agent-reducer run --quiet --output json)
echo $result | jq .data
```
...
def call_agent_reducer(action="run") -> dict:
    proc = subprocess.run(
        ["agent-reducer", action, "--output", "json", "--quiet"],
...
```bash
# 管線串接範例
agent-reducer run --quiet --output json \
  | jq -r '.data' \
  | other-tool process
```
...
# 執行測試
pytest tests/ -v --cov=agent_reducer --cov-report=term-missing

# 快速冒煙測試
agent-reducer run --help
agent-reducer config show
```
...
```python
import pytest
from agent_reducer.core import AgentReducerCore
...
```json
{
  ...
  "number": 104,
  "pain_point": "Agent 結果難以聚合",
  "commercial": "✅",
  "language": "Python",
  "entry_point": "agent-reducer",
  "dependencies": ["click", "rich", "pydantic"],
...
*工具編號 #104 ／ 類別 F — Agent間通訊與協作 ／ 計畫版本 v1.0 ／ 2026-04-27*

...
> **Log Shipping 設定器**
> 分類：Q — DevOps與基礎設施 ｜ 編號：#336 ｜ 商用：✅

## 問題背景

**用戶痛點**：log shipping 設定
...
| 介面 | CLI（`click` 框架） |
| 設定檔 | `~/.config/log-ship/config.json` |
...
# 指定輸出格式
log-ship run --output json

# 詳細模式
log-ship run --verbose

# 靜音（適合 pipe 到其他工具）
log-ship run --quiet | jq .

# 查看設定
log-ship config show

# 匯出 JSON
log-ship export --output json > result.json
```
...
def config():
    """顯示或修改設定"""
    import json, pathlib
    cfg_path = pathlib.Path.home() / ".config" / "log-ship" / "config.json"
    if cfg_path.exists():
...
DB_PATH = pathlib.Path.home() / ".config" / "log-ship" / "data.db"
...
{
  "name": "log-ship",
  "description": "Log Shipping 設定器。解決問題：log shipping 設定",
  "input_schema": {
    "type": "object",
    "properties": {
      "action": {
...
```bash
# Agent 呼叫範例
result=$(log-ship run --quiet --output json)
echo $result | jq .data
```
...
def call_log_ship(action="run") -> dict:
    proc = subprocess.run(
        ["log-ship", action, "--output", "json", "--quiet"],
...
```bash
# 管線串接範例
log-ship run --quiet --output json \
  | jq -r '.data' \
  | other-tool process
```
...
# 執行測試
pytest tests/ -v --cov=log_ship --cov-report=term-missing
...
```python
import pytest
from log_ship.core import LogShipCore
...
```json
{
  "name": "log-ship",
  "version": "0.1.0",
  "description": "Log Shipping 設定器",
  "category": "Q",
  "category_name": "DevOps與基礎設施",
  "number": 336,
  "pain_point": "log shipping 設定",
  "commercial": "✅",
  "language": "Python",
  "entry_point": "log-ship",
  "dependencies": ["click", "rich", "yaml"],
...
*工具編號 #336 ／ 類別 Q — DevOps與基礎設施 ／ 計畫版本 v1.0 ／ 2026-04-27*

...
| 介面 | CLI（`click` 框架） |
| 設定檔 | `~/.config/po-flow/config.json` |
...
# 指定輸出格式
po-flow run --output json

# 詳細模式
po-flow run --verbose

# 靜音（適合 pipe 到其他工具）
po-flow run --quiet | jq .

# 查看設定
po-flow config show

# 匯出 JSON
po-flow export --output json > result.json
```
...
def config():
    """顯示或修改設定"""
    import json, pathlib
    cfg_path = pathlib.Path.home() / ".config" / "po-flow" / "config.json"
    if cfg_path.exists():
...
DB_PATH = pathlib.Path.home() / ".config" / "po-flow" / "data.db"
...
```bash
# Agent 呼叫範例
result=$(po-flow run --quiet --output json)
echo $result | jq .data
```
...
def call_po_flow(action="run") -> dict:
    proc = subprocess.run(
        ["po-flow", action, "--output", "json", "--quiet"],
...
# 執行測試
pytest tests/ -v --cov=po_flow --cov-report=term-missing
...
```python
import pytest
from po_flow.core import PoFlowCore
...
*工具編號 #491 ／ 類別 Y — 法律合約商務 ／ 計畫版本 v1.0 ／ 2026-04-27*

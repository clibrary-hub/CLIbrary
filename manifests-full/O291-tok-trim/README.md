...
| 介面 | CLI（`click` 框架） |
| 設定檔 | `~/.config/tok-trim/config.json` |
...
# 指定輸出格式
tok-trim run --output json

# 詳細模式
tok-trim run --verbose

# 靜音（適合 pipe 到其他工具）
tok-trim run --quiet | jq .

# 查看設定
tok-trim config show

# 匯出 JSON
tok-trim export --output json > result.json
```
...
def config():
    """顯示或修改設定"""
    import json, pathlib
    cfg_path = pathlib.Path.home() / ".config" / "tok-trim" / "config.json"
    if cfg_path.exists():
...
DB_PATH = pathlib.Path.home() / ".config" / "tok-trim" / "data.db"
...
{
  "name": "tok-trim",
  "description": "Token 截斷器。解決問題：Token 截斷器",
  "input_schema": {
    "type": "object",
    "properties": {
      "action": {
...
```bash
# Agent 呼叫範例
result=$(tok-trim run --quiet --output json)
echo $result | jq .data
```
...
def call_tok_trim(action="run") -> dict:
    proc = subprocess.run(
        ["tok-trim", action, "--output", "json", "--quiet"],
...
```bash
# 管線串接範例
tok-trim run --quiet --output json \
  | jq -r '.data' \
  | other-tool process
```
...
# 執行測試
pytest tests/ -v --cov=tok_trim --cov-report=term-missing
...
```python
import pytest
from tok_trim.core import TokTrimCore
...
```json
{
  "name": "tok-trim",
  "version": "0.1.0",
  "description": "Token 截斷器",
  "category": "O",
  "category_name": "AI/ML輔助",
  "number": 291,
  "pain_point": "Token 截斷器",
  "commercial": "✅",
  "language": "Python",
  "entry_point": "tok-trim",
  "dependencies": ["tiktoken", "click", "rich"],
...
*工具編號 #291 ／ 類別 O — AI/ML輔助 ／ 計畫版本 v1.0 ／ 2026-04-27*

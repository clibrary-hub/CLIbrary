...
| 介面 | CLI（`click` 框架） |
| 設定檔 | `~/.config/todo-merge/config.json` |
...
[project.scripts]
todo-merge = "todo_merge.cli:main"
```
...
```bash
todo-merge [OPTIONS] COMMAND [ARGS]...
```
...
# 指定輸出格式
todo-merge run --output json

# 詳細模式
todo-merge run --verbose

# 靜音（適合 pipe 到其他工具）
todo-merge run --quiet | jq .

# 查看設定
todo-merge config show

# 匯出 JSON
todo-merge export --output json > result.json
```
...
from .core import TodoMergeCore
...
def config():
    """顯示或修改設定"""
    import json, pathlib
    cfg_path = pathlib.Path.home() / ".config" / "todo-merge" / "config.json"
    if cfg_path.exists():
...
DB_PATH = pathlib.Path.home() / ".config" / "todo-merge" / "data.db"
...
{
  "name": "todo-merge",
  "description": "待辦集中器。解決問題：待辦事項散落多處",
  "input_schema": {
    "type": "object",
    "properties": {
      "action": {
...
```bash
# Agent 呼叫範例
result=$(todo-merge run --quiet --output json)
echo $result | jq .data
```
...
def call_todo_merge(action="run") -> dict:
    proc = subprocess.run(
        ["todo-merge", action, "--output", "json", "--quiet"],
...
```bash
# 管線串接範例
todo-merge run --quiet --output json \
  | jq -r '.data' \
  | other-tool process
```
...
# 執行測試
pytest tests/ -v --cov=todo_merge --cov-report=term-missing
...
```python
import pytest
from todo_merge.core import TodoMergeCore
...
*工具編號 #121 ／ 類別 G — 個人生產力 ／ 計畫版本 v1.0 ／ 2026-04-27*

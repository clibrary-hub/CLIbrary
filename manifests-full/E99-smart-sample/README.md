...
| 介面 | CLI（`click` 框架） |
| 設定檔 | `~/.config/smart-sample/config.json` |
...
[project.scripts]
smart-sample = "smart_sample.cli:main"
```
...
```bash
smart-sample [OPTIONS] COMMAND [ARGS]...
```
...
# 指定輸出格式
smart-sample run --output json

# 詳細模式
smart-sample run --verbose

# 靜音（適合 pipe 到其他工具）
smart-sample run --quiet | jq .

# 查看設定
smart-sample config show

# 匯出 JSON
smart-sample export --output json > result.json
```
...
from .core import SmartSampleCore
...
def config():
    """顯示或修改設定"""
    import json, pathlib
    cfg_path = pathlib.Path.home() / ".config" / "smart-sample" / "config.json"
    if cfg_path.exists():
...
DB_PATH = pathlib.Path.home() / ".config" / "smart-sample" / "data.db"
...
{
  "name": "smart-sample",
  "description": "智慧抽樣器。解決問題：樣本過大需抽樣",
  "input_schema": {
    "type": "object",
    "properties": {
      "action": {
...
```bash
# Agent 呼叫範例
result=$(smart-sample run --quiet --output json)
echo $result | jq .data
```
...
def call_smart_sample(action="run") -> dict:
    proc = subprocess.run(
        ["smart-sample", action, "--output", "json", "--quiet"],
...
```bash
# 管線串接範例
smart-sample run --quiet --output json \
  | jq -r '.data' \
  | other-tool process
```
...
# 執行測試
pytest tests/ -v --cov=smart_sample --cov-report=term-missing

# 快速冒煙測試
smart-sample run --help
smart-sample config show
```
...
```python
import pytest
from smart_sample.core import SmartSampleCore
...
*工具編號 #099 ／ 類別 E — 資料處理與整合 ／ 計畫版本 v1.0 ／ 2026-04-27*

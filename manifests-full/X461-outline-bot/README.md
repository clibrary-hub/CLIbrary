...
| 介面 | CLI（`click` 框架） |
| 設定檔 | `~/.config/outline-bot/config.json` |
...
[project.scripts]
outline-bot = "outline_bot.cli:main"
```
...
```bash
outline-bot [OPTIONS] COMMAND [ARGS]...
```
...
# 指定輸出格式
outline-bot run --output json

# 詳細模式
outline-bot run --verbose

# 靜音（適合 pipe 到其他工具）
outline-bot run --quiet | jq .

# 查看設定
outline-bot config show

# 匯出 JSON
outline-bot export --output json > result.json
```
...
from .core import OutlineBotCore
...
def config():
    """顯示或修改設定"""
    import json, pathlib
    cfg_path = pathlib.Path.home() / ".config" / "outline-bot" / "config.json"
    if cfg_path.exists():
...
DB_PATH = pathlib.Path.home() / ".config" / "outline-bot" / "data.db"
...
```bash
# Agent 呼叫範例
result=$(outline-bot run --quiet --output json)
echo $result | jq .data
```
...
def call_outline_bot(action="run") -> dict:
    proc = subprocess.run(
        ["outline-bot", action, "--output", "json", "--quiet"],
...
```bash
# 管線串接範例
outline-bot run --quiet --output json \
  | jq -r '.data' \
  | other-tool process
```
...
# 執行測試
pytest tests/ -v --cov=outline_bot --cov-report=term-missing

# 快速冒煙測試
outline-bot run --help
outline-bot config show
```
...
```python
import pytest
from outline_bot.core import OutlineBotCore
...
*工具編號 #461 ／ 類別 X — 寫作與內容創作 ／ 計畫版本 v1.0 ／ 2026-04-27*

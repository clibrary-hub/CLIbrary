...
| 介面 | CLI（`click` 框架） |
| 設定檔 | `~/.config/perf-audit/config.json` |
...
```bash
perf-audit [OPTIONS] COMMAND [ARGS]...
```
...
# 指定輸出格式
perf-audit run --output json

# 詳細模式
perf-audit run --verbose

# 靜音（適合 pipe 到其他工具）
perf-audit run --quiet | jq .

# 查看設定
perf-audit config show

# 匯出 JSON
perf-audit export --output json > result.json
```
...
from .core import PerfAuditCore
...
def config():
    """顯示或修改設定"""
    import json, pathlib
    cfg_path = pathlib.Path.home() / ".config" / "perf-audit" / "config.json"
    if cfg_path.exists():
...
DB_PATH = pathlib.Path.home() / ".config" / "perf-audit" / "data.db"
...
```bash
# Agent 呼叫範例
result=$(perf-audit run --quiet --output json)
echo $result | jq .data
```
...
def call_perf_audit(action="run") -> dict:
    proc = subprocess.run(
        ["perf-audit", action, "--output", "json", "--quiet"],
...
```bash
# 管線串接範例
perf-audit run --quiet --output json \
  | jq -r '.data' \
  | other-tool process
```
...
# 執行測試
pytest tests/ -v --cov=perf_audit --cov-report=term-missing
...
```python
import pytest
from perf_audit.core import PerfAuditCore
...
*工具編號 #250 ／ 類別 M — 網路爬蟲與監控 ／ 計畫版本 v1.0 ／ 2026-04-27*

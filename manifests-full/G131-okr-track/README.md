...
| 介面 | CLI（`click` 框架） |
| 設定檔 | `~/.config/okr-track/config.json` |
...
```bash
okr-track [OPTIONS] COMMAND [ARGS]...
```
...
# 指定輸出格式
okr-track run --output json

# 詳細模式
okr-track run --verbose

# 靜音（適合 pipe 到其他工具）
okr-track run --quiet | jq .

# 查看設定
okr-track config show

# 匯出 JSON
okr-track export --output json > result.json
```
...
def config():
    """顯示或修改設定"""
    import json, pathlib
    cfg_path = pathlib.Path.home() / ".config" / "okr-track" / "config.json"
    if cfg_path.exists():
...
DB_PATH = pathlib.Path.home() / ".config" / "okr-track" / "data.db"
...
{
  "name": "okr-track",
  "description": "OKR 追蹤器。解決問題：OKR 進度難追蹤",
  "input_schema": {
    "type": "object",
    "properties": {
      "action": {
...
```bash
# Agent 呼叫範例
result=$(okr-track run --quiet --output json)
echo $result | jq .data
```
...
def call_okr_track(action="run") -> dict:
    proc = subprocess.run(
        ["okr-track", action, "--output", "json", "--quiet"],
...
```bash
# 管線串接範例
okr-track run --quiet --output json \
  | jq -r '.data' \
  | other-tool process
```
...
# 執行測試
pytest tests/ -v --cov=okr_track --cov-report=term-missing
...
```python
import pytest
from okr_track.core import OkrTrackCore
...
```json
{
  ...
  "category_name": "個人生產力",
  "number": 131,
  "pain_point": "OKR 進度難追蹤",
  "commercial": "✅",
  "language": "Python",
  "entry_point": "okr-track",
  "dependencies": ["sqlite3", "rich", "click"],
...
*工具編號 #131 ／ 類別 G — 個人生產力 ／ 計畫版本 v1.0 ／ 2026-04-27*

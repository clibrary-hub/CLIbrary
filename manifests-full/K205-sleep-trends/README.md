...
| 介面 | CLI（`click` 框架） |
| 設定檔 | `~/.config/sleep-trends/config.json` |
...
[project.scripts]
sleep-trends = "sleep_trends.cli:main"
```
...
```bash
sleep-trends [OPTIONS] COMMAND [ARGS]...
```
...
# 指定輸出格式
sleep-trends run --output json

# 詳細模式
sleep-trends run --verbose

# 靜音（適合 pipe 到其他工具）
sleep-trends run --quiet | jq .

# 查看設定
sleep-trends config show

# 匯出 JSON
sleep-trends export --output json > result.json
```
...
from .core import SleepTrendsCore
...
def config():
    """顯示或修改設定"""
    import json, pathlib
    cfg_path = pathlib.Path.home() / ".config" / "sleep-trends" / "config.json"
    if cfg_path.exists():
...
DB_PATH = pathlib.Path.home() / ".config" / "sleep-trends" / "data.db"
...
{
  "name": "sleep-trends",
  "description": "睡眠趨勢分析。解決問題：睡眠資料無統合",
  "input_schema": {
    "type": "object",
    "properties": {
      "action": {
...
```bash
# Agent 呼叫範例
result=$(sleep-trends run --quiet --output json)
echo $result | jq .data
```
...
def call_sleep_trends(action="run") -> dict:
    proc = subprocess.run(
        ["sleep-trends", action, "--output", "json", "--quiet"],
...
```bash
# 管線串接範例
sleep-trends run --quiet --output json \
  | jq -r '.data' \
  | other-tool process
```
...
# 執行測試
pytest tests/ -v --cov=sleep_trends --cov-report=term-missing

# 快速冒煙測試
sleep-trends run --help
sleep-trends config show
```
...
```python
import pytest
from sleep_trends.core import SleepTrendsCore
...
*工具編號 #205 ／ 類別 K — 健康與健身 ／ 計畫版本 v1.0 ／ 2026-04-27*

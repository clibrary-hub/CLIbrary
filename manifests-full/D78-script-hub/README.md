...
| 介面 | CLI（`click` 框架） |
| 設定檔 | `~/.config/script-hub/config.json` |
...
[project.scripts]
script-hub = "script_hub.cli:main"
```
...
```bash
script-hub [OPTIONS] COMMAND [ARGS]...
```
...
# 指定輸出格式
script-hub run --output json

# 詳細模式
script-hub run --verbose

# 靜音（適合 pipe 到其他工具）
script-hub run --quiet | jq .

# 查看設定
script-hub config show

# 匯出 JSON
script-hub export --output json > result.json
```
...
from .core import ScriptHubCore
...
def config():
    """顯示或修改設定"""
    import json, pathlib
    cfg_path = pathlib.Path.home() / ".config" / "script-hub" / "config.json"
    if cfg_path.exists():
...
DB_PATH = pathlib.Path.home() / ".config" / "script-hub" / "data.db"
...
{
  "name": "script-hub",
  "description": "腳本中央倉。解決問題：自動化腳本到處散落",
  "input_schema": {
    "type": "object",
    "properties": {
      "action": {
...
```bash
# Agent 呼叫範例
result=$(script-hub run --quiet --output json)
echo $result | jq .data
```
...
def call_script_hub(action="run") -> dict:
    proc = subprocess.run(
        ["script-hub", action, "--output", "json", "--quiet"],
...
```bash
# 管線串接範例
script-hub run --quiet --output json \
  | jq -r '.data' \
  | other-tool process
```
...
# 執行測試
pytest tests/ -v --cov=script_hub --cov-report=term-missing
...
```python
import pytest
from script_hub.core import ScriptHubCore
...
```json
{
  ...
  "number": 78,
  "pain_point": "自動化腳本到處散落",
  "commercial": "✅",
  "language": "Python",
  "entry_point": "script-hub",
  "dependencies": ["click", "rich", "sqlite3"],
...
*工具編號 #078 ／ 類別 D — 自動化與工作流程 ／ 計畫版本 v1.0 ／ 2026-04-27*

...
| 介面 | CLI（`click` 框架） |
| 設定檔 | `~/.config/chart-title/config.json` |
...
[project.scripts]
chart-title = "chart_title.cli:main"
```
...
```bash
chart-title [OPTIONS] COMMAND [ARGS]...
```
...
# 指定輸出格式
chart-title run --output json

# 詳細模式
chart-title run --verbose

# 靜音（適合 pipe 到其他工具）
chart-title run --quiet | jq .

# 查看設定
chart-title config show

# 匯出 JSON
chart-title export --output json > result.json
```
...
from .core import ChartTitleCore
...
def config():
    """顯示或修改設定"""
    import json, pathlib
    cfg_path = pathlib.Path.home() / ".config" / "chart-title" / "config.json"
    if cfg_path.exists():
...
DB_PATH = pathlib.Path.home() / ".config" / "chart-title" / "data.db"
...
{
  "name": "chart-title",
  "description": "圖表標題器。解決問題：自動產生圖表標題",
  "input_schema": {
    "type": "object",
    "properties": {
      "action": {
...
```bash
# Agent 呼叫範例
result=$(chart-title run --quiet --output json)
echo $result | jq .data
```
...
def call_chart_title(action="run") -> dict:
    proc = subprocess.run(
        ["chart-title", action, "--output", "json", "--quiet"],
...
```bash
# 管線串接範例
chart-title run --quiet --output json \
  | jq -r '.data' \
  | other-tool process
```
...
# 執行測試
pytest tests/ -v --cov=chart_title --cov-report=term-missing

# 快速冒煙測試
chart-title run --help
chart-title config show
```
...
```python
import pytest
from chart_title.core import ChartTitleCore
...
*工具編號 #272 ／ 類別 N — 資料分析與視覺化 ／ 計畫版本 v1.0 ／ 2026-04-27*

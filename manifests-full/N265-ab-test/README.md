...
| 介面 | CLI（`click` 框架） |
| 設定檔 | `~/.config/ab-test/config.json` |
...
dependencies = ["scipy", "click", "rich"]

[project.scripts]
ab-test = "ab_test.cli:main"
```
...
# 指定輸出格式
ab-test run --output json

# 詳細模式
ab-test run --verbose

# 靜音（適合 pipe 到其他工具）
ab-test run --quiet | jq .

# 查看設定
ab-test config show

# 匯出 JSON
ab-test export --output json > result.json
```
...
def config():
    """顯示或修改設定"""
    import json, pathlib
    cfg_path = pathlib.Path.home() / ".config" / "ab-test" / "config.json"
    if cfg_path.exists():
...
DB_PATH = pathlib.Path.home() / ".config" / "ab-test" / "data.db"
...
{
  "name": "ab-test",
  "description": "AB 檢定器。解決問題：想做 A/B 統計檢定",
  "input_schema": {
    "type": "object",
    "properties": {
      "action": {
...
```bash
# Agent 呼叫範例
result=$(ab-test run --quiet --output json)
echo $result | jq .data
```
...
def call_ab_test(action="run") -> dict:
    proc = subprocess.run(
        ["ab-test", action, "--output", "json", "--quiet"],
...
# 執行測試
pytest tests/ -v --cov=ab_test --cov-report=term-missing
...
```python
import pytest
from ab_test.core import AbTestCore
...
*工具編號 #265 ／ 類別 N — 資料分析與視覺化 ／ 計畫版本 v1.0 ／ 2026-04-27*

...
| 介面 | CLI（`click` 框架） |
| 設定檔 | `~/.config/mcp-bridge/config.json` |
...
```bash
mcp-bridge [OPTIONS] COMMAND [ARGS]...
```
...
# 指定輸出格式
mcp-bridge run --output json

# 詳細模式
mcp-bridge run --verbose

# 靜音（適合 pipe 到其他工具）
mcp-bridge run --quiet | jq .

# 查看設定
mcp-bridge config show

# 匯出 JSON
mcp-bridge export --output json > result.json
```
...
from .core import McpBridgeCore
...
def config():
    """顯示或修改設定"""
    import json, pathlib
    cfg_path = pathlib.Path.home() / ".config" / "mcp-bridge" / "config.json"
    if cfg_path.exists():
...
DB_PATH = pathlib.Path.home() / ".config" / "mcp-bridge" / "data.db"
...
{
  "name": "mcp-bridge",
  "description": "MCP 協議橋接。解決問題：跨模型工具規格不通",
  "input_schema": {
    "type": "object",
    "properties": {
      "action": {
...
```bash
# Agent 呼叫範例
result=$(mcp-bridge run --quiet --output json)
echo $result | jq .data
```
...
def call_mcp_bridge(action="run") -> dict:
    proc = subprocess.run(
        ["mcp-bridge", action, "--output", "json", "--quiet"],
...
```bash
# 管線串接範例
mcp-bridge run --quiet --output json \
  | jq -r '.data' \
  | other-tool process
```
...
# 執行測試
pytest tests/ -v --cov=mcp_bridge --cov-report=term-missing
...
```python
import pytest
from mcp_bridge.core import McpBridgeCore
...
```json
{
  "name": "mcp-bridge",
  "version": "0.1.0",
  "description": "MCP 協議橋接",
  "category": "F",
  "category_name": "Agent間通訊與協作",
  "number": 114,
  "pain_point": "跨模型工具規格不通",
  "commercial": "✅",
  "language": "Python",
  "entry_point": "mcp-bridge",
  "dependencies": ["click", "rich", "requests"],
...
*工具編號 #114 ／ 類別 F — Agent間通訊與協作 ／ 計畫版本 v1.0 ／ 2026-04-27*

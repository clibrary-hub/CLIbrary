...
| 介面 | CLI（`click` 框架） |
| 設定檔 | `~/.config/api-slim/config.json` |
...
dependencies = ["jq", "click", "rich"]

[project.scripts]
api-slim = "api_slim.cli:main"
```
...
# 指定輸出格式
api-slim run --output json

# 詳細模式
api-slim run --verbose

# 靜音（適合 pipe 到其他工具）
api-slim run --quiet | jq .

# 查看設定
api-slim config show

# 匯出 JSON
api-slim export --output json > result.json
```
...
def config():
    """顯示或修改設定"""
    import json, pathlib
    cfg_path = pathlib.Path.home() / ".config" / "api-slim" / "config.json"
    if cfg_path.exists():
...
DB_PATH = pathlib.Path.home() / ".config" / "api-slim" / "data.db"
...
{
  "name": "api-slim",
  "description": "API 回應瘦身器。解決問題：API 回應太大",
  "input_schema": {
    "type": "object",
    "properties": {
      "action": {
...
```bash
# Agent 呼叫範例
result=$(api-slim run --quiet --output json)
echo $result | jq .data
```
...
def call_api_slim(action="run") -> dict:
    proc = subprocess.run(
        ["api-slim", action, "--output", "json", "--quiet"],
...
```bash
# 管線串接範例
api-slim run --quiet --output json \
  | jq -r '.data' \
  | other-tool process
```
...
# 執行測試
pytest tests/ -v --cov=api_slim --cov-report=term-missing
...
```python
import pytest
from api_slim.core import ApiSlimCore
...
*工具編號 #084 ／ 類別 E — 資料處理與整合 ／ 計畫版本 v1.0 ／ 2026-04-27*

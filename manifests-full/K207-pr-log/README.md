...
# 匯出 JSON
pr-log export --output json > result.json
```
...
def config():
    """顯示或修改設定"""
    import json, pathlib
    cfg_path = pathlib.Path.home() / ".config" / "pr-log" / "config.json"
    if cfg_path.exists():
...
DB_PATH = pathlib.Path.home() / ".config" / "pr-log" / "data.db"
...
{
  "name": "pr-log",
  "description": "個人最佳紀錄。解決問題：PR 紀錄分散",
  "input_schema": {
    "type": "object",
    "properties": {
      "action": {
...
```bash
# Agent 呼叫範例
result=$(pr-log run --quiet --output json)
echo $result | jq .data
```
...
def call_pr_log(action="run") -> dict:
    proc = subprocess.run(
        ["pr-log", action, "--output", "json", "--quiet"],
...
# 執行測試
pytest tests/ -v --cov=pr_log --cov-report=term-missing
...
```python
import pytest
from pr_log.core import PrLogCore
...
*工具編號 #207 ／ 類別 K — 健康與健身 ／ 計畫版本 v1.0 ／ 2026-04-27*

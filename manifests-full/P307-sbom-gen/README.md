...
| 介面 | CLI（`click` 框架） |
| 設定檔 | `~/.config/sbom-gen/config.json` |
...
pip install cyclonedx-bom click rich
...
dependencies = ["cyclonedx-bom", "click", "rich"]

[project.scripts]
sbom-gen = "sbom_gen.cli:main"
```
...
# 指定輸出格式
sbom-gen run --output json

# 詳細模式
sbom-gen run --verbose

# 靜音（適合 pipe 到其他工具）
sbom-gen run --quiet | jq .

# 查看設定
sbom-gen config show

# 匯出 JSON
sbom-gen export --output json > result.json
```
...
def config():
    """顯示或修改設定"""
    import json, pathlib
    cfg_path = pathlib.Path.home() / ".config" / "sbom-gen" / "config.json"
    if cfg_path.exists():
...
DB_PATH = pathlib.Path.home() / ".config" / "sbom-gen" / "data.db"
...
class SbomGenCore:
    ...
    核心邏輯類別。
    問題：軟體 SBOM
    解法：透過 cyclonedx-bom, click 實現自動化處理。
    """
...
{
  "name": "sbom-gen",
  "description": "SBOM 產生器。解決問題：軟體 SBOM",
  "input_schema": {
    "type": "object",
    "properties": {
      "action": {
...
```bash
# Agent 呼叫範例
result=$(sbom-gen run --quiet --output json)
echo $result | jq .data
```
...
def call_sbom_gen(action="run") -> dict:
    proc = subprocess.run(
        ["sbom-gen", action, "--output", "json", "--quiet"],
...
```bash
# 管線串接範例
sbom-gen run --quiet --output json \
  | jq -r '.data' \
  | other-tool process
```
...
# 執行測試
pytest tests/ -v --cov=sbom_gen --cov-report=term-missing
...
```python
import pytest
from sbom_gen.core import SbomGenCore
...
*工具編號 #307 ／ 類別 P — 安全與隱私 ／ 計畫版本 v1.0 ／ 2026-04-27*

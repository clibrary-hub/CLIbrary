...
| 介面 | CLI（`click` 框架） |
| 設定檔 | `~/.config/rag-eval/config.json` |
...
# 指定輸出格式
rag-eval run --output json

# 詳細模式
rag-eval run --verbose

# 靜音（適合 pipe 到其他工具）
rag-eval run --quiet | jq .

# 查看設定
rag-eval config show

# 匯出 JSON
rag-eval export --output json > result.json
```
...
def config():
    """顯示或修改設定"""
    import json, pathlib
    cfg_path = pathlib.Path.home() / ".config" / "rag-eval" / "config.json"
    if cfg_path.exists():
...
DB_PATH = pathlib.Path.home() / ".config" / "rag-eval" / "data.db"
...
{
  "name": "rag-eval",
  "description": "RAG 評估器。解決問題：RAG 檢索品質評估",
  "input_schema": {
    "type": "object",
    "properties": {
      "action": {
...
```bash
# Agent 呼叫範例
result=$(rag-eval run --quiet --output json)
echo $result | jq .data
```
...
def call_rag_eval(action="run") -> dict:
    proc = subprocess.run(
        ["rag-eval", action, "--output", "json", "--quiet"],
...
```bash
# 管線串接範例
rag-eval run --quiet --output json \
  | jq -r '.data' \
  | other-tool process
```
...
# 執行測試
pytest tests/ -v --cov=rag_eval --cov-report=term-missing
...
```python
import pytest
from rag_eval.core import RagEvalCore
...
```json
{
  ...
  "number": 284,
  "pain_point": "RAG 檢索品質評估",
  "commercial": "✅",
  "language": "Python",
  "entry_point": "rag-eval",
  "dependencies": ["anthropic", "click", "rich"],
...
*工具編號 #284 ／ 類別 O — AI/ML輔助 ／ 計畫版本 v1.0 ／ 2026-04-27*

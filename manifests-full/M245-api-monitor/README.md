...
| 介面 | CLI（`click` 框架） |
| 設定檔 | `~/.config/api-monitor/config.json` |
...
```bash
api-monitor [OPTIONS] COMMAND [ARGS]...
```
...
# 指定輸出格式
api-monitor run --output json

# 詳細模式
api-monitor run --verbose

# 靜音（適合 pipe 到其他工具）
api-monitor run --quiet | jq .

# 查看設定
api-monitor config show

# 匯出 JSON
api-monitor export --output json > result.json
```
...
from .core import ApiMonitorCore
...
def config():
    """顯示或修改設定"""
    import json, pathlib
    cfg_path = pathlib.Path.home() / ".config" / "api-monitor" / "config.json"
    if cfg_path.exists():
...
DB_PATH = pathlib.Path.home() / ".config" / "api-monitor" / "data.db"
...
{
  "name": "api-monitor",
  "description": "API 監測器。解決問題：API 健康監測",
  "input_schema": {
    "type": "object",
    "properties": {
      "action": {
...
```bash
# Agent 呼叫範例
result=$(api-monitor run --quiet --output json)
echo $result | jq .data
```
...
def call_api_monitor(action="run") -> dict:
    proc = subprocess.run(
        ["api-monitor", action, "--output", "json", "--quiet"],
...
```bash
# 管線串接範例
api-monitor run --quiet --output json \
  | jq -r '.data' \
  | other-tool process
```
...
# 執行測試
pytest tests/ -v --cov=api_monitor --cov-report=term-missing

# 快速冒煙測試
api-monitor run --help
api-monitor config show
```
...
```python
import pytest
from api_monitor.core import ApiMonitorCore
...
*工具編號 #245 ／ 類別 M — 網路爬蟲與監控 ／ 計畫版本 v1.0 ／ 2026-04-27*

...
| 介面 | CLI（`click` 框架） |
| 設定檔 | `~/.config/ft-monitor/config.json` |
...
[project.scripts]
ft-monitor = "ft_monitor.cli:main"
```
...
```bash
ft-monitor [OPTIONS] COMMAND [ARGS]...
```
...
# 指定輸出格式
ft-monitor run --output json

# 詳細模式
ft-monitor run --verbose

# 靜音（適合 pipe 到其他工具）
ft-monitor run --quiet | jq .

# 查看設定
ft-monitor config show

# 匯出 JSON
ft-monitor export --output json > result.json
```
...
from .core import FtMonitorCore
...
def config():
    """顯示或修改設定"""
    import json, pathlib
    cfg_path = pathlib.Path.home() / ".config" / "ft-monitor" / "config.json"
    if cfg_path.exists():
...
DB_PATH = pathlib.Path.home() / ".config" / "ft-monitor" / "data.db"
...
```bash
# Agent 呼叫範例
result=$(ft-monitor run --quiet --output json)
echo $result | jq .data
```
...
def call_ft_monitor(action="run") -> dict:
    proc = subprocess.run(
        ["ft-monitor", action, "--output", "json", "--quiet"],
...
```bash
# 管線串接範例
ft-monitor run --quiet --output json \
  | jq -r '.data' \
  | other-tool process
```
...
# 執行測試
pytest tests/ -v --cov=ft_monitor --cov-report=term-missing
...
```python
import pytest
from ft_monitor.core import FtMonitorCore
...
*工具編號 #297 ／ 類別 O — AI/ML輔助 ／ 計畫版本 v1.0 ／ 2026-04-27*

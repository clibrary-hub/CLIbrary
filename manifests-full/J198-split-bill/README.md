...
| 介面 | CLI（`click` 框架） |
| 設定檔 | `~/.config/split-bill/config.json` |
...
[project.scripts]
split-bill = "split_bill.cli:main"
```
...
```bash
split-bill [OPTIONS] COMMAND [ARGS]...
```
...
# 指定輸出格式
split-bill run --output json

# 詳細模式
split-bill run --verbose

# 靜音（適合 pipe 到其他工具）
split-bill run --quiet | jq .

# 查看設定
split-bill config show

# 匯出 JSON
split-bill export --output json > result.json
```
...
from .core import SplitBillCore
...
def config():
    """顯示或修改設定"""
    import json, pathlib
    cfg_path = pathlib.Path.home() / ".config" / "split-bill" / "config.json"
    if cfg_path.exists():
...
DB_PATH = pathlib.Path.home() / ".config" / "split-bill" / "data.db"
...
```bash
# Agent 呼叫範例
result=$(split-bill run --quiet --output json)
echo $result | jq .data
```
...
def call_split_bill(action="run") -> dict:
    proc = subprocess.run(
        ["split-bill", action, "--output", "json", "--quiet"],
...
```bash
# 管線串接範例
split-bill run --quiet --output json \
  | jq -r '.data' \
  | other-tool process
```
...
# 執行測試
pytest tests/ -v --cov=split_bill --cov-report=term-missing
...
```python
import pytest
from split_bill.core import SplitBillCore
...
*工具編號 #198 ／ 類別 J — 財務與帳務 ／ 計畫版本 v1.0 ／ 2026-04-27*

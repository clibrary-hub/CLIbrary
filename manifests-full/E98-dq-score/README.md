...
| 介面 | CLI（`click` 框架） |
| 設定檔 | `~/.config/dq-score/config.json` |
...
# 指定輸出格式
dq-score run --output json

# 詳細模式
dq-score run --verbose

# 靜音（適合 pipe 到其他工具）
dq-score run --quiet | jq .

# 查看設定
dq-score config show

# 匯出 JSON
dq-score export --output json > result.json
```
...
def config():
    """顯示或修改設定"""
    import json, pathlib
    cfg_path = pathlib.Path.home() / ".config" / "dq-score" / "config.json"
    if cfg_path.exists():
...
DB_PATH = pathlib.Path.home() / ".config" / "dq-score" / "data.db"
...
{
  "name": "dq-score",
  "description": "資料品質評分器。解決問題：資料品質無法量化",
  "input_schema": {
    "type": "object",
    "properties": {
      "action": {
...
```bash
# Agent 呼叫範例
result=$(dq-score run --quiet --output json)
echo $result | jq .data
```
...
def call_dq_score(action="run") -> dict:
    proc = subprocess.run(
        ["dq-score", action, "--output", "json", "--quiet"],
...
```bash
# 管線串接範例
dq-score run --quiet --output json \
  | jq -r '.data' \
  | other-tool process
```
...
# 執行測試
pytest tests/ -v --cov=dq_score --cov-report=term-missing
...
```python
import pytest
from dq_score.core import DqScoreCore
...
*工具編號 #098 ／ 類別 E — 資料處理與整合 ／ 計畫版本 v1.0 ／ 2026-04-27*

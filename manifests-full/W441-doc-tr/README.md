...
# 匯出 JSON
doc-tr export --output json > result.json
```
...
def config():
    """顯示或修改設定"""
    import json, pathlib
    cfg_path = pathlib.Path.home() / ".config" / "doc-tr" / "config.json"
    if cfg_path.exists():
...
DB_PATH = pathlib.Path.home() / ".config" / "doc-tr" / "data.db"
...
```bash
# Agent 呼叫範例
result=$(doc-tr run --quiet --output json)
echo $result | jq .data
```
...
def call_doc_tr(action="run") -> dict:
    proc = subprocess.run(
        ["doc-tr", action, "--output", "json", "--quiet"],
...
# 執行測試
pytest tests/ -v --cov=doc_tr --cov-report=term-missing
...
```python
import pytest
from doc_tr.core import DocTrCore
...
*工具編號 #441 ／ 類別 W — 翻譯與在地化 ／ 計畫版本 v1.0 ／ 2026-04-27*

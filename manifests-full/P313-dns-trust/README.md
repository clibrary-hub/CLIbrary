...
| 介面 | CLI（`click` 框架） |
| 設定檔 | `~/.config/dns-trust/config.json` |
...
pip install dnspython click rich
...
dependencies = ["dnspython", "click", "rich"]

[project.scripts]
dns-trust = "dns_trust.cli:main"
```
...
```bash
dns-trust [OPTIONS] COMMAND [ARGS]...
```
...
# 指定輸出格式
dns-trust run --output json

# 詳細模式
dns-trust run --verbose

# 靜音（適合 pipe 到其他工具）
dns-trust run --quiet | jq .

# 查看設定
dns-trust config show

# 匯出 JSON
dns-trust export --output json > result.json
```
...
def config():
    """顯示或修改設定"""
    import json, pathlib
    cfg_path = pathlib.Path.home() / ".config" / "dns-trust" / "config.json"
    if cfg_path.exists():
...
DB_PATH = pathlib.Path.home() / ".config" / "dns-trust" / "data.db"
...
{
  "name": "dns-trust",
  "description": "DNS 信任查詢。解決問題：DNS 防汙染查詢",
  "input_schema": {
    "type": "object",
    "properties": {
      "action": {
...
```bash
# Agent 呼叫範例
result=$(dns-trust run --quiet --output json)
echo $result | jq .data
```
...
def call_dns_trust(action="run") -> dict:
    proc = subprocess.run(
        ["dns-trust", action, "--output", "json", "--quiet"],
...
```bash
# 管線串接範例
dns-trust run --quiet --output json \
  | jq -r '.data' \
  | other-tool process
```
...
# 執行測試
pytest tests/ -v --cov=dns_trust --cov-report=term-missing
...
```python
import pytest
from dns_trust.core import DnsTrustCore
...
```json
{
  "name": "dns-trust",
  "version": "0.1.0",
  "description": "DNS 信任查詢",
  "category": "P",
  "category_name": "安全與隱私",
  "number": 313,
  "pain_point": "DNS 防汙染查詢",
  "commercial": "✅",
  "language": "Python",
  "entry_point": "dns-trust",
  "dependencies": ["dnspython", "click", "rich"],
...
*工具編號 #313 ／ 類別 P — 安全與隱私 ／ 計畫版本 v1.0 ／ 2026-04-27*

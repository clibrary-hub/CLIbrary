...
| 介面 | CLI（`click` 框架） |
| 設定檔 | `~/.config/data-join/config.json` |
...
```bash
data-join [OPTIONS] COMMAND [ARGS]...
```
...
# 指定輸出格式
data-join run --output json

# 詳細模式
data-join run --verbose

# 靜音（適合 pipe 到其他工具）
data-join run --quiet | jq .

# 查看設定
data-join config show

# 匯出 JSON
data-join export --output json > result.json
```
...
def config():
    """顯示或修改設定"""
    import json, pathlib
    cfg_path = pathlib.Path.home() / ".config" / "data-join" / "config.json"
    if cfg_path.exists():
...
DB_PATH = pathlib.Path.home() / ".config" / "data-join" / "data.db"
...
{
  "name": "data-join",
  "description": "多源 join 器。解決問題：跨來源 join 沒工具",
  "input_schema": {
    "type": "object",
    "properties": {
      "action": {
...
```bash
# Agent 呼叫範例
result=$(data-join run --quiet --output json)
echo $result | jq .data
```
...
def call_data_join(action="run") -> dict:
    proc = subprocess.run(
        ["data-join", action, "--output", "json", "--quiet"],
...
```bash
# 管線串接範例
data-join run --quiet --output json \
  | jq -r '.data' \
  | other-tool process
```
...
# 執行測試
pytest tests/ -v --cov=data_join --cov-report=term-missing
...
```python
import pytest
from data_join.core import DataJoinCore
...
```json
{
  "name": "data-join",
  "version": "0.1.0",
  "description": "多源 join 器",
  "category": "E",
  "category_name": "資料處理與整合",
  "number": 96,
  "pain_point": "跨來源 join 沒工具",
  "commercial": "✅",
  "language": "Python",
  "entry_point": "data-join",
  "dependencies": ["pandas", "click", "rich"],
...
*工具編號 #096 ／ 類別 E — 資料處理與整合 ／ 計畫版本 v1.0 ／ 2026-04-27*

...
| 介面 | CLI（`click` 框架） |
| 設定檔 | `~/.config/dns-audit/config.json` |
...
```bash
dns-audit [OPTIONS] COMMAND [ARGS]...
```
...
# 指定輸出格式
dns-audit run --output json

# 詳細模式
dns-audit run --verbose

# 靜音（適合 pipe 到其他工具）
dns-audit run --quiet | jq .

# 查看設定
dns-audit config show

# 匯出 JSON
dns-audit export --output json > result.json
```
...
def config():
    """顯示或修改設定"""
    import json, pathlib
    cfg_path = pathlib.Path.home() / ".config" / "dns-audit" / "config.json"
    if cfg_path.exists():
...
DB_PATH = pathlib.Path.home() / ".config" / "dns-audit" / "data.db"
...
{
  "name": "dns-audit",
  "description": "DNS 稽核器。解決問題：DNS 設定核對",
  "input_schema": {
    "type": "object",
    "properties": {
      "action": {
...
```bash
# Agent 呼叫範例
result=$(dns-audit run --quiet --output json)
echo $result | jq .data
```
...
def call_dns_audit(action="run") -> dict:
    proc = subprocess.run(
        ["dns-audit", action, "--output", "json", "--quiet"],
...
```bash
# 管線串接範例
dns-audit run --quiet --output json \
  | jq -r '.data' \
  | other-tool process
```
...
# 執行測試
pytest tests/ -v --cov=dns_audit --cov-report=term-missing
...
```python
import pytest
from dns_audit.core import DnsAuditCore
...
*工具編號 #338 ／ 類別 Q — DevOps與基礎設施 ／ 計畫版本 v1.0 ／ 2026-04-27*

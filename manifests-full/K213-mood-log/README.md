...
| 介面 | CLI（`click` 框架） |
| 設定檔 | `~/.config/mood-log/config.json` |
...
# 指定輸出格式
mood-log run --output json

# 詳細模式
mood-log run --verbose

# 靜音（適合 pipe 到其他工具）
mood-log run --quiet | jq .

# 查看設定
mood-log config show

# 匯出 JSON
mood-log export --output json > result.json
```
...
def config():
    """顯示或修改設定"""
    import json, pathlib
    cfg_path = pathlib.Path.home() / ".config" / "mood-log" / "config.json"
    if cfg_path.exists():
...
DB_PATH = pathlib.Path.home() / ".config" / "mood-log" / "data.db"
...
```bash
# Agent 呼叫範例
result=$(mood-log run --quiet --output json)
echo $result | jq .data
```
...
def call_mood_log(action="run") -> dict:
    proc = subprocess.run(
        ["mood-log", action, "--output", "json", "--quiet"],
...
```bash
# 管線串接範例
mood-log run --quiet --output json \
  | jq -r '.data' \
  | other-tool process
```
...
# 執行測試
pytest tests/ -v --cov=mood_log --cov-report=term-missing
...
```python
import pytest
from mood_log.core import MoodLogCore
...
*工具編號 #213 ／ 類別 K — 健康與健身 ／ 計畫版本 v1.0 ／ 2026-04-27*

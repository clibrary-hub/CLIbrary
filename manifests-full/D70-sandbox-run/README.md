...
| 介面 | CLI（`click` 框架） |
| 設定檔 | `~/.config/sandbox-run/config.json` |
...
[project.scripts]
sandbox-run = "sandbox_run.cli:main"
```
...
# 指定輸出格式
sandbox-run run --output json

# 詳細模式
sandbox-run run --verbose

# 靜音（適合 pipe 到其他工具）
sandbox-run run --quiet | jq .

# 查看設定
sandbox-run config show

# 匯出 JSON
sandbox-run export --output json > result.json
```
...
from .core import SandboxRunCore
...
def config():
    """顯示或修改設定"""
    import json, pathlib
    cfg_path = pathlib.Path.home() / ".config" / "sandbox-run" / "config.json"
    if cfg_path.exists():
...
DB_PATH = pathlib.Path.home() / ".config" / "sandbox-run" / "data.db"
...
{
  "name": "sandbox-run",
  "description": "命令沙箱執行。解決問題：命令缺乏安全沙箱",
  "input_schema": {
    "type": "object",
    "properties": {
      "action": {
...
```bash
# Agent 呼叫範例
result=$(sandbox-run run --quiet --output json)
echo $result | jq .data
```
...
def call_sandbox_run(action="run") -> dict:
    proc = subprocess.run(
        ["sandbox-run", action, "--output", "json", "--quiet"],
...
```bash
# 管線串接範例
sandbox-run run --quiet --output json \
  | jq -r '.data' \
  | other-tool process
```
...
# 執行測試
pytest tests/ -v --cov=sandbox_run --cov-report=term-missing
...
```python
import pytest
from sandbox_run.core import SandboxRunCore
...
*工具編號 #070 ／ 類別 D — 自動化與工作流程 ／ 計畫版本 v1.0 ／ 2026-04-27*

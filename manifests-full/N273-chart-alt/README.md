...
| 介面 | CLI（`click` 框架） |
| 設定檔 | `~/.config/chart-alt/config.json` |
...
```bash
chart-alt [OPTIONS] COMMAND [ARGS]...
```
...
# 指定輸出格式
chart-alt run --output json

# 詳細模式
chart-alt run --verbose

# 靜音（適合 pipe 到其他工具）
chart-alt run --quiet | jq .

# 查看設定
chart-alt config show

# 匯出 JSON
chart-alt export --output json > result.json
```
...
def config():
    """顯示或修改設定"""
    import json, pathlib
    cfg_path = pathlib.Path.home() / ".config" / "chart-alt" / "config.json"
    if cfg_path.exists():
...
DB_PATH = pathlib.Path.home() / ".config" / "chart-alt" / "data.db"
...
{
  "name": "chart-alt",
  "description": "Alt 文字器。解決問題：圖表轉可訪問描述",
  "input_schema": {
    "type": "object",
    "properties": {
      "action": {
...
```bash
# Agent 呼叫範例
result=$(chart-alt run --quiet --output json)
echo $result | jq .data
```
...
def call_chart_alt(action="run") -> dict:
    proc = subprocess.run(
        ["chart-alt", action, "--output", "json", "--quiet"],
...
```bash
# 管線串接範例
chart-alt run --quiet --output json \
  | jq -r '.data' \
  | other-tool process
```
...
# 執行測試
pytest tests/ -v --cov=chart_alt --cov-report=term-missing
...
```python
import pytest
from chart_alt.core import ChartAltCore
...
*工具編號 #273 ／ 類別 N — 資料分析與視覺化 ／ 計畫版本 v1.0 ／ 2026-04-27*

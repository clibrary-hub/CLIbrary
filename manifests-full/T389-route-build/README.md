...
| 介面 | CLI（`click` 框架） |
| 設定檔 | `~/.config/route-build/config.json` |
...
[project.scripts]
route-build = "route_build.cli:main"
```
...
```bash
route-build [OPTIONS] COMMAND [ARGS]...
```
...
# 指定輸出格式
route-build run --output json

# 詳細模式
route-build run --verbose

# 靜音（適合 pipe 到其他工具）
route-build run --quiet | jq .

# 查看設定
route-build config show

# 匯出 JSON
route-build export --output json > result.json
```
...
from .core import RouteBuildCore
...
def config():
    """顯示或修改設定"""
    import json, pathlib
    cfg_path = pathlib.Path.home() / ".config" / "route-build" / "config.json"
    if cfg_path.exists():
...
DB_PATH = pathlib.Path.home() / ".config" / "route-build" / "data.db"
...
```bash
# Agent 呼叫範例
result=$(route-build run --quiet --output json)
echo $result | jq .data
```
...
def call_route_build(action="run") -> dict:
    proc = subprocess.run(
        ["route-build", action, "--output", "json", "--quiet"],
...
```bash
# 管線串接範例
route-build run --quiet --output json \
  | jq -r '.data' \
  | other-tool process
```
...
# 執行測試
pytest tests/ -v --cov=route_build --cov-report=term-missing

# 快速冒煙測試
route-build run --help
route-build config show
```
...
```python
import pytest
from route_build.core import RouteBuildCore
...
*工具編號 #389 ／ 類別 T — 旅遊與生活 ／ 計畫版本 v1.0 ／ 2026-04-27*

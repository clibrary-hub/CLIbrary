...
| 介面 | CLI（`click` 框架） |
| 設定檔 | `~/.config/infer-bench/config.json` |
...
dependencies = ["time", "click", "rich"]

[project.scripts]
infer-bench = "infer_bench.cli:main"
```
...
```bash
infer-bench [OPTIONS] COMMAND [ARGS]...
```
...
# 指定輸出格式
infer-bench run --output json

# 詳細模式
infer-bench run --verbose

# 靜音（適合 pipe 到其他工具）
infer-bench run --quiet | jq .

# 查看設定
infer-bench config show

# 匯出 JSON
infer-bench export --output json > result.json
```
...
from .core import InferBenchCore
...
def config():
    """顯示或修改設定"""
    import json, pathlib
    cfg_path = pathlib.Path.home() / ".config" / "infer-bench" / "config.json"
    if cfg_path.exists():
...
DB_PATH = pathlib.Path.home() / ".config" / "infer-bench" / "data.db"
...
class InferBenchCore:
    """
    核心邏輯類別。
    問題：推論速度基準
    解法：透過 time, click 實現自動化處理。
    """
...
```bash
# Agent 呼叫範例
result=$(infer-bench run --quiet --output json)
echo $result | jq .data
```
...
def call_infer_bench(action="run") -> dict:
    proc = subprocess.run(
        ["infer-bench", action, "--output", "json", "--quiet"],
...
```bash
# 管線串接範例
infer-bench run --quiet --output json \
  | jq -r '.data' \
  | other-tool process
```
...
# 執行測試
pytest tests/ -v --cov=infer_bench --cov-report=term-missing

# 快速冒煙測試
infer-bench run --help
infer-bench config show
```
...
```python
import pytest
from infer_bench.core import InferBenchCore
...
*工具編號 #299 ／ 類別 O — AI/ML輔助 ／ 計畫版本 v1.0 ／ 2026-04-27*

...
# 匯出 JSON
zettel export --output json > result.json
```
...
def config():
    """顯示或修改設定"""
    import json, pathlib
    cfg_path = pathlib.Path.home() / ".config" / "zettel" / "config.json"
    if cfg_path.exists():
...
DB_PATH = pathlib.Path.home() / ".config" / "zettel" / "data.db"
...
{
  "name": "zettel",
  "description": "Zettelkasten CLI。解決問題：卡片盒筆記法支援",
  "input_schema": {
    "type": "object",
    "properties": {
      "action": {
...
```bash
# Agent 呼叫範例
result=$(zettel run --quiet --output json)
echo $result | jq .data
```
...
def call_zettel(action="run") -> dict:
    proc = subprocess.run(
        ["zettel", action, "--output", "json", "--quiet"],
...
# 執行測試
pytest tests/ -v --cov=zettel --cov-report=term-missing
...
```python
import pytest
from zettel.core import ZettelCore
...
*工具編號 #343 ／ 類別 R — 知識管理與筆記 ／ 計畫版本 v1.0 ／ 2026-04-27*

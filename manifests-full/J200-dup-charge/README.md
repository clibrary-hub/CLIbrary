...
| 介面 | CLI（`click` 框架） |
| 設定檔 | `~/.config/dup-charge/config.json` |
...
[project.scripts]
dup-charge = "dup_charge.cli:main"
```
...
```bash
dup-charge [OPTIONS] COMMAND [ARGS]...
```
...
# 指定輸出格式
dup-charge run --output json

# 詳細模式
dup-charge run --verbose

# 靜音（適合 pipe 到其他工具）
dup-charge run --quiet | jq .

# 查看設定
dup-charge config show

# 匯出 JSON
dup-charge export --output json > result.json
```
...
from .core import DupChargeCore
...
def config():
    """顯示或修改設定"""
    import json, pathlib
    cfg_path = pathlib.Path.home() / ".config" / "dup-charge" / "config.json"
    if cfg_path.exists():
...
DB_PATH = pathlib.Path.home() / ".config" / "dup-charge" / "data.db"
...
{
  "name": "dup-charge",
  "description": "重複扣款偵測器。解決問題：想找重複扣款",
  "input_schema": {
    "type": "object",
    "properties": {
      "action": {
...
```bash
# Agent 呼叫範例
result=$(dup-charge run --quiet --output json)
echo $result | jq .data
```
...
def call_dup_charge(action="run") -> dict:
    proc = subprocess.run(
        ["dup-charge", action, "--output", "json", "--quiet"],
...
```bash
# 管線串接範例
dup-charge run --quiet --output json \
  | jq -r '.data' \
  | other-tool process
```
...
# 執行測試
pytest tests/ -v --cov=dup_charge --cov-report=term-missing
...
```python
import pytest
from dup_charge.core import DupChargeCore
...
*工具編號 #200 ／ 類別 J — 財務與帳務 ／ 計畫版本 v1.0 ／ 2026-04-27*

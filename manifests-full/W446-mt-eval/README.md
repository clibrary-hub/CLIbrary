...
| 介面 | CLI（`click` 框架） |
| 設定檔 | `~/.config/mt-eval/config.json` |
...
pip install sacrebleu click rich
...
dependencies = ["sacrebleu", "click", "rich"]

[project.scripts]
mt-eval = "mt_eval.cli:main"
```
...
# 指定輸出格式
mt-eval run --output json

# 詳細模式
mt-eval run --verbose

# 靜音（適合 pipe 到其他工具）
mt-eval run --quiet | jq .

# 查看設定
mt-eval config show

# 匯出 JSON
mt-eval export --output json > result.json
```
...
def config():
    """顯示或修改設定"""
    import json, pathlib
    cfg_path = pathlib.Path.home() / ".config" / "mt-eval" / "config.json"
    if cfg_path.exists():
...
DB_PATH = pathlib.Path.home() / ".config" / "mt-eval" / "data.db"
...
{
  "name": "mt-eval",
  "description": "MT 品質評估器。解決問題：機翻品質評估",
  "input_schema": {
    "type": "object",
    "properties": {
      "action": {
...
```bash
# Agent 呼叫範例
result=$(mt-eval run --quiet --output json)
echo $result | jq .data
```
...
def call_mt_eval(action="run") -> dict:
    proc = subprocess.run(
        ["mt-eval", action, "--output", "json", "--quiet"],
...
# 執行測試
pytest tests/ -v --cov=mt_eval --cov-report=term-missing
...
```python
import pytest
from mt_eval.core import MtEvalCore
...
*工具編號 #446 ／ 類別 W — 翻譯與在地化 ／ 計畫版本 v1.0 ／ 2026-04-27*

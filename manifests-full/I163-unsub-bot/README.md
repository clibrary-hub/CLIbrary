...
| 介面 | CLI（`click` 框架） |
| 設定檔 | `~/.config/unsub-bot/config.json` |
...
```bash
unsub-bot [OPTIONS] COMMAND [ARGS]...
```
...
# 指定輸出格式
unsub-bot run --output json

# 詳細模式
unsub-bot run --verbose

# 靜音（適合 pipe 到其他工具）
unsub-bot run --quiet | jq .

# 查看設定
unsub-bot config show

# 匯出 JSON
unsub-bot export --output json > result.json
```
...
def config():
    """顯示或修改設定"""
    import json, pathlib
    cfg_path = pathlib.Path.home() / ".config" / "unsub-bot" / "config.json"
    if cfg_path.exists():
...
DB_PATH = pathlib.Path.home() / ".config" / "unsub-bot" / "data.db"
...
```bash
# Agent 呼叫範例
result=$(unsub-bot run --quiet --output json)
echo $result | jq .data
```
...
def call_unsub_bot(action="run") -> dict:
    proc = subprocess.run(
        ["unsub-bot", action, "--output", "json", "--quiet"],
...
```bash
# 管線串接範例
unsub-bot run --quiet --output json \
  | jq -r '.data' \
  | other-tool process
```
...
# 執行測試
pytest tests/ -v --cov=unsub_bot --cov-report=term-missing
...
```python
import pytest
from unsub_bot.core import UnsubBotCore
...
*工具編號 #163 ／ 類別 I — 郵件與通訊 ／ 計畫版本 v1.0 ／ 2026-04-27*

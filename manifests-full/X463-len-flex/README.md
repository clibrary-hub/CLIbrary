...
| 介面 | CLI（`click` 框架） |
| 設定檔 | `~/.config/len-flex/config.json` |
...
# 指定輸出格式
len-flex run --output json

# 詳細模式
len-flex run --verbose

# 靜音（適合 pipe 到其他工具）
len-flex run --quiet | jq .

# 查看設定
len-flex config show

# 匯出 JSON
len-flex export --output json > result.json
```
...
def config():
    """顯示或修改設定"""
    import json, pathlib
    cfg_path = pathlib.Path.home() / ".config" / "len-flex" / "config.json"
    if cfg_path.exists():
...
DB_PATH = pathlib.Path.home() / ".config" / "len-flex" / "data.db"
...
```bash
# Agent 呼叫範例
result=$(len-flex run --quiet --output json)
echo $result | jq .data
```
...
def call_len_flex(action="run") -> dict:
    proc = subprocess.run(
        ["len-flex", action, "--output", "json", "--quiet"],
...
```bash
# 管線串接範例
len-flex run --quiet --output json \
  | jq -r '.data' \
  | other-tool process
```
...
# 執行測試
pytest tests/ -v --cov=len_flex --cov-report=term-missing
...
```python
import pytest
from len_flex.core import LenFlexCore
...
*工具編號 #463 ／ 類別 X — 寫作與內容創作 ／ 計畫版本 v1.0 ／ 2026-04-27*

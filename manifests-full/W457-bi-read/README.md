...
| 介面 | CLI（`click` 框架） |
| 設定檔 | `~/.config/bi-read/config.json` |
...
# 指定輸出格式
bi-read run --output json

# 詳細模式
bi-read run --verbose

# 靜音（適合 pipe 到其他工具）
bi-read run --quiet | jq .

# 查看設定
bi-read config show

# 匯出 JSON
bi-read export --output json > result.json
```
...
def config():
    """顯示或修改設定"""
    import json, pathlib
    cfg_path = pathlib.Path.home() / ".config" / "bi-read" / "config.json"
    if cfg_path.exists():
...
DB_PATH = pathlib.Path.home() / ".config" / "bi-read" / "data.db"
...
```bash
# Agent 呼叫範例
result=$(bi-read run --quiet --output json)
echo $result | jq .data
```
...
def call_bi_read(action="run") -> dict:
    proc = subprocess.run(
        ["bi-read", action, "--output", "json", "--quiet"],
...
# 執行測試
pytest tests/ -v --cov=bi_read --cov-report=term-missing
...
```python
import pytest
from bi_read.core import BiReadCore
...
*工具編號 #457 ／ 類別 W — 翻譯與在地化 ／ 計畫版本 v1.0 ／ 2026-04-27*

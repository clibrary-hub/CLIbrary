...
| 介面 | CLI（`click` 框架） |
| 設定檔 | `~/.config/price-cmp/config.json` |
...
```bash
price-cmp [OPTIONS] COMMAND [ARGS]...
```
...
# 指定輸出格式
price-cmp run --output json

# 詳細模式
price-cmp run --verbose

# 靜音（適合 pipe 到其他工具）
price-cmp run --quiet | jq .

# 查看設定
price-cmp config show

# 匯出 JSON
price-cmp export --output json > result.json
```
...
def config():
    """顯示或修改設定"""
    import json, pathlib
    cfg_path = pathlib.Path.home() / ".config" / "price-cmp" / "config.json"
    if cfg_path.exists():
...
DB_PATH = pathlib.Path.home() / ".config" / "price-cmp" / "data.db"
...
```bash
# Agent 呼叫範例
result=$(price-cmp run --quiet --output json)
echo $result | jq .data
```
...
def call_price_cmp(action="run") -> dict:
    proc = subprocess.run(
        ["price-cmp", action, "--output", "json", "--quiet"],
...
```bash
# 管線串接範例
price-cmp run --quiet --output json \
  | jq -r '.data' \
  | other-tool process
```
...
# 執行測試
pytest tests/ -v --cov=price_cmp --cov-report=term-missing
...
```python
import pytest
from price_cmp.core import PriceCmpCore
...
*工具編號 #401 ／ 類別 U — 購物與訂閱管理 ／ 計畫版本 v1.0 ／ 2026-04-27*

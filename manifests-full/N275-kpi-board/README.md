...
| 介面 | CLI（`click` 框架） |
| 設定檔 | `~/.config/kpi-board/config.json` |
...
```bash
kpi-board [OPTIONS] COMMAND [ARGS]...
```
...
# 指定輸出格式
kpi-board run --output json

# 詳細模式
kpi-board run --verbose

# 靜音（適合 pipe 到其他工具）
kpi-board run --quiet | jq .

# 查看設定
kpi-board config show

# 匯出 JSON
kpi-board export --output json > result.json
```
...
def config():
    """顯示或修改設定"""
    import json, pathlib
    cfg_path = pathlib.Path.home() / ".config" / "kpi-board" / "config.json"
    if cfg_path.exists():
...
DB_PATH = pathlib.Path.home() / ".config" / "kpi-board" / "data.db"
...
{
  "name": "kpi-board",
  "description": "KPI 儀表板。解決問題：KPI 儀表板",
  "input_schema": {
    "type": "object",
    "properties": {
      "action": {
...
```bash
# Agent 呼叫範例
result=$(kpi-board run --quiet --output json)
echo $result | jq .data
```
...
def call_kpi_board(action="run") -> dict:
    proc = subprocess.run(
        ["kpi-board", action, "--output", "json", "--quiet"],
...
```bash
# 管線串接範例
kpi-board run --quiet --output json \
  | jq -r '.data' \
  | other-tool process
```
...
# 執行測試
pytest tests/ -v --cov=kpi_board --cov-report=term-missing
...
```python
import pytest
from kpi_board.core import KpiBoardCore
...
*工具編號 #275 ／ 類別 N — 資料分析與視覺化 ／ 計畫版本 v1.0 ／ 2026-04-27*

...
| 介面 | CLI（`click` 框架） |
| 設定檔 | `~/.config/human-gate/config.json` |
...
[project.scripts]
human-gate = "human_gate.cli:main"
```
...
```bash
human-gate [OPTIONS] COMMAND [ARGS]...
```
...
# 指定輸出格式
human-gate run --output json

# 詳細模式
human-gate run --verbose

# 靜音（適合 pipe 到其他工具）
human-gate run --quiet | jq .

# 查看設定
human-gate config show

# 匯出 JSON
human-gate export --output json > result.json
```
...
from .core import HumanGateCore
...
def config():
    """顯示或修改設定"""
    import json, pathlib
    cfg_path = pathlib.Path.home() / ".config" / "human-gate" / "config.json"
    if cfg_path.exists():
...
DB_PATH = pathlib.Path.home() / ".config" / "human-gate" / "data.db"
...
{
  "name": "human-gate",
  "description": "人類審批關卡。解決問題：重要任務需要審批",
  "input_schema": {
    "type": "object",
    "properties": {
      "action": {
...
```bash
# Agent 呼叫範例
result=$(human-gate run --quiet --output json)
echo $result | jq .data
```
...
def call_human_gate(action="run") -> dict:
    proc = subprocess.run(
        ["human-gate", action, "--output", "json", "--quiet"],
...
```bash
# 管線串接範例
human-gate run --quiet --output json \
  | jq -r '.data' \
  | other-tool process
```
...
# 執行測試
pytest tests/ -v --cov=human_gate --cov-report=term-missing
...
```python
import pytest
from human_gate.core import HumanGateCore
...
*工具編號 #072 ／ 類別 D — 自動化與工作流程 ／ 計畫版本 v1.0 ／ 2026-04-27*

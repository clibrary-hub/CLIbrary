...
| 介面 | CLI（`click` 框架） |
| 設定檔 | `~/.config/gist-pull/config.json` |
...
```bash
gist-pull [OPTIONS] COMMAND [ARGS]...
```
...
# 指定輸出格式
gist-pull run --output json

# 詳細模式
gist-pull run --verbose

# 靜音（適合 pipe 到其他工具）
gist-pull run --quiet | jq .

# 查看設定
gist-pull config show

# 匯出 JSON
gist-pull export --output json > result.json
```
...
def config():
    """顯示或修改設定"""
    import json, pathlib
    cfg_path = pathlib.Path.home() / ".config" / "gist-pull" / "config.json"
    if cfg_path.exists():
...
DB_PATH = pathlib.Path.home() / ".config" / "gist-pull" / "data.db"
...
```bash
# Agent 呼叫範例
result=$(gist-pull run --quiet --output json)
echo $result | jq .data
```
...
def call_gist_pull(action="run") -> dict:
    proc = subprocess.run(
        ["gist-pull", action, "--output", "json", "--quiet"],
...
```bash
# 管線串接範例
gist-pull run --quiet --output json \
  | jq -r '.data' \
  | other-tool process
```
...
# 執行測試
pytest tests/ -v --cov=gist_pull --cov-report=term-missing
...
```python
import pytest
from gist_pull.core import GistPullCore
...
*工具編號 #478 ／ 類別 X — 寫作與內容創作 ／ 計畫版本 v1.0 ／ 2026-04-27*

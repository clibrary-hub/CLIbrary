...
| 介面 | CLI（`click` 框架） |
| 設定檔 | `~/.config/sla-watch/config.json` |
...
```bash
sla-watch [OPTIONS] COMMAND [ARGS]...
```
...
# 指定輸出格式
sla-watch run --output json

# 詳細模式
sla-watch run --verbose

# 靜音（適合 pipe 到其他工具）
sla-watch run --quiet | jq .

# 查看設定
sla-watch config show

# 匯出 JSON
sla-watch export --output json > result.json
```
...
def config():
    """顯示或修改設定"""
    import json, pathlib
    cfg_path = pathlib.Path.home() / ".config" / "sla-watch" / "config.json"
    if cfg_path.exists():
...
DB_PATH = pathlib.Path.home() / ".config" / "sla-watch" / "data.db"
...
{
  "name": "sla-watch",
  "description": "SLA 守望者。解決問題：SLA 違約偵測",
  "input_schema": {
    "type": "object",
    "properties": {
      "action": {
...
```bash
# Agent 呼叫範例
result=$(sla-watch run --quiet --output json)
echo $result | jq .data
```
...
def call_sla_watch(action="run") -> dict:
    proc = subprocess.run(
        ["sla-watch", action, "--output", "json", "--quiet"],
...
```bash
# 管線串接範例
sla-watch run --quiet --output json \
  | jq -r '.data' \
  | other-tool process
```
...
# 執行測試
pytest tests/ -v --cov=sla_watch --cov-report=term-missing
...
```python
import pytest
from sla_watch.core import SlaWatchCore
...
*工具編號 #497 ／ 類別 Y — 法律合約商務 ／ 計畫版本 v1.0 ／ 2026-04-27*

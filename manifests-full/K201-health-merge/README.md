...
| 介面 | CLI（`click` 框架） |
| 設定檔 | `~/.config/health-merge/config.json` |
...
[project.scripts]
health-merge = "health_merge.cli:main"
```
...
```bash
health-merge [OPTIONS] COMMAND [ARGS]...
```
...
# 指定輸出格式
health-merge run --output json

# 詳細模式
health-merge run --verbose

# 靜音（適合 pipe 到其他工具）
health-merge run --quiet | jq .

# 查看設定
health-merge config show

# 匯出 JSON
health-merge export --output json > result.json
```
...
from .core import HealthMergeCore
...
def config():
    """顯示或修改設定"""
    import json, pathlib
    cfg_path = pathlib.Path.home() / ".config" / "health-merge" / "config.json"
    if cfg_path.exists():
...
DB_PATH = pathlib.Path.home() / ".config" / "health-merge" / "data.db"
...
{
  "name": "health-merge",
  "description": "健康資料整合。解決問題：跨裝置健康資料分散",
  "input_schema": {
    "type": "object",
    "properties": {
      "action": {
...
```bash
# Agent 呼叫範例
result=$(health-merge run --quiet --output json)
echo $result | jq .data
```
...
def call_health_merge(action="run") -> dict:
    proc = subprocess.run(
        ["health-merge", action, "--output", "json", "--quiet"],
...
```bash
# 管線串接範例
health-merge run --quiet --output json \
  | jq -r '.data' \
  | other-tool process
```
...
# 執行測試
pytest tests/ -v --cov=health_merge --cov-report=term-missing

# 快速冒煙測試
health-merge run --help
health-merge config show
```
...
```python
import pytest
from health_merge.core import HealthMergeCore
...
```json
{
  ...
  "category_name": "健康與健身",
  "number": 201,
  "pain_point": "跨裝置健康資料分散",
  "commercial": "⚠️",
  "language": "Python",
  "entry_point": "health-merge",
  "dependencies": ["pandas", "click", "rich"],
...
*工具編號 #201 ／ 類別 K — 健康與健身 ／ 計畫版本 v1.0 ／ 2026-04-27*

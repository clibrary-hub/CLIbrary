...
| 介面 | CLI（`click` 框架） |
| 設定檔 | `~/.config/im-digest/config.json` |
...
```bash
im-digest [OPTIONS] COMMAND [ARGS]...
```
...
# 指定輸出格式
im-digest run --output json

# 詳細模式
im-digest run --verbose

# 靜音（適合 pipe 到其他工具）
im-digest run --quiet | jq .

# 查看設定
im-digest config show

# 匯出 JSON
im-digest export --output json > result.json
```
...
def config():
    """顯示或修改設定"""
    import json, pathlib
    cfg_path = pathlib.Path.home() / ".config" / "im-digest" / "config.json"
    if cfg_path.exists():
...
DB_PATH = pathlib.Path.home() / ".config" / "im-digest" / "data.db"
...
{
  "name": "im-digest",
  "description": "IM 摘要器。解決問題：IM 通知太雜",
  "input_schema": {
    "type": "object",
    "properties": {
      "action": {
...
```bash
# Agent 呼叫範例
result=$(im-digest run --quiet --output json)
echo $result | jq .data
```
...
def call_im_digest(action="run") -> dict:
    proc = subprocess.run(
        ["im-digest", action, "--output", "json", "--quiet"],
...
```bash
# 管線串接範例
im-digest run --quiet --output json \
  | jq -r '.data' \
  | other-tool process
```
...
# 執行測試
pytest tests/ -v --cov=im_digest --cov-report=term-missing
...
```python
import pytest
from im_digest.core import ImDigestCore
...
*工具編號 #171 ／ 類別 I — 郵件與通訊 ／ 計畫版本 v1.0 ／ 2026-04-27*

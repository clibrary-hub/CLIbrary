...
| 介面 | CLI（`click` 框架） |
| 設定檔 | `~/.config/data-scrub/config.json` |
...
[project.scripts]
data-scrub = "data_scrub.cli:main"
```
...
```bash
data-scrub [OPTIONS] COMMAND [ARGS]...
```
...
# 指定輸出格式
data-scrub run --output json

# 詳細模式
data-scrub run --verbose

# 靜音（適合 pipe 到其他工具）
data-scrub run --quiet | jq .

# 查看設定
data-scrub config show

# 匯出 JSON
data-scrub export --output json > result.json
```
...
from .core import DataScrubCore
...
def config():
    """顯示或修改設定"""
    import json, pathlib
    cfg_path = pathlib.Path.home() / ".config" / "data-scrub" / "config.json"
    if cfg_path.exists():
...
DB_PATH = pathlib.Path.home() / ".config" / "data-scrub" / "data.db"
...
{
  "name": "data-scrub",
  "description": "髒資料清洗器。解決問題：髒資料清洗繁瑣",
  "input_schema": {
    "type": "object",
    "properties": {
      "action": {
...
```bash
# Agent 呼叫範例
result=$(data-scrub run --quiet --output json)
echo $result | jq .data
```
...
def call_data_scrub(action="run") -> dict:
    proc = subprocess.run(
        ["data-scrub", action, "--output", "json", "--quiet"],
...
```bash
# 管線串接範例
data-scrub run --quiet --output json \
  | jq -r '.data' \
  | other-tool process
```
...
# 執行測試
pytest tests/ -v --cov=data_scrub --cov-report=term-missing
...
```python
import pytest
from data_scrub.core import DataScrubCore
...
*工具編號 #097 ／ 類別 E — 資料處理與整合 ／ 計畫版本 v1.0 ／ 2026-04-27*

...
| 介面 | CLI（`click` 框架） |
| 設定檔 | `~/.config/gdpr-scan/config.json` |
...
```bash
gdpr-scan [OPTIONS] COMMAND [ARGS]...
```
...
# 指定輸出格式
gdpr-scan run --output json

# 詳細模式
gdpr-scan run --verbose

# 靜音（適合 pipe 到其他工具）
gdpr-scan run --quiet | jq .

# 查看設定
gdpr-scan config show

# 匯出 JSON
gdpr-scan export --output json > result.json
```
...
def config():
    """顯示或修改設定"""
    import json, pathlib
    cfg_path = pathlib.Path.home() / ".config" / "gdpr-scan" / "config.json"
    if cfg_path.exists():
...
DB_PATH = pathlib.Path.home() / ".config" / "gdpr-scan" / "data.db"
...
{
  "name": "gdpr-scan",
  "description": "GDPR 掃描器。解決問題：法遵掃描（GDPR）",
  "input_schema": {
    "type": "object",
    "properties": {
      "action": {
...
```bash
# Agent 呼叫範例
result=$(gdpr-scan run --quiet --output json)
echo $result | jq .data
```
...
def call_gdpr_scan(action="run") -> dict:
    proc = subprocess.run(
        ["gdpr-scan", action, "--output", "json", "--quiet"],
...
```bash
# 管線串接範例
gdpr-scan run --quiet --output json \
  | jq -r '.data' \
  | other-tool process
```
...
# 執行測試
pytest tests/ -v --cov=gdpr_scan --cov-report=term-missing
...
```python
import pytest
from gdpr_scan.core import GdprScanCore
...
```json
{
  "name": "gdpr-scan",
  "version": "0.1.0",
  "description": "GDPR 掃描器",
  "category": "P",
  "category_name": "安全與隱私",
  "number": 320,
  "pain_point": "法遵掃描（GDPR）",
  "commercial": "✅",
  "language": "Python",
  "entry_point": "gdpr-scan",
  "dependencies": ["anthropic", "click", "rich"],
...
*工具編號 #320 ／ 類別 P — 安全與隱私 ／ 計畫版本 v1.0 ／ 2026-04-27*

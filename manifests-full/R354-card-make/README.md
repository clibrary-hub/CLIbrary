...
| 介面 | CLI（`click` 框架） |
| 設定檔 | `~/.config/card-make/config.json` |
...
```bash
card-make [OPTIONS] COMMAND [ARGS]...
```
...
# 指定輸出格式
card-make run --output json

# 詳細模式
card-make run --verbose

# 靜音（適合 pipe 到其他工具）
card-make run --quiet | jq .

# 查看設定
card-make config show

# 匯出 JSON
card-make export --output json > result.json
```
...
def config():
    """顯示或修改設定"""
    import json, pathlib
    cfg_path = pathlib.Path.home() / ".config" / "card-make" / "config.json"
    if cfg_path.exists():
...
DB_PATH = pathlib.Path.home() / ".config" / "card-make" / "data.db"
...
{
  "name": "card-make",
  "description": "知識卡片產生。解決問題：自動生成知識卡片",
  "input_schema": {
    "type": "object",
    "properties": {
      "action": {
...
```bash
# Agent 呼叫範例
result=$(card-make run --quiet --output json)
echo $result | jq .data
```
...
def call_card_make(action="run") -> dict:
    proc = subprocess.run(
        ["card-make", action, "--output", "json", "--quiet"],
...
```bash
# 管線串接範例
card-make run --quiet --output json \
  | jq -r '.data' \
  | other-tool process
```
...
# 執行測試
pytest tests/ -v --cov=card_make --cov-report=term-missing
...
```python
import pytest
from card_make.core import CardMakeCore
...
*工具編號 #354 ／ 類別 R — 知識管理與筆記 ／ 計畫版本 v1.0 ／ 2026-04-27*

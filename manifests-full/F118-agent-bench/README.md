...
| 介面 | CLI（`click` 框架） |
| 設定檔 | `~/.config/agent-bench/config.json` |
...
[project.scripts]
agent-bench = "agent_bench.cli:main"
```
...
```bash
agent-bench [OPTIONS] COMMAND [ARGS]...
```
...
# 指定輸出格式
agent-bench run --output json

# 詳細模式
agent-bench run --verbose

# 靜音（適合 pipe 到其他工具）
agent-bench run --quiet | jq .

# 查看設定
agent-bench config show

# 匯出 JSON
agent-bench export --output json > result.json
```
...
from .core import AgentBenchCore
...
def config():
    """顯示或修改設定"""
    import json, pathlib
    cfg_path = pathlib.Path.home() / ".config" / "agent-bench" / "config.json"
    if cfg_path.exists():
...
DB_PATH = pathlib.Path.home() / ".config" / "agent-bench" / "data.db"
...
{
  "name": "agent-bench",
  "description": "Agent 評測器。解決問題：不知 Agent 該選哪個",
  "input_schema": {
    "type": "object",
    "properties": {
      "action": {
...
```bash
# Agent 呼叫範例
result=$(agent-bench run --quiet --output json)
echo $result | jq .data
```
...
def call_agent_bench(action="run") -> dict:
    proc = subprocess.run(
        ["agent-bench", action, "--output", "json", "--quiet"],
...
```bash
# 管線串接範例
agent-bench run --quiet --output json \
  | jq -r '.data' \
  | other-tool process
```
...
# 執行測試
pytest tests/ -v --cov=agent_bench --cov-report=term-missing

# 快速冒煙測試
agent-bench run --help
agent-bench config show
```
...
```python
import pytest
from agent_bench.core import AgentBenchCore
...
```json
{
  "name": "agent-bench",
  "version": "0.1.0",
  "description": "Agent 評測器",
  "category": "F",
  "category_name": "Agent間通訊與協作",
  "number": 118,
  "pain_point": "不知 Agent 該選哪個",
  "commercial": "✅",
  "language": "Python",
  "entry_point": "agent-bench",
  "dependencies": ["anthropic", "click", "rich"],
...
*工具編號 #118 ／ 類別 F — Agent間通訊與協作 ／ 計畫版本 v1.0 ／ 2026-04-27*

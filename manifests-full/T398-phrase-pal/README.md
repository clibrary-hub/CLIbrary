...
| 介面 | CLI（`click` 框架） |
| 設定檔 | `~/.config/phrase-pal/config.json` |
...
[project.scripts]
phrase-pal = "phrase_pal.cli:main"
```
...
```bash
phrase-pal [OPTIONS] COMMAND [ARGS]...
```
...
# 指定輸出格式
phrase-pal run --output json

# 詳細模式
phrase-pal run --verbose

# 靜音（適合 pipe 到其他工具）
phrase-pal run --quiet | jq .

# 查看設定
phrase-pal config show

# 匯出 JSON
phrase-pal export --output json > result.json
```
...
from .core import PhrasePalCore
...
def config():
    """顯示或修改設定"""
    import json, pathlib
    cfg_path = pathlib.Path.home() / ".config" / "phrase-pal" / "config.json"
    if cfg_path.exists():
...
DB_PATH = pathlib.Path.home() / ".config" / "phrase-pal" / "data.db"
...
```bash
# Agent 呼叫範例
result=$(phrase-pal run --quiet --output json)
echo $result | jq .data
```
...
def call_phrase_pal(action="run") -> dict:
    proc = subprocess.run(
        ["phrase-pal", action, "--output", "json", "--quiet"],
...
```bash
# 管線串接範例
phrase-pal run --quiet --output json \
  | jq -r '.data' \
  | other-tool process
```
...
# 執行測試
pytest tests/ -v --cov=phrase_pal --cov-report=term-missing
...
```python
import pytest
from phrase_pal.core import PhrasePalCore
...
*工具編號 #398 ／ 類別 T — 旅遊與生活 ／ 計畫版本 v1.0 ／ 2026-04-27*

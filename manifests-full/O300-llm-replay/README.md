...
| 介面 | CLI（`click` 框架） |
| 設定檔 | `~/.config/llm-replay/config.json` |
...
[project.scripts]
llm-replay = "llm_replay.cli:main"
```
...
```bash
llm-replay [OPTIONS] COMMAND [ARGS]...
```
...
# 指定輸出格式
llm-replay run --output json

# 詳細模式
llm-replay run --verbose

# 靜音（適合 pipe 到其他工具）
llm-replay run --quiet | jq .

# 查看設定
llm-replay config show

# 匯出 JSON
llm-replay export --output json > result.json
```
...
from .core import LlmReplayCore
...
def config():
    """顯示或修改設定"""
    import json, pathlib
    cfg_path = pathlib.Path.home() / ".config" / "llm-replay" / "config.json"
    if cfg_path.exists():
...
DB_PATH = pathlib.Path.home() / ".config" / "llm-replay" / "data.db"
...
{
  "name": "llm-replay",
  "description": "流量回放器。解決問題：LLM 流量回放",
  "input_schema": {
    "type": "object",
    "properties": {
      "action": {
...
```bash
# Agent 呼叫範例
result=$(llm-replay run --quiet --output json)
echo $result | jq .data
```
...
def call_llm_replay(action="run") -> dict:
    proc = subprocess.run(
        ["llm-replay", action, "--output", "json", "--quiet"],
...
```bash
# 管線串接範例
llm-replay run --quiet --output json \
  | jq -r '.data' \
  | other-tool process
```
...
# 執行測試
pytest tests/ -v --cov=llm_replay --cov-report=term-missing
...
```python
import pytest
from llm_replay.core import LlmReplayCore
...
*工具編號 #300 ／ 類別 O — AI/ML輔助 ／ 計畫版本 v1.0 ／ 2026-04-27*

...
| 介面 | CLI（`click` 框架） |
| 設定檔 | `~/.config/sitemap-gen/config.json` |
...
[project.scripts]
sitemap-gen = "sitemap_gen.cli:main"
```
...
```bash
sitemap-gen [OPTIONS] COMMAND [ARGS]...
```
...
# 指定輸出格式
sitemap-gen run --output json

# 詳細模式
sitemap-gen run --verbose

# 靜音（適合 pipe 到其他工具）
sitemap-gen run --quiet | jq .

# 查看設定
sitemap-gen config show

# 匯出 JSON
sitemap-gen export --output json > result.json
```
...
from .core import SitemapGenCore
...
def config():
    """顯示或修改設定"""
    import json, pathlib
    cfg_path = pathlib.Path.home() / ".config" / "sitemap-gen" / "config.json"
    if cfg_path.exists():
...
DB_PATH = pathlib.Path.home() / ".config" / "sitemap-gen" / "data.db"
...
{
  "name": "sitemap-gen",
  "description": "Sitemap 產生器。解決問題：sitemap 產生",
  "input_schema": {
    "type": "object",
    "properties": {
      "action": {
...
```bash
# Agent 呼叫範例
result=$(sitemap-gen run --quiet --output json)
echo $result | jq .data
```
...
def call_sitemap_gen(action="run") -> dict:
    proc = subprocess.run(
        ["sitemap-gen", action, "--output", "json", "--quiet"],
...
```bash
# 管線串接範例
sitemap-gen run --quiet --output json \
  | jq -r '.data' \
  | other-tool process
```
...
# 執行測試
pytest tests/ -v --cov=sitemap_gen --cov-report=term-missing

# 快速冒煙測試
sitemap-gen run --help
sitemap-gen config show
```
...
```python
import pytest
from sitemap_gen.core import SitemapGenCore
...
```json
{
  "name": "sitemap-gen",
  "version": "0.1.0",
  "description": "Sitemap 產生器",
  "category": "M",
  "category_name": "網路爬蟲與監控",
  "number": 253,
  "pain_point": "sitemap 產生",
  "commercial": "✅",
  "language": "Python",
  "entry_point": "sitemap-gen",
  "dependencies": ["requests", "beautifulsoup4", "click"],
...
*工具編號 #253 ／ 類別 M — 網路爬蟲與監控 ／ 計畫版本 v1.0 ／ 2026-04-27*

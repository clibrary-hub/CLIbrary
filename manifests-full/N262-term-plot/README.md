...
| 介面 | CLI（`click` 框架） |
| 設定檔 | `~/.config/term-plot/config.json` |
...
pip install plotext click rich
...
dependencies = ["plotext", "click", "rich"]

[project.scripts]
term-plot = "term_plot.cli:main"
```
...
```bash
term-plot [OPTIONS] COMMAND [ARGS]...
```
...
# 指定輸出格式
term-plot run --output json

# 詳細模式
term-plot run --verbose

# 靜音（適合 pipe 到其他工具）
term-plot run --quiet | jq .

# 查看設定
term-plot config show

# 匯出 JSON
term-plot export --output json > result.json
```
...
def config():
    """顯示或修改設定"""
    import json, pathlib
    cfg_path = pathlib.Path.home() / ".config" / "term-plot" / "config.json"
    if cfg_path.exists():
...
DB_PATH = pathlib.Path.home() / ".config" / "term-plot" / "data.db"
...
```bash
# Agent 呼叫範例
result=$(term-plot run --quiet --output json)
echo $result | jq .data
```
...
def call_term_plot(action="run") -> dict:
    proc = subprocess.run(
        ["term-plot", action, "--output", "json", "--quiet"],
...
```bash
# 管線串接範例
term-plot run --quiet --output json \
  | jq -r '.data' \
  | other-tool process
```
...
# 執行測試
pytest tests/ -v --cov=term_plot --cov-report=term-missing
...
```python
import pytest
from term_plot.core import TermPlotCore
...
*工具編號 #262 ／ 類別 N — 資料分析與視覺化 ／ 計畫版本 v1.0 ／ 2026-04-27*

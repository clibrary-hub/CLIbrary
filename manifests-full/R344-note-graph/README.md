...
| 介面 | CLI（`click` 框架） |
| 設定檔 | `~/.config/note-graph/config.json` |
...
pip install networkx click rich
...
[project.scripts]
note-graph = "note_graph.cli:main"
```
...
```bash
note-graph [OPTIONS] COMMAND [ARGS]...
```
...
# 指定輸出格式
note-graph run --output json

# 詳細模式
note-graph run --verbose

# 靜音（適合 pipe 到其他工具）
note-graph run --quiet | jq .

# 查看設定
note-graph config show

# 匯出 JSON
note-graph export --output json > result.json
```
...
from .core import NoteGraphCore
...
def config():
    """顯示或修改設定"""
    import json, pathlib
    cfg_path = pathlib.Path.home() / ".config" / "note-graph" / "config.json"
    if cfg_path.exists():
...
DB_PATH = pathlib.Path.home() / ".config" / "note-graph" / "data.db"
...
```bash
# Agent 呼叫範例
result=$(note-graph run --quiet --output json)
echo $result | jq .data
```
...
def call_note_graph(action="run") -> dict:
    proc = subprocess.run(
        ["note-graph", action, "--output", "json", "--quiet"],
...
```bash
# 管線串接範例
note-graph run --quiet --output json \
  | jq -r '.data' \
  | other-tool process
```
...
# 執行測試
pytest tests/ -v --cov=note_graph --cov-report=term-missing
...
```python
import pytest
from note_graph.core import NoteGraphCore
...
*工具編號 #344 ／ 類別 R — 知識管理與筆記 ／ 計畫版本 v1.0 ／ 2026-04-27*

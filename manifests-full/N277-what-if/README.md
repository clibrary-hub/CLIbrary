...
| 介面 | CLI（`click` 框架） |
| 設定檔 | `~/.config/what-if/config.json` |
...
# 指定輸出格式
what-if run --output json

# 詳細模式
what-if run --verbose

# 靜音（適合 pipe 到其他工具）
what-if run --quiet | jq .

# 查看設定
what-if config show

# 匯出 JSON
what-if export --output json > result.json
```
...
def config():
    """顯示或修改設定"""
    import json, pathlib
    cfg_path = pathlib.Path.home() / ".config" / "what-if" / "config.json"
    if cfg_path.exists():
...
DB_PATH = pathlib.Path.home() / ".config" / "what-if" / "data.db"
...
```bash
# Agent 呼叫範例
result=$(what-if run --quiet --output json)
echo $result | jq .data
```
...
def call_what_if(action="run") -> dict:
    proc = subprocess.run(
        ["what-if", action, "--output", "json", "--quiet"],
...
# 執行測試
pytest tests/ -v --cov=what_if --cov-report=term-missing
...
```python
import pytest
from what_if.core import WhatIfCore
...
*工具編號 #277 ／ 類別 N — 資料分析與視覺化 ／ 計畫版本 v1.0 ／ 2026-04-27*

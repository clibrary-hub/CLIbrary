...
| 介面 | CLI（`click` 框架） |
| 設定檔 | `~/.config/pin-msg/config.json` |
...
# 指定輸出格式
pin-msg run --output json

# 詳細模式
pin-msg run --verbose

# 靜音（適合 pipe 到其他工具）
pin-msg run --quiet | jq .

# 查看設定
pin-msg config show

# 匯出 JSON
pin-msg export --output json > result.json
```
...
def config():
    """顯示或修改設定"""
    import json, pathlib
    cfg_path = pathlib.Path.home() / ".config" / "pin-msg" / "config.json"
    if cfg_path.exists():
...
DB_PATH = pathlib.Path.home() / ".config" / "pin-msg" / "data.db"
...
{
  "name": "pin-msg",
  "description": "關鍵訊息釘選器。解決問題：重要指令被前文蓋過",
  "input_schema": {
    "type": "object",
    "properties": {
      "action": {
...
```bash
# Agent 呼叫範例
result=$(pin-msg run --quiet --output json)
echo $result | jq .data
```
...
def call_pin_msg(action="run") -> dict:
    proc = subprocess.run(
        ["pin-msg", action, "--output", "json", "--quiet"],
...
# 執行測試
pytest tests/ -v --cov=pin_msg --cov-report=term-missing
...
```python
import pytest
from pin_msg.core import PinMsgCore
...
*工具編號 #025 ／ 類別 B — 對話與記憶管理 ／ 計畫版本 v1.0 ／ 2026-04-27*

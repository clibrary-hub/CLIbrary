...
| 介面 | CLI（`click` 框架） |
| 設定檔 | `~/.config/nginx-try/config.json` |
...
```bash
nginx-try [OPTIONS] COMMAND [ARGS]...
```
...
# 指定輸出格式
nginx-try run --output json

# 詳細模式
nginx-try run --verbose

# 靜音（適合 pipe 到其他工具）
nginx-try run --quiet | jq .

# 查看設定
nginx-try config show

# 匯出 JSON
nginx-try export --output json > result.json
```
...
def config():
    """顯示或修改設定"""
    import json, pathlib
    cfg_path = pathlib.Path.home() / ".config" / "nginx-try" / "config.json"
    if cfg_path.exists():
...
DB_PATH = pathlib.Path.home() / ".config" / "nginx-try" / "data.db"
...
{
  "name": "nginx-try",
  "description": "Nginx 配置試跑器。解決問題：Nginx 設定試跑",
  "input_schema": {
    "type": "object",
    "properties": {
      "action": {
...
```bash
# Agent 呼叫範例
result=$(nginx-try run --quiet --output json)
echo $result | jq .data
```
...
def call_nginx_try(action="run") -> dict:
    proc = subprocess.run(
        ["nginx-try", action, "--output", "json", "--quiet"],
...
```bash
# 管線串接範例
nginx-try run --quiet --output json \
  | jq -r '.data' \
  | other-tool process
```
...
# 執行測試
pytest tests/ -v --cov=nginx_try --cov-report=term-missing
...
```python
import pytest
from nginx_try.core import NginxTryCore
...
```json
{
  "name": "nginx-try",
  "version": "0.1.0",
  "description": "Nginx 配置試跑器",
  "category": "Q",
  "category_name": "DevOps與基礎設施",
  "number": 332,
  "pain_point": "Nginx 設定試跑",
  "commercial": "✅",
  "language": "Python",
  "entry_point": "nginx-try",
  "dependencies": ["subprocess", "click", "rich"],
...
*工具編號 #332 ／ 類別 Q — DevOps與基礎設施 ／ 計畫版本 v1.0 ／ 2026-04-27*

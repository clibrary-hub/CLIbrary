...
| 介面 | CLI（`click` 框架） |
| 設定檔 | `~/.config/blog-pub/config.json` |
...
# 指定輸出格式
blog-pub run --output json

# 詳細模式
blog-pub run --verbose

# 靜音（適合 pipe 到其他工具）
blog-pub run --quiet | jq .

# 查看設定
blog-pub config show

# 匯出 JSON
blog-pub export --output json > result.json
```
...
def config():
    """顯示或修改設定"""
    import json, pathlib
    cfg_path = pathlib.Path.home() / ".config" / "blog-pub" / "config.json"
    if cfg_path.exists():
...
DB_PATH = pathlib.Path.home() / ".config" / "blog-pub" / "data.db"
...
{
  "name": "blog-pub",
  "description": "部落格發佈器。解決問題：部落格批量發佈",
  "input_schema": {
    "type": "object",
    "properties": {
      "action": {
...
```bash
# Agent 呼叫範例
result=$(blog-pub run --quiet --output json)
echo $result | jq .data
```
...
def call_blog_pub(action="run") -> dict:
    proc = subprocess.run(
        ["blog-pub", action, "--output", "json", "--quiet"],
...
```bash
# 管線串接範例
blog-pub run --quiet --output json \
  | jq -r '.data' \
  | other-tool process
```
...
# 執行測試
pytest tests/ -v --cov=blog_pub --cov-report=term-missing
...
```python
import pytest
from blog_pub.core import BlogPubCore
...
*工具編號 #470 ／ 類別 X — 寫作與內容創作 ／ 計畫版本 v1.0 ／ 2026-04-27*

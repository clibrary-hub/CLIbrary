...
| 介面 | CLI（`click` 框架） |
| 設定檔 | `~/.config/tone-shift/config.json` |
...
[project.scripts]
tone-shift = "tone_shift.cli:main"
```
...
```bash
tone-shift [OPTIONS] COMMAND [ARGS]...
```
...
# 指定輸出格式
tone-shift run --output json

# 詳細模式
tone-shift run --verbose

# 靜音（適合 pipe 到其他工具）
tone-shift run --quiet | jq .

# 查看設定
tone-shift config show

# 匯出 JSON
tone-shift export --output json > result.json
```
...
from .core import ToneShiftCore
...
def config():
    """顯示或修改設定"""
    import json, pathlib
    cfg_path = pathlib.Path.home() / ".config" / "tone-shift" / "config.json"
    if cfg_path.exists():
...
DB_PATH = pathlib.Path.home() / ".config" / "tone-shift" / "data.db"
...
{
  "name": "tone-shift",
  "description": "文字語氣轉換。解決問題：寫信太正式想口語化",
  "input_schema": {
    "type": "object",
    "properties": {
      "action": {
...
```bash
# Agent 呼叫範例
result=$(tone-shift run --quiet --output json)
echo $result | jq .data
```
...
def call_tone_shift(action="run") -> dict:
    proc = subprocess.run(
        ["tone-shift", action, "--output", "json", "--quiet"],
...
```bash
# 管線串接範例
tone-shift run --quiet --output json \
  | jq -r '.data' \
  | other-tool process
```
...
# 執行測試
pytest tests/ -v --cov=tone_shift --cov-report=term-missing
...
```python
import pytest
from tone_shift.core import ToneShiftCore
...
```json
{
  ...
  "category_name": "郵件與通訊",
  "number": 177,
  "pain_point": "寫信太正式想口語化",
  "commercial": "✅",
  "language": "Python",
  "entry_point": "tone-shift",
  "dependencies": ["anthropic", "click", "rich"],
...
*工具編號 #177 ／ 類別 I — 郵件與通訊 ／ 計畫版本 v1.0 ／ 2026-04-27*

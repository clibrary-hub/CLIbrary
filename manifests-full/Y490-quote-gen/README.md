...
| 介面 | CLI（`click` 框架） |
| 設定檔 | `~/.config/quote-gen/config.json` |
...
```bash
quote-gen [OPTIONS] COMMAND [ARGS]...
```
...
# 指定輸出格式
quote-gen run --output json

# 詳細模式
quote-gen run --verbose

# 靜音（適合 pipe 到其他工具）
quote-gen run --quiet | jq .

# 查看設定
quote-gen config show

# 匯出 JSON
quote-gen export --output json > result.json
```
...
def config():
    """顯示或修改設定"""
    import json, pathlib
    cfg_path = pathlib.Path.home() / ".config" / "quote-gen" / "config.json"
    if cfg_path.exists():
...
DB_PATH = pathlib.Path.home() / ".config" / "quote-gen" / "data.db"
...
```bash
# Agent 呼叫範例
result=$(quote-gen run --quiet --output json)
echo $result | jq .data
```
...
def call_quote_gen(action="run") -> dict:
    proc = subprocess.run(
        ["quote-gen", action, "--output", "json", "--quiet"],
...
```bash
# 管線串接範例
quote-gen run --quiet --output json \
  | jq -r '.data' \
  | other-tool process
```
...
# 執行測試
pytest tests/ -v --cov=quote_gen --cov-report=term-missing
...
```python
import pytest
from quote_gen.core import QuoteGenCore
...
*工具編號 #490 ／ 類別 Y — 法律合約商務 ／ 計畫版本 v1.0 ／ 2026-04-27*

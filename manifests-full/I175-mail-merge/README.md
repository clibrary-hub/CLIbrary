...
| 介面 | CLI（`click` 框架） |
| 設定檔 | `~/.config/mail-merge/config.json` |
...
pip install smtplib jinja2 click
...
dependencies = ["smtplib", "jinja2", "click"]

[project.scripts]
mail-merge = "mail_merge.cli:main"
```
...
```bash
mail-merge [OPTIONS] COMMAND [ARGS]...
```
...
# 指定輸出格式
mail-merge run --output json

# 詳細模式
mail-merge run --verbose

# 靜音（適合 pipe 到其他工具）
mail-merge run --quiet | jq .

# 查看設定
mail-merge config show

# 匯出 JSON
mail-merge export --output json > result.json
```
...
from .core import MailMergeCore
...
def config():
    """顯示或修改設定"""
    import json, pathlib
    cfg_path = pathlib.Path.home() / ".config" / "mail-merge" / "config.json"
    if cfg_path.exists():
...
DB_PATH = pathlib.Path.home() / ".config" / "mail-merge" / "data.db"
...
{
  "name": "mail-merge",
  "description": "客製群發器。解決問題：寄發大量個人化信",
  "input_schema": {
    "type": "object",
    "properties": {
      "action": {
...
```bash
# Agent 呼叫範例
result=$(mail-merge run --quiet --output json)
echo $result | jq .data
```
...
def call_mail_merge(action="run") -> dict:
    proc = subprocess.run(
        ["mail-merge", action, "--output", "json", "--quiet"],
...
```bash
# 管線串接範例
mail-merge run --quiet --output json \
  | jq -r '.data' \
  | other-tool process
```
...
# 執行測試
pytest tests/ -v --cov=mail_merge --cov-report=term-missing
...
```python
import pytest
from mail_merge.core import MailMergeCore
...
*工具編號 #175 ／ 類別 I — 郵件與通訊 ／ 計畫版本 v1.0 ／ 2026-04-27*

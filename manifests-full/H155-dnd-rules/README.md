...
| 介面 | CLI（`click` 框架） |
| 設定檔 | `~/.config/dnd-rules/config.json` |
...
```bash
dnd-rules [OPTIONS] COMMAND [ARGS]...
```
...
# 指定輸出格式
dnd-rules run --output json

# 詳細模式
dnd-rules run --verbose

# 靜音（適合 pipe 到其他工具）
dnd-rules run --quiet | jq .

# 查看設定
dnd-rules config show

# 匯出 JSON
dnd-rules export --output json > result.json
```
...
def config():
    """顯示或修改設定"""
    import json, pathlib
    cfg_path = pathlib.Path.home() / ".config" / "dnd-rules" / "config.json"
    if cfg_path.exists():
...
DB_PATH = pathlib.Path.home() / ".config" / "dnd-rules" / "data.db"
...
{
  "name": "dnd-rules",
  "description": "勿擾規則器。解決問題：不在打擾時段想擋訊息",
  "input_schema": {
    "type": "object",
    "properties": {
      "action": {
...
```bash
# Agent 呼叫範例
result=$(dnd-rules run --quiet --output json)
echo $result | jq .data
```
...
def call_dnd_rules(action="run") -> dict:
    proc = subprocess.run(
        ["dnd-rules", action, "--output", "json", "--quiet"],
...
```bash
# 管線串接範例
dnd-rules run --quiet --output json \
  | jq -r '.data' \
  | other-tool process
```
...
# 執行測試
pytest tests/ -v --cov=dnd_rules --cov-report=term-missing
...
```python
import pytest
from dnd_rules.core import DndRulesCore
...
```json
{
  ...
  "number": 155,
  "pain_point": "不在打擾時段想擋訊息",
  "commercial": "⚠️",
  "language": "Python",
  "entry_point": "dnd-rules",
  "dependencies": ["click", "rich", "sqlite3"],
...
*工具編號 #155 ／ 類別 H — 行事曆與時間管理 ／ 計畫版本 v1.0 ／ 2026-04-27*

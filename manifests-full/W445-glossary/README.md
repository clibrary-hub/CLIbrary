...
| 介面 | CLI（`click` 框架） |
| 設定檔 | `~/.config/glossary/config.json` |
...
# 指定輸出格式
glossary run --output json

# 詳細模式
glossary run --verbose

# 靜音（適合 pipe 到其他工具）
glossary run --quiet | jq .

# 查看設定
glossary config show

# 匯出 JSON
glossary export --output json > result.json
```
...
def config():
    """顯示或修改設定"""
    import json, pathlib
    cfg_path = pathlib.Path.home() / ".config" / "glossary" / "config.json"
    if cfg_path.exists():
...
DB_PATH = pathlib.Path.home() / ".config" / "glossary" / "data.db"
...
```bash
# Agent 呼叫範例
result=$(glossary run --quiet --output json)
echo $result | jq .data
```
...
def call_glossary(action="run") -> dict:
    proc = subprocess.run(
        ["glossary", action, "--output", "json", "--quiet"],
...
```bash
# 管線串接範例
glossary run --quiet --output json \
  | jq -r '.data' \
  | other-tool process
```
...
# 執行測試
pytest tests/ -v --cov=glossary --cov-report=term-missing
...
```python
import pytest
from glossary.core import GlossaryCore
...
*工具編號 #445 ／ 類別 W — 翻譯與在地化 ／ 計畫版本 v1.0 ／ 2026-04-27*

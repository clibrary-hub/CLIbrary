...
| 介面 | CLI（`click` 框架） |
| 設定檔 | `~/.config/auto-reply/config.json` |
...
[project.scripts]
auto-reply = "auto_reply.cli:main"
```
...
```bash
auto-reply [OPTIONS] COMMAND [ARGS]...
```
...
# 指定輸出格式
auto-reply run --output json

# 詳細模式
auto-reply run --verbose

# 靜音（適合 pipe 到其他工具）
auto-reply run --quiet | jq .

# 查看設定
auto-reply config show

# 匯出 JSON
auto-reply export --output json > result.json
```
...
from .core import AutoReplyCore
...
def config():
    """顯示或修改設定"""
    import json, pathlib
    cfg_path = pathlib.Path.home() / ".config" / "auto-reply" / "config.json"
    if cfg_path.exists():
...
DB_PATH = pathlib.Path.home() / ".config" / "auto-reply" / "data.db"
...
{
  "name": "auto-reply",
  "description": "規則式自動回。解決問題：想對訊息自動回應",
  "input_schema": {
    "type": "object",
    "properties": {
      "action": {
...
```bash
# Agent 呼叫範例
result=$(auto-reply run --quiet --output json)
echo $result | jq .data
```
...
def call_auto_reply(action="run") -> dict:
    proc = subprocess.run(
        ["auto-reply", action, "--output", "json", "--quiet"],
...
```bash
# 管線串接範例
auto-reply run --quiet --output json \
  | jq -r '.data' \
  | other-tool process
```
...
# 執行測試
pytest tests/ -v --cov=auto_reply --cov-report=term-missing
...
```python
import pytest
from auto_reply.core import AutoReplyCore
...
*工具編號 #172 ／ 類別 I — 郵件與通訊 ／ 計畫版本 v1.0 ／ 2026-04-27*

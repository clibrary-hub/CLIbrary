...
| 介面 | CLI（`click` 框架） |
| 設定檔 | `~/.config/i18n-seo/config.json` |
...
# 指定輸出格式
i18n-seo run --output json

# 詳細模式
i18n-seo run --verbose

# 靜音（適合 pipe 到其他工具）
i18n-seo run --quiet | jq .

# 查看設定
i18n-seo config show

# 匯出 JSON
i18n-seo export --output json > result.json
```
...
def config():
    """顯示或修改設定"""
    import json, pathlib
    cfg_path = pathlib.Path.home() / ".config" / "i18n-seo" / "config.json"
    if cfg_path.exists():
...
DB_PATH = pathlib.Path.home() / ".config" / "i18n-seo" / "data.db"
...
{
  "name": "i18n-seo",
  "description": "i18n SEO 工具。解決問題：多語 SEO 標籤",
  "input_schema": {
    "type": "object",
    "properties": {
      "action": {
...
```bash
# Agent 呼叫範例
result=$(i18n-seo run --quiet --output json)
echo $result | jq .data
```
...
def call_i18n_seo(action="run") -> dict:
    proc = subprocess.run(
        ["i18n-seo", action, "--output", "json", "--quiet"],
...
```bash
# 管線串接範例
i18n-seo run --quiet --output json \
  | jq -r '.data' \
  | other-tool process
```
...
# 執行測試
pytest tests/ -v --cov=i18n_seo --cov-report=term-missing
...
```python
import pytest
from i18n_seo.core import I18NSeoCore
...
```json
{
  "name": "i18n-seo",
  "version": "0.1.0",
  "description": "i18n SEO 工具",
  "category": "W",
  "category_name": "翻譯與在地化",
  "number": 450,
  "pain_point": "多語 SEO 標籤",
  "commercial": "✅",
  "language": "Python",
  "entry_point": "i18n-seo",
  "dependencies": ["click", "rich", "pyyaml"],
...
*工具編號 #450 ／ 類別 W — 翻譯與在地化 ／ 計畫版本 v1.0 ／ 2026-04-27*

...
| 介面 | CLI（`click` 框架） |
| 設定檔 | `~/.config/sub-cost/config.json` |
...
# 指定輸出格式
sub-cost run --output json

# 詳細模式
sub-cost run --verbose

# 靜音（適合 pipe 到其他工具）
sub-cost run --quiet | jq .

# 查看設定
sub-cost config show

# 匯出 JSON
sub-cost export --output json > result.json
```
...
def config():
    """顯示或修改設定"""
    import json, pathlib
    cfg_path = pathlib.Path.home() / ".config" / "sub-cost" / "config.json"
    if cfg_path.exists():
...
DB_PATH = pathlib.Path.home() / ".config" / "sub-cost" / "data.db"
...
{
  "name": "sub-cost",
  "description": "訂閱總帳器。解決問題：訂閱費用不知總額",
  "input_schema": {
    "type": "object",
    "properties": {
      "action": {
...
```bash
# Agent 呼叫範例
result=$(sub-cost run --quiet --output json)
echo $result | jq .data
```
...
def call_sub_cost(action="run") -> dict:
    proc = subprocess.run(
        ["sub-cost", action, "--output", "json", "--quiet"],
...
```bash
# 管線串接範例
sub-cost run --quiet --output json \
  | jq -r '.data' \
  | other-tool process
```
...
# 執行測試
pytest tests/ -v --cov=sub_cost --cov-report=term-missing
...
```python
import pytest
from sub_cost.core import SubCostCore
...
*工具編號 #184 ／ 類別 J — 財務與帳務 ／ 計畫版本 v1.0 ／ 2026-04-27*

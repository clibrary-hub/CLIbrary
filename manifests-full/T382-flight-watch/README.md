...
| 介面 | CLI（`click` 框架） |
| 設定檔 | `~/.config/flight-watch/config.json` |
...
[project.scripts]
flight-watch = "flight_watch.cli:main"
```
...
```bash
flight-watch [OPTIONS] COMMAND [ARGS]...
```
...
# 指定輸出格式
flight-watch run --output json

# 詳細模式
flight-watch run --verbose

# 靜音（適合 pipe 到其他工具）
flight-watch run --quiet | jq .

# 查看設定
flight-watch config show

# 匯出 JSON
flight-watch export --output json > result.json
```
...
from .core import FlightWatchCore
...
def config():
    """顯示或修改設定"""
    import json, pathlib
    cfg_path = pathlib.Path.home() / ".config" / "flight-watch" / "config.json"
    if cfg_path.exists():
...
DB_PATH = pathlib.Path.home() / ".config" / "flight-watch" / "data.db"
...
```bash
# Agent 呼叫範例
result=$(flight-watch run --quiet --output json)
echo $result | jq .data
```
...
def call_flight_watch(action="run") -> dict:
    proc = subprocess.run(
        ["flight-watch", action, "--output", "json", "--quiet"],
...
```bash
# 管線串接範例
flight-watch run --quiet --output json \
  | jq -r '.data' \
  | other-tool process
```
...
# 執行測試
pytest tests/ -v --cov=flight_watch --cov-report=term-missing

# 快速冒煙測試
flight-watch run --help
flight-watch config show
```
...
```python
import pytest
from flight_watch.core import FlightWatchCore
...
*工具編號 #382 ／ 類別 T — 旅遊與生活 ／ 計畫版本 v1.0 ／ 2026-04-27*

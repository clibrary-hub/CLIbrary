...
| 介面 | CLI（`click` 框架） |
| 設定檔 | `~/.config/agent-iface/config.json` |
...
[project.scripts]
agent-iface = "agent_iface.cli:main"
```
...
```bash
agent-iface [OPTIONS] COMMAND [ARGS]...
```
...
# 指定輸出格式
agent-iface run --output json

# 詳細模式
agent-iface run --verbose

# 靜音（適合 pipe 到其他工具）
agent-iface run --quiet | jq .

# 查看設定
agent-iface config show

# 匯出 JSON
agent-iface export --output json > result.json
```
...
from .core import AgentIfaceCore
...
def config():
    """顯示或修改設定"""
    import json, pathlib
    cfg_path = pathlib.Path.home() / ".config" / "agent-iface" / "config.json"
    if cfg_path.exists():
...
DB_PATH = pathlib.Path.home() / ".config" / "agent-iface" / "data.db"
...
{
  "name": "agent-iface",
  "description": "Agent 介面適配器。解決問題：Agent 介面規格不一致",
  "input_schema": {
    "type": "object",
    "properties": {
      "action": {
...
```bash
# Agent 呼叫範例
result=$(agent-iface run --quiet --output json)
echo $result | jq .data
```
...
def call_agent_iface(action="run") -> dict:
    proc = subprocess.run(
        ["agent-iface", action, "--output", "json", "--quiet"],
...
```bash
# 管線串接範例
agent-iface run --quiet --output json \
  | jq -r '.data' \
  | other-tool process
```
...
# 執行測試
pytest tests/ -v --cov=agent_iface --cov-report=term-missing

# 快速冒煙測試
agent-iface run --help
agent-iface config show
```
...
```python
import pytest
from agent_iface.core import AgentIfaceCore
...
```json
{
  "name": "agent-iface",
  "version": "0.1.0",
  "description": "Agent 介面適配器",
  "category": "F",
  "category_name": "Agent間通訊與協作",
  "number": 102,
  "pain_point": "Agent 介面規格不一致",
  "commercial": "✅",
  "language": "Python",
  "entry_point": "agent-iface",
  "dependencies": ["click", "rich", "pydantic"],
...
*工具編號 #102 ／ 類別 F — Agent間通訊與協作 ／ 計畫版本 v1.0 ／ 2026-04-27*

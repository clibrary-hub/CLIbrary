...
| 介面 | CLI（`click` 框架） |
| 設定檔 | `~/.config/crypto-portfolio/config.json` |
...
```bash
crypto-portfolio [OPTIONS] COMMAND [ARGS]...
```
...
```bash
# 基本執行
crypto-portfolio run

# 指定輸出格式
crypto-portfolio run --output json

# 詳細模式
crypto-portfolio run --verbose

# 靜音（適合 pipe 到其他工具）
crypto-portfolio run --quiet | jq .

# 查看設定
crypto-portfolio config show

# 匯出 JSON
crypto-portfolio export --output json > result.json
```
...
from .core import CryptoPortfolioCore
...
def config():
    """顯示或修改設定"""
    import json, pathlib
    cfg_path = pathlib.Path.home() / ".config" / "crypto-portfolio" / "config.json"
    if cfg_path.exists():
...
DB_PATH = pathlib.Path.home() / ".config" / "crypto-portfolio" / "data.db"
...
{
  "name": "crypto-portfolio",
  "description": "加密資產聚合。解決問題：加密貨幣資產統合",
  "input_schema": {
    "type": "object",
    "properties": {
      "action": {
...
```bash
# Agent 呼叫範例
result=$(crypto-portfolio run --quiet --output json)
echo $result | jq .data
```
...
def call_crypto_portfolio(action="run") -> dict:
    proc = subprocess.run(
        ["crypto-portfolio", action, "--output", "json", "--quiet"],
...
```bash
# 管線串接範例
crypto-portfolio run --quiet --output json \
  | jq -r '.data' \
  | other-tool process
```
...
# 執行測試
pytest tests/ -v --cov=crypto_portfolio --cov-report=term-missing

# 快速冒煙測試
crypto-portfolio run --help
crypto-portfolio config show
```
...
```python
import pytest
from crypto_portfolio.core import CryptoPortfolioCore
...
*工具編號 #187 ／ 類別 J — 財務與帳務 ／ 計畫版本 v1.0 ／ 2026-04-27*

...
| 介面 | CLI（`click` 框架） |
| 設定檔 | `~/.config/study-plan/config.json` |
...
[project.scripts]
study-plan = "study_plan.cli:main"
```
...
```bash
study-plan [OPTIONS] COMMAND [ARGS]...
```
...
# 指定輸出格式
study-plan run --output json

# 詳細模式
study-plan run --verbose

# 靜音（適合 pipe 到其他工具）
study-plan run --quiet | jq .

# 查看設定
study-plan config show

# 匯出 JSON
study-plan export --output json > result.json
```
...
from .core import StudyPlanCore
...
def config():
    """顯示或修改設定"""
    import json, pathlib
    cfg_path = pathlib.Path.home() / ".config" / "study-plan" / "config.json"
    if cfg_path.exists():
...
DB_PATH = pathlib.Path.home() / ".config" / "study-plan" / "data.db"
...
{
  "name": "study-plan",
  "description": "學習計畫追蹤。解決問題：學習計畫沒有進度條",
  "input_schema": {
    "type": "object",
    "properties": {
      "action": {
...
```bash
# Agent 呼叫範例
result=$(study-plan run --quiet --output json)
echo $result | jq .data
```
...
def call_study_plan(action="run") -> dict:
    proc = subprocess.run(
        ["study-plan", action, "--output", "json", "--quiet"],
...
```bash
# 管線串接範例
study-plan run --quiet --output json \
  | jq -r '.data' \
  | other-tool process
```
...
# 執行測試
pytest tests/ -v --cov=study_plan --cov-report=term-missing
...
```python
import pytest
from study_plan.core import StudyPlanCore
...
```json
{
  ...
  "number": 157,
  "pain_point": "學習計畫沒有進度條",
  "commercial": "✅",
  "language": "Python",
  "entry_point": "study-plan",
  "dependencies": ["click", "rich", "sqlite3"],
...
*工具編號 #157 ／ 類別 H — 行事曆與時間管理 ／ 計畫版本 v1.0 ／ 2026-04-27*

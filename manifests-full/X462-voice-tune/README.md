...
| 介面 | CLI（`click` 框架） |
| 設定檔 | `~/.config/voice-tune/config.json` |
...
[project.scripts]
voice-tune = "voice_tune.cli:main"
```
...
```bash
voice-tune [OPTIONS] COMMAND [ARGS]...
```
...
# 指定輸出格式
voice-tune run --output json

# 詳細模式
voice-tune run --verbose

# 靜音（適合 pipe 到其他工具）
voice-tune run --quiet | jq .

# 查看設定
voice-tune config show

# 匯出 JSON
voice-tune export --output json > result.json
```
...
from .core import VoiceTuneCore
...
def config():
    """顯示或修改設定"""
    import json, pathlib
    cfg_path = pathlib.Path.home() / ".config" / "voice-tune" / "config.json"
    if cfg_path.exists():
...
DB_PATH = pathlib.Path.home() / ".config" / "voice-tune" / "data.db"
...
```bash
# Agent 呼叫範例
result=$(voice-tune run --quiet --output json)
echo $result | jq .data
```
...
def call_voice_tune(action="run") -> dict:
    proc = subprocess.run(
        ["voice-tune", action, "--output", "json", "--quiet"],
...
```bash
# 管線串接範例
voice-tune run --quiet --output json \
  | jq -r '.data' \
  | other-tool process
```
...
# 執行測試
pytest tests/ -v --cov=voice_tune --cov-report=term-missing
...
```python
import pytest
from voice_tune.core import VoiceTuneCore
...
*工具編號 #462 ／ 類別 X — 寫作與內容創作 ／ 計畫版本 v1.0 ／ 2026-04-27*

...
| 介面 | CLI（`click` 框架） |
| 設定檔 | `~/.config/bizplan-kpi/config.json` |
...
[project.scripts]
bizplan-kpi = "bizplan_kpi.cli:main"
```
...
```bash
bizplan-kpi [OPTIONS] COMMAND [ARGS]...
```
...
# 指定輸出格式
bizplan-kpi run --output json

# 詳細模式
bizplan-kpi run --verbose

# 靜音（適合 pipe 到其他工具）
bizplan-kpi run --quiet | jq .

# 查看設定
bizplan-kpi config show

# 匯出 JSON
bizplan-kpi export --output json > result.json
```
...
from .core import BizplanKpiCore
...
def config():
    """顯示或修改設定"""
    import json, pathlib
    cfg_path = pathlib.Path.home() / ".config" / "bizplan-kpi" / "config.json"
    if cfg_path.exists():
...
DB_PATH = pathlib.Path.home() / ".config" / "bizplan-kpi" / "data.db"
...
{
  "name": "bizplan-kpi",
  "description": "商計 KPI 整理。解決問題：商業計畫 KPI 整理",
  "input_schema": {
    "type": "object",
    "properties": {
      "action": {
...
```bash
# Agent 呼叫範例
result=$(bizplan-kpi run --quiet --output json)
echo $result | jq .data
```
...
def call_bizplan_kpi(action="run") -> dict:
    proc = subprocess.run(
        ["bizplan-kpi", action, "--output", "json", "--quiet"],
...
```bash
# 管線串接範例
bizplan-kpi run --quiet --output json \
  | jq -r '.data' \
  | other-tool process
```
...
# 執行測試
pytest tests/ -v --cov=bizplan_kpi --cov-report=term-missing

# 快速冒煙測試
bizplan-kpi run --help
bizplan-kpi config show
```
...
```python
import pytest
from bizplan_kpi.core import BizplanKpiCore
...
```json
{
  "name": "bizplan-kpi",
  "version": "0.1.0",
  "description": "商計 KPI 整理",
  "category": "Y",
  "category_name": "法律合約商務",
  "number": 499,
  "pain_point": "商業計畫 KPI 整理",
  "commercial": "✅",
  "language": "Python",
  "entry_point": "bizplan-kpi",
  "dependencies": ["anthropic", "click", "rich"],
...
*工具編號 #499 ／ 類別 Y — 法律合約商務 ／ 計畫版本 v1.0 ／ 2026-04-27*

...
| 介面 | CLI（`click` 框架） |
| 設定檔 | `~/.config/tm-prelim/config.json` |
...
```bash
tm-prelim [OPTIONS] COMMAND [ARGS]...
```
...
# 指定輸出格式
tm-prelim run --output json

# 詳細模式
tm-prelim run --verbose

# 靜音（適合 pipe 到其他工具）
tm-prelim run --quiet | jq .

# 查看設定
tm-prelim config show

# 匯出 JSON
tm-prelim export --output json > result.json
```
...
def config():
    """顯示或修改設定"""
    import json, pathlib
    cfg_path = pathlib.Path.home() / ".config" / "tm-prelim" / "config.json"
    if cfg_path.exists():
...
DB_PATH = pathlib.Path.home() / ".config" / "tm-prelim" / "data.db"
...
```bash
# Agent 呼叫範例
result=$(tm-prelim run --quiet --output json)
echo $result | jq .data
```
...
def call_tm_prelim(action="run") -> dict:
    proc = subprocess.run(
        ["tm-prelim", action, "--output", "json", "--quiet"],
...
```bash
# 管線串接範例
tm-prelim run --quiet --output json \
  | jq -r '.data' \
  | other-tool process
```
...
# 執行測試
pytest tests/ -v --cov=tm_prelim --cov-report=term-missing
...
```python
import pytest
from tm_prelim.core import TmPrelimCore
...
*工具編號 #488 ／ 類別 Y — 法律合約商務 ／ 計畫版本 v1.0 ／ 2026-04-27*

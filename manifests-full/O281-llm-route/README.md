...
| 介面 | CLI（`click` 框架） |
| 設定檔 | `~/.config/llm-route/config.json` |
...
```bash
llm-route [OPTIONS] COMMAND [ARGS]...
```
...
# 指定輸出格式
llm-route run --output json

# 詳細模式
llm-route run --verbose

# 靜音（適合 pipe 到其他工具）
llm-route run --quiet | jq .

# 查看設定
llm-route config show

# 匯出 JSON
llm-route export --output json > result.json
```
...
def config():
    """顯示或修改設定"""
    import json, pathlib
    cfg_path = pathlib.Path.home() / ".config" / "llm-route" / "config.json"
    if cfg_path.exists():
...
DB_PATH = pathlib.Path.home() / ".config" / "llm-route" / "data.db"
...
```bash
# Agent 呼叫範例
result=$(llm-route run --quiet --output json)
echo $result | jq .data
```
...
def call_llm_route(action="run") -> dict:
    proc = subprocess.run(
        ["llm-route", action, "--output", "json", "--quiet"],
...
```bash
# 管線串接範例
llm-route run --quiet --output json \
  | jq -r '.data' \
  | other-tool process
```
...
# 執行測試
pytest tests/ -v --cov=llm_route --cov-report=term-missing
...
```python
import pytest
from llm_route.core import LlmRouteCore
...
*工具編號 #281 ／ 類別 O — AI/ML輔助 ／ 計畫版本 v1.0 ／ 2026-04-27*

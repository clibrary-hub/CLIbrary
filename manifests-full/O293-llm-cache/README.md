...
| 介面 | CLI（`click` 框架） |
| 設定檔 | `~/.config/llm-cache/config.json` |
...
```bash
llm-cache [OPTIONS] COMMAND [ARGS]...
```
...
# 指定輸出格式
llm-cache run --output json

# 詳細模式
llm-cache run --verbose

# 靜音（適合 pipe 到其他工具）
llm-cache run --quiet | jq .

# 查看設定
llm-cache config show

# 匯出 JSON
llm-cache export --output json > result.json
```
...
def config():
    """顯示或修改設定"""
    import json, pathlib
    cfg_path = pathlib.Path.home() / ".config" / "llm-cache" / "config.json"
    if cfg_path.exists():
...
DB_PATH = pathlib.Path.home() / ".config" / "llm-cache" / "data.db"
...
{
  "name": "llm-cache",
  "description": "LLM 快取。解決問題：LLM caching 層",
  "input_schema": {
    "type": "object",
    "properties": {
      "action": {
...
```bash
# Agent 呼叫範例
result=$(llm-cache run --quiet --output json)
echo $result | jq .data
```
...
def call_llm_cache(action="run") -> dict:
    proc = subprocess.run(
        ["llm-cache", action, "--output", "json", "--quiet"],
...
```bash
# 管線串接範例
llm-cache run --quiet --output json \
  | jq -r '.data' \
  | other-tool process
```
...
# 執行測試
pytest tests/ -v --cov=llm_cache --cov-report=term-missing
...
```python
import pytest
from llm_cache.core import LlmCacheCore
...
```json
{
  ...
  "number": 293,
  "pain_point": "LLM caching 層",
  "commercial": "⚠️",
  "language": "Python",
  "entry_point": "llm-cache",
  "dependencies": ["redis", "click", "rich"],
...
*工具編號 #293 ／ 類別 O — AI/ML輔助 ／ 計畫版本 v1.0 ／ 2026-04-27*

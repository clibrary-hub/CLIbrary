...
| 介面 | CLI（`click` 框架） |
| 設定檔 | `~/.config/fig-pull/config.json` |
...
# 指定輸出格式
fig-pull run --output json

# 詳細模式
fig-pull run --verbose

# 靜音（適合 pipe 到其他工具）
fig-pull run --quiet | jq .

# 查看設定
fig-pull config show

# 匯出 JSON
fig-pull export --output json > result.json
```
...
def config():
    """顯示或修改設定"""
    import json, pathlib
    cfg_path = pathlib.Path.home() / ".config" / "fig-pull" / "config.json"
    if cfg_path.exists():
...
DB_PATH = pathlib.Path.home() / ".config" / "fig-pull" / "data.db"
...
{
  "name": "fig-pull",
  "description": "圖表抽取器。解決問題：跨論文圖表抽取",
  "input_schema": {
    "type": "object",
    "properties": {
      "action": {
...
```bash
# Agent 呼叫範例
result=$(fig-pull run --quiet --output json)
echo $result | jq .data
```
...
def call_fig_pull(action="run") -> dict:
    proc = subprocess.run(
        ["fig-pull", action, "--output", "json", "--quiet"],
...
```bash
# 管線串接範例
fig-pull run --quiet --output json \
  | jq -r '.data' \
  | other-tool process
```
...
# 執行測試
pytest tests/ -v --cov=fig_pull --cov-report=term-missing
...
```python
import pytest
from fig_pull.core import FigPullCore
...
*工具編號 #374 ／ 類別 S — 學習與研究 ／ 計畫版本 v1.0 ／ 2026-04-27*

...
| 介面 | CLI（`click` 框架） |
| 設定檔 | `~/.config/stay-compare/config.json` |
...
[project.scripts]
stay-compare = "stay_compare.cli:main"
```
...
```bash
stay-compare [OPTIONS] COMMAND [ARGS]...
```
...
# 指定輸出格式
stay-compare run --output json

# 詳細模式
stay-compare run --verbose

# 靜音（適合 pipe 到其他工具）
stay-compare run --quiet | jq .

# 查看設定
stay-compare config show

# 匯出 JSON
stay-compare export --output json > result.json
```
...
from .core import StayCompareCore
...
def config():
    """顯示或修改設定"""
    import json, pathlib
    cfg_path = pathlib.Path.home() / ".config" / "stay-compare" / "config.json"
    if cfg_path.exists():
...
DB_PATH = pathlib.Path.home() / ".config" / "stay-compare" / "data.db"
...
```bash
# Agent 呼叫範例
result=$(stay-compare run --quiet --output json)
echo $result | jq .data
```
...
def call_stay_compare(action="run") -> dict:
    proc = subprocess.run(
        ["stay-compare", action, "--output", "json", "--quiet"],
...
```bash
# 管線串接範例
stay-compare run --quiet --output json \
  | jq -r '.data' \
  | other-tool process
```
...
# 執行測試
pytest tests/ -v --cov=stay_compare --cov-report=term-missing

# 快速冒煙測試
stay-compare run --help
stay-compare config show
```
...
```python
import pytest
from stay_compare.core import StayCompareCore
...
*工具編號 #383 ／ 類別 T — 旅遊與生活 ／ 計畫版本 v1.0 ／ 2026-04-27*

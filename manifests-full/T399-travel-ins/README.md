...
| 介面 | CLI（`click` 框架） |
| 設定檔 | `~/.config/travel-ins/config.json` |
...
[project.scripts]
travel-ins = "travel_ins.cli:main"
```
...
```bash
travel-ins [OPTIONS] COMMAND [ARGS]...
```
...
# 指定輸出格式
travel-ins run --output json

# 詳細模式
travel-ins run --verbose

# 靜音（適合 pipe 到其他工具）
travel-ins run --quiet | jq .

# 查看設定
travel-ins config show

# 匯出 JSON
travel-ins export --output json > result.json
```
...
from .core import TravelInsCore
...
def config():
    """顯示或修改設定"""
    import json, pathlib
    cfg_path = pathlib.Path.home() / ".config" / "travel-ins" / "config.json"
    if cfg_path.exists():
...
DB_PATH = pathlib.Path.home() / ".config" / "travel-ins" / "data.db"
...
```bash
# Agent 呼叫範例
result=$(travel-ins run --quiet --output json)
echo $result | jq .data
```
...
def call_travel_ins(action="run") -> dict:
    proc = subprocess.run(
        ["travel-ins", action, "--output", "json", "--quiet"],
...
```bash
# 管線串接範例
travel-ins run --quiet --output json \
  | jq -r '.data' \
  | other-tool process
```
...
# 執行測試
pytest tests/ -v --cov=travel_ins --cov-report=term-missing
...
```python
import pytest
from travel_ins.core import TravelInsCore
...
*工具編號 #399 ／ 類別 T — 旅遊與生活 ／ 計畫版本 v1.0 ／ 2026-04-27*

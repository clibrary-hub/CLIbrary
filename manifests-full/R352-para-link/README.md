...
| 介面 | CLI（`click` 框架） |
| 設定檔 | `~/.config/para-link/config.json` |
...
```bash
para-link [OPTIONS] COMMAND [ARGS]...
```
...
# 指定輸出格式
para-link run --output json

# 詳細模式
para-link run --verbose

# 靜音（適合 pipe 到其他工具）
para-link run --quiet | jq .

# 查看設定
para-link config show

# 匯出 JSON
para-link export --output json > result.json
```
...
def config():
    """顯示或修改設定"""
    import json, pathlib
    cfg_path = pathlib.Path.home() / ".config" / "para-link" / "config.json"
    if cfg_path.exists():
...
DB_PATH = pathlib.Path.home() / ".config" / "para-link" / "data.db"
...
```bash
# Agent 呼叫範例
result=$(para-link run --quiet --output json)
echo $result | jq .data
```
...
def call_para_link(action="run") -> dict:
    proc = subprocess.run(
        ["para-link", action, "--output", "json", "--quiet"],
...
```bash
# 管線串接範例
para-link run --quiet --output json \
  | jq -r '.data' \
  | other-tool process
```
...
# 執行測試
pytest tests/ -v --cov=para_link --cov-report=term-missing
...
```python
import pytest
from para_link.core import ParaLinkCore
...
*工具編號 #352 ／ 類別 R — 知識管理與筆記 ／ 計畫版本 v1.0 ／ 2026-04-27*

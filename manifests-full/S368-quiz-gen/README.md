...
| 介面 | CLI（`click` 框架） |
| 設定檔 | `~/.config/quiz-gen/config.json` |
...
# 指定輸出格式
quiz-gen run --output json

# 詳細模式
quiz-gen run --verbose

# 靜音（適合 pipe 到其他工具）
quiz-gen run --quiet | jq .

# 查看設定
quiz-gen config show

# 匯出 JSON
quiz-gen export --output json > result.json
```
...
def config():
    """顯示或修改設定"""
    import json, pathlib
    cfg_path = pathlib.Path.home() / ".config" / "quiz-gen" / "config.json"
    if cfg_path.exists():
...
DB_PATH = pathlib.Path.home() / ".config" / "quiz-gen" / "data.db"
...
{
  "name": "quiz-gen",
  "description": "題目產生器。解決問題：自我測驗題目產生",
  "input_schema": {
    "type": "object",
    "properties": {
      "action": {
...
```bash
# Agent 呼叫範例
result=$(quiz-gen run --quiet --output json)
echo $result | jq .data
```
...
def call_quiz_gen(action="run") -> dict:
    proc = subprocess.run(
        ["quiz-gen", action, "--output", "json", "--quiet"],
...
```bash
# 管線串接範例
quiz-gen run --quiet --output json \
  | jq -r '.data' \
  | other-tool process
```
...
# 執行測試
pytest tests/ -v --cov=quiz_gen --cov-report=term-missing
...
```python
import pytest
from quiz_gen.core import QuizGenCore
...
*工具編號 #368 ／ 類別 S — 學習與研究 ／ 計畫版本 v1.0 ／ 2026-04-27*

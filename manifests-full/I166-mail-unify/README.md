...
| 介面 | CLI（`click` 框架） |
| 設定檔 | `~/.config/mail-unify/config.json` |
...
pip install imaplib click rich
...
dependencies = ["imaplib", "click", "rich"]

[project.scripts]
mail-unify = "mail_unify.cli:main"
```
...
```bash
mail-unify [OPTIONS] COMMAND [ARGS]...
```
...
# 指定輸出格式
mail-unify run --output json

# 詳細模式
mail-unify run --verbose

# 靜音（適合 pipe 到其他工具）
mail-unify run --quiet | jq .

# 查看設定
mail-unify config show

# 匯出 JSON
mail-unify export --output json > result.json
```
...
from .core import MailUnifyCore
...
def config():
    """顯示或修改設定"""
    import json, pathlib
    cfg_path = pathlib.Path.home() / ".config" / "mail-unify" / "config.json"
    if cfg_path.exists():
...
DB_PATH = pathlib.Path.home() / ".config" / "mail-unify" / "data.db"
...
{
  "name": "mail-unify",
  "description": "跨帳戶統合。解決問題：跨郵箱整合麻煩",
  "input_schema": {
    "type": "object",
    "properties": {
      "action": {
...
```bash
# Agent 呼叫範例
result=$(mail-unify run --quiet --output json)
echo $result | jq .data
```
...
def call_mail_unify(action="run") -> dict:
    proc = subprocess.run(
        ["mail-unify", action, "--output", "json", "--quiet"],
...
```bash
# 管線串接範例
mail-unify run --quiet --output json \
  | jq -r '.data' \
  | other-tool process
```
...
# 執行測試
pytest tests/ -v --cov=mail_unify --cov-report=term-missing
...
```python
import pytest
from mail_unify.core import MailUnifyCore
...
*工具編號 #166 ／ 類別 I — 郵件與通訊 ／ 計畫版本 v1.0 ／ 2026-04-27*

...
| 介面 | CLI（`click` 框架） |
| 設定檔 | `~/.config/link-check/config.json` |
...
[project.scripts]
link-check = "link_check.cli:main"
```
...
```bash
link-check [OPTIONS] COMMAND [ARGS]...
```
...
# 指定輸出格式
link-check run --output json

# 詳細模式
link-check run --verbose

# 靜音（適合 pipe 到其他工具）
link-check run --quiet | jq .

# 查看設定
link-check config show

# 匯出 JSON
link-check export --output json > result.json
```
...
from .core import LinkCheckCore
...
def config():
    """顯示或修改設定"""
    import json, pathlib
    cfg_path = pathlib.Path.home() / ".config" / "link-check" / "config.json"
    if cfg_path.exists():
...
DB_PATH = pathlib.Path.home() / ".config" / "link-check" / "data.db"
...
```bash
# Agent 呼叫範例
result=$(link-check run --quiet --output json)
echo $result | jq .data
```
...
def call_link_check(action="run") -> dict:
    proc = subprocess.run(
        ["link-check", action, "--output", "json", "--quiet"],
...
```bash
# 管線串接範例
link-check run --quiet --output json \
  | jq -r '.data' \
  | other-tool process
```
...
# 執行測試
pytest tests/ -v --cov=link_check --cov-report=term-missing
...
```python
import pytest
from link_check.core import LinkCheckCore
...
*工具編號 #249 ／ 類別 M — 網路爬蟲與監控 ／ 計畫版本 v1.0 ／ 2026-04-27*

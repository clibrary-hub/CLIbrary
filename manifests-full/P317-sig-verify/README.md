...
| 介面 | CLI（`click` 框架） |
| 設定檔 | `~/.config/sig-verify/config.json` |
...
[project.scripts]
sig-verify = "sig_verify.cli:main"
```
...
```bash
sig-verify [OPTIONS] COMMAND [ARGS]...
```
...
# 指定輸出格式
sig-verify run --output json

# 詳細模式
sig-verify run --verbose

# 靜音（適合 pipe 到其他工具）
sig-verify run --quiet | jq .

# 查看設定
sig-verify config show

# 匯出 JSON
sig-verify export --output json > result.json
```
...
from .core import SigVerifyCore
...
def config():
    """顯示或修改設定"""
    import json, pathlib
    cfg_path = pathlib.Path.home() / ".config" / "sig-verify" / "config.json"
    if cfg_path.exists():
...
DB_PATH = pathlib.Path.home() / ".config" / "sig-verify" / "data.db"
...
```bash
# Agent 呼叫範例
result=$(sig-verify run --quiet --output json)
echo $result | jq .data
```
...
def call_sig_verify(action="run") -> dict:
    proc = subprocess.run(
        ["sig-verify", action, "--output", "json", "--quiet"],
...
```bash
# 管線串接範例
sig-verify run --quiet --output json \
  | jq -r '.data' \
  | other-tool process
```
...
# 執行測試
pytest tests/ -v --cov=sig_verify --cov-report=term-missing
...
```python
import pytest
from sig_verify.core import SigVerifyCore
...
*工具編號 #317 ／ 類別 P — 安全與隱私 ／ 計畫版本 v1.0 ／ 2026-04-27*

...
| 介面 | CLI（`click` 框架） |
| 設定檔 | `~/.config/transit-time/config.json` |
...
[project.scripts]
transit-time = "transit_time.cli:main"
```
...
```bash
transit-time [OPTIONS] COMMAND [ARGS]...
```
...
# 指定輸出格式
transit-time run --output json

# 詳細模式
transit-time run --verbose

# 靜音（適合 pipe 到其他工具）
transit-time run --quiet | jq .

# 查看設定
transit-time config show

# 匯出 JSON
transit-time export --output json > result.json
```
...
from .core import TransitTimeCore
...
def config():
    """顯示或修改設定"""
    import json, pathlib
    cfg_path = pathlib.Path.home() / ".config" / "transit-time" / "config.json"
    if cfg_path.exists():
...
DB_PATH = pathlib.Path.home() / ".config" / "transit-time" / "data.db"
...
{
  "name": "transit-time",
  "description": "大眾運輸時刻。解決問題：公共運輸時刻",
  "input_schema": {
    "type": "object",
    "properties": {
      "action": {
...
```bash
# Agent 呼叫範例
result=$(transit-time run --quiet --output json)
echo $result | jq .data
```
...
def call_transit_time(action="run") -> dict:
    proc = subprocess.run(
        ["transit-time", action, "--output", "json", "--quiet"],
...
```bash
# 管線串接範例
transit-time run --quiet --output json \
  | jq -r '.data' \
  | other-tool process
```
...
# 執行測試
pytest tests/ -v --cov=transit_time --cov-report=term-missing

# 快速冒煙測試
transit-time run --help
transit-time config show
```
...
```python
import pytest
from transit_time.core import TransitTimeCore
...
*工具編號 #395 ／ 類別 T — 旅遊與生活 ／ 計畫版本 v1.0 ／ 2026-04-27*

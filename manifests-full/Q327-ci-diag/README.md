...
| 介面 | CLI（`click` 框架） |
| 設定檔 | `~/.config/ci-diag/config.json` |
...
# 指定輸出格式
ci-diag run --output json

# 詳細模式
ci-diag run --verbose

# 靜音（適合 pipe 到其他工具）
ci-diag run --quiet | jq .

# 查看設定
ci-diag config show

# 匯出 JSON
ci-diag export --output json > result.json
```
...
def config():
    """顯示或修改設定"""
    import json, pathlib
    cfg_path = pathlib.Path.home() / ".config" / "ci-diag" / "config.json"
    if cfg_path.exists():
...
DB_PATH = pathlib.Path.home() / ".config" / "ci-diag" / "data.db"
...
{
  "name": "ci-diag",
  "description": "CI 失敗診斷器。解決問題：CI 失敗自動診斷",
  "input_schema": {
    "type": "object",
    "properties": {
      "action": {
...
```bash
# Agent 呼叫範例
result=$(ci-diag run --quiet --output json)
echo $result | jq .data
```
...
def call_ci_diag(action="run") -> dict:
    proc = subprocess.run(
        ["ci-diag", action, "--output", "json", "--quiet"],
...
# 執行測試
pytest tests/ -v --cov=ci_diag --cov-report=term-missing
...
```python
import pytest
from ci_diag.core import CiDiagCore
...
```json
{
  "name": "ci-diag",
  "version": "0.1.0",
  "description": "CI 失敗診斷器",
  "category": "Q",
  "category_name": "DevOps與基礎設施",
  "number": 327,
  "pain_point": "CI 失敗自動診斷",
  "commercial": "✅",
  "language": "Python",
  "entry_point": "ci-diag",
  "dependencies": ["anthropic", "click", "rich"],
...
*工具編號 #327 ／ 類別 Q — DevOps與基礎設施 ／ 計畫版本 v1.0 ／ 2026-04-27*

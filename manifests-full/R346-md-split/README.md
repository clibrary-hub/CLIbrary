...
| 介面 | CLI（`click` 框架） |
| 設定檔 | `~/.config/md-split/config.json` |
...
# 指定輸出格式
md-split run --output json

# 詳細模式
md-split run --verbose

# 靜音（適合 pipe 到其他工具）
md-split run --quiet | jq .

# 查看設定
md-split config show

# 匯出 JSON
md-split export --output json > result.json
```
...
def config():
    """顯示或修改設定"""
    import json, pathlib
    cfg_path = pathlib.Path.home() / ".config" / "md-split" / "config.json"
    if cfg_path.exists():
...
DB_PATH = pathlib.Path.home() / ".config" / "md-split" / "data.db"
...
{
  "name": "md-split",
  "description": "Markdown 拆分。解決問題：Markdown 大檔拆分",
  "input_schema": {
    "type": "object",
    "properties": {
      "action": {
...
```bash
# Agent 呼叫範例
result=$(md-split run --quiet --output json)
echo $result | jq .data
```
...
def call_md_split(action="run") -> dict:
    proc = subprocess.run(
        ["md-split", action, "--output", "json", "--quiet"],
...
```bash
# 管線串接範例
md-split run --quiet --output json \
  | jq -r '.data' \
  | other-tool process
```
...
# 執行測試
pytest tests/ -v --cov=md_split --cov-report=term-missing
...
```python
import pytest
from md_split.core import MdSplitCore
...
```json
{
  "name": "md-split",
  "version": "0.1.0",
  "description": "Markdown 拆分",
  "category": "R",
  "category_name": "知識管理與筆記",
  "number": 346,
  "pain_point": "Markdown 大檔拆分",
  "commercial": "✅",
  "language": "Python",
  "entry_point": "md-split",
  "dependencies": ["click", "rich", "pathlib"],
...
*工具編號 #346 ／ 類別 R — 知識管理與筆記 ／ 計畫版本 v1.0 ／ 2026-04-27*

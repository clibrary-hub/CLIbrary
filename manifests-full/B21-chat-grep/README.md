...
| 介面 | CLI（`click` 框架） |
| 設定檔 | `~/.config/chat-grep/config.json` |
...
```bash
chat-grep [OPTIONS] COMMAND [ARGS]...
```
...
# 指定輸出格式
chat-grep run --output json

# 詳細模式
chat-grep run --verbose

# 靜音（適合 pipe 到其他工具）
chat-grep run --quiet | jq .

# 查看設定
chat-grep config show

# 匯出 JSON
chat-grep export --output json > result.json
```
...
def config():
    """顯示或修改設定"""
    import json, pathlib
    cfg_path = pathlib.Path.home() / ".config" / "chat-grep" / "config.json"
    if cfg_path.exists():
...
DB_PATH = pathlib.Path.home() / ".config" / "chat-grep" / "data.db"
...
{
  "name": "chat-grep",
  "description": "對話歷史搜尋器。解決問題：歷史對話無法搜尋",
  "input_schema": {
    "type": "object",
    "properties": {
      "action": {
...
```bash
# Agent 呼叫範例
result=$(chat-grep run --quiet --output json)
echo $result | jq .data
```
...
def call_chat_grep(action="run") -> dict:
    proc = subprocess.run(
        ["chat-grep", action, "--output", "json", "--quiet"],
...
```bash
# 管線串接範例
chat-grep run --quiet --output json \
  | jq -r '.data' \
  | other-tool process
```
...
# 執行測試
pytest tests/ -v --cov=chat_grep --cov-report=term-missing
...
```python
import pytest
from chat_grep.core import ChatGrepCore
...
*工具編號 #021 ／ 類別 B — 對話與記憶管理 ／ 計畫版本 v1.0 ／ 2026-04-27*

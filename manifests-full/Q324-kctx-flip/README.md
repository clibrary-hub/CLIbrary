...
| 介面 | CLI（`click` 框架） |
| 設定檔 | `~/.config/kctx-flip/config.json` |
...
```bash
kctx-flip [OPTIONS] COMMAND [ARGS]...
```
...
# 指定輸出格式
kctx-flip run --output json

# 詳細模式
kctx-flip run --verbose

# 靜音（適合 pipe 到其他工具）
kctx-flip run --quiet | jq .

# 查看設定
kctx-flip config show

# 匯出 JSON
kctx-flip export --output json > result.json
```
...
def config():
    """顯示或修改設定"""
    import json, pathlib
    cfg_path = pathlib.Path.home() / ".config" / "kctx-flip" / "config.json"
    if cfg_path.exists():
...
DB_PATH = pathlib.Path.home() / ".config" / "kctx-flip" / "data.db"
...
{
  "name": "kctx-flip",
  "description": "Context 切換器。解決問題：k8s context 切換",
  "input_schema": {
    "type": "object",
    "properties": {
      "action": {
...
```bash
# Agent 呼叫範例
result=$(kctx-flip run --quiet --output json)
echo $result | jq .data
```
...
def call_kctx_flip(action="run") -> dict:
    proc = subprocess.run(
        ["kctx-flip", action, "--output", "json", "--quiet"],
...
```bash
# 管線串接範例
kctx-flip run --quiet --output json \
  | jq -r '.data' \
  | other-tool process
```
...
# 執行測試
pytest tests/ -v --cov=kctx_flip --cov-report=term-missing
...
```python
import pytest
from kctx_flip.core import KctxFlipCore
...
```json
{
  "name": "kctx-flip",
  "version": "0.1.0",
  "description": "Context 切換器",
  "category": "Q",
  "category_name": "DevOps與基礎設施",
  "number": 324,
  "pain_point": "k8s context 切換",
  "commercial": "✅",
  "language": "Python",
  "entry_point": "kctx-flip",
  "dependencies": ["click", "rich", "yaml"],
...
*工具編號 #324 ／ 類別 Q — DevOps與基礎設施 ／ 計畫版本 v1.0 ／ 2026-04-27*

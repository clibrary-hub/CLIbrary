...
| 介面 | CLI（`click` 框架） |
| 設定檔 | `~/.config/llm-compare/config.json` |
...
[project.scripts]
llm-compare = "llm_compare.cli:main"
```
...
```bash
llm-compare [OPTIONS] COMMAND [ARGS]...
```
...
# 指定輸出格式
llm-compare run --output json

# 詳細模式
llm-compare run --verbose

# 靜音（適合 pipe 到其他工具）
llm-compare run --quiet | jq .

# 查看設定
llm-compare config show

# 匯出 JSON
llm-compare export --output json > result.json
```
...
from .core import LlmCompareCore
...
def config():
    """顯示或修改設定"""
    import json, pathlib
    cfg_path = pathlib.Path.home() / ".config" / "llm-compare" / "config.json"
    if cfg_path.exists():
...
DB_PATH = pathlib.Path.home() / ".config" / "llm-compare" / "data.db"
...
{
  "name": "llm-compare",
  "description": "模型比對器。解決問題：多模型結果比對",
  "input_schema": {
    "type": "object",
    "properties": {
      "action": {
...
```bash
# Agent 呼叫範例
result=$(llm-compare run --quiet --output json)
echo $result | jq .data
```
...
def call_llm_compare(action="run") -> dict:
    proc = subprocess.run(
        ["llm-compare", action, "--output", "json", "--quiet"],
...
```bash
# 管線串接範例
llm-compare run --quiet --output json \
  | jq -r '.data' \
  | other-tool process
```
...
# 執行測試
pytest tests/ -v --cov=llm_compare --cov-report=term-missing

# 快速冒煙測試
llm-compare run --help
llm-compare config show
```
...
```python
import pytest
from llm_compare.core import LlmCompareCore
...
*工具編號 #282 ／ 類別 O — AI/ML輔助 ／ 計畫版本 v1.0 ／ 2026-04-27*

...
| 介面 | CLI（`click` 框架） |
| 設定檔 | `~/.config/note-git/config.json` |
...
# 指定輸出格式
note-git run --output json

# 詳細模式
note-git run --verbose

# 靜音（適合 pipe 到其他工具）
note-git run --quiet | jq .

# 查看設定
note-git config show

# 匯出 JSON
note-git export --output json > result.json
```
...
def config():
    """顯示或修改設定"""
    import json, pathlib
    cfg_path = pathlib.Path.home() / ".config" / "note-git" / "config.json"
    if cfg_path.exists():
...
DB_PATH = pathlib.Path.home() / ".config" / "note-git" / "data.db"
...
```bash
# Agent 呼叫範例
result=$(note-git run --quiet --output json)
echo $result | jq .data
```
...
def call_note_git(action="run") -> dict:
    proc = subprocess.run(
        ["note-git", action, "--output", "json", "--quiet"],
...
```bash
# 管線串接範例
note-git run --quiet --output json \
  | jq -r '.data' \
  | other-tool process
```
...
# 執行測試
pytest tests/ -v --cov=note_git --cov-report=term-missing
...
```python
import pytest
from note_git.core import NoteGitCore
...
*工具編號 #347 ／ 類別 R — 知識管理與筆記 ／ 計畫版本 v1.0 ／ 2026-04-27*

...
| 介面 | CLI（`click` 框架） |
| 設定檔 | `~/.config/cert-watch/config.json` |
...
[project.scripts]
cert-watch = "cert_watch.cli:main"
```
...
```bash
cert-watch [OPTIONS] COMMAND [ARGS]...
```
...
# 指定輸出格式
cert-watch run --output json

# 詳細模式
cert-watch run --verbose

# 靜音（適合 pipe 到其他工具）
cert-watch run --quiet | jq .

# 查看設定
cert-watch config show

# 匯出 JSON
cert-watch export --output json > result.json
```
...
from .core import CertWatchCore
...
def config():
    """顯示或修改設定"""
    import json, pathlib
    cfg_path = pathlib.Path.home() / ".config" / "cert-watch" / "config.json"
    if cfg_path.exists():
...
DB_PATH = pathlib.Path.home() / ".config" / "cert-watch" / "data.db"
...
{
  "name": "cert-watch",
  "description": "憑證守望者。解決問題：DNS / 憑證到期監控",
  "input_schema": {
    "type": "object",
    "properties": {
      "action": {
...
```bash
# Agent 呼叫範例
result=$(cert-watch run --quiet --output json)
echo $result | jq .data
```
...
def call_cert_watch(action="run") -> dict:
    proc = subprocess.run(
        ["cert-watch", action, "--output", "json", "--quiet"],
...
```bash
# 管線串接範例
cert-watch run --quiet --output json \
  | jq -r '.data' \
  | other-tool process
```
...
# 執行測試
pytest tests/ -v --cov=cert_watch --cov-report=term-missing
...
```python
import pytest
from cert_watch.core import CertWatchCore
...
```json
{
  ...
  "number": 246,
  "pain_point": "DNS / 憑證到期監控",
  "commercial": "✅",
  "language": "Python",
  "entry_point": "cert-watch",
  "dependencies": ["ssl", "click", "rich"],
...
*工具編號 #246 ／ 類別 M — 網路爬蟲與監控 ／ 計畫版本 v1.0 ／ 2026-04-27*

...
| 介面 | CLI（`click` 框架） |
| 設定檔 | `~/.config/agent-health/config.json` |
...
[project.scripts]
agent-health = "agent_health.cli:main"
```
...
```bash
agent-health [OPTIONS] COMMAND [ARGS]...
```
...
# 指定輸出格式
agent-health run --output json

# 詳細模式
agent-health run --verbose

# 靜音（適合 pipe 到其他工具）
agent-health run --quiet | jq .

# 查看設定
agent-health config show

# 匯出 JSON
agent-health export --output json > result.json
```
...
from .core import AgentHealthCore
...
def config():
    """顯示或修改設定"""
    import json, pathlib
    cfg_path = pathlib.Path.home() / ".config" / "agent-health" / "config.json"
    if cfg_path.exists():
...
DB_PATH = pathlib.Path.home() / ".config" / "agent-health" / "data.db"
...
{
  "name": "agent-health",
  "description": "Agent 健康檢測。解決問題：Agent 心跳監測缺失",
  "input_schema": {
    "type": "object",
    "properties": {
      "action": {
...
```bash
# Agent 呼叫範例
result=$(agent-health run --quiet --output json)
echo $result | jq .data
```
...
def call_agent_health(action="run") -> dict:
    proc = subprocess.run(
        ["agent-health", action, "--output", "json", "--quiet"],
...
```bash
# 管線串接範例
agent-health run --quiet --output json \
  | jq -r '.data' \
  | other-tool process
```
...
# 執行測試
pytest tests/ -v --cov=agent_health --cov-report=term-missing

# 快速冒煙測試
agent-health run --help
agent-health config show
```
...
```python
import pytest
from agent_health.core import AgentHealthCore
...
```json
{
  "name": "agent-health",
  "version": "0.1.0",
  "description": "Agent 健康檢測",
  "category": "F",
  "category_name": "Agent間通訊與協作",
  "number": 112,
  "pain_point": "Agent 心跳監測缺失",
  "commercial": "✅",
  "language": "Python",
  "entry_point": "agent-health",
  "dependencies": ["click", "rich", "requests"],
...
*工具編號 #112 ／ 類別 F — Agent間通訊與協作 ／ 計畫版本 v1.0 ／ 2026-04-27*

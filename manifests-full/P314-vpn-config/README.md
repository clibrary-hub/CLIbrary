...
| 介面 | CLI（`click` 框架） |
| 設定檔 | `~/.config/vpn-config/config.json` |
...
[project.scripts]
vpn-config = "vpn_config.cli:main"
```
...
```bash
vpn-config [OPTIONS] COMMAND [ARGS]...
```
...
# 指定輸出格式
vpn-config run --output json

# 詳細模式
vpn-config run --verbose

# 靜音（適合 pipe 到其他工具）
vpn-config run --quiet | jq .

# 查看設定
vpn-config config show

# 匯出 JSON
vpn-config export --output json > result.json
```
...
from .core import VpnConfigCore
...
def config():
    """顯示或修改設定"""
    import json, pathlib
    cfg_path = pathlib.Path.home() / ".config" / "vpn-config" / "config.json"
    if cfg_path.exists():
...
DB_PATH = pathlib.Path.home() / ".config" / "vpn-config" / "data.db"
...
{
  "name": "vpn-config",
  "description": "VPN 設定器。解決問題：VPN 設定整理",
  "input_schema": {
    "type": "object",
    "properties": {
      "action": {
...
```bash
# Agent 呼叫範例
result=$(vpn-config run --quiet --output json)
echo $result | jq .data
```
...
def call_vpn_config(action="run") -> dict:
    proc = subprocess.run(
        ["vpn-config", action, "--output", "json", "--quiet"],
...
```bash
# 管線串接範例
vpn-config run --quiet --output json \
  | jq -r '.data' \
  | other-tool process
```
...
# 執行測試
pytest tests/ -v --cov=vpn_config --cov-report=term-missing
...
```python
import pytest
from vpn_config.core import VpnConfigCore
...
*工具編號 #314 ／ 類別 P — 安全與隱私 ／ 計畫版本 v1.0 ／ 2026-04-27*

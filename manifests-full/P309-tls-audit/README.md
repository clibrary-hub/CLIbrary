...
| 介面 | CLI（`click` 框架） |
| 設定檔 | `~/.config/tls-audit/config.json` |
...
```bash
tls-audit [OPTIONS] COMMAND [ARGS]...
```
...
# 指定輸出格式
tls-audit run --output json

# 詳細模式
tls-audit run --verbose

# 靜音（適合 pipe 到其他工具）
tls-audit run --quiet | jq .

# 查看設定
tls-audit config show

# 匯出 JSON
tls-audit export --output json > result.json
```
...
def config():
    """顯示或修改設定"""
    import json, pathlib
    cfg_path = pathlib.Path.home() / ".config" / "tls-audit" / "config.json"
    if cfg_path.exists():
...
DB_PATH = pathlib.Path.home() / ".config" / "tls-audit" / "data.db"
...
{
  "name": "tls-audit",
  "description": "TLS 稽核器。解決問題：TLS 設定檢測",
  "input_schema": {
    "type": "object",
    "properties": {
      "action": {
...
```bash
# Agent 呼叫範例
result=$(tls-audit run --quiet --output json)
echo $result | jq .data
```
...
def call_tls_audit(action="run") -> dict:
    proc = subprocess.run(
        ["tls-audit", action, "--output", "json", "--quiet"],
...
```bash
# 管線串接範例
tls-audit run --quiet --output json \
  | jq -r '.data' \
  | other-tool process
```
...
# 執行測試
pytest tests/ -v --cov=tls_audit --cov-report=term-missing
...
```python
import pytest
from tls_audit.core import TlsAuditCore
...
*工具編號 #309 ／ 類別 P — 安全與隱私 ／ 計畫版本 v1.0 ／ 2026-04-27*

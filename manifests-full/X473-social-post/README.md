...
| 介面 | CLI（`click` 框架） |
| 設定檔 | `~/.config/social-post/config.json` |
...
[project.scripts]
social-post = "social_post.cli:main"
```
...
```bash
social-post [OPTIONS] COMMAND [ARGS]...
```
...
# 指定輸出格式
social-post run --output json

# 詳細模式
social-post run --verbose

# 靜音（適合 pipe 到其他工具）
social-post run --quiet | jq .

# 查看設定
social-post config show

# 匯出 JSON
social-post export --output json > result.json
```
...
from .core import SocialPostCore
...
def config():
    """顯示或修改設定"""
    import json, pathlib
    cfg_path = pathlib.Path.home() / ".config" / "social-post" / "config.json"
    if cfg_path.exists():
...
DB_PATH = pathlib.Path.home() / ".config" / "social-post" / "data.db"
...
{
  "name": "social-post",
  "description": "社群貼文器。解決問題：圖文社群貼文產",
  "input_schema": {
    "type": "object",
    "properties": {
      "action": {
...
```bash
# Agent 呼叫範例
result=$(social-post run --quiet --output json)
echo $result | jq .data
```
...
def call_social_post(action="run") -> dict:
    proc = subprocess.run(
        ["social-post", action, "--output", "json", "--quiet"],
...
```bash
# 管線串接範例
social-post run --quiet --output json \
  | jq -r '.data' \
  | other-tool process
```
...
# 執行測試
pytest tests/ -v --cov=social_post --cov-report=term-missing

# 快速冒煙測試
social-post run --help
social-post config show
```
...
```python
import pytest
from social_post.core import SocialPostCore
...
*工具編號 #473 ／ 類別 X — 寫作與內容創作 ／ 計畫版本 v1.0 ／ 2026-04-27*

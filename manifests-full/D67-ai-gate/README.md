...
> **AI 品質關卡**
> 分類：D — 自動化與工作流程 ｜ 編號：#067 ｜ 商用：✅

## 問題背景

**用戶痛點**：CI/CD 缺 AI 品質關卡
...
| 介面 | CLI（`click` 框架） |
| 設定檔 | `~/.config/ai-gate/config.json` |
...
# 指定輸出格式
ai-gate run --output json

# 詳細模式
ai-gate run --verbose

# 靜音（適合 pipe 到其他工具）
ai-gate run --quiet | jq .

# 查看設定
ai-gate config show

# 匯出 JSON
ai-gate export --output json > result.json
```
...
def config():
    """顯示或修改設定"""
    import json, pathlib
    cfg_path = pathlib.Path.home() / ".config" / "ai-gate" / "config.json"
    if cfg_path.exists():
...
DB_PATH = pathlib.Path.home() / ".config" / "ai-gate" / "data.db"
...
{
  "name": "ai-gate",
  "description": "AI 品質關卡。解決問題：CI/CD 缺 AI 品質關卡",
  "input_schema": {
    "type": "object",
    "properties": {
      "action": {
...
```bash
# Agent 呼叫範例
result=$(ai-gate run --quiet --output json)
echo $result | jq .data
```
...
def call_ai_gate(action="run") -> dict:
    proc = subprocess.run(
        ["ai-gate", action, "--output", "json", "--quiet"],
...
# 執行測試
pytest tests/ -v --cov=ai_gate --cov-report=term-missing
...
```python
import pytest
from ai_gate.core import AiGateCore
...
*工具編號 #067 ／ 類別 D — 自動化與工作流程 ／ 計畫版本 v1.0 ／ 2026-04-27*

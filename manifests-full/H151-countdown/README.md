...
| 介面 | CLI（`click` 框架） |
| 設定檔 | `~/.config/countdown/config.json` |
...
```bash
countdown [OPTIONS] COMMAND [ARGS]...
```
...
# 指定輸出格式
countdown run --output json

# 詳細模式
countdown run --verbose

# 靜音（適合 pipe 到其他工具）
countdown run --quiet | jq .

# 查看設定
countdown config show

# 匯出 JSON
countdown export --output json > result.json
```
...
from .core import CountdownCore
...
def config():
    """顯示或修改設定"""
    import json, pathlib
    cfg_path = pathlib.Path.home() / ".config" / "countdown" / "config.json"
    if cfg_path.exists():
...
DB_PATH = pathlib.Path.home() / ".config" / "countdown" / "data.db"
...
{
  "name": "countdown",
  "description": "倒數提醒器。解決問題：倒數活動需提醒",
  "input_schema": {
    "type": "object",
    "properties": {
      "action": {
...
```bash
# Agent 呼叫範例
result=$(countdown run --quiet --output json)
echo $result | jq .data
```
...
def call_countdown(action="run") -> dict:
    proc = subprocess.run(
        ["countdown", action, "--output", "json", "--quiet"],
...
```bash
# 管線串接範例
countdown run --quiet --output json \
  | jq -r '.data' \
  | other-tool process
```
...
# 執行測試
pytest tests/ -v --cov=countdown --cov-report=term-missing
...
```python
import pytest
from countdown.core import CountdownCore
...
*工具編號 #151 ／ 類別 H — 行事曆與時間管理 ／ 計畫版本 v1.0 ／ 2026-04-27*

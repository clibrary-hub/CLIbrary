...
| 介面 | CLI（`click` 框架） |
| 設定檔 | `~/.config/pr-brief/config.json` |
...
pip install gitpython anthropic click
...
dependencies = ["gitpython", "anthropic", "click"]

[project.scripts]
pr-brief = "pr_brief.cli:main"
```
...
# 指定輸出格式
pr-brief run --output json

# 詳細模式
pr-brief run --verbose

# 靜音（適合 pipe 到其他工具）
pr-brief run --quiet | jq .

# 查看設定
pr-brief config show

# 匯出 JSON
pr-brief export --output json > result.json
```
...
def config():
    """顯示或修改設定"""
    import json, pathlib
    cfg_path = pathlib.Path.home() / ".config" / "pr-brief" / "config.json"
    if cfg_path.exists():
...
DB_PATH = pathlib.Path.home() / ".config" / "pr-brief" / "data.db"
...
class PrBriefCore:
    ...
    核心邏輯類別。
    問題：PR 描述太隨便
    解法：透過 gitpython, anthropic 實現自動化處理。
    """
...
{
  "name": "pr-brief",
  "description": "PR 自動摘要器。解決問題：PR 描述太隨便",
  "input_schema": {
    "type": "object",
    "properties": {
      "action": {
...
```bash
# Agent 呼叫範例
result=$(pr-brief run --quiet --output json)
echo $result | jq .data
```
...
def call_pr_brief(action="run") -> dict:
    proc = subprocess.run(
        ["pr-brief", action, "--output", "json", "--quiet"],
...
```bash
# 管線串接範例
pr-brief run --quiet --output json \
  | jq -r '.data' \
  | other-tool process
```
...
# 執行測試
pytest tests/ -v --cov=pr_brief --cov-report=term-missing
...
```python
import pytest
from pr_brief.core import PrBriefCore
...
*工具編號 #056 ／ 類別 C — 程式碼品質與開發輔助 ／ 計畫版本 v1.0 ／ 2026-04-27*

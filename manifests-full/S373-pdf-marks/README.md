...
| 介面 | CLI（`click` 框架） |
| 設定檔 | `~/.config/pdf-marks/config.json` |
...
pip install pymupdf click rich
...
dependencies = ["pymupdf", "click", "rich"]

[project.scripts]
pdf-marks = "pdf_marks.cli:main"
```
...
```bash
pdf-marks [OPTIONS] COMMAND [ARGS]...
```
...
# 指定輸出格式
pdf-marks run --output json

# 詳細模式
pdf-marks run --verbose

# 靜音（適合 pipe 到其他工具）
pdf-marks run --quiet | jq .

# 查看設定
pdf-marks config show

# 匯出 JSON
pdf-marks export --output json > result.json
```
...
def config():
    """顯示或修改設定"""
    import json, pathlib
    cfg_path = pathlib.Path.home() / ".config" / "pdf-marks" / "config.json"
    if cfg_path.exists():
...
DB_PATH = pathlib.Path.home() / ".config" / "pdf-marks" / "data.db"
...
{
  "name": "pdf-marks",
  "description": "PDF 註記器。解決問題：文獻 PDF 註記匯出",
  "input_schema": {
    "type": "object",
    "properties": {
      "action": {
...
```bash
# Agent 呼叫範例
result=$(pdf-marks run --quiet --output json)
echo $result | jq .data
```
...
def call_pdf_marks(action="run") -> dict:
    proc = subprocess.run(
        ["pdf-marks", action, "--output", "json", "--quiet"],
...
```bash
# 管線串接範例
pdf-marks run --quiet --output json \
  | jq -r '.data' \
  | other-tool process
```
...
# 執行測試
pytest tests/ -v --cov=pdf_marks --cov-report=term-missing
...
```python
import pytest
from pdf_marks.core import PdfMarksCore
...
```json
{
  ...
  "category_name": "學習與研究",
  "number": 373,
  "pain_point": "文獻 PDF 註記匯出",
  "commercial": "✅",
  "language": "Python",
  "entry_point": "pdf-marks",
  "dependencies": ["pymupdf", "click", "rich"],
...
*工具編號 #373 ／ 類別 S — 學習與研究 ／ 計畫版本 v1.0 ／ 2026-04-27*

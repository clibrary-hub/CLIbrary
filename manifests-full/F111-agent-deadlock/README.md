...
| 介面 | CLI（`click` 框架） |
| 設定檔 | `~/.config/agent-deadlock/config.json` |
...
[project.scripts]
agent-deadlock = "agent_deadlock.cli:main"
```
...
```bash
agent-deadlock [OPTIONS] COMMAND [ARGS]...
```
...
# 指定輸出格式
agent-deadlock run --output json

# 詳細模式
agent-deadlock run --verbose

# 靜音（適合 pipe 到其他工具）
agent-deadlock run --quiet | jq .

# 查看設定
agent-deadlock config show

# 匯出 JSON
agent-deadlock export --output json > result.json
```
...
### `agent_deadlock/cli.py`

```python
import click
from rich.console import Console
from .core import AgentDeadlockCore
...
def config():
    """顯示或修改設定"""
    import json, pathlib
    cfg_path = pathlib.Path.home() / ".config" / "agent-deadlock" / "config.json"
    if cfg_path.exists():
...
DB_PATH = pathlib.Path.home() / ".config" / "agent-deadlock" / "data.db"
...
{
  "name": "agent-deadlock",
  "description": "死鎖偵測器。解決問題：多 Agent 死鎖無法解",
  "input_schema": {
    "type": "object",
    "properties": {
      "action": {
...
```bash
# Agent 呼叫範例
result=$(agent-deadlock run --quiet --output json)
echo $result | jq .data
```
...
def call_agent_deadlock(action="run") -> dict:
    proc = subprocess.run(
        ["agent-deadlock", action, "--output", "json", "--quiet"],
...
```bash
# 管線串接範例
agent-deadlock run --quiet --output json \
  | jq -r '.data' \
  | other-tool process
```
...
# 執行測試
pytest tests/ -v --cov=agent_deadlock --cov-report=term-missing

# 快速冒煙測試
agent-deadlock run --help
agent-deadlock config show
```
...
```python
import pytest
from agent_deadlock.core import AgentDeadlockCore
...
```json
{
  ...
  "number": 111,
  "pain_point": "多 Agent 死鎖無法解",
  "commercial": "✅",
  "language": "Python",
  "entry_point": "agent-deadlock",
  "dependencies": ["click", "rich", "asyncio"],
...
*工具編號 #111 ／ 類別 F — Agent間通訊與協作 ／ 計畫版本 v1.0 ／ 2026-04-27*

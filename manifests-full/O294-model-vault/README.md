...
| 介面 | CLI（`click` 框架） |
| 設定檔 | `~/.config/model-vault/config.json` |
...
[project.scripts]
model-vault = "model_vault.cli:main"
```
...
```bash
model-vault [OPTIONS] COMMAND [ARGS]...
```
...
# 指定輸出格式
model-vault run --output json

# 詳細模式
model-vault run --verbose

# 靜音（適合 pipe 到其他工具）
model-vault run --quiet | jq .

# 查看設定
model-vault config show

# 匯出 JSON
model-vault export --output json > result.json
```
...
from .core import ModelVaultCore
...
def config():
    """顯示或修改設定"""
    import json, pathlib
    cfg_path = pathlib.Path.home() / ".config" / "model-vault" / "config.json"
    if cfg_path.exists():
...
DB_PATH = pathlib.Path.home() / ".config" / "model-vault" / "data.db"
...
{
  "name": "model-vault",
  "description": "模型版本管理。解決問題：模型版本管理",
  "input_schema": {
    "type": "object",
    "properties": {
      "action": {
...
```bash
# Agent 呼叫範例
result=$(model-vault run --quiet --output json)
echo $result | jq .data
```
...
def call_model_vault(action="run") -> dict:
    proc = subprocess.run(
        ["model-vault", action, "--output", "json", "--quiet"],
...
```bash
# 管線串接範例
model-vault run --quiet --output json \
  | jq -r '.data' \
  | other-tool process
```
...
# 執行測試
pytest tests/ -v --cov=model_vault --cov-report=term-missing

# 快速冒煙測試
model-vault run --help
model-vault config show
```
...
```python
import pytest
from model_vault.core import ModelVaultCore
...
*工具編號 #294 ／ 類別 O — AI/ML輔助 ／ 計畫版本 v1.0 ／ 2026-04-27*

...
pip install pytesseract anthropic click
...
dependencies = ["pytesseract", "anthropic", "click"]

[project.scripts]
img-tr = "img_tr.cli:main"
```
...
# 匯出 JSON
img-tr export --output json > result.json
```
...
def config():
    """顯示或修改設定"""
    import json, pathlib
    cfg_path = pathlib.Path.home() / ".config" / "img-tr" / "config.json"
    if cfg_path.exists():
...
DB_PATH = pathlib.Path.home() / ".config" / "img-tr" / "data.db"
...
class ImgTrCore:
    ...
    核心邏輯類別。
    問題：圖片內文字翻譯
    解法：透過 pytesseract, anthropic 實現自動化處理。
    """
...
{
  "name": "img-tr",
  "description": "圖內文翻譯器。解決問題：圖片內文字翻譯",
  "input_schema": {
    "type": "object",
    "properties": {
      "action": {
...
```bash
# Agent 呼叫範例
result=$(img-tr run --quiet --output json)
echo $result | jq .data
```
...
def call_img_tr(action="run") -> dict:
    proc = subprocess.run(
        ["img-tr", action, "--output", "json", "--quiet"],
...
# 執行測試
pytest tests/ -v --cov=img_tr --cov-report=term-missing
...
```python
import pytest
from img_tr.core import ImgTrCore
...
*工具編號 #458 ／ 類別 W — 翻譯與在地化 ／ 計畫版本 v1.0 ／ 2026-04-27*

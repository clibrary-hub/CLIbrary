...
pip install anthropic icalendar click
...
dependencies = ["anthropic", "icalendar", "click"]

[project.scripts]
nl-cal = "nl_cal.cli:main"
```
...
# 匯出 JSON
nl-cal export --output json > result.json
```
...
def config():
    """顯示或修改設定"""
    import json, pathlib
    cfg_path = pathlib.Path.home() / ".config" / "nl-cal" / "config.json"
    if cfg_path.exists():
...
DB_PATH = pathlib.Path.home() / ".config" / "nl-cal" / "data.db"
...
class NlCalCore:
    ...
    核心邏輯類別。
    問題：行事曆事件 NL 建立
    解法：透過 anthropic, icalendar 實現自動化處理。
    """
...
{
  "name": "nl-cal",
  "description": "自然語言建會。解決問題：行事曆事件 NL 建立",
  "input_schema": {
    "type": "object",
    "properties": {
      "action": {
...
```bash
# Agent 呼叫範例
result=$(nl-cal run --quiet --output json)
echo $result | jq .data
```
...
def call_nl_cal(action="run") -> dict:
    proc = subprocess.run(
        ["nl-cal", action, "--output", "json", "--quiet"],
...
# 執行測試
pytest tests/ -v --cov=nl_cal --cov-report=term-missing
...
```python
import pytest
from nl_cal.core import NlCalCore
...
*工具編號 #154 ／ 類別 H — 行事曆與時間管理 ／ 計畫版本 v1.0 ／ 2026-04-27*

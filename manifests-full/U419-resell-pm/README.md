...
| 介面 | CLI（`click` 框架） |
| 設定檔 | `~/.config/resell-pm/config.json` |
...
```bash
resell-pm [OPTIONS] COMMAND [ARGS]...
```
...
# 指定輸出格式
resell-pm run --output json

# 詳細模式
resell-pm run --verbose

# 靜音（適合 pipe 到其他工具）
resell-pm run --quiet | jq .

# 查看設定
resell-pm config show

# 匯出 JSON
resell-pm export --output json > result.json
```
...
def config():
    """顯示或修改設定"""
    import json, pathlib
    cfg_path = pathlib.Path.home() / ".config" / "resell-pm" / "config.json"
    if cfg_path.exists():
...
DB_PATH = pathlib.Path.home() / ".config" / "resell-pm" / "data.db"
...
{
  "name": "resell-pm",
  "description": "二手再售助手。解決問題：二手售出提醒",
  "input_schema": {
    "type": "object",
    "properties": {
      "action": {
...
```bash
# Agent 呼叫範例
result=$(resell-pm run --quiet --output json)
echo $result | jq .data
```
...
def call_resell_pm(action="run") -> dict:
    proc = subprocess.run(
        ["resell-pm", action, "--output", "json", "--quiet"],
...
```bash
# 管線串接範例
resell-pm run --quiet --output json \
  | jq -r '.data' \
  | other-tool process
```
...
# 執行測試
pytest tests/ -v --cov=resell_pm --cov-report=term-missing
...
```python
import pytest
from resell_pm.core import ResellPmCore
...
*工具編號 #419 ／ 類別 U — 購物與訂閱管理 ／ 計畫版本 v1.0 ／ 2026-04-27*

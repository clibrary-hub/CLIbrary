...
> **Agent 訊息匯流排**
> 分類：F — Agent間通訊與協作 ｜ 編號：#101 ｜ 商用：✅

## 問題背景

**用戶痛點**：Agent 間沒有統一訊息匯流排
...
| 介面 | CLI（`click` 框架） |
| 設定檔 | `~/.config/agent-bus/config.json` |
...
```bash
agent-bus [OPTIONS] COMMAND [ARGS]...
```
...
# 指定輸出格式
agent-bus run --output json

# 詳細模式
agent-bus run --verbose

# 靜音（適合 pipe 到其他工具）
agent-bus run --quiet | jq .

# 查看設定
agent-bus config show

# 匯出 JSON
agent-bus export --output json > result.json
```
...
def config():
    """顯示或修改設定"""
    import json, pathlib
    cfg_path = pathlib.Path.home() / ".config" / "agent-bus" / "config.json"
    if cfg_path.exists():
...
DB_PATH = pathlib.Path.home() / ".config" / "agent-bus" / "data.db"
...
{
  "name": "agent-bus",
  "description": "Agent 訊息匯流排。解決問題：Agent 間沒有統一訊息匯流排",
  "input_schema": {
    "type": "object",
    "properties": {
      "action": {
...
```bash
# Agent 呼叫範例
result=$(agent-bus run --quiet --output json)
echo $result | jq .data
```
...
def call_agent_bus(action="run") -> dict:
    proc = subprocess.run(
        ["agent-bus", action, "--output", "json", "--quiet"],
...
```bash
# 管線串接範例
agent-bus run --quiet --output json \
  | jq -r '.data' \
  | other-tool process
```
...
# 執行測試
pytest tests/ -v --cov=agent_bus --cov-report=term-missing
...
```python
import pytest
from agent_bus.core import AgentBusCore
...
```json
{
  "name": "agent-bus",
  "version": "0.1.0",
  "description": "Agent 訊息匯流排",
  "category": "F",
  "category_name": "Agent間通訊與協作",
  "number": 101,
  "pain_point": "Agent 間沒有統一訊息匯流排",
  "commercial": "✅",
  "language": "Python",
  "entry_point": "agent-bus",
  "dependencies": ["redis", "click", "rich"],
...
*工具編號 #101 ／ 類別 F — Agent間通訊與協作 ／ 計畫版本 v1.0 ／ 2026-04-27*

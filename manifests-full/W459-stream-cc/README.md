...
| 介面 | CLI（`click` 框架） |
| 設定檔 | `~/.config/stream-cc/config.json` |
...
```bash
stream-cc [OPTIONS] COMMAND [ARGS]...
```
...
# 指定輸出格式
stream-cc run --output json

# 詳細模式
stream-cc run --verbose

# 靜音（適合 pipe 到其他工具）
stream-cc run --quiet | jq .

# 查看設定
stream-cc config show

# 匯出 JSON
stream-cc export --output json > result.json
```
...
def config():
    """顯示或修改設定"""
    import json, pathlib
    cfg_path = pathlib.Path.home() / ".config" / "stream-cc" / "config.json"
    if cfg_path.exists():
...
DB_PATH = pathlib.Path.home() / ".config" / "stream-cc" / "data.db"
...
```bash
# Agent 呼叫範例
result=$(stream-cc run --quiet --output json)
echo $result | jq .data
```
...
def call_stream_cc(action="run") -> dict:
    proc = subprocess.run(
        ["stream-cc", action, "--output", "json", "--quiet"],
...
```bash
# 管線串接範例
stream-cc run --quiet --output json \
  | jq -r '.data' \
  | other-tool process
```
...
# 執行測試
pytest tests/ -v --cov=stream_cc --cov-report=term-missing
...
```python
import pytest
from stream_cc.core import StreamCcCore
...
*工具編號 #459 ／ 類別 W — 翻譯與在地化 ／ 計畫版本 v1.0 ／ 2026-04-27*

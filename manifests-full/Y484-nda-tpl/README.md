...
| 介面 | CLI（`click` 框架） |
| 設定檔 | `~/.config/nda-tpl/config.json` |
...
# 指定輸出格式
nda-tpl run --output json

# 詳細模式
nda-tpl run --verbose

# 靜音（適合 pipe 到其他工具）
nda-tpl run --quiet | jq .

# 查看設定
nda-tpl config show

# 匯出 JSON
nda-tpl export --output json > result.json
```
...
def config():
    """顯示或修改設定"""
    import json, pathlib
    cfg_path = pathlib.Path.home() / ".config" / "nda-tpl" / "config.json"
    if cfg_path.exists():
...
DB_PATH = pathlib.Path.home() / ".config" / "nda-tpl" / "data.db"
...
{
  "name": "nda-tpl",
  "description": "NDA 模板器。解決問題：NDA 標準模板",
  "input_schema": {
    "type": "object",
    "properties": {
      "action": {
...
```bash
# Agent 呼叫範例
result=$(nda-tpl run --quiet --output json)
echo $result | jq .data
```
...
def call_nda_tpl(action="run") -> dict:
    proc = subprocess.run(
        ["nda-tpl", action, "--output", "json", "--quiet"],
...
# 執行測試
pytest tests/ -v --cov=nda_tpl --cov-report=term-missing
...
```python
import pytest
from nda_tpl.core import NdaTplCore
...
*工具編號 #484 ／ 類別 Y — 法律合約商務 ／ 計畫版本 v1.0 ／ 2026-04-27*

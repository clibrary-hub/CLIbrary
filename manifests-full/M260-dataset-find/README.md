...
| 介面 | CLI（`click` 框架） |
| 設定檔 | `~/.config/dataset-find/config.json` |
...
[project.scripts]
dataset-find = "dataset_find.cli:main"
```
...
```bash
dataset-find [OPTIONS] COMMAND [ARGS]...
```
...
# 指定輸出格式
dataset-find run --output json

# 詳細模式
dataset-find run --verbose

# 靜音（適合 pipe 到其他工具）
dataset-find run --quiet | jq .

# 查看設定
dataset-find config show

# 匯出 JSON
dataset-find export --output json > result.json
```
...
from .core import DatasetFindCore
...
def config():
    """顯示或修改設定"""
    import json, pathlib
    cfg_path = pathlib.Path.home() / ".config" / "dataset-find" / "config.json"
    if cfg_path.exists():
...
DB_PATH = pathlib.Path.home() / ".config" / "dataset-find" / "data.db"
...
{
  "name": "dataset-find",
  "description": "資料集發現器。解決問題：公開資料集發現",
  "input_schema": {
    "type": "object",
    "properties": {
      "action": {
...
```bash
# Agent 呼叫範例
result=$(dataset-find run --quiet --output json)
echo $result | jq .data
```
...
def call_dataset_find(action="run") -> dict:
    proc = subprocess.run(
        ["dataset-find", action, "--output", "json", "--quiet"],
...
```bash
# 管線串接範例
dataset-find run --quiet --output json \
  | jq -r '.data' \
  | other-tool process
```
...
# 執行測試
pytest tests/ -v --cov=dataset_find --cov-report=term-missing

# 快速冒煙測試
dataset-find run --help
dataset-find config show
```
...
```python
import pytest
from dataset_find.core import DatasetFindCore
...
*工具編號 #260 ／ 類別 M — 網路爬蟲與監控 ／ 計畫版本 v1.0 ／ 2026-04-27*

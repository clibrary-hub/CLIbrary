...
| 介面 | CLI（`click` 框架） |
| 設定檔 | `~/.config/web-clean/config.json` |
...
```bash
web-clean [OPTIONS] COMMAND [ARGS]...
```
...
# 指定輸出格式
web-clean run --output json

# 詳細模式
web-clean run --verbose

# 靜音（適合 pipe 到其他工具）
web-clean run --quiet | jq .

# 查看設定
web-clean config show

# 匯出 JSON
web-clean export --output json > result.json
```
...
def config():
    """顯示或修改設定"""
    import json, pathlib
    cfg_path = pathlib.Path.home() / ".config" / "web-clean" / "config.json"
    if cfg_path.exists():
...
DB_PATH = pathlib.Path.home() / ".config" / "web-clean" / "data.db"
...
{
  "name": "web-clean",
  "description": "網頁清洗器。解決問題：網頁內容難乾淨餵入",
  "input_schema": {
    "type": "object",
    "properties": {
      "action": {
...
```bash
# Agent 呼叫範例
result=$(web-clean run --quiet --output json)
echo $result | jq .data
```
...
def call_web_clean(action="run") -> dict:
    proc = subprocess.run(
        ["web-clean", action, "--output", "json", "--quiet"],
...
```bash
# 管線串接範例
web-clean run --quiet --output json \
  | jq -r '.data' \
  | other-tool process
```
...
# 執行測試
pytest tests/ -v --cov=web_clean --cov-report=term-missing
...
```python
import pytest
from web_clean.core import WebCleanCore
...
```json
{
  ...
  "number": 81,
  "pain_point": "網頁內容難乾淨餵入",
  "commercial": "⚠️",
  "language": "Python",
  "entry_point": "web-clean",
  "dependencies": ["requests", "beautifulsoup4", "markdownify"],
...
*工具編號 #081 ／ 類別 E — 資料處理與整合 ／ 計畫版本 v1.0 ／ 2026-04-27*

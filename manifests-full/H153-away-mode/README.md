...
| 介面 | CLI（`click` 框架） |
| 設定檔 | `~/.config/away-mode/config.json` |
...
```bash
away-mode [OPTIONS] COMMAND [ARGS]...
```
...
# 指定輸出格式
away-mode run --output json

# 詳細模式
away-mode run --verbose

# 靜音（適合 pipe 到其他工具）
away-mode run --quiet | jq .

# 查看設定
away-mode config show

# 匯出 JSON
away-mode export --output json > result.json
```
...
def config():
    """顯示或修改設定"""
    import json, pathlib
    cfg_path = pathlib.Path.home() / ".config" / "away-mode" / "config.json"
    if cfg_path.exists():
...
DB_PATH = pathlib.Path.home() / ".config" / "away-mode" / "data.db"
...
{
  "name": "away-mode",
  "description": "出差代辦模式。解決問題：旅遊期間自動代辦",
  "input_schema": {
    "type": "object",
    "properties": {
      "action": {
...
```bash
# Agent 呼叫範例
result=$(away-mode run --quiet --output json)
echo $result | jq .data
```
...
def call_away_mode(action="run") -> dict:
    proc = subprocess.run(
        ["away-mode", action, "--output", "json", "--quiet"],
...
```bash
# 管線串接範例
away-mode run --quiet --output json \
  | jq -r '.data' \
  | other-tool process
```
...
# 執行測試
pytest tests/ -v --cov=away_mode --cov-report=term-missing
...
```python
import pytest
from away_mode.core import AwayModeCore
...
*工具編號 #153 ／ 類別 H — 行事曆與時間管理 ／ 計畫版本 v1.0 ／ 2026-04-27*

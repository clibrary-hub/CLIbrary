...
| 介面 | CLI（`click` 框架） |
| 設定檔 | `~/.config/menu-tr/config.json` |
...
# 指定輸出格式
menu-tr run --output json

# 詳細模式
menu-tr run --verbose

# 靜音（適合 pipe 到其他工具）
menu-tr run --quiet | jq .

# 查看設定
menu-tr config show

# 匯出 JSON
menu-tr export --output json > result.json
```
...
def config():
    """顯示或修改設定"""
    import json, pathlib
    cfg_path = pathlib.Path.home() / ".config" / "menu-tr" / "config.json"
    if cfg_path.exists():
...
DB_PATH = pathlib.Path.home() / ".config" / "menu-tr" / "data.db"
...
{
  "name": "menu-tr",
  "description": "菜單翻譯器。解決問題：點餐圖片菜單翻譯",
  "input_schema": {
    "type": "object",
    "properties": {
      "action": {
...
```bash
# Agent 呼叫範例
result=$(menu-tr run --quiet --output json)
echo $result | jq .data
```
...
def call_menu_tr(action="run") -> dict:
    proc = subprocess.run(
        ["menu-tr", action, "--output", "json", "--quiet"],
...
# 執行測試
pytest tests/ -v --cov=menu_tr --cov-report=term-missing
...
```python
import pytest
from menu_tr.core import MenuTrCore
...
*工具編號 #391 ／ 類別 T — 旅遊與生活 ／ 計畫版本 v1.0 ／ 2026-04-27*

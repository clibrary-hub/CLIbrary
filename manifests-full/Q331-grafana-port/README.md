...
| 介面 | CLI（`click` 框架） |
| 設定檔 | `~/.config/grafana-port/config.json` |
...
[project.scripts]
grafana-port = "grafana_port.cli:main"
```
...
```bash
grafana-port [OPTIONS] COMMAND [ARGS]...
```
...
# 指定輸出格式
grafana-port run --output json

# 詳細模式
grafana-port run --verbose

# 靜音（適合 pipe 到其他工具）
grafana-port run --quiet | jq .

# 查看設定
grafana-port config show

# 匯出 JSON
grafana-port export --output json > result.json
```
...
from .core import GrafanaPortCore
...
def config():
    """顯示或修改設定"""
    import json, pathlib
    cfg_path = pathlib.Path.home() / ".config" / "grafana-port" / "config.json"
    if cfg_path.exists():
...
DB_PATH = pathlib.Path.home() / ".config" / "grafana-port" / "data.db"
...
{
  "name": "grafana-port",
  "description": "Grafana 搬遷器。解決問題：Grafana 儀表搬遷",
  "input_schema": {
    "type": "object",
    "properties": {
      "action": {
...
```bash
# Agent 呼叫範例
result=$(grafana-port run --quiet --output json)
echo $result | jq .data
```
...
def call_grafana_port(action="run") -> dict:
    proc = subprocess.run(
        ["grafana-port", action, "--output", "json", "--quiet"],
...
```bash
# 管線串接範例
grafana-port run --quiet --output json \
  | jq -r '.data' \
  | other-tool process
```
...
# 執行測試
pytest tests/ -v --cov=grafana_port --cov-report=term-missing

# 快速冒煙測試
grafana-port run --help
grafana-port config show
```
...
```python
import pytest
from grafana_port.core import GrafanaPortCore
...
```json
{
  "name": "grafana-port",
  "version": "0.1.0",
  "description": "Grafana 搬遷器",
  "category": "Q",
  "category_name": "DevOps與基礎設施",
  "number": 331,
  "pain_point": "Grafana 儀表搬遷",
  "commercial": "⚠️",
  "language": "Python",
  "entry_point": "grafana-port",
  "dependencies": ["requests", "click", "rich"],
...
*工具編號 #331 ／ 類別 Q — DevOps與基礎設施 ／ 計畫版本 v1.0 ／ 2026-04-27*

...
| 介面 | CLI（`click` 框架） |
| 設定檔 | `~/.config/eats-suggest/config.json` |
...
[project.scripts]
eats-suggest = "eats_suggest.cli:main"
```
...
```bash
eats-suggest [OPTIONS] COMMAND [ARGS]...
```
...
# 指定輸出格式
eats-suggest run --output json

# 詳細模式
eats-suggest run --verbose

# 靜音（適合 pipe 到其他工具）
eats-suggest run --quiet | jq .

# 查看設定
eats-suggest config show

# 匯出 JSON
eats-suggest export --output json > result.json
```
...
from .core import EatsSuggestCore
...
def config():
    """顯示或修改設定"""
    import json, pathlib
    cfg_path = pathlib.Path.home() / ".config" / "eats-suggest" / "config.json"
    if cfg_path.exists():
...
DB_PATH = pathlib.Path.home() / ".config" / "eats-suggest" / "data.db"
...
```bash
# Agent 呼叫範例
result=$(eats-suggest run --quiet --output json)
echo $result | jq .data
```
...
def call_eats_suggest(action="run") -> dict:
    proc = subprocess.run(
        ["eats-suggest", action, "--output", "json", "--quiet"],
...
```bash
# 管線串接範例
eats-suggest run --quiet --output json \
  | jq -r '.data' \
  | other-tool process
```
...
# 執行測試
pytest tests/ -v --cov=eats_suggest --cov-report=term-missing

# 快速冒煙測試
eats-suggest run --help
eats-suggest config show
```
...
```python
import pytest
from eats_suggest.core import EatsSuggestCore
...
*工具編號 #390 ／ 類別 T — 旅遊與生活 ／ 計畫版本 v1.0 ／ 2026-04-27*

...
| 介面 | CLI（`click` 框架） |
| 設定檔 | `~/.config/focus-log/config.json` |
...
```bash
focus-log [OPTIONS] COMMAND [ARGS]...
```
...
# 指定輸出格式
focus-log run --output json

# 詳細模式
focus-log run --verbose

# 靜音（適合 pipe 到其他工具）
focus-log run --quiet | jq .

# 查看設定
focus-log config show

# 匯出 JSON
focus-log export --output json > result.json
```
...
def config():
    """顯示或修改設定"""
    import json, pathlib
    cfg_path = pathlib.Path.home() / ".config" / "focus-log" / "config.json"
    if cfg_path.exists():
...
DB_PATH = pathlib.Path.home() / ".config" / "focus-log" / "data.db"
...
{
  "name": "focus-log",
  "description": "專注時間記錄器。解決問題：想記錄專注時間",
  "input_schema": {
    "type": "object",
    "properties": {
      "action": {
...
```bash
# Agent 呼叫範例
result=$(focus-log run --quiet --output json)
echo $result | jq .data
```
...
def call_focus_log(action="run") -> dict:
    proc = subprocess.run(
        ["focus-log", action, "--output", "json", "--quiet"],
...
```bash
# 管線串接範例
focus-log run --quiet --output json \
  | jq -r '.data' \
  | other-tool process
```
...
# 執行測試
pytest tests/ -v --cov=focus_log --cov-report=term-missing
...
```python
import pytest
from focus_log.core import FocusLogCore
...
*工具編號 #123 ／ 類別 G — 個人生產力 ／ 計畫版本 v1.0 ／ 2026-04-27*

...
| 介面 | CLI（`click` 框架） |
| 設定檔 | `~/.config/web-fetch/config.json` |
...
# 指定輸出格式
web-fetch run --output json

# 詳細模式
web-fetch run --verbose

# 靜音（適合 pipe 到其他工具）
web-fetch run --quiet | jq .

# 查看設定
web-fetch config show

# 匯出 JSON
web-fetch export --output json > result.json
```
...
def config():
    """顯示或修改設定"""
    import json, pathlib
    cfg_path = pathlib.Path.home() / ".config" / "web-fetch" / "config.json"
    if cfg_path.exists():
...
DB_PATH = pathlib.Path.home() / ".config" / "web-fetch" / "data.db"
...
```bash
# Agent 呼叫範例
result=$(web-fetch run --quiet --output json)
echo $result | jq .data
```
...
def call_web_fetch(action="run") -> dict:
    proc = subprocess.run(
        ["web-fetch", action, "--output", "json", "--quiet"],
...
```bash
# 管線串接範例
web-fetch run --quiet --output json \
  | jq -r '.data' \
  | other-tool process
```
...
# 執行測試
pytest tests/ -v --cov=web_fetch --cov-report=term-missing
...
```python
import pytest
from web_fetch.core import WebFetchCore
...
*工具編號 #241 ／ 類別 M — 網路爬蟲與監控 ／ 計畫版本 v1.0 ／ 2026-04-27*

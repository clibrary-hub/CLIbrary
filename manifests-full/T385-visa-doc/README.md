...
| 介面 | CLI（`click` 框架） |
| 設定檔 | `~/.config/visa-doc/config.json` |
...
# 指定輸出格式
visa-doc run --output json

# 詳細模式
visa-doc run --verbose

# 靜音（適合 pipe 到其他工具）
visa-doc run --quiet | jq .

# 查看設定
visa-doc config show

# 匯出 JSON
visa-doc export --output json > result.json
```
...
def config():
    """顯示或修改設定"""
    import json, pathlib
    cfg_path = pathlib.Path.home() / ".config" / "visa-doc" / "config.json"
    if cfg_path.exists():
...
DB_PATH = pathlib.Path.home() / ".config" / "visa-doc" / "data.db"
...
{
  "name": "visa-doc",
  "description": "簽證文件清單。解決問題：通關文件清單",
  "input_schema": {
    "type": "object",
    "properties": {
      "action": {
...
```bash
# Agent 呼叫範例
result=$(visa-doc run --quiet --output json)
echo $result | jq .data
```
...
def call_visa_doc(action="run") -> dict:
    proc = subprocess.run(
        ["visa-doc", action, "--output", "json", "--quiet"],
...
```bash
# 管線串接範例
visa-doc run --quiet --output json \
  | jq -r '.data' \
  | other-tool process
```
...
# 執行測試
pytest tests/ -v --cov=visa_doc --cov-report=term-missing
...
```python
import pytest
from visa_doc.core import VisaDocCore
...
*工具編號 #385 ／ 類別 T — 旅遊與生活 ／ 計畫版本 v1.0 ／ 2026-04-27*

...
| 介面 | CLI（`click` 框架） |
| 設定檔 | `~/.config/llm-cost-opt/config.json` |
...
[project.scripts]
llm-cost-opt = "llm_cost_opt.cli:main"
```
...
```bash
llm-cost-opt [OPTIONS] COMMAND [ARGS]...
```
...
# 指定輸出格式
llm-cost-opt run --output json

# 詳細模式
llm-cost-opt run --verbose

# 靜音（適合 pipe 到其他工具）
llm-cost-opt run --quiet | jq .

# 查看設定
llm-cost-opt config show

# 匯出 JSON
llm-cost-opt export --output json > result.json
```
...
from .core import LlmCostOptCore
...
def config():
    """顯示或修改設定"""
    import json, pathlib
    cfg_path = pathlib.Path.home() / ".config" / "llm-cost-opt" / "config.json"
    if cfg_path.exists():
...
DB_PATH = pathlib.Path.home() / ".config" / "llm-cost-opt" / "data.db"
...
{
  "name": "llm-cost-opt",
  "description": "成本最佳化器。解決問題：成本最佳化建議",
  "input_schema": {
    "type": "object",
    "properties": {
      "action": {
...
```bash
# Agent 呼叫範例
result=$(llm-cost-opt run --quiet --output json)
echo $result | jq .data
```
...
def call_llm_cost_opt(action="run") -> dict:
    proc = subprocess.run(
        ["llm-cost-opt", action, "--output", "json", "--quiet"],
...
```bash
# 管線串接範例
llm-cost-opt run --quiet --output json \
  | jq -r '.data' \
  | other-tool process
```
...
# 執行測試
pytest tests/ -v --cov=llm_cost_opt --cov-report=term-missing

# 快速冒煙測試
llm-cost-opt run --help
llm-cost-opt config show
```
...
```python
import pytest
from llm_cost_opt.core import LlmCostOptCore
...
*工具編號 #290 ／ 類別 O — AI/ML輔助 ／ 計畫版本 v1.0 ／ 2026-04-27*

...
| 介面 | CLI（`click` 框架） |
| 設定檔 | `~/.config/slow-sql/config.json` |
...
# 指定輸出格式
slow-sql run --output json

# 詳細模式
slow-sql run --verbose

# 靜音（適合 pipe 到其他工具）
slow-sql run --quiet | jq .

# 查看設定
slow-sql config show

# 匯出 JSON
slow-sql export --output json > result.json
```
...
def config():
    """顯示或修改設定"""
    import json, pathlib
    cfg_path = pathlib.Path.home() / ".config" / "slow-sql" / "config.json"
    if cfg_path.exists():
...
DB_PATH = pathlib.Path.home() / ".config" / "slow-sql" / "data.db"
...
{
  "name": "slow-sql",
  "description": "慢查詢分析器。解決問題：資料庫慢查詢分析",
  "input_schema": {
    "type": "object",
    "properties": {
      "action": {
...
```bash
# Agent 呼叫範例
result=$(slow-sql run --quiet --output json)
echo $result | jq .data
```
...
def call_slow_sql(action="run") -> dict:
    proc = subprocess.run(
        ["slow-sql", action, "--output", "json", "--quiet"],
...
```bash
# 管線串接範例
slow-sql run --quiet --output json \
  | jq -r '.data' \
  | other-tool process
```
...
# 執行測試
pytest tests/ -v --cov=slow_sql --cov-report=term-missing
...
```python
import pytest
from slow_sql.core import SlowSqlCore
...
*工具編號 #333 ／ 類別 Q — DevOps與基礎設施 ／ 計畫版本 v1.0 ／ 2026-04-27*

...
| 介面 | CLI（`click` 框架） |
| 設定檔 | `~/.config/book-flow/config.json` |
...
```bash
book-flow [OPTIONS] COMMAND [ARGS]...
```
...
# 指定輸出格式
book-flow run --output json

# 詳細模式
book-flow run --verbose

# 靜音（適合 pipe 到其他工具）
book-flow run --quiet | jq .

# 查看設定
book-flow config show

# 匯出 JSON
book-flow export --output json > result.json
```
...
def config():
    """顯示或修改設定"""
    import json, pathlib
    cfg_path = pathlib.Path.home() / ".config" / "book-flow" / "config.json"
    if cfg_path.exists():
...
DB_PATH = pathlib.Path.home() / ".config" / "book-flow" / "data.db"
...
{
  "name": "book-flow",
  "description": "預約流程器。解決問題：預約跑流程繁瑣",
  "input_schema": {
    "type": "object",
    "properties": {
      "action": {
...
```bash
# Agent 呼叫範例
result=$(book-flow run --quiet --output json)
echo $result | jq .data
```
...
def call_book_flow(action="run") -> dict:
    proc = subprocess.run(
        ["book-flow", action, "--output", "json", "--quiet"],
...
```bash
# 管線串接範例
book-flow run --quiet --output json \
  | jq -r '.data' \
  | other-tool process
```
...
# 執行測試
pytest tests/ -v --cov=book_flow --cov-report=term-missing
...
```python
import pytest
from book_flow.core import BookFlowCore
...
*工具編號 #144 ／ 類別 H — 行事曆與時間管理 ／ 計畫版本 v1.0 ／ 2026-04-27*

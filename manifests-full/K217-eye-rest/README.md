...
| 介面 | CLI（`click` 框架） |
| 設定檔 | `~/.config/eye-rest/config.json` |
...
# 指定輸出格式
eye-rest run --output json

# 詳細模式
eye-rest run --verbose

# 靜音（適合 pipe 到其他工具）
eye-rest run --quiet | jq .

# 查看設定
eye-rest config show

# 匯出 JSON
eye-rest export --output json > result.json
```
...
def config():
    """顯示或修改設定"""
    import json, pathlib
    cfg_path = pathlib.Path.home() / ".config" / "eye-rest" / "config.json"
    if cfg_path.exists():
...
DB_PATH = pathlib.Path.home() / ".config" / "eye-rest" / "data.db"
...
```bash
# Agent 呼叫範例
result=$(eye-rest run --quiet --output json)
echo $result | jq .data
```
...
def call_eye_rest(action="run") -> dict:
    proc = subprocess.run(
        ["eye-rest", action, "--output", "json", "--quiet"],
...
```bash
# 管線串接範例
eye-rest run --quiet --output json \
  | jq -r '.data' \
  | other-tool process
```
...
# 執行測試
pytest tests/ -v --cov=eye_rest --cov-report=term-missing
...
```python
import pytest
from eye_rest.core import EyeRestCore
...
*工具編號 #217 ／ 類別 K — 健康與健身 ／ 計畫版本 v1.0 ／ 2026-04-27*

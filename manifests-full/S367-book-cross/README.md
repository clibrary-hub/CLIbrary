...
| 介面 | CLI（`click` 框架） |
| 設定檔 | `~/.config/book-cross/config.json` |
...
[project.scripts]
book-cross = "book_cross.cli:main"
```
...
```bash
book-cross [OPTIONS] COMMAND [ARGS]...
```
...
# 指定輸出格式
book-cross run --output json

# 詳細模式
book-cross run --verbose

# 靜音（適合 pipe 到其他工具）
book-cross run --quiet | jq .

# 查看設定
book-cross config show

# 匯出 JSON
book-cross export --output json > result.json
```
...
from .core import BookCrossCore
...
def config():
    """顯示或修改設定"""
    import json, pathlib
    cfg_path = pathlib.Path.home() / ".config" / "book-cross" / "config.json"
    if cfg_path.exists():
...
DB_PATH = pathlib.Path.home() / ".config" / "book-cross" / "data.db"
...
{
  "name": "book-cross",
  "description": "跨書比對器。解決問題：多教材交叉比對",
  "input_schema": {
    "type": "object",
    "properties": {
      "action": {
...
```bash
# Agent 呼叫範例
result=$(book-cross run --quiet --output json)
echo $result | jq .data
```
...
def call_book_cross(action="run") -> dict:
    proc = subprocess.run(
        ["book-cross", action, "--output", "json", "--quiet"],
...
```bash
# 管線串接範例
book-cross run --quiet --output json \
  | jq -r '.data' \
  | other-tool process
```
...
# 執行測試
pytest tests/ -v --cov=book_cross --cov-report=term-missing
...
```python
import pytest
from book_cross.core import BookCrossCore
...
*工具編號 #367 ／ 類別 S — 學習與研究 ／ 計畫版本 v1.0 ／ 2026-04-27*

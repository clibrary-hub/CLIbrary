...
| 介面 | CLI（`click` 框架） |
| 設定檔 | `~/.config/name-lint/config.json` |
...
```bash
name-lint [OPTIONS] COMMAND [ARGS]...
```
...
# 指定輸出格式
name-lint run --output json

# 詳細模式
name-lint run --verbose

# 靜音（適合 pipe 到其他工具）
name-lint run --quiet | jq .

# 查看設定
name-lint config show

# 匯出 JSON
name-lint export --output json > result.json
```
...
def config():
    """顯示或修改設定"""
    import json, pathlib
    cfg_path = pathlib.Path.home() / ".config" / "name-lint" / "config.json"
    if cfg_path.exists():
...
DB_PATH = pathlib.Path.home() / ".config" / "name-lint" / "data.db"
...
{
  "name": "name-lint",
  "description": "命名規約檢查器。解決問題：命名不一致",
  "input_schema": {
    "type": "object",
    "properties": {
      "action": {
...
```bash
# Agent 呼叫範例
result=$(name-lint run --quiet --output json)
echo $result | jq .data
```
...
def call_name_lint(action="run") -> dict:
    proc = subprocess.run(
        ["name-lint", action, "--output", "json", "--quiet"],
...
```bash
# 管線串接範例
name-lint run --quiet --output json \
  | jq -r '.data' \
  | other-tool process
```
...
# 執行測試
pytest tests/ -v --cov=name_lint --cov-report=term-missing
...
```python
import pytest
from name_lint.core import NameLintCore
...
*工具編號 #053 ／ 類別 C — 程式碼品質與開發輔助 ／ 計畫版本 v1.0 ／ 2026-04-27*

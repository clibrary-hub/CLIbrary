...
| 介面 | CLI（`click` 框架） |
| 設定檔 | `~/.config/plural-rules/config.json` |
...
[project.scripts]
plural-rules = "plural_rules.cli:main"
```
...
```bash
plural-rules [OPTIONS] COMMAND [ARGS]...
```
...
# 指定輸出格式
plural-rules run --output json

# 詳細模式
plural-rules run --verbose

# 靜音（適合 pipe 到其他工具）
plural-rules run --quiet | jq .

# 查看設定
plural-rules config show

# 匯出 JSON
plural-rules export --output json > result.json
```
...
from .core import PluralRulesCore
...
def config():
    """顯示或修改設定"""
    import json, pathlib
    cfg_path = pathlib.Path.home() / ".config" / "plural-rules" / "config.json"
    if cfg_path.exists():
...
DB_PATH = pathlib.Path.home() / ".config" / "plural-rules" / "data.db"
...
{
  "name": "plural-rules",
  "description": "複數規則器。解決問題：自動單複數規則",
  "input_schema": {
    "type": "object",
    "properties": {
      "action": {
...
```bash
# Agent 呼叫範例
result=$(plural-rules run --quiet --output json)
echo $result | jq .data
```
...
def call_plural_rules(action="run") -> dict:
    proc = subprocess.run(
        ["plural-rules", action, "--output", "json", "--quiet"],
...
```bash
# 管線串接範例
plural-rules run --quiet --output json \
  | jq -r '.data' \
  | other-tool process
```
...
# 執行測試
pytest tests/ -v --cov=plural_rules --cov-report=term-missing

# 快速冒煙測試
plural-rules run --help
plural-rules config show
```
...
```python
import pytest
from plural_rules.core import PluralRulesCore
...
*工具編號 #452 ／ 類別 W — 翻譯與在地化 ／ 計畫版本 v1.0 ／ 2026-04-27*

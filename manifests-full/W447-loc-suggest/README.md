...
| 介面 | CLI（`click` 框架） |
| 設定檔 | `~/.config/loc-suggest/config.json` |
...
[project.scripts]
loc-suggest = "loc_suggest.cli:main"
```
...
```bash
loc-suggest [OPTIONS] COMMAND [ARGS]...
```
...
# 指定輸出格式
loc-suggest run --output json

# 詳細模式
loc-suggest run --verbose

# 靜音（適合 pipe 到其他工具）
loc-suggest run --quiet | jq .

# 查看設定
loc-suggest config show

# 匯出 JSON
loc-suggest export --output json > result.json
```
...
from .core import LocSuggestCore
...
def config():
    """顯示或修改設定"""
    import json, pathlib
    cfg_path = pathlib.Path.home() / ".config" / "loc-suggest" / "config.json"
    if cfg_path.exists():
...
DB_PATH = pathlib.Path.home() / ".config" / "loc-suggest" / "data.db"
...
{
  "name": "loc-suggest",
  "description": "在地化建議器。解決問題：文化在地化建議",
  "input_schema": {
    "type": "object",
    "properties": {
      "action": {
...
```bash
# Agent 呼叫範例
result=$(loc-suggest run --quiet --output json)
echo $result | jq .data
```
...
def call_loc_suggest(action="run") -> dict:
    proc = subprocess.run(
        ["loc-suggest", action, "--output", "json", "--quiet"],
...
```bash
# 管線串接範例
loc-suggest run --quiet --output json \
  | jq -r '.data' \
  | other-tool process
```
...
# 執行測試
pytest tests/ -v --cov=loc_suggest --cov-report=term-missing

# 快速冒煙測試
loc-suggest run --help
loc-suggest config show
```
...
```python
import pytest
from loc_suggest.core import LocSuggestCore
...
*工具編號 #447 ／ 類別 W — 翻譯與在地化 ／ 計畫版本 v1.0 ／ 2026-04-27*

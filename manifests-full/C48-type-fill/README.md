...
| 介面 | CLI（`click` 框架） |
| 設定檔 | `~/.config/type-fill/config.json` |
...
pip install ast mypy click rich
...
dependencies = ["ast", "mypy", "click", "rich"]

[project.scripts]
type-fill = "type_fill.cli:main"
```
...
```bash
type-fill [OPTIONS] COMMAND [ARGS]...
```
...
# 指定輸出格式
type-fill run --output json

# 詳細模式
type-fill run --verbose

# 靜音（適合 pipe 到其他工具）
type-fill run --quiet | jq .

# 查看設定
type-fill config show

# 匯出 JSON
type-fill export --output json > result.json
```
...
def config():
    """顯示或修改設定"""
    import json, pathlib
    cfg_path = pathlib.Path.home() / ".config" / "type-fill" / "config.json"
    if cfg_path.exists():
...
DB_PATH = pathlib.Path.home() / ".config" / "type-fill" / "data.db"
...
{
  "name": "type-fill",
  "description": "型別補全器。解決問題：型別定義不完整",
  "input_schema": {
    "type": "object",
    "properties": {
      "action": {
...
```bash
# Agent 呼叫範例
result=$(type-fill run --quiet --output json)
echo $result | jq .data
```
...
def call_type_fill(action="run") -> dict:
    proc = subprocess.run(
        ["type-fill", action, "--output", "json", "--quiet"],
...
```bash
# 管線串接範例
type-fill run --quiet --output json \
  | jq -r '.data' \
  | other-tool process
```
...
# 執行測試
pytest tests/ -v --cov=type_fill --cov-report=term-missing
...
```python
import pytest
from type_fill.core import TypeFillCore
...
*工具編號 #048 ／ 類別 C — 程式碼品質與開發輔助 ／ 計畫版本 v1.0 ／ 2026-04-27*

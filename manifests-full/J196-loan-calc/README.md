...
| 介面 | CLI（`click` 框架） |
| 設定檔 | `~/.config/loan-calc/config.json` |
...
```bash
loan-calc [OPTIONS] COMMAND [ARGS]...
```
...
# 指定輸出格式
loan-calc run --output json

# 詳細模式
loan-calc run --verbose

# 靜音（適合 pipe 到其他工具）
loan-calc run --quiet | jq .

# 查看設定
loan-calc config show

# 匯出 JSON
loan-calc export --output json > result.json
```
...
def config():
    """顯示或修改設定"""
    import json, pathlib
    cfg_path = pathlib.Path.home() / ".config" / "loan-calc" / "config.json"
    if cfg_path.exists():
...
DB_PATH = pathlib.Path.home() / ".config" / "loan-calc" / "data.db"
...
```bash
# Agent 呼叫範例
result=$(loan-calc run --quiet --output json)
echo $result | jq .data
```
...
def call_loan_calc(action="run") -> dict:
    proc = subprocess.run(
        ["loan-calc", action, "--output", "json", "--quiet"],
...
```bash
# 管線串接範例
loan-calc run --quiet --output json \
  | jq -r '.data' \
  | other-tool process
```
...
# 執行測試
pytest tests/ -v --cov=loan_calc --cov-report=term-missing
...
```python
import pytest
from loan_calc.core import LoanCalcCore
...
*工具編號 #196 ／ 類別 J — 財務與帳務 ／ 計畫版本 v1.0 ／ 2026-04-27*

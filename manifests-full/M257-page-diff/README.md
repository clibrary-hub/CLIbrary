...
| 介面 | CLI（`click` 框架） |
| 設定檔 | `~/.config/page-diff/config.json` |
...
pip install requests difflib click
...
dependencies = ["requests", "difflib", "click"]

[project.scripts]
page-diff = "page_diff.cli:main"
```
...
```bash
page-diff [OPTIONS] COMMAND [ARGS]...
```
...
# 指定輸出格式
page-diff run --output json

# 詳細模式
page-diff run --verbose

# 靜音（適合 pipe 到其他工具）
page-diff run --quiet | jq .

# 查看設定
page-diff config show

# 匯出 JSON
page-diff export --output json > result.json
```
...
def config():
    """顯示或修改設定"""
    import json, pathlib
    cfg_path = pathlib.Path.home() / ".config" / "page-diff" / "config.json"
    if cfg_path.exists():
...
DB_PATH = pathlib.Path.home() / ".config" / "page-diff" / "data.db"
...
class PageDiffCore:
    ...
    核心邏輯類別。
    問題：網站變動 diff
    解法：透過 requests, difflib 實現自動化處理。
    """
...
{
  "name": "page-diff",
  "description": "網頁差異器。解決問題：網站變動 diff",
  "input_schema": {
    "type": "object",
    "properties": {
      "action": {
...
```bash
# Agent 呼叫範例
result=$(page-diff run --quiet --output json)
echo $result | jq .data
```
...
def call_page_diff(action="run") -> dict:
    proc = subprocess.run(
        ["page-diff", action, "--output", "json", "--quiet"],
...
```bash
# 管線串接範例
page-diff run --quiet --output json \
  | jq -r '.data' \
  | other-tool process
```
...
# 執行測試
pytest tests/ -v --cov=page_diff --cov-report=term-missing
...
```python
import pytest
from page_diff.core import PageDiffCore
...
```json
{
  ...
  "number": 257,
  "pain_point": "網站變動 diff",
  "commercial": "⚠️",
  "language": "Python",
  "entry_point": "page-diff",
  "dependencies": ["requests", "difflib", "click"],
...
*工具編號 #257 ／ 類別 M — 網路爬蟲與監控 ／ 計畫版本 v1.0 ／ 2026-04-27*

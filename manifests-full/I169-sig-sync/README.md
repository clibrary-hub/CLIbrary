...
| 介面 | CLI（`click` 框架） |
| 設定檔 | `~/.config/sig-sync/config.json` |
...
# 指定輸出格式
sig-sync run --output json

# 詳細模式
sig-sync run --verbose

# 靜音（適合 pipe 到其他工具）
sig-sync run --quiet | jq .

# 查看設定
sig-sync config show

# 匯出 JSON
sig-sync export --output json > result.json
```
...
def config():
    """顯示或修改設定"""
    import json, pathlib
    cfg_path = pathlib.Path.home() / ".config" / "sig-sync" / "config.json"
    if cfg_path.exists():
...
DB_PATH = pathlib.Path.home() / ".config" / "sig-sync" / "data.db"
...
{
  "name": "sig-sync",
  "description": "簽名檔同步。解決問題：公司簽名檔不統一",
  "input_schema": {
    "type": "object",
    "properties": {
      "action": {
...
```bash
# Agent 呼叫範例
result=$(sig-sync run --quiet --output json)
echo $result | jq .data
```
...
def call_sig_sync(action="run") -> dict:
    proc = subprocess.run(
        ["sig-sync", action, "--output", "json", "--quiet"],
...
```bash
# 管線串接範例
sig-sync run --quiet --output json \
  | jq -r '.data' \
  | other-tool process
```
...
# 執行測試
pytest tests/ -v --cov=sig_sync --cov-report=term-missing
...
```python
import pytest
from sig_sync.core import SigSyncCore
...
*工具編號 #169 ／ 類別 I — 郵件與通訊 ／ 計畫版本 v1.0 ／ 2026-04-27*

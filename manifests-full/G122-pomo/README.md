...
# 匯出 JSON
pomo export --output json > result.json
```
...
def config():
    """顯示或修改設定"""
    import json, pathlib
    cfg_path = pathlib.Path.home() / ".config" / "pomo" / "config.json"
    if cfg_path.exists():
...
DB_PATH = pathlib.Path.home() / ".config" / "pomo" / "data.db"
...
{
  "name": "pomo",
  "description": "終端番茄鐘。解決問題：番茄鐘工具切換麻煩",
  "input_schema": {
    "type": "object",
    "properties": {
      "action": {
...
```bash
# Agent 呼叫範例
result=$(pomo run --quiet --output json)
echo $result | jq .data
```
...
def call_pomo(action="run") -> dict:
    proc = subprocess.run(
        ["pomo", action, "--output", "json", "--quiet"],
...
# 執行測試
pytest tests/ -v --cov=pomo --cov-report=term-missing
...
```python
import pytest
from pomo.core import PomoCore
...
*工具編號 #122 ／ 類別 G — 個人生產力 ／ 計畫版本 v1.0 ／ 2026-04-27*

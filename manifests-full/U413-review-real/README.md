...
| 介面 | CLI（`click` 框架） |
| 設定檔 | `~/.config/review-real/config.json` |
...
[project.scripts]
review-real = "review_real.cli:main"
```
...
```bash
review-real [OPTIONS] COMMAND [ARGS]...
```
...
# 指定輸出格式
review-real run --output json

# 詳細模式
review-real run --verbose

# 靜音（適合 pipe 到其他工具）
review-real run --quiet | jq .

# 查看設定
review-real config show

# 匯出 JSON
review-real export --output json > result.json
```
...
from .core import ReviewRealCore
...
def config():
    """顯示或修改設定"""
    import json, pathlib
    cfg_path = pathlib.Path.home() / ".config" / "review-real" / "config.json"
    if cfg_path.exists():
...
DB_PATH = pathlib.Path.home() / ".config" / "review-real" / "data.db"
...
```bash
# Agent 呼叫範例
result=$(review-real run --quiet --output json)
echo $result | jq .data
```
...
def call_review_real(action="run") -> dict:
    proc = subprocess.run(
        ["review-real", action, "--output", "json", "--quiet"],
...
```bash
# 管線串接範例
review-real run --quiet --output json \
  | jq -r '.data' \
  | other-tool process
```
...
# 執行測試
pytest tests/ -v --cov=review_real --cov-report=term-missing

# 快速冒煙測試
review-real run --help
review-real config show
```
...
```python
import pytest
from review_real.core import ReviewRealCore
...
*工具編號 #413 ／ 類別 U — 購物與訂閱管理 ／ 計畫版本 v1.0 ／ 2026-04-27*

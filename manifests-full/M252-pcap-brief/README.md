...
| 介面 | CLI（`click` 框架） |
| 設定檔 | `~/.config/pcap-brief/config.json` |
...
[project.scripts]
pcap-brief = "pcap_brief.cli:main"
```
...
```bash
pcap-brief [OPTIONS] COMMAND [ARGS]...
```
...
# 指定輸出格式
pcap-brief run --output json

# 詳細模式
pcap-brief run --verbose

# 靜音（適合 pipe 到其他工具）
pcap-brief run --quiet | jq .

# 查看設定
pcap-brief config show

# 匯出 JSON
pcap-brief export --output json > result.json
```
...
from .core import PcapBriefCore
...
def config():
    """顯示或修改設定"""
    import json, pathlib
    cfg_path = pathlib.Path.home() / ".config" / "pcap-brief" / "config.json"
    if cfg_path.exists():
...
DB_PATH = pathlib.Path.home() / ".config" / "pcap-brief" / "data.db"
...
```bash
# Agent 呼叫範例
result=$(pcap-brief run --quiet --output json)
echo $result | jq .data
```
...
def call_pcap_brief(action="run") -> dict:
    proc = subprocess.run(
        ["pcap-brief", action, "--output", "json", "--quiet"],
...
```bash
# 管線串接範例
pcap-brief run --quiet --output json \
  | jq -r '.data' \
  | other-tool process
```
...
# 執行測試
pytest tests/ -v --cov=pcap_brief --cov-report=term-missing
...
```python
import pytest
from pcap_brief.core import PcapBriefCore
...
*工具編號 #252 ／ 類別 M — 網路爬蟲與監控 ／ 計畫版本 v1.0 ／ 2026-04-27*

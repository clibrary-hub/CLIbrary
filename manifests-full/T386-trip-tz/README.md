...
| 介面 | CLI（`click` 框架） |
| 設定檔 | `~/.config/trip-tz/config.json` |
...
# 指定輸出格式
trip-tz run --output json

# 詳細模式
trip-tz run --verbose

# 靜音（適合 pipe 到其他工具）
trip-tz run --quiet | jq .

# 查看設定
trip-tz config show

# 匯出 JSON
trip-tz export --output json > result.json
```
...
def config():
    """顯示或修改設定"""
    import json, pathlib
    cfg_path = pathlib.Path.home() / ".config" / "trip-tz" / "config.json"
    if cfg_path.exists():
...
DB_PATH = pathlib.Path.home() / ".config" / "trip-tz" / "data.db"
...
{
  "name": "trip-tz",
  "description": "行程時區器。解決問題：多時區行程顯示",
  "input_schema": {
    "type": "object",
    "properties": {
      "action": {
...
```bash
# Agent 呼叫範例
result=$(trip-tz run --quiet --output json)
echo $result | jq .data
```
...
def call_trip_tz(action="run") -> dict:
    proc = subprocess.run(
        ["trip-tz", action, "--output", "json", "--quiet"],
...
# 執行測試
pytest tests/ -v --cov=trip_tz --cov-report=term-missing
...
```python
import pytest
from trip_tz.core import TripTzCore
...
*工具編號 #386 ／ 類別 T — 旅遊與生活 ／ 計畫版本 v1.0 ／ 2026-04-27*

...
| 介面 | CLI（`click` 框架） |
| 設定檔 | `~/.config/paper-brief/config.json` |
...
[project.scripts]
paper-brief = "paper_brief.cli:main"
```
...
```bash
paper-brief [OPTIONS] COMMAND [ARGS]...
```
...
# 指定輸出格式
paper-brief run --output json

# 詳細模式
paper-brief run --verbose

# 靜音（適合 pipe 到其他工具）
paper-brief run --quiet | jq .

# 查看設定
paper-brief config show

# 匯出 JSON
paper-brief export --output json > result.json
```
...
from .core import PaperBriefCore
...
def config():
    """顯示或修改設定"""
    import json, pathlib
    cfg_path = pathlib.Path.home() / ".config" / "paper-brief" / "config.json"
    if cfg_path.exists():
...
DB_PATH = pathlib.Path.home() / ".config" / "paper-brief" / "data.db"
...
```bash
# Agent 呼叫範例
result=$(paper-brief run --quiet --output json)
echo $result | jq .data
```
...
def call_paper_brief(action="run") -> dict:
    proc = subprocess.run(
        ["paper-brief", action, "--output", "json", "--quiet"],
...
```bash
# 管線串接範例
paper-brief run --quiet --output json \
  | jq -r '.data' \
  | other-tool process
```
...
# 執行測試
pytest tests/ -v --cov=paper_brief --cov-report=term-missing

# 快速冒煙測試
paper-brief run --help
paper-brief config show
```
...
```python
import pytest
from paper_brief.core import PaperBriefCore
...
*工具編號 #362 ／ 類別 S — 學習與研究 ／ 計畫版本 v1.0 ／ 2026-04-27*

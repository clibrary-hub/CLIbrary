...
| 介面 | CLI（`click` 框架） |
| 設定檔 | `~/.config/lab-explain/config.json` |
...
[project.scripts]
lab-explain = "lab_explain.cli:main"
```
...
```bash
lab-explain [OPTIONS] COMMAND [ARGS]...
```
...
# 指定輸出格式
lab-explain run --output json

# 詳細模式
lab-explain run --verbose

# 靜音（適合 pipe 到其他工具）
lab-explain run --quiet | jq .

# 查看設定
lab-explain config show

# 匯出 JSON
lab-explain export --output json > result.json
```
...
from .core import LabExplainCore
...
def config():
    """顯示或修改設定"""
    import json, pathlib
    cfg_path = pathlib.Path.home() / ".config" / "lab-explain" / "config.json"
    if cfg_path.exists():
...
DB_PATH = pathlib.Path.home() / ".config" / "lab-explain" / "data.db"
...
{
  "name": "lab-explain",
  "description": "健檢報告解讀器。解決問題：健檢報告解讀難",
  "input_schema": {
    "type": "object",
    "properties": {
      "action": {
...
```bash
# Agent 呼叫範例
result=$(lab-explain run --quiet --output json)
echo $result | jq .data
```
...
def call_lab_explain(action="run") -> dict:
    proc = subprocess.run(
        ["lab-explain", action, "--output", "json", "--quiet"],
...
```bash
# 管線串接範例
lab-explain run --quiet --output json \
  | jq -r '.data' \
  | other-tool process
```
...
# 執行測試
pytest tests/ -v --cov=lab_explain --cov-report=term-missing

# 快速冒煙測試
lab-explain run --help
lab-explain config show
```
...
```python
import pytest
from lab_explain.core import LabExplainCore
...
*工具編號 #216 ／ 類別 K — 健康與健身 ／ 計畫版本 v1.0 ／ 2026-04-27*

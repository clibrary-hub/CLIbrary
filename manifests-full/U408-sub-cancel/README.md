...
| 介面 | CLI（`click` 框架） |
| 設定檔 | `~/.config/sub-cancel/config.json` |
...
[project.scripts]
sub-cancel = "sub_cancel.cli:main"
```
...
```bash
sub-cancel [OPTIONS] COMMAND [ARGS]...
```
...
# 指定輸出格式
sub-cancel run --output json

# 詳細模式
sub-cancel run --verbose

# 靜音（適合 pipe 到其他工具）
sub-cancel run --quiet | jq .

# 查看設定
sub-cancel config show

# 匯出 JSON
sub-cancel export --output json > result.json
```
...
from .core import SubCancelCore
...
def config():
    """顯示或修改設定"""
    import json, pathlib
    cfg_path = pathlib.Path.home() / ".config" / "sub-cancel" / "config.json"
    if cfg_path.exists():
...
DB_PATH = pathlib.Path.home() / ".config" / "sub-cancel" / "data.db"
...
{
  "name": "sub-cancel",
  "description": "訂閱取消助手。解決問題：自動取消未用訂閱",
  "input_schema": {
    "type": "object",
    "properties": {
      "action": {
...
```bash
# Agent 呼叫範例
result=$(sub-cancel run --quiet --output json)
echo $result | jq .data
```
...
def call_sub_cancel(action="run") -> dict:
    proc = subprocess.run(
        ["sub-cancel", action, "--output", "json", "--quiet"],
...
```bash
# 管線串接範例
sub-cancel run --quiet --output json \
  | jq -r '.data' \
  | other-tool process
```
...
# 執行測試
pytest tests/ -v --cov=sub_cancel --cov-report=term-missing
...
```python
import pytest
from sub_cancel.core import SubCancelCore
...
*工具編號 #408 ／ 類別 U — 購物與訂閱管理 ／ 計畫版本 v1.0 ／ 2026-04-27*

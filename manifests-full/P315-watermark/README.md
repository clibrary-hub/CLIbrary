...
| 介面 | CLI（`click` 框架） |
| 設定檔 | `~/.config/watermark/config.json` |
...
```bash
watermark [OPTIONS] COMMAND [ARGS]...
```
...
# 指定輸出格式
watermark run --output json

# 詳細模式
watermark run --verbose

# 靜音（適合 pipe 到其他工具）
watermark run --quiet | jq .

# 查看設定
watermark config show

# 匯出 JSON
watermark export --output json > result.json
```
...
def config():
    """顯示或修改設定"""
    import json, pathlib
    cfg_path = pathlib.Path.home() / ".config" / "watermark" / "config.json"
    if cfg_path.exists():
...
DB_PATH = pathlib.Path.home() / ".config" / "watermark" / "data.db"
...
```bash
# Agent 呼叫範例
result=$(watermark run --quiet --output json)
echo $result | jq .data
```
...
def call_watermark(action="run") -> dict:
    proc = subprocess.run(
        ["watermark", action, "--output", "json", "--quiet"],
...
```bash
# 管線串接範例
watermark run --quiet --output json \
  | jq -r '.data' \
  | other-tool process
```
...
# 執行測試
pytest tests/ -v --cov=watermark --cov-report=term-missing
...
```python
import pytest
from watermark.core import WatermarkCore
...
*工具編號 #315 ／ 類別 P — 安全與隱私 ／ 計畫版本 v1.0 ／ 2026-04-27*

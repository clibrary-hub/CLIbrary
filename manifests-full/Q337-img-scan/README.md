...
| 介面 | CLI（`click` 框架） |
| 設定檔 | `~/.config/img-scan/config.json` |
...
# 指定輸出格式
img-scan run --output json

# 詳細模式
img-scan run --verbose

# 靜音（適合 pipe 到其他工具）
img-scan run --quiet | jq .

# 查看設定
img-scan config show

# 匯出 JSON
img-scan export --output json > result.json
```
...
def config():
    """顯示或修改設定"""
    import json, pathlib
    cfg_path = pathlib.Path.home() / ".config" / "img-scan" / "config.json"
    if cfg_path.exists():
...
DB_PATH = pathlib.Path.home() / ".config" / "img-scan" / "data.db"
...
{
  "name": "img-scan",
  "description": "Image 漏洞掃描。解決問題：容器漏洞掃描",
  "input_schema": {
    "type": "object",
    "properties": {
      "action": {
...
```bash
# Agent 呼叫範例
result=$(img-scan run --quiet --output json)
echo $result | jq .data
```
...
def call_img_scan(action="run") -> dict:
    proc = subprocess.run(
        ["img-scan", action, "--output", "json", "--quiet"],
...
```bash
# 管線串接範例
img-scan run --quiet --output json \
  | jq -r '.data' \
  | other-tool process
```
...
# 執行測試
pytest tests/ -v --cov=img_scan --cov-report=term-missing
...
```python
import pytest
from img_scan.core import ImgScanCore
...
*工具編號 #337 ／ 類別 Q — DevOps與基礎設施 ／ 計畫版本 v1.0 ／ 2026-04-27*

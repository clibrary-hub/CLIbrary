...
| 介面 | CLI（`click` 框架） |
| 設定檔 | `~/.config/trip-recap/config.json` |
...
[project.scripts]
trip-recap = "trip_recap.cli:main"
```
...
```bash
trip-recap [OPTIONS] COMMAND [ARGS]...
```
...
# 指定輸出格式
trip-recap run --output json

# 詳細模式
trip-recap run --verbose

# 靜音（適合 pipe 到其他工具）
trip-recap run --quiet | jq .

# 查看設定
trip-recap config show

# 匯出 JSON
trip-recap export --output json > result.json
```
...
from .core import TripRecapCore
...
def config():
    """顯示或修改設定"""
    import json, pathlib
    cfg_path = pathlib.Path.home() / ".config" / "trip-recap" / "config.json"
    if cfg_path.exists():
...
DB_PATH = pathlib.Path.home() / ".config" / "trip-recap" / "data.db"
...
{
  "name": "trip-recap",
  "description": "遊記產生器。解決問題：行程後遊記產生",
  "input_schema": {
    "type": "object",
    "properties": {
      "action": {
...
```bash
# Agent 呼叫範例
result=$(trip-recap run --quiet --output json)
echo $result | jq .data
```
...
def call_trip_recap(action="run") -> dict:
    proc = subprocess.run(
        ["trip-recap", action, "--output", "json", "--quiet"],
...
```bash
# 管線串接範例
trip-recap run --quiet --output json \
  | jq -r '.data' \
  | other-tool process
```
...
# 執行測試
pytest tests/ -v --cov=trip_recap --cov-report=term-missing
...
```python
import pytest
from trip_recap.core import TripRecapCore
...
*工具編號 #396 ／ 類別 T — 旅遊與生活 ／ 計畫版本 v1.0 ／ 2026-04-27*

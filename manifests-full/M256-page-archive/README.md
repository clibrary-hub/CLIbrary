...
| 介面 | CLI（`click` 框架） |
| 設定檔 | `~/.config/page-archive/config.json` |
...
dependencies = ["requests", "beautifulsoup4", "click"]

[project.scripts]
page-archive = "page_archive.cli:main"
```
...
```bash
page-archive [OPTIONS] COMMAND [ARGS]...
```
...
# 指定輸出格式
page-archive run --output json

# 詳細模式
page-archive run --verbose

# 靜音（適合 pipe 到其他工具）
page-archive run --quiet | jq .

# 查看設定
page-archive config show

# 匯出 JSON
page-archive export --output json > result.json
```
...
from .core import PageArchiveCore
...
def config():
    """顯示或修改設定"""
    import json, pathlib
    cfg_path = pathlib.Path.home() / ".config" / "page-archive" / "config.json"
    if cfg_path.exists():
...
DB_PATH = pathlib.Path.home() / ".config" / "page-archive" / "data.db"
...
class PageArchiveCore:
    """
    核心邏輯類別。
    問題：抓單頁存離線
    解法：透過 requests, beautifulsoup4 實現自動化處理。
    """
...
```bash
# Agent 呼叫範例
result=$(page-archive run --quiet --output json)
echo $result | jq .data
```
...
def call_page_archive(action="run") -> dict:
    proc = subprocess.run(
        ["page-archive", action, "--output", "json", "--quiet"],
...
```bash
# 管線串接範例
page-archive run --quiet --output json \
  | jq -r '.data' \
  | other-tool process
```
...
# 執行測試
pytest tests/ -v --cov=page_archive --cov-report=term-missing

# 快速冒煙測試
page-archive run --help
page-archive config show
```
...
```python
import pytest
from page_archive.core import PageArchiveCore
...
*工具編號 #256 ／ 類別 M — 網路爬蟲與監控 ／ 計畫版本 v1.0 ／ 2026-04-27*

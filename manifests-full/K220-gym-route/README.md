...
| 介面 | CLI（`click` 框架） |
| 設定檔 | `~/.config/gym-route/config.json` |
...
```bash
gym-route [OPTIONS] COMMAND [ARGS]...
```
...
# 指定輸出格式
gym-route run --output json

# 詳細模式
gym-route run --verbose

# 靜音（適合 pipe 到其他工具）
gym-route run --quiet | jq .

# 查看設定
gym-route config show

# 匯出 JSON
gym-route export --output json > result.json
```
...
def config():
    """顯示或修改設定"""
    import json, pathlib
    cfg_path = pathlib.Path.home() / ".config" / "gym-route" / "config.json"
    if cfg_path.exists():
...
DB_PATH = pathlib.Path.home() / ".config" / "gym-route" / "data.db"
...
{
  "name": "gym-route",
  "description": "健身房路徑器。解決問題：健身房路線安排",
  "input_schema": {
    "type": "object",
    "properties": {
      "action": {
...
```bash
# Agent 呼叫範例
result=$(gym-route run --quiet --output json)
echo $result | jq .data
```
...
def call_gym_route(action="run") -> dict:
    proc = subprocess.run(
        ["gym-route", action, "--output", "json", "--quiet"],
...
```bash
# 管線串接範例
gym-route run --quiet --output json \
  | jq -r '.data' \
  | other-tool process
```
...
# 執行測試
pytest tests/ -v --cov=gym_route --cov-report=term-missing
...
```python
import pytest
from gym_route.core import GymRouteCore
...
*工具編號 #220 ／ 類別 K — 健康與健身 ／ 計畫版本 v1.0 ／ 2026-04-27*

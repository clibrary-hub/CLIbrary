...
| 介面 | CLI（`click` 框架） |
| 設定檔 | `~/.config/drug-check/config.json` |
...
[project.scripts]
drug-check = "drug_check.cli:main"
```
...
```bash
drug-check [OPTIONS] COMMAND [ARGS]...
```
...
# 指定輸出格式
drug-check run --output json

# 詳細模式
drug-check run --verbose

# 靜音（適合 pipe 到其他工具）
drug-check run --quiet | jq .

# 查看設定
drug-check config show

# 匯出 JSON
drug-check export --output json > result.json
```
...
from .core import DrugCheckCore
...
def config():
    """顯示或修改設定"""
    import json, pathlib
    cfg_path = pathlib.Path.home() / ".config" / "drug-check" / "config.json"
    if cfg_path.exists():
...
DB_PATH = pathlib.Path.home() / ".config" / "drug-check" / "data.db"
...
{
  "name": "drug-check",
  "description": "藥物交互查詢。解決問題：用藥交互作用查詢",
  "input_schema": {
    "type": "object",
    "properties": {
      "action": {
...
```bash
# Agent 呼叫範例
result=$(drug-check run --quiet --output json)
echo $result | jq .data
```
...
def call_drug_check(action="run") -> dict:
    proc = subprocess.run(
        ["drug-check", action, "--output", "json", "--quiet"],
...
```bash
# 管線串接範例
drug-check run --quiet --output json \
  | jq -r '.data' \
  | other-tool process
```
...
# 執行測試
pytest tests/ -v --cov=drug_check --cov-report=term-missing
...
```python
import pytest
from drug_check.core import DrugCheckCore
...
*工具編號 #219 ／ 類別 K — 健康與健身 ／ 計畫版本 v1.0 ／ 2026-04-27*

...
| 介面 | CLI（`click` 框架） |
| 設定檔 | `~/.config/lang-align/config.json` |
...
[project.scripts]
lang-align = "lang_align.cli:main"
```
...
```bash
lang-align [OPTIONS] COMMAND [ARGS]...
```
...
# 指定輸出格式
lang-align run --output json

# 詳細模式
lang-align run --verbose

# 靜音（適合 pipe 到其他工具）
lang-align run --quiet | jq .

# 查看設定
lang-align config show

# 匯出 JSON
lang-align export --output json > result.json
```
...
from .core import LangAlignCore
...
def config():
    """顯示或修改設定"""
    import json, pathlib
    cfg_path = pathlib.Path.home() / ".config" / "lang-align" / "config.json"
    if cfg_path.exists():
...
DB_PATH = pathlib.Path.home() / ".config" / "lang-align" / "data.db"
...
```bash
# Agent 呼叫範例
result=$(lang-align run --quiet --output json)
echo $result | jq .data
```
...
def call_lang_align(action="run") -> dict:
    proc = subprocess.run(
        ["lang-align", action, "--output", "json", "--quiet"],
...
```bash
# 管線串接範例
lang-align run --quiet --output json \
  | jq -r '.data' \
  | other-tool process
```
...
# 執行測試
pytest tests/ -v --cov=lang_align --cov-report=term-missing
...
```python
import pytest
from lang_align.core import LangAlignCore
...
*工具編號 #448 ／ 類別 W — 翻譯與在地化 ／ 計畫版本 v1.0 ／ 2026-04-27*

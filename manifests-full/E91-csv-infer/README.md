...
| 介面 | CLI（`click` 框架） |
| 設定檔 | `~/.config/csv-infer/config.json` |
...
```bash
csv-infer [OPTIONS] COMMAND [ARGS]...
```
...
# 指定輸出格式
csv-infer run --output json

# 詳細模式
csv-infer run --verbose

# 靜音（適合 pipe 到其他工具）
csv-infer run --quiet | jq .

# 查看設定
csv-infer config show

# 匯出 JSON
csv-infer export --output json > result.json
```
...
def config():
    """顯示或修改設定"""
    import json, pathlib
    cfg_path = pathlib.Path.home() / ".config" / "csv-infer" / "config.json"
    if cfg_path.exists():
...
DB_PATH = pathlib.Path.home() / ".config" / "csv-infer" / "data.db"
...
{
  "name": "csv-infer",
  "description": "CSV 結構推斷器。解決問題：CSV 欄位推斷不準",
  "input_schema": {
    "type": "object",
    "properties": {
      "action": {
...
```bash
# Agent 呼叫範例
result=$(csv-infer run --quiet --output json)
echo $result | jq .data
```
...
def call_csv_infer(action="run") -> dict:
    proc = subprocess.run(
        ["csv-infer", action, "--output", "json", "--quiet"],
...
```bash
# 管線串接範例
csv-infer run --quiet --output json \
  | jq -r '.data' \
  | other-tool process
```
...
# 執行測試
pytest tests/ -v --cov=csv_infer --cov-report=term-missing
...
```python
import pytest
from csv_infer.core import CsvInferCore
...
```json
{
  "name": "csv-infer",
  "version": "0.1.0",
  "description": "CSV 結構推斷器",
  "category": "E",
  "category_name": "資料處理與整合",
  "number": 91,
  "pain_point": "CSV 欄位推斷不準",
  "commercial": "✅",
  "language": "Python",
  "entry_point": "csv-infer",
  "dependencies": ["pandas", "click", "rich"],
...
*工具編號 #091 ／ 類別 E — 資料處理與整合 ／ 計畫版本 v1.0 ／ 2026-04-27*

...
# 匯出 JSON
xpost export --output json > result.json
```
...
def config():
    """顯示或修改設定"""
    import json, pathlib
    cfg_path = pathlib.Path.home() / ".config" / "xpost" / "config.json"
    if cfg_path.exists():
...
DB_PATH = pathlib.Path.home() / ".config" / "xpost" / "data.db"
...
{
  "name": "xpost",
  "description": "跨平台重排器。解決問題：跨平台貼文重排",
  "input_schema": {
    "type": "object",
    "properties": {
      "action": {
...
```bash
# Agent 呼叫範例
result=$(xpost run --quiet --output json)
echo $result | jq .data
```
...
def call_xpost(action="run") -> dict:
    proc = subprocess.run(
        ["xpost", action, "--output", "json", "--quiet"],
...
# 執行測試
pytest tests/ -v --cov=xpost --cov-report=term-missing
...
```python
import pytest
from xpost.core import XpostCore
...
*工具編號 #474 ／ 類別 X — 寫作與內容創作 ／ 計畫版本 v1.0 ／ 2026-04-27*

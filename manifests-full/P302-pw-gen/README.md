...
pip install secrets click rich
...
dependencies = ["secrets", "click", "rich"]

[project.scripts]
pw-gen = "pw_gen.cli:main"
```
...
# 匯出 JSON
pw-gen export --output json > result.json
```
...
def config():
    """顯示或修改設定"""
    import json, pathlib
    cfg_path = pathlib.Path.home() / ".config" / "pw-gen" / "config.json"
    if cfg_path.exists():
...
DB_PATH = pathlib.Path.home() / ".config" / "pw-gen" / "data.db"
...
```bash
# Agent 呼叫範例
result=$(pw-gen run --quiet --output json)
echo $result | jq .data
```
...
def call_pw_gen(action="run") -> dict:
    proc = subprocess.run(
        ["pw-gen", action, "--output", "json", "--quiet"],
...
# 執行測試
pytest tests/ -v --cov=pw_gen --cov-report=term-missing
...
```python
import pytest
from pw_gen.core import PwGenCore
...
*工具編號 #302 ／ 類別 P — 安全與隱私 ／ 計畫版本 v1.0 ／ 2026-04-27*

...
> **任務分派路由器**
> 分類：F — Agent間通訊與協作 ｜ 編號：#103 ｜ 商用：✅

## 問題背景

**用戶痛點**：多 Agent 任務分派無策略
...
| 介面 | CLI（`click` 框架） |
| 設定檔 | `~/.config/agent-router/config.json` |
...
[project.scripts]
agent-router = "agent_router.cli:main"
```
...
```bash
agent-router [OPTIONS] COMMAND [ARGS]...
```
...
# 指定輸出格式
agent-router run --output json

# 詳細模式
agent-router run --verbose

# 靜音（適合 pipe 到其他工具）
agent-router run --quiet | jq .

# 查看設定
agent-router config show

# 匯出 JSON
agent-router export --output json > result.json
```
...
from .core import AgentRouterCore
...
def config():
    """顯示或修改設定"""
    import json, pathlib
    cfg_path = pathlib.Path.home() / ".config" / "agent-router" / "config.json"
    if cfg_path.exists():
...
DB_PATH = pathlib.Path.home() / ".config" / "agent-router" / "data.db"
...
{
  "name": "agent-router",
  "description": "任務分派路由器。解決問題：多 Agent 任務分派無策略",
  "input_schema": {
    "type": "object",
    "properties": {
      "action": {
...
```bash
# Agent 呼叫範例
result=$(agent-router run --quiet --output json)
echo $result | jq .data
```
...
def call_agent_router(action="run") -> dict:
    proc = subprocess.run(
        ["agent-router", action, "--output", "json", "--quiet"],
...
```bash
# 管線串接範例
agent-router run --quiet --output json \
  | jq -r '.data' \
  | other-tool process
```
...
# 執行測試
pytest tests/ -v --cov=agent_router --cov-report=term-missing

# 快速冒煙測試
agent-router run --help
agent-router config show
```
...
```python
import pytest
from agent_router.core import AgentRouterCore
...
```json
{
  ...
  "number": 103,
  "pain_point": "多 Agent 任務分派無策略",
  "commercial": "✅",
  "language": "Python",
  "entry_point": "agent-router",
  "dependencies": ["click", "rich", "pydantic"],
...
*工具編號 #103 ／ 類別 F — Agent間通訊與協作 ／ 計畫版本 v1.0 ／ 2026-04-27*

...
| 介面 | CLI（`click` 框架） |
| 設定檔 | `~/.config/api-sniff/config.json` |
...
```bash
api-sniff [OPTIONS] COMMAND [ARGS]...
```
...
# 指定輸出格式
api-sniff run --output json

# 詳細模式
api-sniff run --verbose

# 靜音（適合 pipe 到其他工具）
api-sniff run --quiet | jq .

# 查看設定
api-sniff config show

# 匯出 JSON
api-sniff export --output json > result.json
```
...
def config():
    """顯示或修改設定"""
    import json, pathlib
    cfg_path = pathlib.Path.home() / ".config" / "api-sniff" / "config.json"
    if cfg_path.exists():
...
DB_PATH = pathlib.Path.home() / ".config" / "api-sniff" / "data.db"
...
{
  "name": "api-sniff",
  "description": "API 規格嗅探器。解決問題：API 規格自動探測",
  "input_schema": {
    "type": "object",
    "properties": {
      "action": {
...
```bash
# Agent 呼叫範例
result=$(api-sniff run --quiet --output json)
echo $result | jq .data
```
...
def call_api_sniff(action="run") -> dict:
    proc = subprocess.run(
        ["api-sniff", action, "--output", "json", "--quiet"],
...
```bash
# 管線串接範例
api-sniff run --quiet --output json \
  | jq -r '.data' \
  | other-tool process
```
...
# 執行測試
pytest tests/ -v --cov=api_sniff --cov-report=term-missing
...
```python
import pytest
from api_sniff.core import ApiSniffCore
...
```json
{
  "name": "api-sniff",
  "version": "0.1.0",
  "description": "API 規格嗅探器",
  "category": "M",
  "category_name": "網路爬蟲與監控",
  "number": 259,
  "pain_point": "API 規格自動探測",
  "commercial": "⚠️",
  "language": "Python",
  "entry_point": "api-sniff",
  "dependencies": ["requests", "click", "rich"],
...
*工具編號 #259 ／ 類別 M — 網路爬蟲與監控 ／ 計畫版本 v1.0 ／ 2026-04-27*

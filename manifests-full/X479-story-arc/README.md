...
| 介面 | CLI（`click` 框架） |
| 設定檔 | `~/.config/story-arc/config.json` |
...
```bash
story-arc [OPTIONS] COMMAND [ARGS]...
```
...
# 指定輸出格式
story-arc run --output json

# 詳細模式
story-arc run --verbose

# 靜音（適合 pipe 到其他工具）
story-arc run --quiet | jq .

# 查看設定
story-arc config show

# 匯出 JSON
story-arc export --output json > result.json
```
...
def config():
    """顯示或修改設定"""
    import json, pathlib
    cfg_path = pathlib.Path.home() / ".config" / "story-arc" / "config.json"
    if cfg_path.exists():
...
DB_PATH = pathlib.Path.home() / ".config" / "story-arc" / "data.db"
...
```bash
# Agent 呼叫範例
result=$(story-arc run --quiet --output json)
echo $result | jq .data
```
...
def call_story_arc(action="run") -> dict:
    proc = subprocess.run(
        ["story-arc", action, "--output", "json", "--quiet"],
...
```bash
# 管線串接範例
story-arc run --quiet --output json \
  | jq -r '.data' \
  | other-tool process
```
...
# 執行測試
pytest tests/ -v --cov=story_arc --cov-report=term-missing
...
```python
import pytest
from story_arc.core import StoryArcCore
...
*工具編號 #479 ／ 類別 X — 寫作與內容創作 ／ 計畫版本 v1.0 ／ 2026-04-27*

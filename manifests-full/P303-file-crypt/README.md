...
| 介面 | CLI（`click` 框架） |
| 設定檔 | `~/.config/file-crypt/config.json` |
...
[project.scripts]
file-crypt = "file_crypt.cli:main"
```
...
```bash
file-crypt [OPTIONS] COMMAND [ARGS]...
```
...
# 指定輸出格式
file-crypt run --output json

# 詳細模式
file-crypt run --verbose

# 靜音（適合 pipe 到其他工具）
file-crypt run --quiet | jq .

# 查看設定
file-crypt config show

# 匯出 JSON
file-crypt export --output json > result.json
```
...
from .core import FileCryptCore
...
def config():
    """顯示或修改設定"""
    import json, pathlib
    cfg_path = pathlib.Path.home() / ".config" / "file-crypt" / "config.json"
    if cfg_path.exists():
...
DB_PATH = pathlib.Path.home() / ".config" / "file-crypt" / "data.db"
...
```bash
# Agent 呼叫範例
result=$(file-crypt run --quiet --output json)
echo $result | jq .data
```
...
def call_file_crypt(action="run") -> dict:
    proc = subprocess.run(
        ["file-crypt", action, "--output", "json", "--quiet"],
...
```bash
# 管線串接範例
file-crypt run --quiet --output json \
  | jq -r '.data' \
  | other-tool process
```
...
# 執行測試
pytest tests/ -v --cov=file_crypt --cov-report=term-missing
...
```python
import pytest
from file_crypt.core import FileCryptCore
...
*工具編號 #303 ／ 類別 P — 安全與隱私 ／ 計畫版本 v1.0 ／ 2026-04-27*

...
| 介面 | CLI（`click` 框架） |
| 設定檔 | `~/.config/quote-story/config.json` |
...
[project.scripts]
quote-story = "quote_story.cli:main"
```
...
```bash
quote-story [OPTIONS] COMMAND [ARGS]...
```
...
# 指定輸出格式
quote-story run --output json

# 詳細模式
quote-story run --verbose

# 靜音（適合 pipe 到其他工具）
quote-story run --quiet | jq .

# 查看設定
quote-story config show

# 匯出 JSON
quote-story export --output json > result.json
```
...
from .core import QuoteStoryCore
...
def config():
    """顯示或修改設定"""
    import json, pathlib
    cfg_path = pathlib.Path.home() / ".config" / "quote-story" / "config.json"
    if cfg_path.exists():
...
DB_PATH = pathlib.Path.home() / ".config" / "quote-story" / "data.db"
...
```bash
# Agent 呼叫範例
result=$(quote-story run --quiet --output json)
echo $result | jq .data
```
...
def call_quote_story(action="run") -> dict:
    proc = subprocess.run(
        ["quote-story", action, "--output", "json", "--quiet"],
...
```bash
# 管線串接範例
quote-story run --quiet --output json \
  | jq -r '.data' \
  | other-tool process
```
...
# 執行測試
pytest tests/ -v --cov=quote_story --cov-report=term-missing

# 快速冒煙測試
quote-story run --help
quote-story config show
```
...
```python
import pytest
from quote_story.core import QuoteStoryCore
...
*工具編號 #480 ／ 類別 X — 寫作與內容創作 ／ 計畫版本 v1.0 ／ 2026-04-27*

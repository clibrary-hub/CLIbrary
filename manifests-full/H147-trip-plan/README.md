...
| 介面 | CLI（`click` 框架） |
| 設定檔 | `~/.config/trip-plan/config.json` |
...
```bash
trip-plan [OPTIONS] COMMAND [ARGS]...
```
...
# 指定輸出格式
trip-plan run --output json

# 詳細模式
trip-plan run --verbose

# 靜音（適合 pipe 到其他工具）
trip-plan run --quiet | jq .

# 查看設定
trip-plan config show

# 匯出 JSON
trip-plan export --output json > result.json
```
...
def config():
    """顯示或修改設定"""
    import json, pathlib
    cfg_path = pathlib.Path.home() / ".config" / "trip-plan" / "config.json"
    if cfg_path.exists():
...
DB_PATH = pathlib.Path.home() / ".config" / "trip-plan" / "data.db"
...
{
  "name": "trip-plan",
  "description": "行程排程器。解決問題：旅行行程規劃難",
  "input_schema": {
    "type": "object",
    "properties": {
      "action": {
...
```bash
# Agent 呼叫範例
result=$(trip-plan run --quiet --output json)
echo $result | jq .data
```
...
def call_trip_plan(action="run") -> dict:
    proc = subprocess.run(
        ["trip-plan", action, "--output", "json", "--quiet"],
...
```bash
# 管線串接範例
trip-plan run --quiet --output json \
  | jq -r '.data' \
  | other-tool process
```
...
# 執行測試
pytest tests/ -v --cov=trip_plan --cov-report=term-missing
...
```python
import pytest
from trip_plan.core import TripPlanCore
...
*工具編號 #147 ／ 類別 H — 行事曆與時間管理 ／ 計畫版本 v1.0 ／ 2026-04-27*

...
| 介面 | CLI（`click` 框架） |
| 設定檔 | `~/.config/meditate/config.json` |
...
dependencies = ["rich", "click", "plyer"]

[project.scripts]
meditate = "meditate.cli:main"
```
...
# 指定輸出格式
meditate run --output json

# 詳細模式
meditate run --verbose

# 靜音（適合 pipe 到其他工具）
meditate run --quiet | jq .

# 查看設定
meditate config show

# 匯出 JSON
meditate export --output json > result.json
```
...
def config():
    """顯示或修改設定"""
    import json, pathlib
    cfg_path = pathlib.Path.home() / ".config" / "meditate" / "config.json"
    if cfg_path.exists():
...
DB_PATH = pathlib.Path.home() / ".config" / "meditate" / "data.db"
...
```bash
# Agent 呼叫範例
result=$(meditate run --quiet --output json)
echo $result | jq .data
```
...
def call_meditate(action="run") -> dict:
    proc = subprocess.run(
        ["meditate", action, "--output", "json", "--quiet"],
...
```bash
# 管線串接範例
meditate run --quiet --output json \
  | jq -r '.data' \
  | other-tool process
```
...
# 執行測試
pytest tests/ -v --cov=meditate --cov-report=term-missing
...
```python
import pytest
from meditate.core import MeditateCore
...
*工具編號 #214 ／ 類別 K — 健康與健身 ／ 計畫版本 v1.0 ／ 2026-04-27*

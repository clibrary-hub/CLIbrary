...
| 介面 | CLI（`click` 框架） |
| 設定檔 | `~/.config/wm-detect/config.json` |
...
```bash
wm-detect [OPTIONS] COMMAND [ARGS]...
```
...
# 指定輸出格式
wm-detect run --output json

# 詳細模式
wm-detect run --verbose

# 靜音（適合 pipe 到其他工具）
wm-detect run --quiet | jq .

# 查看設定
wm-detect config show

# 匯出 JSON
wm-detect export --output json > result.json
```
...
def config():
    """顯示或修改設定"""
    import json, pathlib
    cfg_path = pathlib.Path.home() / ".config" / "wm-detect" / "config.json"
    if cfg_path.exists():
...
DB_PATH = pathlib.Path.home() / ".config" / "wm-detect" / "data.db"
...
{
  "name": "wm-detect",
  "description": "浮水印偵測器。解決問題：隱形浮水印偵測",
  "input_schema": {
    "type": "object",
    "properties": {
      "action": {
...
```bash
# Agent 呼叫範例
result=$(wm-detect run --quiet --output json)
echo $result | jq .data
```
...
def call_wm_detect(action="run") -> dict:
    proc = subprocess.run(
        ["wm-detect", action, "--output", "json", "--quiet"],
...
```bash
# 管線串接範例
wm-detect run --quiet --output json \
  | jq -r '.data' \
  | other-tool process
```
...
# 執行測試
pytest tests/ -v --cov=wm_detect --cov-report=term-missing
...
```python
import pytest
from wm_detect.core import WmDetectCore
...
*工具編號 #316 ／ 類別 P — 安全與隱私 ／ 計畫版本 v1.0 ／ 2026-04-27*

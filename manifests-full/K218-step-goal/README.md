...
| 介面 | CLI（`click` 框架） |
| 設定檔 | `~/.config/step-goal/config.json` |
...
```bash
step-goal [OPTIONS] COMMAND [ARGS]...
```
...
# 指定輸出格式
step-goal run --output json

# 詳細模式
step-goal run --verbose

# 靜音（適合 pipe 到其他工具）
step-goal run --quiet | jq .

# 查看設定
step-goal config show

# 匯出 JSON
step-goal export --output json > result.json
```
...
def config():
    """顯示或修改設定"""
    import json, pathlib
    cfg_path = pathlib.Path.home() / ".config" / "step-goal" / "config.json"
    if cfg_path.exists():
...
DB_PATH = pathlib.Path.home() / ".config" / "step-goal" / "data.db"
...
{
  "name": "step-goal",
  "description": "步數目標追蹤。解決問題：步數目標追蹤",
  "input_schema": {
    "type": "object",
    "properties": {
      "action": {
...
```bash
# Agent 呼叫範例
result=$(step-goal run --quiet --output json)
echo $result | jq .data
```
...
def call_step_goal(action="run") -> dict:
    proc = subprocess.run(
        ["step-goal", action, "--output", "json", "--quiet"],
...
```bash
# 管線串接範例
step-goal run --quiet --output json \
  | jq -r '.data' \
  | other-tool process
```
...
# 執行測試
pytest tests/ -v --cov=step_goal --cov-report=term-missing
...
```python
import pytest
from step_goal.core import StepGoalCore
...
*工具編號 #218 ／ 類別 K — 健康與健身 ／ 計畫版本 v1.0 ／ 2026-04-27*

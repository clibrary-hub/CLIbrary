...
| 介面 | CLI（`click` 框架） |
| 設定檔 | `~/.config/restock/config.json` |
...
# 指定輸出格式
restock run --output json

# 詳細模式
restock run --verbose

# 靜音（適合 pipe 到其他工具）
restock run --quiet | jq .

# 查看設定
restock config show

# 匯出 JSON
restock export --output json > result.json
```
...
def config():
    """顯示或修改設定"""
    import json, pathlib
    cfg_path = pathlib.Path.home() / ".config" / "restock" / "config.json"
    if cfg_path.exists():
...
DB_PATH = pathlib.Path.home() / ".config" / "restock" / "data.db"
...
```bash
# Agent 呼叫範例
result=$(restock run --quiet --output json)
echo $result | jq .data
```
...
def call_restock(action="run") -> dict:
    proc = subprocess.run(
        ["restock", action, "--output", "json", "--quiet"],
...
# 執行測試
pytest tests/ -v --cov=restock --cov-report=term-missing
...
```python
import pytest
from restock.core import RestockCore
...
*工具編號 #409 ／ 類別 U — 購物與訂閱管理 ／ 計畫版本 v1.0 ／ 2026-04-27*

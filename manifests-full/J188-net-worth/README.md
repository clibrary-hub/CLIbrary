...
| 介面 | CLI（`click` 框架） |
| 設定檔 | `~/.config/net-worth/config.json` |
...
```bash
net-worth [OPTIONS] COMMAND [ARGS]...
```
...
# 指定輸出格式
net-worth run --output json

# 詳細模式
net-worth run --verbose

# 靜音（適合 pipe 到其他工具）
net-worth run --quiet | jq .

# 查看設定
net-worth config show

# 匯出 JSON
net-worth export --output json > result.json
```
...
def config():
    """顯示或修改設定"""
    import json, pathlib
    cfg_path = pathlib.Path.home() / ".config" / "net-worth" / "config.json"
    if cfg_path.exists():
...
DB_PATH = pathlib.Path.home() / ".config" / "net-worth" / "data.db"
...
{
  "name": "net-worth",
  "description": "淨值追蹤器。解決問題：個人資產淨值追蹤",
  "input_schema": {
    "type": "object",
    "properties": {
      "action": {
...
```bash
# Agent 呼叫範例
result=$(net-worth run --quiet --output json)
echo $result | jq .data
```
...
def call_net_worth(action="run") -> dict:
    proc = subprocess.run(
        ["net-worth", action, "--output", "json", "--quiet"],
...
```bash
# 管線串接範例
net-worth run --quiet --output json \
  | jq -r '.data' \
  | other-tool process
```
...
# 執行測試
pytest tests/ -v --cov=net_worth --cov-report=term-missing
...
```python
import pytest
from net_worth.core import NetWorthCore
...
*工具編號 #188 ／ 類別 J — 財務與帳務 ／ 計畫版本 v1.0 ／ 2026-04-27*

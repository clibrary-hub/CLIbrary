...
| 介面 | CLI（`click` 框架） |
| 設定檔 | `~/.config/paper-find/config.json` |
...
[project.scripts]
paper-find = "paper_find.cli:main"
```
...
```bash
paper-find [OPTIONS] COMMAND [ARGS]...
```
...
# 指定輸出格式
paper-find run --output json

# 詳細模式
paper-find run --verbose

# 靜音（適合 pipe 到其他工具）
paper-find run --quiet | jq .

# 查看設定
paper-find config show

# 匯出 JSON
paper-find export --output json > result.json
```
...
from .core import PaperFindCore
...
def config():
    """顯示或修改設定"""
    import json, pathlib
    cfg_path = pathlib.Path.home() / ".config" / "paper-find" / "config.json"
    if cfg_path.exists():
...
DB_PATH = pathlib.Path.home() / ".config" / "paper-find" / "data.db"
...
```bash
# Agent 呼叫範例
result=$(paper-find run --quiet --output json)
echo $result | jq .data
```
...
def call_paper_find(action="run") -> dict:
    proc = subprocess.run(
        ["paper-find", action, "--output", "json", "--quiet"],
...
```bash
# 管線串接範例
paper-find run --quiet --output json \
  | jq -r '.data' \
  | other-tool process
```
...
# 執行測試
pytest tests/ -v --cov=paper_find --cov-report=term-missing
...
```python
import pytest
from paper_find.core import PaperFindCore
...
*工具編號 #361 ／ 類別 S — 學習與研究 ／ 計畫版本 v1.0 ／ 2026-04-27*

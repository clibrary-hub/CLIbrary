...
| 介面 | CLI（`click` 框架） |
| 設定檔 | `~/.config/agent-card/config.json` |
...
[project.scripts]
agent-card = "agent_card.cli:main"
```
...
```bash
agent-card [OPTIONS] COMMAND [ARGS]...
```
...
# 指定輸出格式
agent-card run --output json

# 詳細模式
agent-card run --verbose

# 靜音（適合 pipe 到其他工具）
agent-card run --quiet | jq .

# 查看設定
agent-card config show

# 匯出 JSON
agent-card export --output json > result.json
```
...
from .core import AgentCardCore
...
def config():
    """顯示或修改設定"""
    import json, pathlib
    cfg_path = pathlib.Path.home() / ".config" / "agent-card" / "config.json"
    if cfg_path.exists():
...
DB_PATH = pathlib.Path.home() / ".config" / "agent-card" / "data.db"
...
{
  "name": "agent-card",
  "description": "Agent 能力名片。解決問題：Agent 能力無法廣告",
  "input_schema": {
    "type": "object",
    "properties": {
      "action": {
...
```bash
# Agent 呼叫範例
result=$(agent-card run --quiet --output json)
echo $result | jq .data
```
...
def call_agent_card(action="run") -> dict:
    proc = subprocess.run(
        ["agent-card", action, "--output", "json", "--quiet"],
...
```bash
# 管線串接範例
agent-card run --quiet --output json \
  | jq -r '.data' \
  | other-tool process
```
...
# 執行測試
pytest tests/ -v --cov=agent_card --cov-report=term-missing
...
```python
import pytest
from agent_card.core import AgentCardCore
...
```json
{
  "name": "agent-card",
  "version": "0.1.0",
  "description": "Agent 能力名片",
  "category": "F",
  "category_name": "Agent間通訊與協作",
  "number": 109,
  "pain_point": "Agent 能力無法廣告",
  "commercial": "✅",
  "language": "Python",
  "entry_point": "agent-card",
  "dependencies": ["click", "rich", "pydantic"],
...
*工具編號 #109 ／ 類別 F — Agent間通訊與協作 ／ 計畫版本 v1.0 ／ 2026-04-27*

...
| 介面 | CLI（`click` 框架） |
| 設定檔 | `~/.config/slot-find/config.json` |
...
```bash
slot-find [OPTIONS] COMMAND [ARGS]...
```
...
# 指定輸出格式
slot-find run --output json

# 詳細模式
slot-find run --verbose

# 靜音（適合 pipe 到其他工具）
slot-find run --quiet | jq .

# 查看設定
slot-find config show

# 匯出 JSON
slot-find export --output json > result.json
```
...
def config():
    """顯示或修改設定"""
    import json, pathlib
    cfg_path = pathlib.Path.home() / ".config" / "slot-find" / "config.json"
    if cfg_path.exists():
...
DB_PATH = pathlib.Path.home() / ".config" / "slot-find" / "data.db"
...
{
  "name": "slot-find",
  "description": "共同時段搜尋。解決問題：開會時段難找",
  "input_schema": {
    "type": "object",
    "properties": {
      "action": {
...
```bash
# Agent 呼叫範例
result=$(slot-find run --quiet --output json)
echo $result | jq .data
```
...
def call_slot_find(action="run") -> dict:
    proc = subprocess.run(
        ["slot-find", action, "--output", "json", "--quiet"],
...
```bash
# 管線串接範例
slot-find run --quiet --output json \
  | jq -r '.data' \
  | other-tool process
```
...
# 執行測試
pytest tests/ -v --cov=slot_find --cov-report=term-missing
...
```python
import pytest
from slot_find.core import SlotFindCore
...
*工具編號 #142 ／ 類別 H — 行事曆與時間管理 ／ 計畫版本 v1.0 ／ 2026-04-27*

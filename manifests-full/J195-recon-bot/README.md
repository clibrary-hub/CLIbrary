...
| 介面 | CLI（`click` 框架） |
| 設定檔 | `~/.config/recon-bot/config.json` |
...
```bash
recon-bot [OPTIONS] COMMAND [ARGS]...
```
...
# 指定輸出格式
recon-bot run --output json

# 詳細模式
recon-bot run --verbose

# 靜音（適合 pipe 到其他工具）
recon-bot run --quiet | jq .

# 查看設定
recon-bot config show

# 匯出 JSON
recon-bot export --output json > result.json
```
...
def config():
    """顯示或修改設定"""
    import json, pathlib
    cfg_path = pathlib.Path.home() / ".config" / "recon-bot" / "config.json"
    if cfg_path.exists():
...
DB_PATH = pathlib.Path.home() / ".config" / "recon-bot" / "data.db"
...
```bash
# Agent 呼叫範例
result=$(recon-bot run --quiet --output json)
echo $result | jq .data
```
...
def call_recon_bot(action="run") -> dict:
    proc = subprocess.run(
        ["recon-bot", action, "--output", "json", "--quiet"],
...
```bash
# 管線串接範例
recon-bot run --quiet --output json \
  | jq -r '.data' \
  | other-tool process
```
...
# 執行測試
pytest tests/ -v --cov=recon_bot --cov-report=term-missing
...
```python
import pytest
from recon_bot.core import ReconBotCore
...
*工具編號 #195 ／ 類別 J — 財務與帳務 ／ 計畫版本 v1.0 ／ 2026-04-27*

...
| 介面 | CLI（`click` 框架） |
| 設定檔 | `~/.config/prompt-eval/config.json` |
...
[project.scripts]
prompt-eval = "prompt_eval.cli:main"
```
...
# 指定輸出格式
prompt-eval run --output json

# 詳細模式
prompt-eval run --verbose

# 靜音（適合 pipe 到其他工具）
prompt-eval run --quiet | jq .

# 查看設定
prompt-eval config show

# 匯出 JSON
prompt-eval export --output json > result.json
```
...
from .core import PromptEvalCore
...
def config():
    """顯示或修改設定"""
    import json, pathlib
    cfg_path = pathlib.Path.home() / ".config" / "prompt-eval" / "config.json"
    if cfg_path.exists():
...
DB_PATH = pathlib.Path.home() / ".config" / "prompt-eval" / "data.db"
...
{
  "name": "prompt-eval",
  "description": "Prompt 評測器。解決問題：Prompt 黃金測試",
  "input_schema": {
    "type": "object",
    "properties": {
      "action": {
...
```bash
# Agent 呼叫範例
result=$(prompt-eval run --quiet --output json)
echo $result | jq .data
```
...
def call_prompt_eval(action="run") -> dict:
    proc = subprocess.run(
        ["prompt-eval", action, "--output", "json", "--quiet"],
...
```bash
# 管線串接範例
prompt-eval run --quiet --output json \
  | jq -r '.data' \
  | other-tool process
```
...
# 執行測試
pytest tests/ -v --cov=prompt_eval --cov-report=term-missing

# 快速冒煙測試
prompt-eval run --help
prompt-eval config show
```
...
```python
import pytest
from prompt_eval.core import PromptEvalCore
...
```json
{
  "name": "prompt-eval",
  "version": "0.1.0",
  "description": "Prompt 評測器",
  "category": "O",
  "category_name": "AI/ML輔助",
  "number": 285,
  "pain_point": "Prompt 黃金測試",
  "commercial": "✅",
  "language": "Python",
  "entry_point": "prompt-eval",
  "dependencies": ["anthropic", "click", "rich"],
...
*工具編號 #285 ／ 類別 O — AI/ML輔助 ／ 計畫版本 v1.0 ／ 2026-04-27*

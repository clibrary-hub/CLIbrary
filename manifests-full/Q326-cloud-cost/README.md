...
| 介面 | CLI（`click` 框架） |
| 設定檔 | `~/.config/cloud-cost/config.json` |
...
[project.scripts]
cloud-cost = "cloud_cost.cli:main"
```
...
```bash
cloud-cost [OPTIONS] COMMAND [ARGS]...
```
...
# 指定輸出格式
cloud-cost run --output json

# 詳細模式
cloud-cost run --verbose

# 靜音（適合 pipe 到其他工具）
cloud-cost run --quiet | jq .

# 查看設定
cloud-cost config show

# 匯出 JSON
cloud-cost export --output json > result.json
```
...
from .core import CloudCostCore
...
def config():
    """顯示或修改設定"""
    import json, pathlib
    cfg_path = pathlib.Path.home() / ".config" / "cloud-cost" / "config.json"
    if cfg_path.exists():
...
DB_PATH = pathlib.Path.home() / ".config" / "cloud-cost" / "data.db"
...
{
  "name": "cloud-cost",
  "description": "雲端花費守門員。解決問題：雲花費異常告警",
  "input_schema": {
    "type": "object",
    "properties": {
      "action": {
...
```bash
# Agent 呼叫範例
result=$(cloud-cost run --quiet --output json)
echo $result | jq .data
```
...
def call_cloud_cost(action="run") -> dict:
    proc = subprocess.run(
        ["cloud-cost", action, "--output", "json", "--quiet"],
...
```bash
# 管線串接範例
cloud-cost run --quiet --output json \
  | jq -r '.data' \
  | other-tool process
```
...
# 執行測試
pytest tests/ -v --cov=cloud_cost --cov-report=term-missing
...
```python
import pytest
from cloud_cost.core import CloudCostCore
...
*工具編號 #326 ／ 類別 Q — DevOps與基礎設施 ／ 計畫版本 v1.0 ／ 2026-04-27*

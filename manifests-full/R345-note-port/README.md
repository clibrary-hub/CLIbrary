...
| 介面 | CLI（`click` 框架） |
| 設定檔 | `~/.config/note-port/config.json` |
...
```bash
note-port [OPTIONS] COMMAND [ARGS]...
```
...
# 指定輸出格式
note-port run --output json

# 詳細模式
note-port run --verbose

# 靜音（適合 pipe 到其他工具）
note-port run --quiet | jq .

# 查看設定
note-port config show

# 匯出 JSON
note-port export --output json > result.json
```
...
def config():
    """顯示或修改設定"""
    import json, pathlib
    cfg_path = pathlib.Path.home() / ".config" / "note-port" / "config.json"
    if cfg_path.exists():
...
DB_PATH = pathlib.Path.home() / ".config" / "note-port" / "data.db"
...
{
  "name": "note-port",
  "description": "筆記搬遷器。解決問題：不同筆記應用搬遷",
  "input_schema": {
    "type": "object",
    "properties": {
      "action": {
...
```bash
# Agent 呼叫範例
result=$(note-port run --quiet --output json)
echo $result | jq .data
```
...
def call_note_port(action="run") -> dict:
    proc = subprocess.run(
        ["note-port", action, "--output", "json", "--quiet"],
...
```bash
# 管線串接範例
note-port run --quiet --output json \
  | jq -r '.data' \
  | other-tool process
```
...
# 執行測試
pytest tests/ -v --cov=note_port --cov-report=term-missing
...
```python
import pytest
from note_port.core import NotePortCore
...
*工具編號 #345 ／ 類別 R — 知識管理與筆記 ／ 計畫版本 v1.0 ／ 2026-04-27*

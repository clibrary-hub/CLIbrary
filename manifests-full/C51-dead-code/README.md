...
| 介面 | CLI（`click` 框架） |
| 設定檔 | `~/.config/dead-code/config.json` |
...
pip install vulture click rich
...
dependencies = ["vulture", "click", "rich"]

[project.scripts]
dead-code = "dead_code.cli:main"
```
...
```bash
dead-code [OPTIONS] COMMAND [ARGS]...
```
...
# 指定輸出格式
dead-code run --output json

# 詳細模式
dead-code run --verbose

# 靜音（適合 pipe 到其他工具）
dead-code run --quiet | jq .

# 查看設定
dead-code config show

# 匯出 JSON
dead-code export --output json > result.json
```
...
def config():
    """顯示或修改設定"""
    import json, pathlib
    cfg_path = pathlib.Path.home() / ".config" / "dead-code" / "config.json"
    if cfg_path.exists():
...
DB_PATH = pathlib.Path.home() / ".config" / "dead-code" / "data.db"
...
```bash
# Agent 呼叫範例
result=$(dead-code run --quiet --output json)
echo $result | jq .data
```
...
def call_dead_code(action="run") -> dict:
    proc = subprocess.run(
        ["dead-code", action, "--output", "json", "--quiet"],
...
```bash
# 管線串接範例
dead-code run --quiet --output json \
  | jq -r '.data' \
  | other-tool process
```
...
# 執行測試
pytest tests/ -v --cov=dead_code --cov-report=term-missing
...
```python
import pytest
from dead_code.core import DeadCodeCore
...
*工具編號 #051 ／ 類別 C — 程式碼品質與開發輔助 ／ 計畫版本 v1.0 ／ 2026-04-27*

...
| 介面 | CLI（`click` 框架） |
| 設定檔 | `~/.config/cart-nudge/config.json` |
...
[project.scripts]
cart-nudge = "cart_nudge.cli:main"
```
...
```bash
cart-nudge [OPTIONS] COMMAND [ARGS]...
```
...
# 指定輸出格式
cart-nudge run --output json

# 詳細模式
cart-nudge run --verbose

# 靜音（適合 pipe 到其他工具）
cart-nudge run --quiet | jq .

# 查看設定
cart-nudge config show

# 匯出 JSON
cart-nudge export --output json > result.json
```
...
from .core import CartNudgeCore
...
def config():
    """顯示或修改設定"""
    import json, pathlib
    cfg_path = pathlib.Path.home() / ".config" / "cart-nudge" / "config.json"
    if cfg_path.exists():
...
DB_PATH = pathlib.Path.home() / ".config" / "cart-nudge" / "data.db"
...
{
  "name": "cart-nudge",
  "description": "結帳提醒器。解決問題：購物車提醒結帳",
  "input_schema": {
    "type": "object",
    "properties": {
      "action": {
...
```bash
# Agent 呼叫範例
result=$(cart-nudge run --quiet --output json)
echo $result | jq .data
```
...
def call_cart_nudge(action="run") -> dict:
    proc = subprocess.run(
        ["cart-nudge", action, "--output", "json", "--quiet"],
...
```bash
# 管線串接範例
cart-nudge run --quiet --output json \
  | jq -r '.data' \
  | other-tool process
```
...
# 執行測試
pytest tests/ -v --cov=cart_nudge --cov-report=term-missing
...
```python
import pytest
from cart_nudge.core import CartNudgeCore
...
*工具編號 #411 ／ 類別 U — 購物與訂閱管理 ／ 計畫版本 v1.0 ／ 2026-04-27*

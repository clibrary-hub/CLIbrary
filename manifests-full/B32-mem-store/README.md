...
| 介面 | CLI（`click` 框架） |
| 設定檔 | `~/.config/mem-store/config.json` |
...
```bash
mem-store [OPTIONS] COMMAND [ARGS]...
```
...
# 指定輸出格式
mem-store run --output json

# 詳細模式
mem-store run --verbose

# 靜音（適合 pipe 到其他工具）
mem-store run --quiet | jq .

# 查看設定
mem-store config show

# 匯出 JSON
mem-store export --output json > result.json
```
...
def config():
    """顯示或修改設定"""
    import json, pathlib
    cfg_path = pathlib.Path.home() / ".config" / "mem-store" / "config.json"
    if cfg_path.exists():
...
DB_PATH = pathlib.Path.home() / ".config" / "mem-store" / "data.db"
...
{
  "name": "mem-store",
  "description": "本地長期記憶庫。解決問題：無長期記憶儲存",
  "input_schema": {
    "type": "object",
    "properties": {
      "action": {
...
```bash
# Agent 呼叫範例
result=$(mem-store run --quiet --output json)
echo $result | jq .data
```
...
def call_mem_store(action="run") -> dict:
    proc = subprocess.run(
        ["mem-store", action, "--output", "json", "--quiet"],
...
```bash
# 管線串接範例
mem-store run --quiet --output json \
  | jq -r '.data' \
  | other-tool process
```
...
# 執行測試
pytest tests/ -v --cov=mem_store --cov-report=term-missing
...
```python
import pytest
from mem_store.core import MemStoreCore
...
*工具編號 #032 ／ 類別 B — 對話與記憶管理 ／ 計畫版本 v1.0 ／ 2026-04-27*

...
| 介面 | CLI（`click` 框架） |
| 設定檔 | `~/.config/combo-opt/config.json` |
...
pip install click rich itertools
...
dependencies = ["click", "rich", "itertools"]

[project.scripts]
combo-opt = "combo_opt.cli:main"
```
...
```bash
combo-opt [OPTIONS] COMMAND [ARGS]...
```
...
# 指定輸出格式
combo-opt run --output json

# 詳細模式
combo-opt run --verbose

# 靜音（適合 pipe 到其他工具）
combo-opt run --quiet | jq .

# 查看設定
combo-opt config show

# 匯出 JSON
combo-opt export --output json > result.json
```
...
def config():
    """顯示或修改設定"""
    import json, pathlib
    cfg_path = pathlib.Path.home() / ".config" / "combo-opt" / "config.json"
    if cfg_path.exists():
...
DB_PATH = pathlib.Path.home() / ".config" / "combo-opt" / "data.db"
...
{
  "name": "combo-opt",
  "description": "換購最佳化器。解決問題：換購最佳組合",
  "input_schema": {
    "type": "object",
    "properties": {
      "action": {
...
```bash
# Agent 呼叫範例
result=$(combo-opt run --quiet --output json)
echo $result | jq .data
```
...
def call_combo_opt(action="run") -> dict:
    proc = subprocess.run(
        ["combo-opt", action, "--output", "json", "--quiet"],
...
```bash
# 管線串接範例
combo-opt run --quiet --output json \
  | jq -r '.data' \
  | other-tool process
```
...
# 執行測試
pytest tests/ -v --cov=combo_opt --cov-report=term-missing
...
```python
import pytest
from combo_opt.core import ComboOptCore
...
*工具編號 #417 ／ 類別 U — 購物與訂閱管理 ／ 計畫版本 v1.0 ／ 2026-04-27*

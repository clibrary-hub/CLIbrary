...
| 介面 | CLI（`click` 框架） |
| 設定檔 | `~/.config/cite-graph/config.json` |
...
pip install networkx requests click
...
dependencies = ["networkx", "requests", "click"]

[project.scripts]
cite-graph = "cite_graph.cli:main"
```
...
```bash
cite-graph [OPTIONS] COMMAND [ARGS]...
```
...
# 指定輸出格式
cite-graph run --output json

# 詳細模式
cite-graph run --verbose

# 靜音（適合 pipe 到其他工具）
cite-graph run --quiet | jq .

# 查看設定
cite-graph config show

# 匯出 JSON
cite-graph export --output json > result.json
```
...
from .core import CiteGraphCore
...
def config():
    """顯示或修改設定"""
    import json, pathlib
    cfg_path = pathlib.Path.home() / ".config" / "cite-graph" / "config.json"
    if cfg_path.exists():
...
DB_PATH = pathlib.Path.home() / ".config" / "cite-graph" / "data.db"
...
class CiteGraphCore:
    ...
    核心邏輯類別。
    問題：文獻引用網絡
    解法：透過 networkx, requests 實現自動化處理。
    """
...
```bash
# Agent 呼叫範例
result=$(cite-graph run --quiet --output json)
echo $result | jq .data
```
...
def call_cite_graph(action="run") -> dict:
    proc = subprocess.run(
        ["cite-graph", action, "--output", "json", "--quiet"],
...
```bash
# 管線串接範例
cite-graph run --quiet --output json \
  | jq -r '.data' \
  | other-tool process
```
...
# 執行測試
pytest tests/ -v --cov=cite_graph --cov-report=term-missing
...
```python
import pytest
from cite_graph.core import CiteGraphCore
...
*工具編號 #363 ／ 類別 S — 學習與研究 ／ 計畫版本 v1.0 ／ 2026-04-27*

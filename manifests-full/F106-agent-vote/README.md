...
| 介面 | CLI（`click` 框架） |
| 設定檔 | `~/.config/agent-vote/config.json` |
...
[project.scripts]
agent-vote = "agent_vote.cli:main"
```
...
```bash
agent-vote [OPTIONS] COMMAND [ARGS]...
```
...
# 指定輸出格式
agent-vote run --output json

# 詳細模式
agent-vote run --verbose

# 靜音（適合 pipe 到其他工具）
agent-vote run --quiet | jq .

# 查看設定
agent-vote config show

# 匯出 JSON
agent-vote export --output json > result.json
```
...
from .core import AgentVoteCore
...
def config():
    """顯示或修改設定"""
    import json, pathlib
    cfg_path = pathlib.Path.home() / ".config" / "agent-vote" / "config.json"
    if cfg_path.exists():
...
DB_PATH = pathlib.Path.home() / ".config" / "agent-vote" / "data.db"
...
{
  "name": "agent-vote",
  "description": "Agent 投票決策器。解決問題：Agent 共識難達成",
  "input_schema": {
    "type": "object",
    "properties": {
      "action": {
...
```bash
# Agent 呼叫範例
result=$(agent-vote run --quiet --output json)
echo $result | jq .data
```
...
def call_agent_vote(action="run") -> dict:
    proc = subprocess.run(
        ["agent-vote", action, "--output", "json", "--quiet"],
...
```bash
# 管線串接範例
agent-vote run --quiet --output json \
  | jq -r '.data' \
  | other-tool process
```
...
# 執行測試
pytest tests/ -v --cov=agent_vote --cov-report=term-missing
...
```python
import pytest
from agent_vote.core import AgentVoteCore
...
```json
{
  "name": "agent-vote",
  "version": "0.1.0",
  "description": "Agent 投票決策器",
  "category": "F",
  "category_name": "Agent間通訊與協作",
  "number": 106,
  "pain_point": "Agent 共識難達成",
  "commercial": "✅",
  "language": "Python",
  "entry_point": "agent-vote",
  "dependencies": ["click", "rich", "pydantic"],
...
*工具編號 #106 ／ 類別 F — Agent間通訊與協作 ／ 計畫版本 v1.0 ／ 2026-04-27*

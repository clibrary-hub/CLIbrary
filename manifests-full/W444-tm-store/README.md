...
| 介面 | CLI（`click` 框架） |
| 設定檔 | `~/.config/tm-store/config.json` |
...
# 指定輸出格式
tm-store run --output json

# 詳細模式
tm-store run --verbose

# 靜音（適合 pipe 到其他工具）
tm-store run --quiet | jq .

# 查看設定
tm-store config show

# 匯出 JSON
tm-store export --output json > result.json
```
...
def config():
    """顯示或修改設定"""
    import json, pathlib
    cfg_path = pathlib.Path.home() / ".config" / "tm-store" / "config.json"
    if cfg_path.exists():
...
DB_PATH = pathlib.Path.home() / ".config" / "tm-store" / "data.db"
...
{
  "name": "tm-store",
  "description": "翻譯記憶庫。解決問題：翻譯記憶庫 TM",
  "input_schema": {
    "type": "object",
    "properties": {
      "action": {
...
```bash
# Agent 呼叫範例
result=$(tm-store run --quiet --output json)
echo $result | jq .data
```
...
def call_tm_store(action="run") -> dict:
    proc = subprocess.run(
        ["tm-store", action, "--output", "json", "--quiet"],
...
```bash
# 管線串接範例
tm-store run --quiet --output json \
  | jq -r '.data' \
  | other-tool process
```
...
# 執行測試
pytest tests/ -v --cov=tm_store --cov-report=term-missing
...
```python
import pytest
from tm_store.core import TmStoreCore
...
*工具編號 #444 ／ 類別 W — 翻譯與在地化 ／ 計畫版本 v1.0 ／ 2026-04-27*

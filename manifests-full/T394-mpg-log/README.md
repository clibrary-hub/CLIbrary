...
| 介面 | CLI（`click` 框架） |
| 設定檔 | `~/.config/mpg-log/config.json` |
...
# 指定輸出格式
mpg-log run --output json

# 詳細模式
mpg-log run --verbose

# 靜音（適合 pipe 到其他工具）
mpg-log run --quiet | jq .

# 查看設定
mpg-log config show

# 匯出 JSON
mpg-log export --output json > result.json
```
...
def config():
    """顯示或修改設定"""
    import json, pathlib
    cfg_path = pathlib.Path.home() / ".config" / "mpg-log" / "config.json"
    if cfg_path.exists():
...
DB_PATH = pathlib.Path.home() / ".config" / "mpg-log" / "data.db"
...
```bash
# Agent 呼叫範例
result=$(mpg-log run --quiet --output json)
echo $result | jq .data
```
...
def call_mpg_log(action="run") -> dict:
    proc = subprocess.run(
        ["mpg-log", action, "--output", "json", "--quiet"],
...
# 執行測試
pytest tests/ -v --cov=mpg_log --cov-report=term-missing
...
```python
import pytest
from mpg_log.core import MpgLogCore
...
*工具編號 #394 ／ 類別 T — 旅遊與生活 ／ 計畫版本 v1.0 ／ 2026-04-27*

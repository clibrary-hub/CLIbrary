...
| 介面 | CLI（`click` 框架） |
| 設定檔 | `~/.config/hook-gen/config.json` |
...
# 指定輸出格式
hook-gen run --output json

# 詳細模式
hook-gen run --verbose

# 靜音（適合 pipe 到其他工具）
hook-gen run --quiet | jq .

# 查看設定
hook-gen config show

# 匯出 JSON
hook-gen export --output json > result.json
```
...
def config():
    """顯示或修改設定"""
    import json, pathlib
    cfg_path = pathlib.Path.home() / ".config" / "hook-gen" / "config.json"
    if cfg_path.exists():
...
DB_PATH = pathlib.Path.home() / ".config" / "hook-gen" / "data.db"
...
{
  "name": "hook-gen",
  "description": "開頭吊鉤產生器。解決問題：文章引導開頭",
  "input_schema": {
    "type": "object",
    "properties": {
      "action": {
...
```bash
# Agent 呼叫範例
result=$(hook-gen run --quiet --output json)
echo $result | jq .data
```
...
def call_hook_gen(action="run") -> dict:
    proc = subprocess.run(
        ["hook-gen", action, "--output", "json", "--quiet"],
...
```bash
# 管線串接範例
hook-gen run --quiet --output json \
  | jq -r '.data' \
  | other-tool process
```
...
# 執行測試
pytest tests/ -v --cov=hook_gen --cov-report=term-missing
...
```python
import pytest
from hook_gen.core import HookGenCore
...
*工具編號 #464 ／ 類別 X — 寫作與內容創作 ／ 計畫版本 v1.0 ／ 2026-04-27*

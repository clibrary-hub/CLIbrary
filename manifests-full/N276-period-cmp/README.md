...
| 介面 | CLI（`click` 框架） |
| 設定檔 | `~/.config/period-cmp/config.json` |
...
[project.scripts]
period-cmp = "period_cmp.cli:main"
```
...
```bash
period-cmp [OPTIONS] COMMAND [ARGS]...
```
...
# 指定輸出格式
period-cmp run --output json

# 詳細模式
period-cmp run --verbose

# 靜音（適合 pipe 到其他工具）
period-cmp run --quiet | jq .

# 查看設定
period-cmp config show

# 匯出 JSON
period-cmp export --output json > result.json
```
...
from .core import PeriodCmpCore
...
def config():
    """顯示或修改設定"""
    import json, pathlib
    cfg_path = pathlib.Path.home() / ".config" / "period-cmp" / "config.json"
    if cfg_path.exists():
...
DB_PATH = pathlib.Path.home() / ".config" / "period-cmp" / "data.db"
...
{
  "name": "period-cmp",
  "description": "期比計算器。解決問題：同期比/環比計算",
  "input_schema": {
    "type": "object",
    "properties": {
      "action": {
...
```bash
# Agent 呼叫範例
result=$(period-cmp run --quiet --output json)
echo $result | jq .data
```
...
def call_period_cmp(action="run") -> dict:
    proc = subprocess.run(
        ["period-cmp", action, "--output", "json", "--quiet"],
...
```bash
# 管線串接範例
period-cmp run --quiet --output json \
  | jq -r '.data' \
  | other-tool process
```
...
# 執行測試
pytest tests/ -v --cov=period_cmp --cov-report=term-missing
...
```python
import pytest
from period_cmp.core import PeriodCmpCore
...
*工具編號 #276 ／ 類別 N — 資料分析與視覺化 ／ 計畫版本 v1.0 ／ 2026-04-27*

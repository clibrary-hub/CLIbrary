...
| 介面 | CLI（`click` 框架） |
| 設定檔 | `~/.config/expense-merge/config.json` |
...
[project.scripts]
expense-merge = "expense_merge.cli:main"
```
...
```bash
expense-merge [OPTIONS] COMMAND [ARGS]...
```
...
# 指定輸出格式
expense-merge run --output json

# 詳細模式
expense-merge run --verbose

# 靜音（適合 pipe 到其他工具）
expense-merge run --quiet | jq .

# 查看設定
expense-merge config show

# 匯出 JSON
expense-merge export --output json > result.json
```
...
### `expense_merge/cli.py`

```python
import click
from rich.console import Console
from .core import ExpenseMergeCore
...
def config():
    """顯示或修改設定"""
    import json, pathlib
    cfg_path = pathlib.Path.home() / ".config" / "expense-merge" / "config.json"
    if cfg_path.exists():
...
DB_PATH = pathlib.Path.home() / ".config" / "expense-merge" / "data.db"
...
{
  "name": "expense-merge",
  "description": "跨帳戶記帳。解決問題：多帳戶支出難整合",
  "input_schema": {
    "type": "object",
    "properties": {
      "action": {
...
```bash
# Agent 呼叫範例
result=$(expense-merge run --quiet --output json)
echo $result | jq .data
```
...
def call_expense_merge(action="run") -> dict:
    proc = subprocess.run(
        ["expense-merge", action, "--output", "json", "--quiet"],
...
```bash
# 管線串接範例
expense-merge run --quiet --output json \
  | jq -r '.data' \
  | other-tool process
```
...
# 執行測試
pytest tests/ -v --cov=expense_merge --cov-report=term-missing

# 快速冒煙測試
expense-merge run --help
expense-merge config show
```
...
```python
import pytest
from expense_merge.core import ExpenseMergeCore
...
*工具編號 #181 ／ 類別 J — 財務與帳務 ／ 計畫版本 v1.0 ／ 2026-04-27*

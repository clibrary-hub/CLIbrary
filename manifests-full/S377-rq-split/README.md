...
| 介面 | CLI（`click` 框架） |
| 設定檔 | `~/.config/rq-split/config.json` |
...
# 指定輸出格式
rq-split run --output json

# 詳細模式
rq-split run --verbose

# 靜音（適合 pipe 到其他工具）
rq-split run --quiet | jq .

# 查看設定
rq-split config show

# 匯出 JSON
rq-split export --output json > result.json
```
...
def config():
    """顯示或修改設定"""
    import json, pathlib
    cfg_path = pathlib.Path.home() / ".config" / "rq-split" / "config.json"
    if cfg_path.exists():
...
DB_PATH = pathlib.Path.home() / ".config" / "rq-split" / "data.db"
...
```bash
# Agent 呼叫範例
result=$(rq-split run --quiet --output json)
echo $result | jq .data
```
...
def call_rq_split(action="run") -> dict:
    proc = subprocess.run(
        ["rq-split", action, "--output", "json", "--quiet"],
...
```bash
# 管線串接範例
rq-split run --quiet --output json \
  | jq -r '.data' \
  | other-tool process
```
...
# 執行測試
pytest tests/ -v --cov=rq_split --cov-report=term-missing
...
```python
import pytest
from rq_split.core import RqSplitCore
...
*工具編號 #377 ／ 類別 S — 學習與研究 ／ 計畫版本 v1.0 ／ 2026-04-27*

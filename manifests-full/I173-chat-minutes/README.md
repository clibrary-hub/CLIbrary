...
| 介面 | CLI（`click` 框架） |
| 設定檔 | `~/.config/chat-minutes/config.json` |
...
[project.scripts]
chat-minutes = "chat_minutes.cli:main"
```
...
```bash
chat-minutes [OPTIONS] COMMAND [ARGS]...
```
...
# 指定輸出格式
chat-minutes run --output json

# 詳細模式
chat-minutes run --verbose

# 靜音（適合 pipe 到其他工具）
chat-minutes run --quiet | jq .

# 查看設定
chat-minutes config show

# 匯出 JSON
chat-minutes export --output json > result.json
```
...
from .core import ChatMinutesCore
...
def config():
    """顯示或修改設定"""
    import json, pathlib
    cfg_path = pathlib.Path.home() / ".config" / "chat-minutes" / "config.json"
    if cfg_path.exists():
...
DB_PATH = pathlib.Path.home() / ".config" / "chat-minutes" / "data.db"
...
{
  "name": "chat-minutes",
  "description": "IM 會議紀錄。解決問題：群組事件要寫紀錄",
  "input_schema": {
    "type": "object",
    "properties": {
      "action": {
...
```bash
# Agent 呼叫範例
result=$(chat-minutes run --quiet --output json)
echo $result | jq .data
```
...
def call_chat_minutes(action="run") -> dict:
    proc = subprocess.run(
        ["chat-minutes", action, "--output", "json", "--quiet"],
...
```bash
# 管線串接範例
chat-minutes run --quiet --output json \
  | jq -r '.data' \
  | other-tool process
```
...
# 執行測試
pytest tests/ -v --cov=chat_minutes --cov-report=term-missing

# 快速冒煙測試
chat-minutes run --help
chat-minutes config show
```
...
```python
import pytest
from chat_minutes.core import ChatMinutesCore
...
*工具編號 #173 ／ 類別 I — 郵件與通訊 ／ 計畫版本 v1.0 ／ 2026-04-27*

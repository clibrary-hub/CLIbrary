...
| 介面 | CLI（`click` 框架） |
| 設定檔 | `~/.config/seo-audit/config.json` |
...
```bash
seo-audit [OPTIONS] COMMAND [ARGS]...
```
...
# 指定輸出格式
seo-audit run --output json

# 詳細模式
seo-audit run --verbose

# 靜音（適合 pipe 到其他工具）
seo-audit run --quiet | jq .

# 查看設定
seo-audit config show

# 匯出 JSON
seo-audit export --output json > result.json
```
...
def config():
    """顯示或修改設定"""
    import json, pathlib
    cfg_path = pathlib.Path.home() / ".config" / "seo-audit" / "config.json"
    if cfg_path.exists():
...
DB_PATH = pathlib.Path.home() / ".config" / "seo-audit" / "data.db"
...
{
  "name": "seo-audit",
  "description": "SEO 稽核器。解決問題：SEO 概況檢查",
  "input_schema": {
    "type": "object",
    "properties": {
      "action": {
...
```bash
# Agent 呼叫範例
result=$(seo-audit run --quiet --output json)
echo $result | jq .data
```
...
def call_seo_audit(action="run") -> dict:
    proc = subprocess.run(
        ["seo-audit", action, "--output", "json", "--quiet"],
...
```bash
# 管線串接範例
seo-audit run --quiet --output json \
  | jq -r '.data' \
  | other-tool process
```
...
# 執行測試
pytest tests/ -v --cov=seo_audit --cov-report=term-missing
...
```python
import pytest
from seo_audit.core import SeoAuditCore
...
*工具編號 #251 ／ 類別 M — 網路爬蟲與監控 ／ 計畫版本 v1.0 ／ 2026-04-27*

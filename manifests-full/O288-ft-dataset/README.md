...
| 介面 | CLI（`click` 框架） |
| 設定檔 | `~/.config/ft-dataset/config.json` |
...
[project.scripts]
ft-dataset = "ft_dataset.cli:main"
```
...
```bash
ft-dataset [OPTIONS] COMMAND [ARGS]...
```
...
# 指定輸出格式
ft-dataset run --output json

# 詳細模式
ft-dataset run --verbose

# 靜音（適合 pipe 到其他工具）
ft-dataset run --quiet | jq .

# 查看設定
ft-dataset config show

# 匯出 JSON
ft-dataset export --output json > result.json
```
...
from .core import FtDatasetCore
...
def config():
    """顯示或修改設定"""
    import json, pathlib
    cfg_path = pathlib.Path.home() / ".config" / "ft-dataset" / "config.json"
    if cfg_path.exists():
...
DB_PATH = pathlib.Path.home() / ".config" / "ft-dataset" / "data.db"
...
{
  "name": "ft-dataset",
  "description": "微調資料器。解決問題：模型微調資料整理",
  "input_schema": {
    "type": "object",
    "properties": {
      "action": {
...
```bash
# Agent 呼叫範例
result=$(ft-dataset run --quiet --output json)
echo $result | jq .data
```
...
def call_ft_dataset(action="run") -> dict:
    proc = subprocess.run(
        ["ft-dataset", action, "--output", "json", "--quiet"],
...
```bash
# 管線串接範例
ft-dataset run --quiet --output json \
  | jq -r '.data' \
  | other-tool process
```
...
# 執行測試
pytest tests/ -v --cov=ft_dataset --cov-report=term-missing
...
```python
import pytest
from ft_dataset.core import FtDatasetCore
...
*工具編號 #288 ／ 類別 O — AI/ML輔助 ／ 計畫版本 v1.0 ／ 2026-04-27*

...
> **預演模式包裝器**
> 分類：D — 自動化與工作流程 ｜ 編號：#079 ｜ 商用：✅

## 問題背景

**用戶痛點**：流程缺少 dry-run 模式
...
| 介面 | CLI（`click` 框架） |
| 設定檔 | `~/.config/dry-run/config.json` |
...
# 指定輸出格式
dry-run run --output json

# 詳細模式
dry-run run --verbose

# 靜音（適合 pipe 到其他工具）
dry-run run --quiet | jq .

# 查看設定
dry-run config show

# 匯出 JSON
dry-run export --output json > result.json
```
...
def config():
    """顯示或修改設定"""
    import json, pathlib
    cfg_path = pathlib.Path.home() / ".config" / "dry-run" / "config.json"
    if cfg_path.exists():
...
DB_PATH = pathlib.Path.home() / ".config" / "dry-run" / "data.db"
...
{
  "name": "dry-run",
  "description": "預演模式包裝器。解決問題：流程缺少 dry-run 模式",
  "input_schema": {
    "type": "object",
    "properties": {
      "action": {
...
```bash
# Agent 呼叫範例
result=$(dry-run run --quiet --output json)
echo $result | jq .data
```
...
def call_dry_run(action="run") -> dict:
    proc = subprocess.run(
        ["dry-run", action, "--output", "json", "--quiet"],
...
# 執行測試
pytest tests/ -v --cov=dry_run --cov-report=term-missing
...
```python
import pytest
from dry_run.core import DryRunCore
...
*工具編號 #079 ／ 類別 D — 自動化與工作流程 ／ 計畫版本 v1.0 ／ 2026-04-27*

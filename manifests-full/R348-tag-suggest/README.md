...
| 介面 | CLI（`click` 框架） |
| 設定檔 | `~/.config/tag-suggest/config.json` |
...
[project.scripts]
tag-suggest = "tag_suggest.cli:main"
```
...
```bash
tag-suggest [OPTIONS] COMMAND [ARGS]...
```
...
# 指定輸出格式
tag-suggest run --output json

# 詳細模式
tag-suggest run --verbose

# 靜音（適合 pipe 到其他工具）
tag-suggest run --quiet | jq .

# 查看設定
tag-suggest config show

# 匯出 JSON
tag-suggest export --output json > result.json
```
...
from .core import TagSuggestCore
...
def config():
    """顯示或修改設定"""
    import json, pathlib
    cfg_path = pathlib.Path.home() / ".config" / "tag-suggest" / "config.json"
    if cfg_path.exists():
...
DB_PATH = pathlib.Path.home() / ".config" / "tag-suggest" / "data.db"
...
```bash
# Agent 呼叫範例
result=$(tag-suggest run --quiet --output json)
echo $result | jq .data
```
...
def call_tag_suggest(action="run") -> dict:
    proc = subprocess.run(
        ["tag-suggest", action, "--output", "json", "--quiet"],
...
```bash
# 管線串接範例
tag-suggest run --quiet --output json \
  | jq -r '.data' \
  | other-tool process
```
...
# 執行測試
pytest tests/ -v --cov=tag_suggest --cov-report=term-missing

# 快速冒煙測試
tag-suggest run --help
tag-suggest config show
```
...
```python
import pytest
from tag_suggest.core import TagSuggestCore
...
*工具編號 #348 ／ 類別 R — 知識管理與筆記 ／ 計畫版本 v1.0 ／ 2026-04-27*

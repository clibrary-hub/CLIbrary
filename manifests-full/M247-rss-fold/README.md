...
| 介面 | CLI（`click` 框架） |
| 設定檔 | `~/.config/rss-fold/config.json` |
...
# 指定輸出格式
rss-fold run --output json

# 詳細模式
rss-fold run --verbose

# 靜音（適合 pipe 到其他工具）
rss-fold run --quiet | jq .

# 查看設定
rss-fold config show

# 匯出 JSON
rss-fold export --output json > result.json
```
...
def config():
    """顯示或修改設定"""
    import json, pathlib
    cfg_path = pathlib.Path.home() / ".config" / "rss-fold" / "config.json"
    if cfg_path.exists():
...
DB_PATH = pathlib.Path.home() / ".config" / "rss-fold" / "data.db"
...
{
  "name": "rss-fold",
  "description": "RSS 聚合器。解決問題：RSS 聚合",
  "input_schema": {
    "type": "object",
    "properties": {
      "action": {
...
```bash
# Agent 呼叫範例
result=$(rss-fold run --quiet --output json)
echo $result | jq .data
```
...
def call_rss_fold(action="run") -> dict:
    proc = subprocess.run(
        ["rss-fold", action, "--output", "json", "--quiet"],
...
```bash
# 管線串接範例
rss-fold run --quiet --output json \
  | jq -r '.data' \
  | other-tool process
```
...
# 執行測試
pytest tests/ -v --cov=rss_fold --cov-report=term-missing
...
```python
import pytest
from rss_fold.core import RssFoldCore
...
*工具編號 #247 ／ 類別 M — 網路爬蟲與監控 ／ 計畫版本 v1.0 ／ 2026-04-27*

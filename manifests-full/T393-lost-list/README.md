...
| 介面 | CLI（`click` 框架） |
| 設定檔 | `~/.config/lost-list/config.json` |
...
```bash
lost-list [OPTIONS] COMMAND [ARGS]...
```
...
# 指定輸出格式
lost-list run --output json

# 詳細模式
lost-list run --verbose

# 靜音（適合 pipe 到其他工具）
lost-list run --quiet | jq .

# 查看設定
lost-list config show

# 匯出 JSON
lost-list export --output json > result.json
```
...
def config():
    """顯示或修改設定"""
    import json, pathlib
    cfg_path = pathlib.Path.home() / ".config" / "lost-list" / "config.json"
    if cfg_path.exists():
...
DB_PATH = pathlib.Path.home() / ".config" / "lost-list" / "data.db"
...
```bash
# Agent 呼叫範例
result=$(lost-list run --quiet --output json)
echo $result | jq .data
```
...
def call_lost_list(action="run") -> dict:
    proc = subprocess.run(
        ["lost-list", action, "--output", "json", "--quiet"],
...
```bash
# 管線串接範例
lost-list run --quiet --output json \
  | jq -r '.data' \
  | other-tool process
```
...
# 執行測試
pytest tests/ -v --cov=lost_list --cov-report=term-missing
...
```python
import pytest
from lost_list.core import LostListCore
...
*工具編號 #393 ／ 類別 T — 旅遊與生活 ／ 計畫版本 v1.0 ／ 2026-04-27*
